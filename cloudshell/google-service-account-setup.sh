...
#   ・GCP プロジェクト作成（または既存）／必要 API 有効化（Calendar/Gmail/Meet/Drive ＋ IAMCredentials/STS）
#   ・サービスアカウント作成（鍵なし）
...
step 3 "API の有効化（Calendar / Gmail / Meet / Drive ＋ IAMCredentials / STS）"
...
# WIF設定JSONを端末経由でクリップボードへ自動コピー（Cloud Shell の hterm は OSC52 対応）。効かない端末でも下に表示。
...
printf '\n%s① WIF設定（クリップボードにコピー済み）%s\n' "$B" "$N"
printf '   設定画面の「WIF設定」欄をクリックして貼り付け（Ctrl+V / Cmd+V）。効かなければ下の枠を手動コピー。\n'
printf '%s┌──────────────────────────────────────────────────────────%s\n' "$D" "$N"
printf '%s\n' "${WIF_JSON}"
printf '%s└──────────────────────────────────────────────────────────%s\n' "$D" "$N"
