...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_CDgvn5Qk.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_X9wJ7pe3.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_RgvQ0ags.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CT3c9yT8.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_AVdbmGFq.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DLPZncV7.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_BlED5qVw.mjs");
