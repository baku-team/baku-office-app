...
import { o as ownedSession, c as createSession, g as getMessages, a as appendMessage, e as ensureTitle, t as toTurns } from "./chat-sessions_BTO_5scg.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (inboundFiles.length) {
    const { prepareAttachment } = await import("./chat-flow_ChMN6Zhi.mjs");
...
`));
  ...
  (async () => {
    try {
      send({ type: "step", label: "依頼を確認しています…" });
      const origin = new URL(request.url).origin;
      const { tryPreAgentRouting, tryProHopsContinuation, finalizeAssistantReply } = await import("./chat-flow_ChMN6Zhi.mjs");
      ...
      const usedTools = [];
      const onEvent = (ev) => {
        if (ev.type === "thinking") send({ type: "step", label: usedTools.length ? "結果を整理しています…" : "内容を考えています…" });
        else if (ev.type === "tool") {
          usedTools.push(ev.name);
          send({ type: "step", label: toolStepLabel(ev.name) });
        } else if (ev.type === "text") send({ type: "delta", text: ev.text });
        else if (ev.type === "delegated_build") send({ type: "step", label: "アプリの作成を始めています…" });
      };
      ...
      const maxHops = await getWorkersPaid(env).catch(() => false) ? 12 : 8;
      ...
      let reply;
      try {
        reply = await ctx.agent.run({ owner: ses.uid, text: prompt, attachments: visionAttachments, role: ses.role, baseUrl: origin, history: toTurns(prior), model, modelId, sessionId, onEvent, signal: request.signal, mode: b.mode, maxHops, waitUntil });
      } finally {
        clearInterval(ka);
      }
      ...
      await (await import("./diag_-P_u6Fn4.mjs")).logDiag(env, "error", "chat", `stream失敗(model=${b.model ?? "auto"}): ${msg}`).catch(() => {
      });
