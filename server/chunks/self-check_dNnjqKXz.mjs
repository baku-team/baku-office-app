...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_UV2ckwv1.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_DWKsGqE6.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
