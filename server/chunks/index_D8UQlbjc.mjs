...
import { r as renderTemplate, e as renderSlot, m as maybeRenderHead, a as addAttribute, F as Fragment } from "./sequence_I_kcixDX.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$Index;
  const { getSession } = await import("./auth__5t3T1Cz.mjs");
  ...
  const homeActions = isOrg ? [
    ...
    { href: "/schedule", label: "予定", sub: "行事・スケジュール", icon: "M4 5h16v15H4zM4 9h16M8 3v4M16 3v4", tone: "cal" },
    { href: "/chat", label: "AI", sub: "質問・相談・作成", icon: "M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8z", tone: "ai" },
    { href: "/apps", label: "アプリ", sub: "使えるアプリ一覧", icon: "M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z", tone: "apps" }
  ];
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_K_W-4z9g.mjs");
  const { cmpVersion } = await import("./update_B7o6l0Fh.mjs");
  await import("./index_Cbs2uaDg.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Dd4rnlyx.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C7VQSZdL.mjs");
  ...
  const { reviewQueue } = await import("./users_P2YEHT8X.mjs");
  const { backupAlert } = await import("./backup_C35cXGeg.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CMUWRR6d.mjs").then((n) => n.p);
  const homeApps = await installedAppLaunchers(ctx).catch(() => []);
