...
const BRIDGE_SDK = `(function(){
  ...
  window.addEventListener("message", function(e){
    ...
    if (m.type === "result") {
      ...
      // 失敗は握り潰さず必ず可視化（自己デバッグの土台・プロンプト遵守に依存しない）。アプリが自前表示していても
      // 二重になるだけで、無音失敗（保存できたのに出ない・ボタン無反応）より遥かに安全。既存アプリにも配信時注入で効く。
      if (!m.ok) { try { console.error("[bo] op failed:", m.error, m.code); } catch(_){} boToast(m.error || "操作に失敗しました。"); }
      // #6 P1：成功(ok:true)でも、途中の連携op（Googleシート/LINE等）が未設定skip/権限不足/失敗なら強制トーストで真実を併記。
      // アプリが固定文言「追記しました」を出しても、プラットフォーム層で「実際には記録されていない」を必ず surface する（偽成功の封じ）。
      if (m.ok && m.notices && m.notices.length) { for (var _i=0; _i<m.notices.length; _i++) { try { boToast(String(m.notices[_i])); } catch(_){} } }
      cb({ ok: !!m.ok, output: m.output, error: m.error, code: m.code, notices: m.notices });
      return;
    }
  ...
  // 失敗トースト（プラットフォーム保証の可視化）。data操作の ok:false は自動でこれを出す＝無音失敗をゼロにする。
  function boToast(msg){
    try{
      if(!msg) return;
      var host=document.getElementById("__bo_toasts");
      if(!host){ host=document.createElement("div"); host.id="__bo_toasts"; host.setAttribute("style","position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:2147483647;display:flex;flex-direction:column;gap:8px;max-width:92%;pointer-events:none"); (document.body||document.documentElement).appendChild(host); }
      var el=document.createElement("div");
      el.setAttribute("style","pointer-events:auto;background:#b00020;color:#fff;padding:10px 14px;border-radius:10px;font:14px/1.5 -apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 4px 16px rgba(0,0,0,.25);max-width:100%;word-break:break-word;cursor:pointer");
      el.textContent=String(msg);
      el.onclick=function(){ if(el.parentNode) el.parentNode.removeChild(el); };
      host.appendChild(el); resize();
      setTimeout(function(){ if(el.parentNode){ el.style.transition="opacity .4s"; el.style.opacity="0"; setTimeout(function(){ if(el.parentNode){ el.parentNode.removeChild(el); } resize(); },400); } }, 6000);
    }catch(_){}
  }
  window.bo = {
    ...
    resize: resize,
    // 失敗の可視化トースト（アプリからも呼べる）。data操作の ok:false は自動でこれを表示する（無音失敗の根絶）。
    toast: function(msg){ boToast(String(msg||"")); },
...
const CHART_ANIM_OFF = '(function(){function f(){try{if(window.Chart&&window.Chart.defaults){window.Chart.defaults.animation=false;if("animations" in window.Chart.defaults)window.Chart.defaults.animations=false;}}catch(e){}}window.addEventListener("DOMContentLoaded",f);window.addEventListener("load",f);})();';
const READY_GUARD = '(function(){var f=false;function r(){if(f)return;if(!window.__app||!window.__app.emit)return;f=true;try{window.__app.emit("ready");}catch(e){}}setTimeout(function(){if(document.readyState==="complete")r();},0);window.addEventListener("pageshow",function(e){if(e.persisted)r();});})();';
function buildFrameSrcdoc(html, sdk = BRIDGE_SDK, params = {}, viewer = {}, frame = {}) {
  ...
  return [
    ...
    `<script${nonceAttr}>window.__BO_PARAMS=${paramsJson};window.__BO_VIEWER=${viewerJson};${STORAGE_SHIM}${CHART_ANIM_OFF}${READY_GUARD}<\/script>`,
    "</head><body>",
    body,
...
function buildPublicFullPage(html, opts) {
  ...
  return [
    ...
    head,
    `<style>${BAKU_APP_BASE_CSS}</style>`,
    `<script${nonceAttr}>window.__BO_SLUG=${JSON.stringify(opts.slug)};window.__BO_PARAMS=${paramsJson};${CHART_ANIM_OFF}<\/script>`,
    "</head><body>",
    body,
