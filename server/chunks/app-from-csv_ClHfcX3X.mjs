...
import { g as startAppBuild, k as kickBuild } from "./ctx_BZ9IFLfW.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const spec = `「${name}」というデータ管理アプリ（CRUD）をカスタムUIで作ってください。次のCSVの列を入力項目にし、登録・一覧（id付き）・編集・削除（data.remove、{op:'data.remove',recordId:'$id'}）ができるようにしてください。一覧は見やすい表で、各行に編集/削除ボタンを置く。
列：${headers.join(", ")}
サンプル行：
${samples || "(なし)"}`;
  const paid = await getWorkersPaid(env).catch(() => false);
  const buildId = await startAppBuild(locals.ctx, { owner: ses.uid, spec, paid });
  await kickBuild(locals.ctx, buildId, new URL(request.url).origin, "csv");
