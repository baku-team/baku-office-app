...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BYlf0VP7.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_uMlTsag-.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_DX8TS5d0.mjs");
  ...
  const { buildCtx } = await import("./ctx_CZ0BwZwu.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_CZ0BwZwu.mjs").then((n) => n.M);
