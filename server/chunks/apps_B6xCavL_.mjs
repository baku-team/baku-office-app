...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_iZjiHMy7.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_Ige4T837.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_B17j1uG_.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BCH4lAnd.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CVjeTeB8.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DdRsAqqg.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_Z-no6Ohb.mjs");
