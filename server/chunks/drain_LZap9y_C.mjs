...
import { getApiKey } from "./client_CkzMAmmV.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_BYlP_hKo.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_GcjRF9RO.mjs";
import { processAgentJobs } from "./agent-jobs_CropO8WF.mjs";
import { reclaimStaleApprovals } from "./approvals_DmWkKqQd.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_ZGMfkpHW.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_CvY9YF-V.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_BDFBxfKv.mjs";
import { flushReports } from "./reports_D6PxWzes.mjs";
import { addNotification, pushWebhook } from "./notifications_BF4NlyCu.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_CzB15Zqw.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_FZj-WNeE.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_C7zudgPi.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
