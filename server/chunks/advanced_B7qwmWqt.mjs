...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BZYsk0AP.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BEZoUF8-.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_TBpzvJRs.mjs");
