...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await (await import("./ctx_DloQDFvv.mjs").then((n) => n.O)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_DloQDFvv.mjs").then((n) => n.Q)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_cqz-aSmr.mjs");
