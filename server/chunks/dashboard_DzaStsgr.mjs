...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_0pHY0CaD.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_omQ7RKES.mjs");
  await import("./index_DF6Mutvv.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DF9qKz49.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Bu3d3jys.mjs");
  ...
  const { reviewQueue } = await import("./users_vyOgg6uk.mjs");
  const { backupAlert } = await import("./backup_Ndxo0-kJ.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DzHldYd8.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CPYUMyFX.mjs");
