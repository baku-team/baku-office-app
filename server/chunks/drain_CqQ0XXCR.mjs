...
import { getApiKey } from "./client_CB_YAuNH.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_ByywAkrg.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_C1TyftTU.mjs";
import { processAgentJobs } from "./agent-jobs_BepmVpvw.mjs";
import { reclaimStaleApprovals } from "./approvals_DfVHaJ5K.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_BJM3oGjI.mjs";
import { guardHeavy } from "./diag_CY3PJnOM.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_BJ_kLfef.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_CEC5ZlvD.mjs";
import { flushReports } from "./reports_D2FYLNjd.mjs";
import { addNotification, pushWebhook } from "./notifications_0VPJXKcx.mjs";
import { getNotifyWebhook } from "./settings_dTyIbTCR.mjs";
import { purgeFiles } from "./storage_Cvzvo6pu.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_BP-0z6W5.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_B6Gdy7_n.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_Dmvv5Mgv.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_CuAuR4T-.mjs").then((m) => m.flushUsageDigest(env)));
