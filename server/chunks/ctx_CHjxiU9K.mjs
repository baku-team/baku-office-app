...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_SJF4hL77.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_D3kajqFf.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CX6PTfWW.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_Bbj5MCVQ.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CgzlDI-b.mjs";
import "./index_Dj0mRxJ1.mjs";
import { recordUsage, recordTokens, overBudget, recordCallLog, estimateUsd } from "./usage_BDQhcX3s.mjs";
...
import { getAiEngine, getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, setPaidSwitchOk, resolveModelSelection, getMemberModel, getSmartRouting, getWorkersAiModel, maxParallelAgents, agentMaxHops, getPaidSwitchOk } from "./settings_hJXdQlLE.mjs";
import { googleFetch } from "./google_Cd9kHlGa.mjs";
import { t as toolsOf, a as scopeCtx, p as partCatalog, e as enabledPartIds, s as setEnabledPartIds, c as enabledParts, d as partOfTool } from "./parts_D92YfUNk.mjs";
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_sxz5fLSK.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CEtfGWwh.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DaVyqv6R.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_Bt5lkRjr.mjs";
import { logDiag } from "./diag_ByGiCOpc.mjs";
import { i as isRunnableDefinition } from "./preflight_Dk9zNDMQ.mjs";
import { c as catalogPromptHint } from "./cdn-allowlist_C8Th7j6o.mjs";
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_DoI_T6HY.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CHbV2BMe.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_0835KCOB.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DYtEz2tN.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_lTbw593U.mjs";
import { saveLayoutDraft, listSites } from "./sites_BMlqbpaA.mjs";
...
import { storeCatalog } from "./store_LArcC0KF.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_DMeeHCS8.mjs";
...
${spec}` }], [], void 0, { maxTokens: attempt === 0 ? 1500 : 2200, thinking: "disabled", jsonSchema: PLAN_SCHEMA }).catch(() => null);
...
async function implementRender(env, model, plan, spec, builtScreens, paid) {
  ...
  const sys = await getAiKnowledge(env) + "\n\nあなたはカスタムUIの HTML ジェネレータです。出力は HTML 文書そのものだけ（前置き・説明・コードフェンス・JSON を一切付けない）。サンドボックス iframe（別オリジン・外部通信なし）で描画される。inline の <style>/<script> は可。外部リソースは原則不可（グラフ等の可視化に限り、下記【リッチ描画（relaxed）】の厳選CDNのみ可）。" + scopeRule + DESIGN_BASELINE + `データ操作（保存/一覧）が要る場合のみ window.bo.run(screenId, inputs)（戻り値 {ok,output:{type,value},error?} の Promise）を使う。呼べる口：${opList}。output.value が table の時は JSON 文字列なので JSON.parse して描画。`;
  const maxTokens = paid ? 32e3 : 12e3;
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_BMlqbpaA.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_ClMKyXcf.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_ClMKyXcf.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_ClMKyXcf.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_D3kajqFf.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_ClMKyXcf.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_SJF4hL77.mjs")).storageMode(env) === "r2";
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。"));
    ...
    try {
      ...
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_hJXdQlLE.mjs");
...
function cfAgent(ctx2) {
  return {
    run: (i) => runAgent(ctx2, i.owner, i.text, i.image, i.baseUrl ?? "", i.role ?? "member", { history: i.history, model: i.model, modelId: i.modelId, sessionId: i.sessionId, onEvent: i.onEvent, signal: i.signal, mode: i.mode, feature: i.feature })
  };
}
