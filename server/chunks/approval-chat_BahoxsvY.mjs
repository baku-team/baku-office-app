...
import { getApproval, decideApproval } from "./approvals_D3RhLZVX.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_BgvE5w_L.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_Cpc8zRmY.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_FQgJZWOl.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_XKaks_hD.mjs";
import { joinWithInvite } from "./users_BBnHZMEg.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BqaGF6MH.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_BBnHZMEg.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_Ctjicwyv.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_pESJxJvo.mjs");
