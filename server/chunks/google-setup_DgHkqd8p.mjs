...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_C5HMqpnO.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_7jAecm_N.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_B6c1gZ6p.mjs");
  ...
  const { buildCtx } = await import("./ctx_CIL4AtJU.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_CIL4AtJU.mjs").then((n) => n.M);
