...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BTLpH8fW.mjs");
  const { cmpVersion } = await import("./update_iOLSMgqz.mjs");
  await import("./index_DO-Y0Vx2.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B5O5FLC5.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Y492_yZB.mjs");
  ...
  const { reviewQueue } = await import("./users_BebFQZ_w.mjs");
  const { backupAlert } = await import("./backup_CkAPbV02.mjs");
