...
import { r as renderComponent } from "./worker-entry_BKQSpYXf.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BSxN55fT.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_OC_jDX3F.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_OC_jDX3F.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_Cgc_Tc-o.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_DhfR4ryc.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DAI5Xutz.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CkjQoz3l.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DahLZ_Ik.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_pg7WpKY1.mjs");
