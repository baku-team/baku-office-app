...
import { r as renderComponent } from "./worker-entry_Clrd0clV.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Brfm6wpv.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CIgUj-G6.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_o_bCkQmM.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_Bn_fgYPT.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_C_FojZs_.mjs"),
    import("./scheduled-tasks_CBAQ1-g5.mjs")
  ]);
  ...
  await import("./index_f2x1sCW8.mjs");
