...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CFhfgOdP.mjs");
  const { cmpVersion } = await import("./update_BUQRCkUd.mjs");
  await import("./index_DNEehGc8.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bm85HqIr.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_vfbsfwg6.mjs");
  ...
  const { reviewQueue } = await import("./users_DRl31INi.mjs");
  const { backupAlert } = await import("./backup_5HJDlGH7.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CND5X7DG.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B4jFnDvF.mjs");
