...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_Dpe2zM3e.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_DWCSrGCf.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
