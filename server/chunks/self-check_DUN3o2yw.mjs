...
import { APP_SCHEMA, validateDefinition } from "./appdef_CAZMqTui.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_DlKVNdA9.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BjrAYDQX.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
