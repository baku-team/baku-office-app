...
import { r as renderComponent } from "./worker-entry_CZJkz03S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Bqf-wiDE.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_ZQlc5_jo.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_ZQlc5_jo.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CzpgTqhF.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_0Vd7eIvZ.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Dm3QXHiv.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DwBfCXoi.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CVNZFet_.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Tcmn7fmY.mjs");
  ...
  const folders = await (await import("./settings_CBzcH790.mjs")).getAppFolders(env, ses.uid).catch(() => []);
