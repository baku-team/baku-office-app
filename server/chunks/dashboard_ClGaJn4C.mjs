...
import { r as renderComponent } from "./worker-entry_hbh81n-9.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_XI1yg1QR.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_Bh_tJP0x.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BKYtLCd6.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BL1ENf5s.mjs");
  await import("./index_B4Tk4Zwd.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DMCgfXnA.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_9uI8ycKM.mjs");
  ...
  const { reviewQueue } = await import("./users_CAsNPlru.mjs");
  const { backupAlert } = await import("./backup_C1ZaPmSf.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_C0xk2SHi.mjs");
