...
import { cachedEntitlement, getApiKey } from "./client_C_RhHGpz.mjs";
import { DiscordInbound } from "./discord_MitODRRb.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { joinWithInvite } from "./users_BSfpZ42G.mjs";
import { F as transcribeAudio, G as verifyLineSignature, H as lineReply, B as linePush, I as lineReplyQuick, t as cfEgressGateway } from "./ctx_BWY7KuVP.mjs";
import { inboundFileLimitBytes, saveFile } from "./storage_DWaRDBDI.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { logDiag, looksLikeLimit, PAID_HINT } from "./diag_DWKsGqE6.mjs";
import { getWorkersPaid } from "./settings_DHOyCLdr.mjs";
...
async function joinViaInvite(env2, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_BSfpZ42G.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_DQQfVOwz.mjs");
  const bins = [];
  const failedNames = [];
...
返信で「読み込み」「保管する」「何もしない」のいずれかを送ってください。` : ack.text) + failNote;
  ...
  if (sid) await saveSession(env2, msg.connector, sid, [...history, { role: "user", text: prompt }]).catch(() => {
  });
  const { getMemberModel, parseRequestModel, detectInlineModel } = await import("./settings_DHOyCLdr.mjs");
  const inlineModel = detectInlineModel(prompt);
  if (inlineModel.engine) prompt = inlineModel.stripped;
  const memberSel = parseRequestModel(await getMemberModel(env2, acc.uid).catch(() => null) ?? "");
  ...
  const { recordTaskFromReply } = await import("./task-log_DFVySHMg.mjs");
...
class LineInbound {
    ...
    for (const ev of events) {
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS } = await import("./upload-queue_DQQfVOwz.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_DQQfVOwz.mjs");
