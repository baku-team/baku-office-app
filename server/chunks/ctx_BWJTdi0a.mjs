...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_dk4WezxQ.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Dd9FH8gm.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CIyssDNw.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_MRHM8AL0.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_tV2l6_F4.mjs";
import "./index_BHmUVA_J.mjs";
...
import { googleStatus, SCOPE_GROUPS, googleFetch } from "./google_D_wgy0NO.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, opCatalogText } from "./appdef_O_0C7_c6.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_mug4D6f4.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_ZYkQvia3.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_CYlQ6k0K.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_nwTnM64M.mjs";
import { logDiag } from "./diag_CY3PJnOM.mjs";
import { i as isRunnableDefinition } from "./preflight_p6fjU_XC.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_tccqCPg-.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CB0z-M_x.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_w_QpqFZo.mjs";
import { storeCatalog } from "./store_CUZVwu_2.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_B5cgUMAK.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_wlxweYCT.mjs";
import { saveLayoutDraft, listSites } from "./sites_DC33-JtF.mjs";
...
import { p as pushToUser } from "./push_Coq9tyWm.mjs";
...
function normalizePlan(raw) {
  ...
  const followUps = Array.isArray(r.followUps) ? r.followUps.map((s) => String(s).trim().replace(/\s+/g, " ").slice(0, 120)).filter(Boolean).slice(0, 6) : [];
...
async function finalize$1(ctx2, row, plan, def, model) {
  ...
  const judged = row.spec && !light ? await judgeCoverage(await buildChatModel(ctx2.env, row.model ?? void 0, "fast") ?? model, row.spec, plan, def).catch(() => []) : [];
  const unmet = [.../* @__PURE__ */ new Set([...plan.followUps ?? [], ...judged])].slice(0, 6);
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DC33-JtF.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_CMHDnBbG.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_CMHDnBbG.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_CMHDnBbG.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Dd9FH8gm.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_CMHDnBbG.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_dk4WezxQ.mjs")).storageMode(env) === "r2";
...
function cfAgent(ctx2) {
  return {
    run: (i) => runAgent(ctx2, i.owner, i.text, i.image, i.baseUrl ?? "", i.role ?? "member", { history: i.history, model: i.model, modelId: i.modelId, sessionId: i.sessionId, onEvent: i.onEvent, signal: i.signal, mode: i.mode, feature: i.feature, audience: i.audience, attachments: i.attachments })
  };
}
