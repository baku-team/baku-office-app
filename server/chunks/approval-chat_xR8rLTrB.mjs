...
import { getApproval, decideApproval } from "./approvals_BRjh5QAu.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_BDZXx1aq.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_-pDHDD3Z.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_RPvkBnXt.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_peeVfBbE.mjs";
import { joinWithInvite } from "./users_Dt8Au497.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_DAEZsEe4.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_Dt8Au497.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_C0Fkqr3v.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_C4B6qwAp.mjs");
