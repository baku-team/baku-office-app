...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_mfWxKLIy.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BjKlgqy9.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_D7p_P15A.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_Cj-LVoHK.mjs"),
    import("./scheduled-tasks_DFBKLagv.mjs")
  ]);
  ...
  await import("./index_NXtqd6gx.mjs");
