...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Index;
  const acc = await import("./accounting_BipJ8jvJ.mjs").then((n) => n.j);
  ...
  const { getSession } = await import("./auth_BxYQEyFX.mjs");
  const ses = await getSession(env, Astro2.request);
  const isOrgAdmin = ses?.role === "admin" && ses?.ctx === "org";
  const { getBookkeepingMode } = await import("./settings_BubM6K6A.mjs");
  const mode = await getBookkeepingMode(env);
  const acctItems = await import("./accounting_BipJ8jvJ.mjs").then((n) => n.i);
  const WALLET_TYPES = acctItems.WALLET_TYPES;
  const walletTypeLabel = (t) => WALLET_TYPES.find((w) => w.type === t)?.label ?? t;
  const jr = await import("./journal_B47kwBjB.mjs");
  ...
  const fa = await import("./fixed-assets_DcBCbvl8.mjs");
  ...
  const reg = await import("./register_Bz5-AVR7.mjs");
