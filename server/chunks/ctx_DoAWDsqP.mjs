...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_Czk76zwb.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CZasnwOK.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Cm3ihAVM.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_DUD3EVsR.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DJ7cZUGv.mjs";
import "./index_6IcBz0fb.mjs";
...
import { googleFetch } from "./google_CljRQN3C.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_CmY52iyG.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_21esnpu8.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DJ8U_BkY.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_C5jKow07.mjs";
import { logDiag } from "./diag_DWCSrGCf.mjs";
import { i as isRunnableDefinition } from "./preflight_BO3KIk2s.mjs";
import { c as catalogPromptHint } from "./cdn-allowlist_Ck6a6kXM.mjs";
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BdhGvqNE.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_gh3ULBV2.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_BFYcvNMO.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_D1cpdn3R.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_CD5TN9TN.mjs";
import { saveLayoutDraft, listSites } from "./sites_Eyer3Ba1.mjs";
...
import { storeCatalog } from "./store_Bh6hTC8W.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_EejwKhwP.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_Eyer3Ba1.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_Dz3OM5_n.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_Dz3OM5_n.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_Dz3OM5_n.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CZasnwOK.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_Dz3OM5_n.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_Czk76zwb.mjs")).storageMode(env) === "r2";
