...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_yx4APBBm.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DLN3EmD2.mjs");
  await import("./index_C_ej8KZQ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BeNzI-n3.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BxJYKzU1.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_CSS355mB.mjs");
  ...
  const { reviewQueue } = await import("./users_BeF5YPT8.mjs");
  const { backupAlert } = await import("./backup_Chy-mJ6E.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BEfZxNjt.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_eLhCSUGc.mjs");
  const { getWorkersPaid } = await import("./settings_FobH8uUC.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_Bqk7Mxtc.mjs");
