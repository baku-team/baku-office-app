...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_Cknt-1Jr.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_B7kuZSwA.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DmYZEEDZ.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_DmYZEEDZ.mjs").then((n) => n.P);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DtyjwiHH.mjs").then((n) => n.z);
