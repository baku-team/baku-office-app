...
import { DiscordInbound } from "./discord_DnYpWqHF.mjs";
import { c as checkAccess, m as markEventSeen, d as decideApprovalFromChat, j as joinViaInvite, r as respondInbound } from "./approval-chat_DmB-4H0L.mjs";
import { r as resolveUserLabels, e as extOfName, i as isSecretName, m as maskVal } from "./storage-admin_CgDhwmn-.mjs";
import { softDeleteFile, audit, inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BthdaNTE.mjs";
import { kvPut } from "./kv_ryUreqJI.mjs";
import { I as verifyLineSignature, J as lineLoadingStart, K as lineReply, l as linePush, L as lineReplyQuick, c as cfEgressGateway } from "./ctx_Ige4T837.mjs";
import { getDriveSave } from "./drive_ARKOgIQO.mjs";
import { getWorkersPaid } from "./settings_C4tL9AHR.mjs";
import { logDiag, looksLikeLimit, PAID_HINT } from "./diag_-P_u6Fn4.mjs";
import { recordInboundEvent, completeInboundEvent } from "./inbound-durable_C63tN5W3.mjs";
...
class SlackInbound {
  ...
  async handleInbound(req, ic) {
    ...
    if (ev && !ev.bot_id && !ev.subtype && addressed) {
      if (data.event_id && ic.seen && await ic.seen("slack", data.event_id)) return ok();
...
class LineInbound {
  ...
  async process(events, ic) {
    for (const ev of events) {
      if (ev.webhookEventId && ic.seen && await ic.seen("line", ev.webhookEventId)) continue;
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS, WAIT_SEC, processReadyUploads } = await import("./upload-queue_Dd3X8Oj6.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_Dd3X8Oj6.mjs");
...
const POST = async ({ request, params, locals }) => {
  ...
  return handler.handleInbound(request, {
    ...
    // MSG-01: 処理前に payload を durable 記録し、完了で done・例外で failed 化する（best-effort＝記録失敗でも処理は続行）。
    // pending のまま残った未完了イベントは cron drain が検知し owner へ再送依頼を push＝無通知の消失と監査欠落を防ぐ。
    respond: async (msg) => {
      const recId = await recordInboundEvent(env, msg);
      try {
        const reply = await respondInbound(locals.ctx, env, origin, msg);
        if (recId) await completeInboundEvent(env, recId, "done");
        return reply;
      } catch (e) {
        if (recId) await completeInboundEvent(env, recId, "failed", String(e?.message ?? e));
        throw e;
      }
    },
    ...
    seen: (conn, eventId) => markEventSeen(env, conn, eventId)
  });
};
