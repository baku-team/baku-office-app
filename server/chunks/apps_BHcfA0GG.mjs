...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_ByywAkrg.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CB_YAuNH.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CVpv3OCE.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CCYyWIXk.mjs");
  const { registeredParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DbldLbJu.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_BpsZcsJX.mjs");
  ...
  const folders = await (await import("./settings_dTyIbTCR.mjs")).getAppFolders(env, ses.uid).catch(() => []);
