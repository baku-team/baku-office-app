...
import { l as loadDraft, c as checkDefinition, a as checkTransitions, b as checkSandbox, d as checkFabrication, e as checkRun, f as finalizePrecheck } from "./precheck_BHeKCxgA.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_DWKsGqE6.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
