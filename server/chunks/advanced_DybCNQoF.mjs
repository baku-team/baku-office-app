...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BCk4EGYy.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_CQDnhpKq.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CoDdssK4.mjs");
