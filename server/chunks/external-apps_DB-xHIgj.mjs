...
import { getToken, hostFetch, getVerifyJwk } from "./client_CdHBXZZx.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_BC4pw_bA.mjs";
import { sanitizeRenderHtml, validateDefinition } from "./appdef_CtwKFZ_L.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_CtwKFZ_L.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
  ...
  if (d.definition) {
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        const approvedHosts = approvedRaw ? JSON.parse(approvedRaw).allowHosts ?? [] : null;
        const okApproved = approvedHosts !== null && want.every((h) => approvedHosts.includes(h));
        if (!okApproved) {
          parsed.render.isolation = "sandboxed";
          d.definition = JSON.stringify(parsed);
          const { createApproval } = await import("./approvals_CUXReKcn.mjs");
          await createApproval(
            ctx.env,
            by ?? "system",
            "app_render_isolation_relaxed",
            { appId: d.id, allowHosts: want },
            `【ネイティブ描画（relaxed）への昇格】アプリ「${d.name}」を別オリジン緩和(T1)で描画します。許可ホスト：${want.join("、") || "（なし）"}。CDN等の外部コード読込を許可しますが、別オリジン隔離は維持され親データ(D1/Cookie)への到達は引き続き不可です。承認するまでは安全側（sandboxed）で描画します。`,
  ...
  if (prev && (prev.definition !== d.definition || prev.permissions !== d.permissions)) {
    return { ok: false, error: "同じバージョン番号で内容の異なる定義は登録できません。変更を反映するにはバージョンを上げてください。" };
  }
  await ctx.db.run(
  ...
  try {
    if (d.definition) {
      const { rebuildPublicPageFromDef } = await import("./public-pages_BaPDC316.mjs");
...
async function deleteLocalApp(ctx, appId, deleteData = false) {
  ...
  await (await import("./public-pages_BaPDC316.mjs")).deletePublicPagesByApp(ctx.env, appId).catch(() => {
  });
...
async function installedAppDefs(ctx, role) {
  const rows = await ctx.db.all("SELECT id,name,description,definition,allowed_roles FROM external_apps WHERE definition IS NOT NULL");
  const out = [];
  for (const r of rows) {
...
async function appsBrief(ctx) {
  const drafts = await ctx.db.all("SELECT id,name,version,gate_status FROM app_drafts ORDER BY created_at DESC");
  const inst = await ctx.db.all("SELECT id,name,version FROM external_apps WHERE definition IS NOT NULL");
  const byId = /* @__PURE__ */ new Map();
  for (const d of drafts) byId.set(d.id, { id: d.id, name: d.name, version: d.version, state: d.gate_status === "ready" ? "実装可" : d.gate_status === "blocked" ? "ブロック" : "企画中" });
  for (const e of inst) {
    const cur = byId.get(e.id);
    if (cur) cur.state = "導入済み";
    else byId.set(e.id, { id: e.id, name: e.name, version: e.version, state: "導入済み" });
  }
  return [...byId.values()];
}
...
export {
  ...
  setAppAllowedRoles as v,
  installedAppDefs as w,
  appsBrief as x,
  appMeta as y,
  externalApps as z
};
