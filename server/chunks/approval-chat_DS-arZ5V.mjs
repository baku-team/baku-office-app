...
import { getApproval, decideApproval } from "./approvals_DPodwbLT.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_DED0iuNZ.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_AX6IIuyG.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_hJdQx8dJ.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_DEBuc12A.mjs";
import { joinWithInvite } from "./users_3Hp8b7YO.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_B-P5Zaq2.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_3Hp8b7YO.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_T6XhlY8X.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_CfkDkbhq.mjs");
