...
import { setRetentionDays, setMaxUploadMb, fileBelongsTo, getFile, saveFile, storageMode } from "./storage_CpD6nnHA.mjs";
import { m as makeDocument, w as webSearch, b as summarizeTranscript, c as extractInvoiceData, t as transcribeAudio, i as inferApp } from "./media-ai_DBWNifyL.mjs";
import { googleFetch } from "./google_DvQoLsEb.mjs";
import { getApiKey, entitlementForGate } from "./client_BXVeeS3h.mjs";
...
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp } from "./apps_omiWNHKX.mjs";
import { v as validateDefinition, A as APP_SCHEMA, R as RENDER_HTML_MAX, o as opCatalogText } from "./appdef_CRQzZn2a.mjs";
import { runToolLoop } from "./ai_CCPaNrnh.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CThzfPha.mjs";
import { searchDirectory } from "./directory_DJJxxwVj.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_CnS-zWu8.mjs";
import { logDiag } from "./diag_Cu0HTcAn.mjs";
import { i as isRunnableDefinition } from "./preflight_CdN8K1wC.mjs";
...
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage } from "./public-pages_DEc1MUZq.mjs";
import { g as getAppDesign, m as createDraft, n as repairOutputLiterals, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_CSNxZelj.mjs";
...
import "./index_CL76Ttcr.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_CUARreq3.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CLx0S4o8.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BPQd_5aE.mjs";
import { overBudget, recordUsage, estimateUsd, recordTokens } from "./usage_DN6pGBs1.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_JGw45ks-.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers } from "./users_nVitkZLK.mjs";
import { listActions, createAction, deleteAction, updateAction } from "./a2a-actions_Bksozya3.mjs";
import { p as pushToUser } from "./push_CT47OKeq.mjs";
...
- 【公開フォームは管理ビューとセットで作る】公開フォームを作るときは別アプリにせず“1つのアプリ”に「公開フォーム」と「社内の管理・可視化ビュー」を同梱する。同じ render.html を公開ページと社内アプリ画面の両方が描画するので window.bo.context（'app'=社内ログイン済み / 'public'=匿名公開）で出し分ける：公開では管理ビューを隠してフォームだけ見せ、社内では管理ビューを表示して起動時に bo.run('list') で送信データを取得しテーブル/カード/件数・簡易集計で可視化する。定義(screens)には保存用(save:data.create)に加え必ず一覧用(list:data.list)を含める。可視化は外部ライブラリ不可なので HTML/CSS だけで作る。公開送信は承認制なので管理ビューに出るのは承認済みデータ（未承認は承認待ち一覧 /settings/public 経由で承認すると反映）。
...
async function editStep(ctx, row, model) {
  ...
  const { patch, truncated, text } = await makePatch(env, model, design.definition, row.spec ?? "", row.paid === 1);
  if (!patch) {
    if (truncated) {
      await failBuild(ctx, row, explainStop("ai", "修正内容が大きく、AIが1回で生成しきれませんでした（出力上限到達）。", "対象を絞って（例：特定の画面・機能だけ）小分けに再依頼してください。"));
      return false;
    }
    const assess = (text || "").replace(/```[\s\S]*?```/g, "").trim();
    if (assess.length >= 10 && !assess.startsWith("{") && !assess.startsWith("[")) {
      const msg = `確認しました（変更なし）：${assess.slice(0, 1500)}`;
    ...
    await failBuild(ctx, row, explainStop("ai", "AIが修正案を生成できませんでした（応答が空または解釈不能）。", "指示を具体的にして（どの画面の何をどう変えるか）再度お試しください。別のAIモデルへの切替も有効です。"));
    return false;
  }
...
検証エラー：${issueText}`;
  ...
  if (!vr.ok) {
    await failBuild(ctx, row, explainStop("ai", "AIが生成した修正後の定義が検証に通りませんでした（" + vr.issues.slice(0, 2).map((i) => i.message).join(" / ") + "）。", "どの画面・項目をどう変えたいかを具体的にして再依頼してください。別のAIモデルへの切替も有効です。"));
    return false;
  }
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BXVeeS3h.mjs");
