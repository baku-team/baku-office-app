...
const $$Chat = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Chat;
  const { getSession } = await import("./auth_D-SE8ibz.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CyzvxOBO.mjs");
  ...
  const ENGINE_META = {
    gemini: { label: "標準AI", sub: "無料の範囲" },
    claude: { label: "高精度AI", sub: "高精度" },
    local: { label: "クラウドAI", sub: "標準" }
  };
  const availEngines = ["gemini", "claude", "local"].filter((e) => e === "gemini" ? hasGemini : e === "claude" ? hasClaude : hasLocal);
  const { getMemberEngine } = await import("./settings_C2uu0Zu7.mjs");
  const savedEngine = hasPlus ? await getMemberEngine(env, ses.uid).catch(() => null) : null;
  const fallbackModel = availEngines.includes("gemini") ? "gemini" : availEngines[0] ?? "gemini";
  const activeModel = savedEngine && availEngines.includes(savedEngine) ? savedEngine : fallbackModel;
  const { listSessions } = await import("./chat-sessions_D1ns0SmC.mjs");
  const sessions = hasPlus ? await listSessions(Astro2.locals.ctx, ses.uid).catch(() => []) : [];
  const isOrgAdmin = ses.role === "admin" && ses.ctx === "org";
  const { listSkills } = await import("./skills_waeWRO07.mjs");
  const skills = hasPlus && isOrgAdmin ? await listSkills(env).catch(() => []) : [];
  const { WORKERS_AI_MODELS } = await import("./config_BOdbJfMp.mjs");
  const waModelActive = isOrgAdmin && env.AI ? await (await import("./settings_C2uu0Zu7.mjs")).getWorkersAiModel(env).catch(() => "") : "";
  return renderTemplate`${renderComponent($$result, "App", $$App, { "title": "AI", "active": "/chat" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<h1>AI</h1> ${!hasPlus && renderTemplate`<div class="card"> <div class="banner banner-warn">この機能は <strong>Plus 以上</strong>のプランで利用できます。</div> <p class="muted">集計・DB／ファイル検索・文書作成を会話で実行し、結果を PDF／TXT／md／HTML／CSV でダウンロードできます。</p> <a class="btn btn-primary" href="/billing">プラン・課金へ</a> </div>`}${hasPlus && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate` <div class="chat"> <aside class="sbar"> <button class="btn btn-primary" id="new-ses" style="width:100%">＋ 新しい会話</button> <div class="ses-list" id="ses-list"> ${sessions.map((s) => renderTemplate`<div class="ses-row"${addAttribute(s.id, "data-ses")}> <a href="#" class="ses-open"${addAttribute(s.id, "data-id")}>${s.title || "（無題）"}</a> <button class="ses-del"${addAttribute(s.id, "data-id")} title="削除">×</button> </div>`)} ${sessions.length === 0 && renderTemplate`<p class="muted" style="font-size:.82rem;padding:6px">会話はまだありません。</p>`} </div> </aside> <section class="pane"> <div class="cbar">  ${availEngines.length >= 2 && renderTemplate`<div class="model-seg" id="model-seg" role="group" aria-label="AIの種類"> ${availEngines.map((e) => renderTemplate`<button${addAttribute("model-opt" + (activeModel === e ? " on" : ""), "class")} type="button"${addAttribute(e, "data-m")}>${ENGINE_META[e].label}<small>${ENGINE_META[e].sub}</small></button>`)} </div>`}  <select id="model" hidden> ${availEngines.map((e) => renderTemplate`<option${addAttribute(e, "value")}${addAttribute(activeModel === e, "selected")}>${ENGINE_META[e].label}</option>`)} ${availEngines.length === 0 && renderTemplate`<option value="gemini" selected>標準AI</option>`} </select> <button class="btn btn-sm ses-toggle" type="button" id="ses-toggle">会話履歴</button> </div> ${availEngines.length >= 1 && renderTemplate`<div class="model-note" id="model-note"></div>`} ${isOrgAdmin && env.AI && renderTemplate`<details class="adv" style="margin:.2rem 0 .4rem"> <summary>クラウドAIのモデルを選ぶ（管理者向け）</summary> <div style="padding:.45rem 0"> <label for="wa-model" class="muted" style="display:block;font-size:.85rem;margin-bottom:5px">「クラウドAI」で使うモデルです。上位ほど賢いですが、少し遅く・費用が増えます。</label> <select id="wa-model"> ${WORKERS_AI_MODELS.map((m) => renderTemplate`<option${addAttribute(m.id, "value")}${addAttribute(m.id === waModelActive, "selected")}>${m.label}（${m.note}）</option>`)} </select> </div> </details>`} <div class="ai-examples" aria-label="例文"> <span class="muted" style="font-size:.85rem">かんたん入力：</span> <button class="chip ex" type="button" data-ex="今月の会費の入金状況をまとめて">今月の会費の入金状況をまとめて</button> <button class="chip ex" type="button" data-ex="先月の支出を一覧にして">先月の支出を一覧にして</button> <button class="chip ex" type="button" data-ex="次の定例会の案内文を作って">次の定例会の案内文を作って</button> </div> <div class="chat-dl"> <span class="muted">この会話を保存：</span> <button class="btn btn-sm dlc" type="button" data-ext="txt" title="文字だけのシンプルな形式。どんな端末でも開け、メモ帳などで読めます。" aria-label="TXT形式で保存（文字だけのシンプルな形式）">TXT</button> <button class="btn btn-sm dlc" type="button" data-ext="md" title="見出しや箇条書きの体裁を残せる形式。技術系のメモやNotion等への貼り付け向き。" aria-label="MD（Markdown）形式で保存（見出し・箇条書きを残せる）">MD</button> <button class="btn btn-sm dlc" type="button" data-ext="html" title="ブラウザでそのまま開けて見やすい形式。体裁を保って共有したいとき向き。" aria-label="HTML形式で保存（ブラウザで開け、体裁を保てる）">HTML</button> <button class="btn btn-sm dlc" type="button" data-ext="pdf" title="印刷や正式な配布・保管向き。レイアウトが固定され、改変されにくい形式。" aria-label="PDF形式で保存（印刷・配布向き、レイアウト固定）">PDF</button> </div> <div id="log"></div> ${hasPro && renderTemplate`<label class="muted" style="font-size:.8rem;display:flex;align-items:center;gap:6px;margin-top:8px"> <input type="checkbox" id="bgrun"> バックグラウンドで実行（重い・多段の依頼向け。完了するとこの会話に結果が追記されます）
    ...
    (function () {
        ...
        const NOTE = {
          gemini: { use: "標準AI（無料の範囲）", can: "会計入力・ファイル/名簿の検索・文書作成・調べもの など", cant: "とても長い文章や複雑な推論は高精度AIが得意です" },
          claude: { use: "高精度AI", can: "上記に加えて、より高度な作業・長文・複雑な推論", cant: "（特になし）" },
          local: { use: "クラウドAI（標準）", can: "会話・要約・文章づくり", cant: "会計の登録や検索などの操作（必要なときは他のAIに切り替えてください）" },
        };
        ...
        // モデルセグメント：選択を hidden select(#model) に同期し、メンバー単位で保存（次回以降の既定に）。
        ...
        if (mseg && sel) mseg.querySelectorAll(".model-opt").forEach((b) => b.addEventListener("click", () => {
          ...
          setNote(b.dataset.m);
          window.bo?.api?.("/api/settings", { _action: "member_engine", engine: b.dataset.m }, { successMsg: "使うAIを切り替えました" });
        }));
