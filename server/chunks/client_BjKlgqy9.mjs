...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_DWnRrE-m.mjs").then((n) => n.U), import("./notifications_BozZHqe1.mjs")]);
    ...
    await (await import("./ctx_DWnRrE-m.mjs").then((n) => n.Q)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_DWnRrE-m.mjs").then((n) => n.L)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_D3Qw7oqJ.mjs");
