...
const POST = async ({ request, locals }) => {
  ...
  if (b.action === "rebuild") {
    ...
    const { rebuildPublicPage } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.R);
  ...
  if (b.action === "toggle") {
    ...
    if (enable) {
      ...
      if (page2) {
        const { gatePublishApp } = await import("./self-check_C_afD0eq.mjs");
        const { appendMessage } = await import("./chat-sessions_DJA_I8xR.mjs").then((n) => n.i);
