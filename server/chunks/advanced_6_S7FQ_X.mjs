...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_C1ICAElS.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DZBk9gei.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DJiP0NOX.mjs");
