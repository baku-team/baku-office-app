...
import { r as renderComponent } from "./worker-entry_pxpSrggW.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BgzpDssl.mjs";
...
const $$Update = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Update;
  const { getSession } = await import("./auth_UTOIdkPi.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  if (ses.role !== "admin") return Astro2.redirect("/forbidden", 302);
  const { pollHost, APP_VERSION } = await import("./client_CB_YAuNH.mjs");
  const { hasDeployHook, cmpVersion, getAutoUpdate, getAutoUpdateStatus } = await import("./update_B6Gdy7_n.mjs");
  ...
  const REPO_NAME = "baku-office-app";
...
このアプリは配布リポ（<code data-astro-cid-curm2vkq>${UPSTREAM}</code>）を<strong data-astro-cid-curm2vkq>あなたの GitHub に複製</strong>して動いています。複製が古いと更新の取り込み部分が旧版のままになります。<strong data-astro-cid-curm2vkq>フォークを1回同期</strong>すれば修正版が入り、以降は自動で反映されるようになります。<strong data-astro-cid-curm2vkq>同じ Worker の再ビルドのためデータ・設定は保持</strong>されます（Deploy ボタンの押し直しはしないでください）。
    ...
    if (syncF) syncF.addEventListener("click", () => {
      let v = (document.getElementById("gh-owner").value || "").trim().replace(/^@/, "");
      if (!v) { window.bo.toast("GitHub のユーザー名または組織名を入力してください", "err"); return; }
      const repo = syncF.dataset.repo || "baku-office-app";
      // 入力を GitHub 同一オリジンのパスへ正規化（ユーザー名のみ→/user/repo、owner/repo、フルURLのいずれも受ける）。
      let path;
      if (/^https?:/i.test(v)) { try { path = new URL(v).pathname.replace(/\\/+$/, ""); } catch { path = ""; } }
      else if (v.includes("/")) { path = "/" + v.replace(/^\\/+/, "").replace(/\\/+$/, ""); }
      else { path = "/" + v + "/" + repo; }
      if (!path || path === "/") { window.bo.toast("ユーザー名を確認してください", "err"); return; }
      // 未ログインでもログイン後に該当フォークへ遷移させる（return_to は同一オリジンのパスのみ許可される）。
      window.open("https://github.com/login?return_to=" + encodeURIComponent(path), "_blank", "noopener");
    });
