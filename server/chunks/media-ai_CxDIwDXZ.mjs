...
import { getFile, saveFile } from "./storage_BU1BbtLn.mjs";
import { n as nowSec } from "./accounting_CtvChRxg.mjs";
import { recordUsage, recordTokens, overBudget } from "./usage_C0o2RuAe.mjs";
import { claudeModelId, geminiModelId } from "./config_dJ2QM2XC.mjs";
...
${prompt}` : prompt;
  ...
  if (gkey && await overBudget(env, "gemini") === "ok") {
    ...
    const parts = [{ text: full }, ...atts.map((a) => ({ inlineData: { mimeType: a.mime, data: toB64(a.buf) } }))];
    return geminiGenerate(env, gkey, parts);
  }
...
${new TextDecoder().decode(a.buf).slice(0, 1e5)}` });
    const content = blocks.length ? [...blocks, { type: "text", text: full }] : full;
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "content-type": "application/json", "x-api-key": ckey, "anthropic-version": "2023-06-01" },
      body: JSON.stringify({ model: claudeModelId(env), max_tokens: opts.maxTokens ?? 4e3, ...opts.system ? { system: opts.system } : {}, messages: [{ role: "user", content }] })
    });
    if (!r.ok) {
      console.log("[infer-claude]", r.status);
      return "";
    }
    const d = await r.json();
