...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_C9_qjgRP.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_DWaRDBDI.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_Co5n8Ezg.mjs");
      ...
      const { getMemberModel, parseRequestModel } = await import("./settings_DHOyCLdr.mjs");
      const { modelId: siteModelId } = parseRequestModel(await getMemberModel(env, ses.uid).catch(() => null) ?? "");
      const { startSiteBuild, processSiteBuild } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.S);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.S);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_C9_qjgRP.mjs");
