...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_runN5xu5.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DT4CiOpW.mjs");
  ...
  const { hasApiKey } = await import("./client_runN5xu5.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BZM8Ok-T.mjs");
