...
import { getToken, hostFetch, getVerifyJwk } from "./client_B17j1uG_.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_Bmz9f5Zb.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, rewriteFabricatedDateSlots, rewriteBareCountSlots, repairMisplacedCreateTemplate, deriveConfigFieldsFromUsage, validateDefinition, isBlockedHost } from "./appdef_Cmp4V_Cj.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_CVjeTeB8.mjs";
const MAX_TAGS = 8;
...
async function fetchAndInstall(ctx, id) {
  ...
  const existingApp = await ctx.db.first("SELECT source FROM external_apps WHERE id=?", [p.id]).catch(() => null);
  if (existingApp && existingApp.source !== "store") {
    return { ok: false, error: "このIDは既にこの団体の別アプリで使われています。安全のためストアアプリの取込を中止しました（同名の自作アプリがある場合は、そのアプリの名前を変えるか開発元へご連絡ください）。" };
  }
  await ctx.db.run(
...
async function listExternalApps(ctx) {
  const results = await ctx.db.all("SELECT id,name,version,category,description,permissions,source,allow_fork,allowed_roles,forked_from_id,forked_from_name,forked_from_version,json_extract(definition,'$.configFields') AS config_fields FROM external_apps ORDER BY installed_at DESC");
  return results.map((r) => ({ ...r, permissions: JSON.parse(r.permissions || "[]"), allow_fork: r.allow_fork || 0, allowed_roles: (() => {
    ...
    if (!r.config_fields) return [];
    try {
      const a = JSON.parse(r.config_fields);
...
async function createDraft(ctx, d, by) {
  if (d.definition && typeof d.definition === "object") {
    ...
    rewriteFabricatedDateSlots(d.definition);
    rewriteBareCountSlots(d.definition);
    repairMisplacedCreateTemplate(d.definition);
    deriveConfigFieldsFromUsage(d.definition);
  }
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_Cmp4V_Cj.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_w6xNIQ3l.mjs");
  ...
  try {
    if (d.definition) {
      const { rebuildPublicPageFromDef } = await import("./public-pages_Bg1BWixA.mjs");
...
async function deleteLocalApp(ctx, appId, deleteData = false) {
  ...
  await (await import("./public-pages_Bg1BWixA.mjs")).deletePublicPagesByApp(ctx.env, appId).catch(() => {
  });
...
async function purgeStaleDrafts(ctx) {
  const r = await ctx.db.run(
    `DELETE FROM app_drafts WHERE id NOT IN (SELECT id FROM external_apps) AND (
       gate_status='blocked'
       OR created_at < (SELECT MAX(d2.created_at) FROM app_drafts d2 WHERE d2.name = app_drafts.name AND d2.id NOT IN (SELECT id FROM external_apps))
     )`,
    []
  );
...
export {
  ...
  getAppDesign as g,
  purgeStaleDrafts as h,
  installLocalApp as i,
  appVersions as j,
  activateAppVersion as k,
  listDrafts as l,
  deleteLocalApp as m,
  appsBrief as n,
  createDraft as o,
  proposeUpstreamMerge as p,
  repairOutputLiterals as q,
  roleCanUseApp as r,
  submitDraft as s,
  getAppMeta as t,
  ...
  setAppMeta as v,
  suggestAppMeta as w,
  pendingRelaxedHosts as x,
  isRegisteredPart as y,
  setAppAllowedRoles as z
};
