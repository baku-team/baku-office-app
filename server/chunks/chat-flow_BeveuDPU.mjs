...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_BcmxKHWi.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BTO_5scg.mjs";
import { canDevelopApps } from "./auth_DrSavdul.mjs";
import { cachedEntitlement } from "./client_BNXMhaMG.mjs";
...
${txt}` };
  ...
  if (mime.startsWith("image/") || mime === "application/pdf") {
    ...
    const { inferApp } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.Q);
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, parseDeleteConfirm, DELETE_CONFIRM_MARK } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.X);
  const del = parseDeleteConfirm(message);
  const wantsDelete = looksLikeAppDelete(message);
  if (!wantsDelete && !del.confirm) return null;
  const { latestSessionApp } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.V);
  const appId = del.confirm && del.appId ? del.appId : await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CKPNCc45.mjs").then((n) => n.D);
...
async function tryPreAgentRouting(ctx, cfContext, args) {
  ...
  if (notPlan && canDevelopApps(role) && atLeast(await cachedEntitlement(env), "pro")) {
    ...
    if (idm || continueWord) {
      const { latestSessionApp, startAppEdit, kickBuild, buildModelGuide } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.V);
  ...
  const { looksLikeBuildConfirmation, looksLikeUiModeChoice } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.X);
  if (notPlan && canDevelopApps(role) && looksLikeBuildConfirmation(message, priorAssistant)) {
    const { startAppBuild, kickBuild, buildModelGuide, resolveAppByName } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.V);
    const { parseBuildMarks, BUILD_MARKS, estimateBuildScale, extractFeatureBullets } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.X);
...
※業務が独立する場合は、完成後に「別の新しい◯◯アプリを作って」で別アプリに分けられます。`;
    ...
    if (marks.del) {
      ...
      if (delId) {
        const { deleteDraft } = await import("./external-apps_CKPNCc45.mjs").then((n) => n.D);
  ...
  if (notPlan && canDevelopApps(role) && atLeast(await cachedEntitlement(env), "pro")) {
    const { looksLikeAppEdit, looksLikeNewAppRequest, looksLikeStrongEditRequest, looksLikeSeparateNewApp } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.X);
    if (looksLikeAppEdit(message)) {
      const { latestSessionApp, resolveAppByName, startAppEdit, kickBuild, buildModelGuide } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.V);
...
async function tryProHopsContinuation(ctx, cfContext, args) {
  ...
  const { enqueueAgentJob, kickAgentJob } = await import("./agent-jobs_CbaDOIPv.mjs");
...
async function finalizeAssistantReply(ctx, args) {
  ...
  if (confirmIdx >= 0) {
    try {
      const { estimateBuildScale, BUILD_MARKS } = await import("./ctx_BZ9IFLfW.mjs").then((n) => n.X);
      const { estimateBuildStart, formatBuildEstimate } = await import("./build-estimate_CemRpS8h.mjs");
      const { getWorkersPaid } = await import("./settings_C4tL9AHR.mjs");
      const paid = await getWorkersPaid(env).catch(() => false);
