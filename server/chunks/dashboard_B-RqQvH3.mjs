...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DSiKbs27.mjs");
  const { cmpVersion } = await import("./update_CSyiNJl8.mjs");
  await import("./index_BehDur4O.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Dnwa3KfN.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DpFK7cn8.mjs");
  ...
  const { reviewQueue } = await import("./users_C-MGuiyE.mjs");
  const { backupAlert } = await import("./backup_ByNaem6z.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_JQBtFvEn.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_COf58ej6.mjs");
