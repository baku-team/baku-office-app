...
import { AppError, explainStop } from "./errors_Bw4GoHz5.mjs";
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, fileBelongsTo, storageMode } from "./storage_aIMHFm6C.mjs";
import { getApiKey, getToken, hostFetch, entitlementForGate, masterKeyCtx } from "./client_BCrexcwr.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification } from "./notifications_CdoRv0_M.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_Ck_fNAQ4.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_AfFtxawQ.mjs";
import "./index_C75Q_n4c.mjs";
import { recordUsage, recordTokens, overBudget, estimateUsd } from "./usage_gjxCIj9B.mjs";
...
import { googleFetch } from "./google_SQBWdaD4.mjs";
...
import { validateDefinition, APP_SCHEMA, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, RENDER_HTML_MAX, opCatalogText } from "./appdef_BJ9kCwMY.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dutk61oa.mjs";
import { getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, resolveModelSelection, getMemberModel, getWorkersAiModel, maxParallelAgents, agentMaxHops } from "./settings_CBzcH790.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C-JKkZIZ.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_NFB7E335.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BKibubDI.mjs";
import { logDiag } from "./diag_BS4K6Q4H.mjs";
import { i as isRunnableDefinition } from "./preflight_3Vuphe9D.mjs";
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage, collectionFromDefinition } from "./public-pages_BgLGyBVf.mjs";
import { g as getAppDesign, m as repairOutputLiterals, n as createDraft, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, i as installLocalApp, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_Ish93S4w.mjs";
import { a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { a as resolveChannel } from "./connectors-store_CHWkgRyl.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_RF-0odTN.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_BZEpu5BE.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CUSmsEak.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_1OyiTkBt.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { p as pushToUser } from "./push_38DKr_B_.mjs";
...
const DEFAULT_PROVIDER_REGISTRY = {
  version: 1,
  providers: [
    { id: "claude", label: "Claude", rank: 1, reasoning: 5, speed: 3, cost: "premium", free: false, trains: false, caps: { tools: true, vision: true, audioIn: false, longContext: true }, note: "最高精度・道具/画像対応・API学習なし" },
    { id: "gemini", label: "Gemini", rank: 2, reasoning: 4, speed: 4, cost: "cheap", free: true, trains: true, caps: { tools: true, vision: true, audioIn: true, longContext: true }, note: "画像/音声/長文対応・無料枠（無料層は学習利用）" },
    { id: "openai", label: "ChatGPT(OpenAI)", rank: 3, reasoning: 4, speed: 3, cost: "mid", free: false, trains: false, caps: { tools: true, vision: true, audioIn: false, longContext: true }, note: "道具/画像対応・API学習なし" },
    { id: "grok", label: "Grok", rank: 4, reasoning: 4, speed: 4, cost: "premium", free: false, trains: false, caps: { tools: true, vision: true, audioIn: false, longContext: true } },
    { id: "github_models", label: "GitHub Models", rank: 5, reasoning: 4, speed: 3, cost: "free", free: true, trains: true, caps: { tools: true, vision: true, audioIn: false, longContext: true }, note: "無料枠（既定で学習利用・要オプトアウト）" },
    { id: "groq", label: "Groq", rank: 6, reasoning: 3, speed: 5, cost: "free", free: true, trains: false, caps: { tools: true, vision: false, audioIn: false, longContext: false }, note: "無料・超高速・学習なし" },
    { id: "cerebras", label: "Cerebras", rank: 7, reasoning: 3, speed: 5, cost: "free", free: true, trains: false, caps: { tools: true, vision: false, audioIn: false, longContext: false }, note: "無料・超高速・学習なし" },
    { id: "workers_ai", label: "Workers AI", rank: 8, reasoning: 2, speed: 4, cost: "free", free: true, trains: false, caps: { tools: false, vision: false, audioIn: false, longContext: false }, note: "CF内蔵・無料の最後の砦・道具/画像は不向き" }
  ]
};
  ...
  if (!key) {
    ...
    if (stuck.length) {
      ...
      for (const r of stuck) {
        ...
        await addNotification(ctx, { owner: r.owner, kind: "summary", body: "資料の要約には Gemini APIキーが必要です（設定→APIキー）。" }).catch(() => {
        });
      }
    }
    return 0;
...
function claudeModel(key, modelId = DEFAULT_MODELS.claude) {
  return {
    ...
    async turn(system, history, tools, force, opts) {
      ...
      const body = { model: modelId, max_tokens: opts?.maxTokens ?? (force ? 4e3 : 1500), system: sys, tools: t, ...toolChoice ? { tool_choice: toolChoice } : {}, messages: toMessages$2(history) };
...
const NOTIFY_PURPOSES = [
  ...
  { id: CH_MANUAL, label: "AIからの手動送信（Discord等）" }
];
...
async function editStep(ctx, row, model) {
  ...
  if (!design || !design.definition || typeof design.definition !== "object") {
    await failBuild(ctx, row, explainStop("system", "修正対象のアプリが見つからないか、定義が壊れています。", "アプリ画面（/apps）で対象アプリが存在するか確認し、無ければ作り直してください。"), "no-target");
    return false;
  }
  if (row.status === "planning") {
    const curDef = design.definition;
    await post(ctx, row, `🔍 「${design.name}」を確認し、修正計画を立てています…`);
    ...
    await post(ctx, row, editPlanMessage(design.name, phases2));
    return true;
  }
  const plan = JSON.parse(row.plan ?? "{}");
  const phases = plan.phases ?? [];
  const def = row.definition ? JSON.parse(row.definition) : design.definition;
  if (row.cursor >= phases.length || phases.length === 0) return finalizeEdit(ctx, row, model, design, def, phases);
...
【厳守】説明文・断り文は一切返さず、有効なJSONパッチ1個だけを返す。定義は作り直さず render.html への htmlEdits（find/replace）で、該当箇所を必要な数だけ列挙する。サイズ・文字数を理由に断らないこと。`;
  ...
  await post(ctx, row, head + label + (phaseNote ? `
${phaseNote}` : ""));
  await ctx.db.run(
...
async function finalizeEdit(ctx, row, model, design, def, phases) {
  const env = ctx.env;
...
検証エラー：${issueText}`;
  ...
  if (!vr.ok) {
    await failBuild(ctx, row, explainStop("ai", "修正後の定義が検証に通りませんでした（" + vr.issues.slice(0, 2).map((i) => i.message).join(" / ") + "）。", "対象を「この画面のこの項目」のように絞って再依頼してください。別のAIモデルへの切替も有効です。"), "validate");
    return false;
  }
  ...
  if (noChange) {
    ...
    await ctx.db.run("UPDATE app_builds SET status='done', stop_reason='nochange', error=?, updated_at=? WHERE id=?", [msg.slice(0, 500), nowSec(), row.id]);
    return false;
  }
  const uiCheck = await verifyAndRepairUi(ctx, model, def, row.paid === 1, (m) => post(ctx, row, m));
  def = uiCheck.def;
  def.permissions = [.../* @__PURE__ */ new Set([...Array.isArray(def.permissions) ? def.permissions : [], ...vr.requiredPermissions])];
  const res = await createDraft(ctx, { name: design.name, description: design.spec ?? void 0, permissions: def.permissions, definition: def, version: String(def.version), role: "admin", changelog: `修正（${doneCount}/${phases.length}工程）` }, row.owner);
  let applied = false;
  if (res.gate === "ready" && res.id) {
    const inst = await installLocalApp(ctx, res.id, row.owner).catch(() => ({ ok: false }));
    applied = !!inst.ok;
...
※ 未適用の工程：${failed.join("・")}（もう一度同じ依頼で続けて直せます）` : "") + (applied ? "\n✅ アプリに即時反映しました（画面を再読み込みすると反映が見えます）。" : res.gate === "ready" ? "\nプレビューで確認し、「アプリ」画面の「アプリ開発」で登録すると反映されます。" : "\n※ 事前確認に課題があります。内容をご確認ください。");
  ...
  await ctx.db.run("UPDATE app_builds SET status=?, stop_reason=?, error=?, updated_at=? WHERE id=?", [editStatus, editReason, finalMsg.slice(0, 500), nowSec(), row.id]);
  return false;
}
...
async function stepBuild(ctx, row, model, paid) {
  ...
  if (row.status === "planning") {
    ...
    if (!plan2) {
      await failBuild(ctx, row, explainStop("ai", "AIが実装プランを生成できませんでした。", "要件をもう少し具体化して再度ご依頼ください。別のAIモデルへの切替も有効です。"), "no-plan");
      return false;
    }
  ...
  const phases = plan.phases ?? [];
  if (row.cursor >= phases.length) return finalize(ctx, row, plan, def, model);
  const phase = phases[row.cursor];
...
async function verifyAndRepairUi(ctx, model, def, paid, onStep, maxPass = 3) {
  const env = ctx.env;
...
async function finalize(ctx, row, plan, def, model) {
  const env = ctx.env;
  repairOutputLiterals(def);
  if (!isRunnableDefinition(def)) {
    await failBuild(ctx, row, explainStop("ai", "各工程は実装しましたが、AIが実行可能な形にまとめきれませんでした。", "要件を分けて（機能・画面ごとに）再度ご依頼ください。"), "validate");
    return false;
  }
  const uiCheck = await verifyAndRepairUi(ctx, model, def, row.paid === 1, (m) => post(ctx, row, m));
  def = uiCheck.def;
  ...
  const won = await ctx.db.run("UPDATE app_builds SET status=?, stop_reason=?, updated_at=? WHERE id=? AND status IN ('planning','building')", [buildStatus, buildReason, nowSec(), row.id]);
...
async function rebuildPublicPage(ctx, slug) {
  const design = await getAppDesign(ctx, slug);
  if (!design || !design.definition || typeof design.definition !== "object") return { ok: false, error: "アプリの定義が見つかりません。" };
  return rebuildPublicPageFromDef(ctx.env, slug, design.definition, design.name);
}
...
async function failBuild(ctx, row, msg, reason = "system") {
  await post(ctx, row, /^\s*⚠/.test(msg) ? msg : "⚠️ " + msg, true);
  await ctx.db.run("UPDATE app_builds SET status='error', stop_reason=?, error=?, updated_at=? WHERE id=?", [reason, msg.slice(0, 500), nowSec(), row.id]);
}
...
async function ensureSkeletonDraft(ctx, row) {
  try {
    if (row.app_id) {
      const existing = await getAppDesign(ctx, row.app_id).catch(() => null);
    ...
    await createDraft(ctx, { name, description: (row.spec ?? "").slice(0, 80) || name, spec: row.spec ?? void 0, permissions: [], definition: def, role: "admin", changelog: "最小スケルトン（作成中断のため・続きから再開可）" }, row.owner).catch(() => {
    });
  } catch {
  }
}
async function post(ctx, row, text, notify = false, actions) {
  if (row.session_id) {
    ...
    if (!(last && last.role === "assistant" && last.content === text)) {
      await appendMessage(ctx, row.session_id, "assistant", text, actions).catch(() => {
      });
    }
  }
  if (notify) {
...
async function processAppBuilds(ctx, baseUrl = "", limit = 2, opts) {
  const env = ctx.env;
  ...
  for (const row of rows) {
    const more = await runBatch(ctx, row, paid, baseUrl, opts);
    processed++;
...
async function runBatch(ctx, row, paid, baseUrl, opts) {
  ...
  if (nowSec() - row.created_at > MAX_BUILD_AGE) {
    await ensureSkeletonDraft(ctx, row);
    await failBuild(ctx, row, explainStop("ai", "実装が長引き、制限時間に達したため中断しました（これまでの工程は草案に保存済み）。", "要件を分けて（機能・画面ごとに）再度ご依頼ください。Workers Paid 有効化で一度に長く処理できます（設定→高度なオプション /settings/advanced）。"), "age");
    return false;
  }
  if (row.attempts >= MAX_BUILD_ATTEMPTS) {
    await ensureSkeletonDraft(ctx, row);
    await failBuild(ctx, row, explainStop("system", "内部処理が繰り返し失敗したため中断しました（これまでの工程は草案に保存済み）。", "要件を分けて再度ご依頼ください。続く場合は管理者へご連絡ください。"), "attempts");
    return false;
  }
  const model = await buildChatModel(ctx.env, row.model ?? void 0);
  if (!model) {
    await failBuild(ctx, row, explainStop("system", "AIモデルが未設定のため実装できません。", "管理者が『設定→連携 /settings/messaging』で Gemini / Claude / ChatGPT のAPIキーを登録してください。"), "model-error");
    return false;
  }
  ...
  for (let i = 0; i < budget && active; i++) {
    ...
    active = await stepBuild(ctx, row, model, paid).catch(async (e) => {
      ...
      await logDiag(ctx.env, "warn", "build", `step throw: ${emsg}`).catch(() => {
      });
      ...
      await ctx.db.run("UPDATE app_builds SET attempts=attempts+?, updated_at=? WHERE id=?", [inc, nowSec(), row.id]).catch(() => {
      });
      return true;
    });
  }
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "remember_fact": {
      ...
      const r = await rememberUserFact(ctx, owner, String(args.fact ?? ""));
      return r.ok ? `覚えました${r.note}。次回以降の会話でも思い出します。` : "記憶する内容がありませんでした。";
    }
    case "forget_fact": {
      const { forgetUserFact } = await import("./memory_CYyQ4i1p.mjs");
      const r = await forgetUserFact(ctx, owner, String(args.query ?? ""));
    ...
    case "send_notice": {
      ...
      if (channel === "email") {
        ...
        const r = await ctx.notify.email(to, subject, message);
      ...
      if (channel === "discord") {
        ...
        const r = await sendToChannel2(ctx, CH_MANUAL2, { text: message });
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BCrexcwr.mjs");
  ...
  const memories = await (await import("./memory_CYyQ4i1p.mjs")).recallUserMemory(ctx, owner).catch(() => []);
...
${custom}`].filter(Boolean).join("\n");
  ...
  if (!want) {
    ...
    const topId = topAvailable(await getProviderRegistry(env), availableIds);
  ...
  if (want === "grok" && grokKey) {
    if (await overBudget(env, "grok") === "pause") return "Grok の今月の利用上限に達しました（設定 → 使用量・上限 で変更できます）。";
    await recordUsage(env, "grok");
    model = grokModel(grokKey, grokId);
    provider = "grok";
  } else if (want === "github_models" && githubModelsKey) {
    if (await overBudget(env, "github_models") === "pause") return "GitHub Models の今月の上限に達しました（設定 → 使用量・上限 で変更できます）。";
    await recordUsage(env, "github_models");
    model = githubModelsModel(githubModelsKey, githubId);
    provider = "github_models";
  } else if (want === "groq" && groqKey) {
    if (await overBudget(env, "groq") === "pause") return "Groq の今月の上限に達しました（設定 → 使用量・上限 で変更できます）。";
    await recordUsage(env, "groq");
    model = groqModel(groqKey, groqId);
    provider = "groq";
  } else if (want === "cerebras" && cerebrasKey) {
    if (await overBudget(env, "cerebras") === "pause") return "Cerebras の今月の上限に達しました（設定 → 使用量・上限 で変更できます）。";
    await recordUsage(env, "cerebras");
    model = cerebrasModel(cerebrasKey, cerebrasId);
    provider = "cerebras";
  } else if (wantLocal && env.AI) {
    ...
    const useClaude = !!claudeKey && (prefer === "claude" || prefer !== "openai" && prefer !== "gemini" && !geminiKey && !openaiKey);
    ...
    if (!model && githubModelsKey && !image) {
      await recordUsage(env, "github_models");
      model = githubModelsModel(githubModelsKey, githubId);
      provider = "github_models";
    }
    if (!model && groqKey && !image) {
      await recordUsage(env, "groq");
      model = groqModel(groqKey, groqId);
      provider = "groq";
    }
    if (!model && cerebrasKey && !image) {
      await recordUsage(env, "cerebras");
      model = cerebrasModel(cerebrasKey, cerebrasId);
      provider = "cerebras";
    }
    if (!model) return "選択中のエンジンが未設定です。『設定 → 連携設定』で Gemini / Claude / OpenAI / Grok / GitHub Models / Groq / Cerebras のAPIキーを登録してください。";
  }
  let fellBack = false;
  let switchErr;
...
${PLAN_NOTE}` : "");
  ...
  if (fellBack) {
    ...
    return reason + " 会計登録・検索などのツール操作は一時的に行えません。\n\n" + out;
  }
...
${reqText}` }], [], void 0, { maxTokens: 8e3 }).catch((e) => ({ error: { message: String(e?.message ?? e) } }));
    ...
    if (isRunnableDefinition(def)) {
      const uiFix = await verifyAndRepairUi(ctx, synthModel, def, false, void 0, 1).catch(() => null);
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。");
    ...
    try {
      ...
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_CBzcH790.mjs");
      const paid = await getWorkersPaid2(env).catch(() => false);
      await startAppBuild2(ctx, { owner, sessionId: opts.sessionId, spec: reqText, model: opts.modelId, paid });
...
function cfAgent(ctx) {
  return {
    run: (i) => runAgent(ctx, i.owner, i.text, i.image, i.baseUrl ?? "", i.role ?? "member", { history: i.history, model: i.model, modelId: i.modelId, sessionId: i.sessionId, onEvent: i.onEvent, signal: i.signal, mode: i.mode })
  };
}
...
function buildCtx(env) {
  const ctx = { profile: detectProfile(env).id, env, db: cfSqlStore(env), storage: cfStorage(env), ai: cfAi(env), google: cfGoogle(env), notify: cfNotify(env), messaging: cfMessaging(env), knowledge: cfKnowledge(env), egress: { fetch: (req) => cfEgressGateway(env).appFetch(req) } };
