...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CyRmMA6L.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_8JOsEvWt.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CxiQNFHb.mjs");
