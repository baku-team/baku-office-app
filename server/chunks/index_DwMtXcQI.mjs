...
import { $ as $$MoneyTabs } from "./MoneyTabs_Bb4ABon6.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Index;
  const { getSession, canAccess } = await import("./auth_B043h97J.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  if (!canAccess(ses.role, "accounting")) return Astro2.redirect("/forbidden", 302);
  const acc = await import("./accounting_CtvChRxg.mjs").then((n) => n.j);
  ...
  const { getBookkeepingMode } = await import("./settings_BgFAT_Wp.mjs");
  const mode = await getBookkeepingMode(env);
  const acctItems = await import("./accounting_CtvChRxg.mjs").then((n) => n.i);
  const WALLET_TYPES = acctItems.WALLET_TYPES;
  const walletTypeLabel = (t) => WALLET_TYPES.find((w) => w.type === t)?.label ?? t;
  const jr = await import("./journal_DVyUoOj8.mjs");
  ...
  const fa = await import("./fixed-assets_12fkz9WC.mjs");
  ...
  const reg = await import("./register_Hmgh0CDo.mjs");
