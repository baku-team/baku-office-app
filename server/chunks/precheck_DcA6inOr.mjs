...
import { validateDefinition } from "./appdef_0geZcejn.mjs";
import { g as getAppDesign } from "./external-apps_BTgdbc5l.mjs";
import { p as preflight } from "./preflight_rNRXjiKE.mjs";
import { r as runDraftApp } from "./app-runtime_D0ttewuz.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_BTgdbc5l.mjs").then((n) => n.z)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BRkvvUfW.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
