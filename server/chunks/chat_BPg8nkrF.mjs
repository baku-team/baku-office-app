...
import { cachedEntitlement } from "./client_BV1rSndT.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  let visionImage;
  if (b.image?.dataB64) {
    const { prepareAttachment } = await import("./chat-flow_DCDaQmCn.mjs");
  ...
  if (b.background) {
    ...
    const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_C3lEsTfq.mjs");
  ...
  }
  {
    ...
    const { looksLikeBuildConfirmation, looksLikeUiModeChoice, UI_MODE_QUESTION, UI_MODE_ACTIONS } = await import("./ctx_CfcRI8wU.mjs").then((n) => n.T);
    ...
    if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant)) {
      ...
      const { startAppBuild, processAppBuild } = await import("./ctx_CfcRI8wU.mjs").then((n) => n.R);
  ...
  if (b.mode !== "plan" && !visionImage) {
    const { tryHandleAppDelete } = await import("./chat-flow_DCDaQmCn.mjs");
  ...
  }
  {
    ...
    if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
      const { looksLikeAppEdit } = await import("./ctx_CfcRI8wU.mjs").then((n) => n.T);
      if (looksLikeAppEdit(message)) {
        const { latestSessionApp, resolveAppByName, startAppEdit, processAppBuild } = await import("./ctx_CfcRI8wU.mjs").then((n) => n.R);
  ...
  const { buildReplyActions } = await import("./chat-flow_DCDaQmCn.mjs");
