...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BjKlgqy9.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_QQysPA0e.mjs");
  await import("./index_NXtqd6gx.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_X5x_Eyo3.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CE-bwhfl.mjs");
  ...
  const { reviewQueue } = await import("./users_DRz6Swln.mjs");
  const { backupAlert } = await import("./backup_oUcOtZSh.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_xzpbaRmZ.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_BlqFC5PS.mjs");
