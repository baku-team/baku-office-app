...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DE3oxZSo.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CzHcRT_f.mjs");
  await import("./index_CXDHY44j.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CRTeJApz.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BIoVPjl4.mjs");
  ...
  const { reviewQueue } = await import("./users_BaBNFGXy.mjs");
  const { backupAlert } = await import("./backup_DIpM-7-b.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_qrcuL6Wt.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_DJmKJuaK.mjs");
