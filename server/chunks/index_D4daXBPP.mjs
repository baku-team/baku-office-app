...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_icI3Uf5x.mjs");
  const { cmpVersion } = await import("./update_BxmxXg4Z.mjs");
  await import("./index_DymXVEW2.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bd5lhL2K.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_T8hrbjzD.mjs");
  ...
  const { reviewQueue } = await import("./users_CulnRmNt.mjs");
  const { backupAlert } = await import("./backup_DBYLEv4j.mjs");
