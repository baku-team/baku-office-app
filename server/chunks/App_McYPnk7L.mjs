...
import { f as createRenderInstruction, a as addAttribute, r as renderTemplate, b as renderSlot, F as Fragment, c as renderHead, u as unescapeHTML } from "./sequence_2tuU57vh.mjs";
...
const $$App = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const navIcon = { "/dashboard": "dash", "/apps": "grid", "/settings": "gear", "/my-events": "home" };
  ...
  const [themeMod, navMod, diagMod, notifMod, clientMod] = await Promise.all([
    ...
    import("./diag_B7sefss8.mjs"),
    import("./notifications_CdoRv0_M.mjs"),
    import("./client_mgVrvX2Z.mjs")
  ]);
  const [theme, navOv, limitErr, unreadNotif, entitlement, hasClaude, hasGemini, recentRaw, favRaw] = await Promise.all([
    ...
    ses && !isGuest ? import("./chat-sessions_Bk4GZRxh.mjs").then((n) => n.f).then((m) => m.listSessions(ctx, ses.uid)).then((r) => r.slice(0, 6)).catch(() => []) : Promise.resolve([]),
    ses && !isGuest ? Promise.all([
      import("./settings_BGnJ4d3n.mjs").then((m) => m.getFavApps(env, ses.uid)).catch(() => []),
      import("./external-apps_WS9MhPJy.mjs").then((n) => n.y).then((m) => m.installedAppDefs(ctx, ses.role)).catch(() => [])
