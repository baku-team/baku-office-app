...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_DLxGrjcM.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_CzJPn-cz.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_CNJN-IF9.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.V);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.V);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_DLxGrjcM.mjs");
