...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CJh_mVEi.mjs");
  const { cmpVersion } = await import("./update_Dy9UGrY-.mjs");
  await import("./index_Cos_vIXW.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_8YmkCTHi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dm60WfRn.mjs");
  ...
  const { reviewQueue } = await import("./users_CF00ZgaD.mjs");
  const { backupAlert } = await import("./backup_fGP2rkSZ.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DO3nkheO.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_cf3JyuGL.mjs");
