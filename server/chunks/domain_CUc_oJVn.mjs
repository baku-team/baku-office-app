...
const $$Domain = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getCustomDomain } = await import("./custom-domain_BlEKJqw-.mjs");
  ...
  const cfTokenReady = cfg ? await (await import("./autonomy_YYoCxNfB.mjs")).getAutonomyConfig(env).then((a) => a.cfToken).catch(() => false) : false;
