...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_sRscislI.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DUnAdmMv.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DMuH0rYg.mjs");
