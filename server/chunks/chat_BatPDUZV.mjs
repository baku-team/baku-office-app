...
import { r as renderTemplate, m as maybeRenderHead, F as Fragment, a as addAttribute, u as unescapeHTML } from "./sequence_I_kcixDX.mjs";
...
const $$Chat = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, hasApiKey } = await import("./client_BbGjOLTN.mjs");
  ...
  const { listSkills } = await import("./skills_juoW6Oiz.mjs");
  const skills = hasPlus && isOrgAdmin ? await listSkills(env).catch(() => []) : [];
  await import("./index_BvPbBcDJ.mjs");
  const { enabledParts, enabledPartIds } = await import("./parts_D1i9CXVc.mjs").then((n) => n.f);
  const { disabledBuiltins } = await import("./client_BbGjOLTN.mjs");
