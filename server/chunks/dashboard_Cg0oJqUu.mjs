...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_B1mWWadE.mjs");
  const { cmpVersion } = await import("./update_DnAqA7ZN.mjs");
  await import("./index_Cz_s9PCZ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BfOqoIdF.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CHEb15ny.mjs");
  ...
  const { reviewQueue } = await import("./users_DJZEQd2x.mjs");
  const { backupAlert } = await import("./backup_TEHB8u5R.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BHSE11sT.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_BGXYQVtj.mjs");
