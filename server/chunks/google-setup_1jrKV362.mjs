...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_TofNzP0i.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_D1D8V9ss.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_Cg3mGgJJ.mjs");
  ...
  const { buildCtx } = await import("./ctx_-pDHDD3Z.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_-pDHDD3Z.mjs").then((n) => n.M);
