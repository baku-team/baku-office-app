...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_kiIL6iv0.mjs");
  const { cmpVersion } = await import("./update_DXphwmlH.mjs");
  await import("./index_BMK2adnV.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_GjR2Wx6H.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C67dzqMq.mjs");
  ...
  const { reviewQueue } = await import("./users_DYcPALfj.mjs");
  const { backupAlert } = await import("./backup_CzN37oUC.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B8q51KTm.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DhH-wMJZ.mjs");
