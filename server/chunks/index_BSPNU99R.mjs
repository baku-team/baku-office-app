...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DuhUR81K.mjs");
  const { cmpVersion } = await import("./update_BcbHRxvW.mjs");
  await import("./index_CFlgHdOa.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_D9v53KLK.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C_tYdv-t.mjs");
  ...
  const { reviewQueue } = await import("./users_DNi8qC2-.mjs");
  const { backupAlert } = await import("./backup_zauSLR3z.mjs");
