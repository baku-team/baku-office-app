...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_FnSEomav.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_FnSEomav.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_DBX5cnQS.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_DdrEUTKx.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D7U8yzuf.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_vCtql7Lf.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CjQOD0Ds.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BmrnNc4T.mjs");
