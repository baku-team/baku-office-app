...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_-spfSM1I.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CJF3PIpc.mjs");
  await import("./index_B-J9Qckz.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BYed8Rw1.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_hg012NSQ.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_DEN7iKIl.mjs");
  ...
  const { reviewQueue } = await import("./users_COVMctXM.mjs");
  const { backupAlert } = await import("./backup_D4wY_wsc.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Bb659Sv7.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DD2sf3yv.mjs");
  const { getWorkersPaid } = await import("./settings_C_1axsCz.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_D0PTUXDo.mjs");
