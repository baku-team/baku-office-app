...
import { getApproval, decideApproval } from "./approvals_DzwE9jZU.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_4L0ewUEU.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_CrtWLNXo.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Dfr4Wlvv.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_D0aobTMW.mjs";
import { joinWithInvite } from "./users_CI6JXwVH.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_CzJPn-cz.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CI6JXwVH.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_D73IfWbd.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_C5P8wVrB.mjs");
