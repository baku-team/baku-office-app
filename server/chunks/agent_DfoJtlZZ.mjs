...
import { entitlementForGate, getApiKey } from "./client_C23diL6K.mjs";
...
import { permissionCatalogText } from "./apps_BVCI0C7p.mjs";
import { A as APP_SCHEMA, o as opCatalogText, j as createDraft, k as installedAppDefs } from "./external-apps_D4vbFip2.mjs";
import { getAiEngine, getCustomPrompt, resolveModelSelection, getMemberModel, getWorkersAiModel, maxParallelAgents, agentMaxHops } from "./settings_BgFAT_Wp.mjs";
import { callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CIMpQUE-.mjs";
import { searchDirectory } from "./directory_DUKlfNyA.mjs";
import { autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_CdESEoWA.mjs";
import { DEFAULT_MODELS, isValidWorkersAiModel, workersAiModelId, isValidClaudeModel, claudeModelId, isValidGeminiModel, geminiModelId } from "./config_dJ2QM2XC.mjs";
import "./index_ByOZSNRu.mjs";
import { m as makeDocument, w as webSearch } from "./media-ai_BMVK6MY_.mjs";
import { runSkill, generateSkill, listSkills } from "./skills_DeM1szYf.mjs";
import { r as runInstalledApp } from "./app-runtime_0C0V7y4e.mjs";
import { videoStatusText, invokeCapability, listCapabilities, capabilitySummary } from "./capabilities_P2gLs6_q.mjs";
import { overBudget, recordUsage, recordTokens, estimateUsd } from "./usage_C0o2RuAe.mjs";
import { getApprovalMode, previewFor, createApproval, A2A_OUTWARD } from "./approvals_BR5C_fTn.mjs";
...
const CORE_TOOLS = [
  ...
  { name: "propose_app", description: "アプリ（業務機能）の草案を作成。まず企画・仕様(spec)をまとめ、要求権限・推定トークンを添えて呼ぶ。保存時に実装前の事前確認（環境/権限/安全/コスト）を自動実行し、全て問題なければ実装可となる。", parameters: { type: "object", properties: { name: { type: "string" }, description: { type: "string" }, spec: { type: "string", description: "企画・仕様（目的・データ・操作・画面・想定利用）" }, permissions: { type: "array", items: { type: "string" }, description: `要求権限。利用可能なのは次の8種だけで、これ以外（file/fs/upload 等の独自名）は宣言不可。ファイルの受け取り・保存が要るアプリは storage:read / storage:write を宣言する。一覧：${permissionCatalogText()}` }, definition: { type: "object", description: `宣言的アプリ定義（schema:"${APP_SCHEMA}"）。{schema,id,name,version,permissions,inputs[],steps[],output,allowHosts?}。inputs=フォーム入力（text/textarea/number/boolean/select/file）。steps=処理。利用可能 op：${opCatalogText()}。【API節約の原則】整形・置換・集計・保存・取得・外部取得など知能が不要な処理は transform/file.*/db.*/http.fetch で決定的に行う。ai.infer（AI/API）は要約・分類・自然言語生成・非定型データの解釈など知能が要る箇所だけに使い、不要なら ai 権限も付けない。http.fetch は allowHosts 宣言必須。前ステップ結果は as で束縛し $名 で参照。` }, changelog: { type: "string", description: "更新時の変更ログ（何を・なぜ変えたかを日本語で簡潔に）" }, estimated_tokens: { type: "number", description: "1実行あたりの推定消費トークン" } }, required: ["name", "spec"] } }
];
const GEMINI_TOOLS = [
...
「承認待ち」一覧（/approvals）で管理者が承認すると実行されます。承認ID: ${id}`;
  ...
  switch (name) {
    ...
    case "propose_app": {
      ...
      const res = await createDraft(ctx, { name: name2, description: args.description ? String(args.description) : void 0, spec, permissions: perms, definition: args.definition, estTokens: Number(args.estimated_tokens) || void 0, role, changelog: args.changelog ? String(args.changelog) : void 0 }, owner);
      const icon = (s) => s === "ok" ? "✅" : s === "warn" ? "⚠️" : "⛔";
      const lines = res.preflight.checks.map((c) => `${icon(c.status)} ${c.label}：${c.detail}`).join("\n");
      const readyMsg = role === "admin" ? "→ 4確認OK。あなたは管理者権限のため、別途の承認は不要です。このまま実装に進めます（「アプリ」画面の「アプリ開発」で草案の確認・公開申請ができます）。" : "→ 4確認OK。実装に進めます。管理者が「アプリ」画面の「アプリ開発」でレビュー・公開申請すると公開できます。";
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_C23diL6K.mjs");
  ...
  const appDefsAll = await installedAppDefs(ctx).catch(() => []);
  const appDefs = opts.unattended ? appDefsAll.filter((a) => !(a.definition.permissions ?? []).includes("net")) : appDefsAll;
  const appDecls = appDefs.map((a) => ({ name: appToolName(a.id), description: `${a.name}${a.description ? "：" + a.description : ""}（導入済みアプリ）`, parameters: appInputsToParams(a.definition.inputs) }));
  ...
  const decls = [...partDecls, ...appDecls, ...CORE_TOOLS, ...GEMINI_TOOLS, ...hasClaude ? CLAUDE_TOOLS : [], ...isPro ? multiTools : [], ...isPlus ? dirTools : [], ...autonomy ? AUTONOMY_TOOLS : [], ...enabledSkills.length ? [skillTool(enabledSkills.map((s) => s.name))] : [], ...capDecls];
...
割り当てられたタスクのみを遂行し、結果を簡潔に返す。`, { text: task || "（タスク）" }, subDeclsFor(subTools), subExec, 3, [], onUsage, abort);
  ...
  const exec = async (n, a) => {
    ...
    if (appMap.has(n)) return formatRunResult(await runInstalledApp(ctx, appMap.get(n), a, owner), baseUrl);
...
${WORKERS_AI_NOTE}` : sys;
  const out = await runToolLoop(model, sysForRun, first, decls, exec, hops, history, onUsage, abort, opts.onEvent);
