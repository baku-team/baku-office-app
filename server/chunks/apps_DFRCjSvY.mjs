...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DxLzC322.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CY40n_Ph.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_aE8388t6.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_f89fKOqu.mjs");
  const { registeredParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_CDC9FeHL.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_FYPH2hvQ.mjs");
