...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Dx3CMt_7.mjs");
  const { cmpVersion } = await import("./update_427_-gax.mjs");
  await import("./index_DUiQLzG_.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_D2lB-MzN.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C-3xK6OK.mjs");
  ...
  const { reviewQueue } = await import("./users_Co9pSHvm.mjs");
  const { backupAlert } = await import("./backup_C0YaOuK9.mjs");
