...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_C0wNwuGo.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BFl8qM-S.mjs");
  ...
  const { hasApiKey } = await import("./client_C0wNwuGo.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DLy4Ktsu.mjs");
