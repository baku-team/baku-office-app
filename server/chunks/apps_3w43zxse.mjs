...
import { r as renderComponent } from "./worker-entry_RQryW0sw.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BWm4B9KU.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BifgPhj3.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BifgPhj3.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_C0kTytCf.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BvxaQHJg.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Dj-b6uSu.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DG8Orboc.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_OSwLzA_S.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DB4Fse-p.mjs");
