...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_AxuQz5dA.mjs");
  const { cmpVersion } = await import("./update_Dd6ZZfYZ.mjs");
  await import("./index_BvR2HJE_.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BsdZaID6.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BjaLyFrr.mjs");
  ...
  const { reviewQueue } = await import("./users_-sPlWLZj.mjs");
  const { backupAlert } = await import("./backup_DE4r8Ybb.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_TTFEFEgh.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DEGbNk-F.mjs");
