...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BaLSnevk.mjs");
  const { cmpVersion } = await import("./update_CD6BNyNR.mjs");
  await import("./index_BhxwzBes.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BR9A7zOm.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Ic6IZxRr.mjs");
  ...
  const { reviewQueue } = await import("./users_CvQbtwYE.mjs");
  const { backupAlert } = await import("./backup_DVjBW2xK.mjs");
