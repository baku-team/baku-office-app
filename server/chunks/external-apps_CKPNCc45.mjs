...
import { getToken, hostFetch, getVerifyJwk } from "./client_BNXMhaMG.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_C0cN2tKz.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, rewriteFabricatedDateSlots, rewriteBareCountSlots, repairMisplacedCreateTemplate, deriveConfigFieldsFromUsage, validateDefinition, isBlockedHost } from "./appdef_Bg6y08G0.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_Bh0BoI_b.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_Bg6y08G0.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_kjrS3dQ0.mjs");
