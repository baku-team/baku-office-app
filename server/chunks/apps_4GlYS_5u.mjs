...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BAkPbG1h.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DI-uUz7k.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_CJh_mVEi.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Cos_vIXW.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_8YmkCTHi.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DO3nkheO.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DkjAilSU.mjs");
