...
import { cachedEntitlement, getApiKey } from "./client_sRscislI.mjs";
import { DiscordInbound } from "./discord_MitODRRb.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { joinWithInvite } from "./users_CS2WTucu.mjs";
import { F as transcribeAudio, G as verifyLineSignature, H as lineReply, I as lineReplyQuick, t as cfEgressGateway } from "./ctx_DRb3V3LZ.mjs";
import { inboundFileLimitBytes, saveFile } from "./storage_DMuH0rYg.mjs";
...
async function joinViaInvite(env2, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CS2WTucu.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_C4vHdEjJ.mjs");
  const bins = [];
  const failedNames = [];
...
class LineInbound {
    ...
    for (const ev of events) {
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS } = await import("./upload-queue_C4vHdEjJ.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_C4vHdEjJ.mjs");
