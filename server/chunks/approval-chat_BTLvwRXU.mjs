...
import { getApproval, decideApproval } from "./approvals_CZq7PEAR.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_DfwCgcjk.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_BHqm0CfS.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_DHRGXFWv.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_CCzD9Z58.mjs";
import { joinWithInvite } from "./users_ERIG_80T.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_B0LY-udU.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_ERIG_80T.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_DxSx--NI.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BKotyb5n.mjs");
