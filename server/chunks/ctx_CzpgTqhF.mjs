...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, fileBelongsTo, storageMode } from "./storage_BWA_jk-s.mjs";
import { getApiKey, getToken, hostFetch, entitlementForGate, masterKeyCtx } from "./client_0Vd7eIvZ.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification } from "./notifications_CdoRv0_M.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CyrBIHr-.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DwBfCXoi.mjs";
import "./index_Dm3QXHiv.mjs";
...
import { googleFetch } from "./google_BjuiNiSH.mjs";
...
import { validateDefinition, APP_SCHEMA, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, RENDER_HTML_MAX, opCatalogText } from "./appdef_DyM-L_iC.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DRKQ2ci1.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_CYQ8qNlM.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_D233Ibud.mjs";
import { logDiag } from "./diag_BS4K6Q4H.mjs";
import { i as isRunnableDefinition } from "./preflight_Crl3xrQ-.mjs";
...
import { g as getAppDesign, m as repairOutputLiterals, n as createDraft, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, i as installLocalApp, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_CVNZFet_.mjs";
import { a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { a as resolveChannel } from "./connectors-store_CHWkgRyl.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_B8eziynA.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_D9zDeAkY.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_Dn6dVyaY.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_C4hhcZHX.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { p as pushToUser } from "./push_CLNBd-6f.mjs";
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_0Vd7eIvZ.mjs");
...
${custom}`].filter(Boolean).join("\n");
  ...
  if (!want) {
    ...
    const routed = routeRequest(reg0, { text: text || "", hasImage: !!image, historyChars, availableIds });
...
`;
    ...
    if (!model && env.AI) {
      ...
      model = workersAiChatModel(env.AI, waModel);
      provider = "workers_ai";
      if (skippedCap) budgetNote = "⚠️ 通常のAIがクレジット枠超過のため、Cloudflare Workers AI（簡易AI）に切り替えました。継続するにはクレジット補充、または無料の Groq / Cerebras / GitHub Models（クレカ不要）を登録できます（設定 → 連携設定）。会計登録・検索などのツール操作は一時的に行えません。\n\n";
    }
