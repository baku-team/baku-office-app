...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_CDCAJlBm.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DZbVdp3s.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_B0LtoEmz.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DnpuPih6.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_7tMaKadi.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BP0r-k3z.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_DKjhUCbZ.mjs");
