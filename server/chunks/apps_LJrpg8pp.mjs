...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_DqeGQnea.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DSsg1gMl.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_rF0xK8SJ.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DFmNUVmQ.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Fv0mgUHT.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CbPM17XJ.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CoZXb96-.mjs");
