...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_C_RhHGpz.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DQSrHVYT.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DQ67RXU1.mjs");
  const { registeredParts } = await import("./parts_CVGBl5kS.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_BYZkU0fI.mjs");
  ...
  const folders = await (await import("./settings_DHOyCLdr.mjs")).getAppFolders(env, ses.uid).catch(() => []);
