...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CdSGakTK.mjs");
  const { cmpVersion } = await import("./update_TZvD8KUB.mjs");
  await import("./index_BmteSGwA.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_sdCYmWZn.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_zyaP20WU.mjs");
  ...
  const { reviewQueue } = await import("./users_ClyOEoCZ.mjs");
  const { backupAlert } = await import("./backup_BD-i0-rT.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BcG2B-uJ.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DZNylPHb.mjs");
