...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BzmGqf3D.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_8UIY9kKI.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BT1HjjIF.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BnrRQwqf.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BlQX5b9g.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_rV9_WCfp.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_HlxhhAZC.mjs");
