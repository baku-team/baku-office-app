...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BkhaD8kM.mjs");
  const { cmpVersion } = await import("./update_BBRVmFNM.mjs");
  await import("./index_BJ3WaWAr.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_7c7w8_zY.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_3T4X3BXX.mjs");
  ...
  const { reviewQueue } = await import("./users_Cl69hEgZ.mjs");
  const { backupAlert } = await import("./backup_5wmiQVty.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_WPZGR1xq.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CSCQeY7c.mjs");
