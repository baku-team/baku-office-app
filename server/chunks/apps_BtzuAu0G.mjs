...
import { r as renderComponent } from "./worker-entry_Dj9zrVgp.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BBQpMtQc.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_yaE9M1Sr.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_yaE9M1Sr.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_CJeNxlvr.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_C6i3Usi3.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BwDnm4vX.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BLO8JKYO.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_isqg69Qv.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CXK7LO0d.mjs");
