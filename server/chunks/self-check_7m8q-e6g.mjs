...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_DaLdYWCT.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_CY3PJnOM.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
