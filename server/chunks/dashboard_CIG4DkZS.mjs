...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CQaGQZPz.mjs");
  const { cmpVersion } = await import("./update_CSEkjZZX.mjs");
  await import("./index_K8_PuiPo.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DxBrf3Eu.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BwEC4q-t.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_Cpme2L5u.mjs");
  ...
  const { reviewQueue } = await import("./users_8g2GLrsR.mjs");
  const { backupAlert } = await import("./backup_BBloq6pH.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dn5Vnq79.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DGxwrYo0.mjs");
  const { getWorkersPaid } = await import("./settings_CGAf54Rj.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_BeMAbUf8.mjs");
  const { NEURON_USD } = await import("./config_D7WVsdyn.mjs");
  let cf = null;
  if (isOrgAdmin) {
