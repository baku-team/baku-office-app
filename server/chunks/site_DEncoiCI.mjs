...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_Cbgrxt0R.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_Bm-CmJY_.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_Cg-HVE1W.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_DkS04vDB.mjs").then((n) => n.W);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_DkS04vDB.mjs").then((n) => n.W);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_Cbgrxt0R.mjs");
