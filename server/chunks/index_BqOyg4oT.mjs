...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CG1mOonk.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_DSXOrOmp.mjs");
  ...
  const agentName = await (await import("./settings_hJXdQlLE.mjs")).getAgentName(env).catch(() => "相棒");
  const ttsCloudReady = await (await import("./capabilities_DzfOEU8z.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    import("./settings_hJXdQlLE.mjs"),
    import("./skills_BoOTsRbP.mjs"),
    import("./scheduled-tasks_DUSn4rGV.mjs")
  ]);
  ...
  await import("./index_DM4gS8c2.mjs");
    ...
    (function () {
        ...
        async function send() {
          ...
          try {
            ...
            if (done) {
              window.bo.pollAgent?.();
              addMsg("a", done.reply, true, done.actions, done.messageId);
