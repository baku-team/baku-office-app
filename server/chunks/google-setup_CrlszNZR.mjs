...
import { r as renderComponent } from "./worker-entry_Cl6sZ58F.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DKVQjxh1.mjs";
...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_BjxCEIvt.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin" && ses.ctx === "org";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CROoCO64.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_D6JanvQD.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured, driveConnected } = await import("./drive_kNuTSby3.mjs");
  const driveConf = isAdmin ? await driveConfigured(env).catch(() => false) : false;
  const driveConn = isAdmin ? await driveConnected(env).catch(() => false) : false;
  const driveRedirect = `${Astro2.url.origin}/api/drive/callback`;
...
まず上の手順 <strong data-astro-cid-v63rqogn>1〜4 で Workspace 連携</strong>を済ませてください。その際、手順2の利用機能で <strong data-astro-cid-v63rqogn>「ドライブ（書類の取り込み・読み取り）」にもチェック</strong>を入れると、ドライブもそのまま使えるようになります（<strong data-astro-cid-v63rqogn>OAuth クライアントの作成は不要</strong>です）。
</div>`}<details class="adv" style="margin-top:.8rem" data-astro-cid-v63rqogn> <summary data-astro-cid-v63rqogn>（上級者向け）OAuth クライアントで個別に連携する場合</summary> <p class="muted small" style="margin-top:.5rem" data-astro-cid-v63rqogn>Workspace 連携を使わず、ドライブ専用に OAuth クライアントを作る方法です（通常は不要）。現在の状態：OAuth資格情報 <strong data-astro-cid-v63rqogn>${driveConf ? "登録済み" : "未登録"}</strong>／ドライブ個別連携 <strong data-astro-cid-v63rqogn>${driveConn ? "連携済み" : "未連携"}</strong>。</p> <ol class="dwd-steps drive-steps" data-astro-cid-v63rqogn> <li data-astro-cid-v63rqogn> <strong data-astro-cid-v63rqogn>OAuth クライアント（ウェブアプリ）を作成</strong> <ul style="margin:.3rem 0 0;padding-left:1.1rem" data-astro-cid-v63rqogn> <li data-astro-cid-v63rqogn><a href="https://console.cloud.google.com/auth/clients" target="_blank" rel="noopener" data-astro-cid-v63rqogn>Google Cloud Console の「クライアント」画面 ↗</a> を開きます。</li> <li data-astro-cid-v63rqogn>上部の <strong data-astro-cid-v63rqogn>「＋ クライアントを作成」</strong> をクリックします。</li> <li data-astro-cid-v63rqogn>アプリケーションの種類で <strong data-astro-cid-v63rqogn>「ウェブ アプリケーション」</strong> を選びます。</li> <li data-astro-cid-v63rqogn>名前は任意（例：baku-office ドライブ）。</li> <li data-astro-cid-v63rqogn><strong data-astro-cid-v63rqogn>「承認済みのリダイレクト URI」</strong>の「URI を追加」に、下のURLを貼り付けます。</li> </ul> <div class="row" style="margin-top:.4rem" data-astro-cid-v63rqogn><input id="drive-redirect" type="text" readonly${addAttribute(driveRedirect, "value")} style="flex:1;font-family:monospace;font-size:.78rem" data-astro-cid-v63rqogn><button class="btn" id="copy-drive-redirect" type="button" data-astro-cid-v63rqogn>コピー</button></div> <ul style="margin:.3rem 0 0;padding-left:1.1rem" data-astro-cid-v63rqogn><li data-astro-cid-v63rqogn>「作成」を押すと <strong data-astro-cid-v63rqogn>クライアントID</strong> と <strong data-astro-cid-v63rqogn>クライアントシークレット</strong> が表示されます。</li></ul> </li> <li data-astro-cid-v63rqogn> <strong data-astro-cid-v63rqogn>クライアントID／シークレットを登録</strong> <div class="muted small" data-astro-cid-v63rqogn>作成した ID／シークレットを <a href="/settings/keys" data-astro-cid-v63rqogn>連携設定（APIキー）</a> の「Google クライアントID／シークレット」に貼り付けて保存します。</div> </li> <li data-astro-cid-v63rqogn> <strong data-astro-cid-v63rqogn>ドライブを連携</strong> <span class="dstatus" data-astro-cid-v63rqogn>${driveConn ? "✓ 連携済み" : "未連携"}</span> <div style="margin-top:.4rem" data-astro-cid-v63rqogn> ${driveConf ? renderTemplate`<a class="btn btn-primary" href="/api/drive/start" data-astro-cid-v63rqogn>${driveConn ? "再連携する" : "Google ドライブを連携"}</a>` : renderTemplate`<span class="muted small" data-astro-cid-v63rqogn>先に上の手順でクライアントID／シークレットを登録してください。</span>`} </div> </li> </ol> </details> ` })}`} `, "scripts": async ($$result2) => renderTemplate(_a || (_a = __template([`<script>
        ...
        (function () {
          ...
          document.getElementById("copy-drive-redirect")?.addEventListener("click", () => copy("drive-redirect"));
