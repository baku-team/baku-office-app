...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CCzD9Z58.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DkIjVYjn.mjs");
  await import("./index_Cw9M4CD3.mjs");
  const { appCatalog, installedAppIds } = await import("./apps__zNnisb7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BX-vf23e.mjs");
  ...
  const { reviewQueue } = await import("./users_ERIG_80T.mjs");
  const { backupAlert } = await import("./backup_DGO2INk6.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_B0LY-udU.mjs");
