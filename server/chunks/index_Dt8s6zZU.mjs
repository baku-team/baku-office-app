...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CryjS_8W.mjs");
  const { cmpVersion } = await import("./update_C4Wl5sEi.mjs");
  await import("./index_CyzQ5YzI.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Diahlliv.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_ycwW0EeO.mjs");
  ...
  const { reviewQueue } = await import("./users_BCIsoFzV.mjs");
  const { backupAlert } = await import("./backup_DvbQN7rU.mjs");
