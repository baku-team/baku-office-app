...
import { getApiKey } from "./client_DkJb1xQq.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_DPaLCj7k.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_D7yFgcIz.mjs";
import { processAgentJobs } from "./agent-jobs_CfMEP3-r.mjs";
import { reclaimStaleApprovals } from "./approvals_5yA-LsTn.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_CA6X_3bd.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_x_94XUuy.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_CGdB7tT3.mjs";
import { flushReports } from "./reports_Bs3VQCVF.mjs";
import { addNotification, pushWebhook } from "./notifications_B-SHicf4.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_BpQ232ZA.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_BSCMkUu-.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_Cp6RIs0n.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
