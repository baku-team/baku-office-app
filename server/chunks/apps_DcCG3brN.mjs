...
import { r as renderComponent } from "./worker-entry_BJIkbGwy.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CEcKvPxf.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_Ddwkb9TX.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_Ddwkb9TX.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_ga_0ayig.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_DcubbLWL.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BQNK-3Dt.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DGSaVCvE.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Dr0mvakg.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Nib3U6zO.mjs");
