...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CXgE4FGk.mjs");
  const { cmpVersion } = await import("./update_bnU2JrhI.mjs");
  await import("./index_BUgtM5Dr.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_ClgaNh4b.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_LB-iVO2N.mjs");
  ...
  const { reviewQueue } = await import("./users_DOsUmTw8.mjs");
  const { backupAlert } = await import("./backup_DoojZkJ1.mjs");
