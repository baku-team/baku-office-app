...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_B8CX3HBm.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_B8CX3HBm.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_ODKWvbUB.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_PSBt_dAs.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D1fW0wtH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BJv6y5C1.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CcTtB-z4.mjs");
