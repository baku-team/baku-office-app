...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await (await import("./ctx_C8LO9Go2.mjs").then((n) => n.N)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_C8LO9Go2.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_69psxMdE.mjs");
