...
import { rateLimited, getPublicPage, honeypotTripped, validateSubmission, createSubmission, setSubmissionPayment, moderate, fireSubmitTriggers } from "./public-pages_CgImd4LL.mjs";
import { createCheckoutSession } from "./payments_C9GFEGGC.mjs";
import { maxUploadMb, saveFile } from "./storage_HzCeCgFJ.mjs";
...
const POST = async ({ request, params, locals }) => {
  ...
  if (!v.ok) {
    await (await import("./diag_BjrAYDQX.mjs")).logDiag(env, "warn", "public", `公開フォーム送信NG slug=${slug}: ${v.error}`, `送信キー=[${Object.keys(values).join(",")}] / 必須項目=[${page2.fields.filter((f) => f.required).map((f) => f.name).join(",")}]`).catch(() => {
    });
  ...
  const sub = await createSubmission(env, page2, v.clean, refs, ip);
  if (page2.price > 0) {
