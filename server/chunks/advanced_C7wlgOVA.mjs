...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_G-wMh__R.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_f58sZPB-.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CmFAV3Wf.mjs");
