...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_DZ4X55g1.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Bf1r67RB.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Yp4r_6gu.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BRJ_vBZf.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_yNmZnfox.mjs";
import "./index_DjDLCqan.mjs";
...
import { googleFetch } from "./google_S8JQkCgN.mjs";
...
import { APP_SCHEMA, validateDefinition, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_BVdOhSVT.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C3rc1AfM.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DEmstRtg.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_YyxVQuoE.mjs";
import { logDiag } from "./diag_CY3PJnOM.mjs";
import { i as isRunnableDefinition } from "./preflight_DU2V1-Oe.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_D9mw6_ut.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CvTm6T5r.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_iW6a4cRN.mjs";
import { storeCatalog } from "./store_Dqumkj6n.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BcGgaiPh.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_6LeGYTL9.mjs";
import { saveLayoutDraft, listSites } from "./sites_DB2Dyxn6.mjs";
...
import { p as pushToUser } from "./push_gqwzKqbe.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DB2Dyxn6.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_lE39ruPr.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_lE39ruPr.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_lE39ruPr.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Bf1r67RB.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_lE39ruPr.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_DZ4X55g1.mjs")).storageMode(env) === "r2";
