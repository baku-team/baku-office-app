...
import { cachedEntitlement } from "./client_qGL1F133.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  let visionImage;
  if (b.image?.dataB64) {
    const { prepareAttachment } = await import("./chat-flow_DfOysPWt.mjs");
...
`));
  ...
  (async () => {
    try {
      ...
      const { looksLikeBuildConfirmation } = await import("./ctx_D1FmXRm9.mjs").then((n) => n.R);
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant)) {
        const { startAppBuild, processAppBuild } = await import("./ctx_D1FmXRm9.mjs").then((n) => n.Q);
      ...
      if (b.mode !== "plan" && !visionImage) {
        const { tryHandleAppDelete } = await import("./chat-flow_DfOysPWt.mjs");
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
        const { looksLikeAppEdit } = await import("./ctx_D1FmXRm9.mjs").then((n) => n.R);
        if (looksLikeAppEdit(message)) {
          const { latestSessionApp, startAppEdit, processAppBuild } = await import("./ctx_D1FmXRm9.mjs").then((n) => n.Q);
      ...
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_uoUaTVh0.mjs");
        ...
        const { buildReplyActions } = await import("./chat-flow_DfOysPWt.mjs");
