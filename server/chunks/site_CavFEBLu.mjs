...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_FASA0RI1.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_CQY6ET0s.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_BXymRng6.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_DZmKLyOO.mjs").then((n) => n.W);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_DZmKLyOO.mjs").then((n) => n.W);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_FASA0RI1.mjs");
