...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_sRscislI.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CAdJB7AV.mjs");
  await import("./index_BJcJsswH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DEzXoTbR.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DPrVtOZ8.mjs");
  ...
  const { reviewQueue } = await import("./users_CS2WTucu.mjs");
  const { backupAlert } = await import("./backup_BuJ_6YXn.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DAQnWVVG.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_DMuH0rYg.mjs");
