...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, getApiKey } = await import("./client_DNkcBUdZ.mjs");
  ...
  let steps = [];
  if (isAdminOrg) {
    const { googleConfigured } = await import("./google_B00iOqPm.mjs");
    const aiReady = !!await getApiKey(env, "gemini") || !!await getApiKey(env, "claude") || !!await getApiKey(env, "openai") || !!env.LOCAL_AI_BASE_URL;
  ...
  const groups = [
    ...
    { title: "外部サービス設定", items: [
      { href: "/settings/integrations", label: "外部サービス設定（AI／Discord・Slack・LINE／Google／クラウド保存）", desc: "外部サービスとの接続設定を1か所で", show: isAdminOrg }
    ] },
