...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_D0PzYluR.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BsuDyowk.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D2wnbrnI.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_ZgdMwRke.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_5eUXNvfD.mjs");
