...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_w07XlKo6.mjs");
  const { cmpVersion } = await import("./update_pV1UCKFT.mjs");
  await import("./index_CyuXw7nH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BsMJJaFf.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DwhWCCzp.mjs");
  ...
  const { reviewQueue } = await import("./users_BmV3NSF3.mjs");
  const { backupAlert } = await import("./backup_C6OGyHjw.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B-0nE2nO.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_CbtCJxWn.mjs");
