...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_Cd9kHlGa.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_CM0oE9Aw.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_awWY3CKv.mjs");
  ...
  const { randomId } = await import("./ctx_CHjxiU9K.mjs").then((n) => n.J);
