...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_DBc4AdQ5.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DaPtUtWl.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_WWbnZ3SD.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_H2tcQp6t.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_xBf3QcGR.mjs");
