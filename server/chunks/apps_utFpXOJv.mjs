...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DCqlLMUd.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_DSXOrOmp.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DM4gS8c2.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CjLxCdtu.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DszGd1gL.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_CdMunyy5.mjs");
