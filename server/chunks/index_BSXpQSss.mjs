...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_runN5xu5.mjs");
  const { cmpVersion } = await import("./update_DqV_GZKZ.mjs");
  await import("./index_BlHQGgc4.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DcLoUjgu.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DZOVxVBH.mjs");
  ...
  const { reviewQueue } = await import("./users_B8aVK5B7.mjs");
  const { backupAlert } = await import("./backup_Cfk-PC9c.mjs");
