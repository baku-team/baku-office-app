...
import { r as renderComponent } from "./worker-entry_Ch-C9Fxp.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CM1pU2UX.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CATBeV5o.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BHT8hMec.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CO1s1d8D.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_CQbiju9H.mjs"),
    import("./scheduled-tasks_CH16hdxy.mjs")
  ]);
  ...
  await import("./index_11AnePHL.mjs");
