...
import { getApproval, decideApproval } from "./approvals_Bee2ijMS.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CIgUj-G6.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_BRlB-70J.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CKlVnnzp.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_o_bCkQmM.mjs";
import { joinWithInvite } from "./users_Bp2KjBvV.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_DlJhVZeY.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_Bp2KjBvV.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_CsBZEIjQ.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DY71tuz5.mjs");
