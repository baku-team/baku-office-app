...
import { getFile } from "./storage_C0xk2SHi.mjs";
import { getDriveSave } from "./drive_DSp2oymh.mjs";
import { n as notifyOwnerDirect } from "./ctx_C3hyS9pN.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.U);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_C0xk2SHi.mjs");
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.P);
