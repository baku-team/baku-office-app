...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BCGuPb9o.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_COWOhsgK.mjs");
  ...
  const { hasApiKey } = await import("./client_BCGuPb9o.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DCN_Hros.mjs");
