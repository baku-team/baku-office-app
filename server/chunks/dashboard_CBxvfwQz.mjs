...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_G-wMh__R.mjs");
  const { cmpVersion } = await import("./update_BN-4EfQj.mjs");
  await import("./index_DTs4ENJE.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C2htMoA7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dnb4-RxN.mjs");
  ...
  const { reviewQueue } = await import("./users_CuA2lQ7M.mjs");
  const { backupAlert } = await import("./backup_Dl17FP0s.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CjtNtWNm.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CmFAV3Wf.mjs");
