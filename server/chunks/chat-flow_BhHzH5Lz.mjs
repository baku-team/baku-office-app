...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CmFAV3Wf.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_Cdoe3Kr3.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DwmjFwH6.mjs").then((n) => n.Q);
  ...
  const { latestSessionApp } = await import("./ctx_DwmjFwH6.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CjtNtWNm.mjs").then((n) => n.z);
