...
import { r as renderComponent } from "./worker-entry_Cm_HZS-P.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DwsGRgt8.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_xAmHz7U5.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_xAmHz7U5.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CCHdD_Bj.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_Dkd4XO8R.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DHfqgO_N.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BgIyC1x0.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_C08IdFiZ.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Byn7HpJj.mjs");
