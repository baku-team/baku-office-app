...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DR7-1RrZ.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DR7-1RrZ.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_9oy9gh5-.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_Bp7iwF3-.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C3Al9_ge.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_B9x0OXHg.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_3q9Cmcyt.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Blzgn2Ll.mjs");
