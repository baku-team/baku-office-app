...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_B-lkqngg.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BUGuDJa-.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BBucxay0.mjs");
