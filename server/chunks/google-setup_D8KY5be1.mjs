...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_Cr-MU0wf.mjs");
  const { hasApiKey } = await import("./client_CyRmMA6L.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C7zn0crz.mjs");
...
scripts/google-oauth-setup.sh</code></pre> <p class="muted">完了後、残りの手動ステップへのリンクと貼り付け用リダイレクト URI が出力されます。</p> </details> </div> <style>
        ...
        (function () {
          ...
          syncScopes();
          // WIF設定(JSON)をパース。必須キーが揃っていなければ null。
          const resolveWif = () => {
