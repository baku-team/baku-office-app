...
import { r as renderComponent } from "./worker-entry_CnXwkYZI.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_C4BnUZle.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BDKZng8J.mjs");
  ...
  const { atLeast, planLabel } = await import("./cf-adapter_CqurLfVa.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_kiIL6iv0.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BMK2adnV.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_GjR2Wx6H.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B8q51KTm.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_letfyuQ-.mjs");
