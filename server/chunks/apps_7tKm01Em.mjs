...
import { r as renderComponent } from "./worker-entry_CeahDxCb.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DT9xIT2S.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BwhX8UhD.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BwhX8UhD.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_CP8STYz0.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_BkhaD8kM.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BJ3WaWAr.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_7c7w8_zY.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_WPZGR1xq.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BmhIo9uv.mjs");
