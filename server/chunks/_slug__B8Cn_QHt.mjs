...
import { createCheckoutSession } from "./payments_DPsBDCZv.mjs";
import { maxUploadMb, saveFile } from "./storage_AVD7qBz2.mjs";
...
const POST = async ({ request, params, locals }) => {
  ...
  if (!v.ok) {
    await (await import("./diag_DWCSrGCf.mjs")).logDiag(env, "warn", "public", `公開フォーム送信NG slug=${slug}: ${v.error}`, `送信キー=[${Object.keys(values).join(",")}] / 必須項目=[${page2.fields.filter((f) => f.required).map((f) => f.name).join(",")}]`).catch(() => {
    });
