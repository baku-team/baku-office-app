...
import { canBootstrapOrgAdmin, makeSessionCookie, signPending, sessionExp } from "./auth_iZjiHMy7.mjs";
import { getVerifyJwk, getToken, hostFetch, saveToken } from "./client_B17j1uG_.mjs";
...
const GET = async ({ url, request, locals }) => {
  ...
  const isOrgAdmin = stored ? stored === p.sub : canBootstrapOrgAdmin(env, { externalId: p.sub, email: p.email });
  if (!isOrgAdmin) {
