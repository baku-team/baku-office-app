...
import { r as renderComponent } from "./worker-entry_lhglepq3.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BV4Z7uoQ.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_PY8RFBfq.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_PY8RFBfq.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DugSuSUS.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_-spfSM1I.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B-J9Qckz.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BYed8Rw1.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Bb659Sv7.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DedmkOc1.mjs");
  ...
  const folders = await (await import("./settings_C_1axsCz.mjs")).getAppFolders(env, ses.uid).catch(() => []);
