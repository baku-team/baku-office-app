...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_C48CCjoS.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_0pHY0CaD.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_B4Wt0CUx.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_k1dAsV_z.mjs"),
    import("./scheduled-tasks_DvgJR7r0.mjs")
  ]);
  ...
  await import("./index_DF6Mutvv.mjs");
