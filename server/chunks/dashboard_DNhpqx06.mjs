...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BjQkw6mR.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Db6F-MfU.mjs");
  await import("./index_Cw432j-u.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DrWbSOGs.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B1Doa-g1.mjs");
  ...
  const { reviewQueue } = await import("./users_Dfaoq2IJ.mjs");
  const { backupAlert } = await import("./backup_DtVFiEZl.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BGP-5Wom.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_B7Xn3k8N.mjs");
