...
import { isValidGeminiModel, isValidClaudeModel, isValidOpenAiModel, isValidWorkersAiModel, isValidGrokModel, isValidGithubModelsModel, DEFAULT_MODELS } from "./config_DIXTf4RD.mjs";
...
function isMemberModel(v) {
  ...
  return v === "gemini" || v === "claude" || v === "openai" || v === "local" || v === "grok" || v === "github_models" || isValidGeminiModel(v) || isValidClaudeModel(v) || isValidOpenAiModel(v) || isValidWorkersAiModel(v) || isValidGrokModel(v) || isValidGithubModelsModel(v);
}
...
function parseRequestModel(raw) {
  if (!raw) return {};
  if (raw === "gemini" || raw === "claude" || raw === "openai" || raw === "local" || raw === "grok" || raw === "github_models") return { engine: raw };
