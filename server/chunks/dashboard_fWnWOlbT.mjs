...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_dimZBE3u.mjs");
  const { cmpVersion } = await import("./update_CBSmthPG.mjs");
  await import("./index_mSn5WQnS.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C5DRyoDI.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CwDaWm5h.mjs");
  ...
  const { reviewQueue } = await import("./users_Ce1V_hT9.mjs");
  const { backupAlert } = await import("./backup_BObkQxV6.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DS8tDlHa.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_BlGYdbwv.mjs");
