...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BpQ232ZA.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_DkJb1xQq.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_B-SHicf4.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_DPntK0eN.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DX3YoKbL.mjs";
import "./index_DPUrRbwk.mjs";
...
import { googleFetch } from "./google_Cd3M1ltz.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_C4DJM6El.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_TA1sKT3a.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DnzBFfmz.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_CgPcSC7K.mjs";
import { logDiag } from "./diag_D9aXYLsc.mjs";
import { i as isRunnableDefinition } from "./preflight_De4tH6Sq.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DpWwHMpZ.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_DQoKcPEY.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_D7yFgcIz.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_5yA-LsTn.mjs";
import { saveLayoutDraft, listSites } from "./sites_DT2rMr19.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_CLW1ozyH.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_CvnxiS00.mjs";
...
💡 改善チェック項目（任意・このままでも使えます）：
${editSuggestions.map((s) => `・${s}`).join("\n")}
気になる点があれば下の「${editWorks ? "💡 改善案を反映する" : "🔧 直す"}」で反映できます。` : "";
...
💡 改善チェック項目（任意・アプリはこのままでも使えます）：
${suggestions.map((s) => `・${s}`).join("\n")}
気になる点があれば下の「${works ? "💡 改善案を反映する" : "🔧 直す"}」で反映できます。` : "";
  await post(
    ctx2,
    row,
    head + (failed.length ? `
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DT2rMr19.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_B0qDOEVs.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_B0qDOEVs.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_B0qDOEVs.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DkJb1xQq.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_B0qDOEVs.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BpQ232ZA.mjs")).storageMode(env) === "r2";
