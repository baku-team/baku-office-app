...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BlqFC5PS.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BjKlgqy9.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_BozZHqe1.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_DRz6Swln.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_X5x_Eyo3.mjs";
import "./index_NXtqd6gx.mjs";
...
import { googleFetch } from "./google_CAVqvIP-.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_BcH-onZm.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CsHQ0Sg8.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_CiQ0G_Bk.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_DzQ02g07.mjs";
import { logDiag } from "./diag_D9aXYLsc.mjs";
import { i as isRunnableDefinition } from "./preflight_CJgoml1U.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_Cj-LVoHK.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CN9lQNXj.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_D7p_P15A.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_ClSMmMgm.mjs";
import { saveLayoutDraft, listSites } from "./sites_CzU-w4nh.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_CZzW_Q4A.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_CZPXRGZU.mjs";
...
async function continueRenderHtml(env, model, html, paid, rounds = 4) {
  ...
  for (let i = 0; i < rounds; i++) {
    if (!looksTruncated(html) || html.length > RENDER_HTML_MAX) break;
...
${spec}` }], [], void 0, { maxTokens: paid ? 24e3 : 12e3 }).catch(() => null);
  ...
  if (html && looksTruncated(html) && html.length <= RENDER_HTML_MAX) {
    await logDiag(env, "info", "build", `render truncated, continuation loop: app=${plan.id}`).catch(() => {
    });
    html = await continueRenderHtml(env, model, html, paid);
  }
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_CzU-w4nh.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_scwaK1dt.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_scwaK1dt.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_scwaK1dt.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BjKlgqy9.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_scwaK1dt.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BlqFC5PS.mjs")).storageMode(env) === "r2";
