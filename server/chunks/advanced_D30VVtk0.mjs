...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_Dx3CMt_7.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_D6iItoV9.mjs");
  ...
  const { hasApiKey } = await import("./client_Dx3CMt_7.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_iZjhVw_T.mjs");
