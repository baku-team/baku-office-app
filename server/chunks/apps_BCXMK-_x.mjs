...
import { r as renderComponent } from "./worker-entry_CfTiUuZq.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CnFLG-_U.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_C0Raud0A.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_C0Raud0A.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DloQDFvv.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_CdHBXZZx.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_07W0wUoP.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DWmuHuDh.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DB-xHIgj.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BZxrSuvK.mjs");
