...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, hasApiKey } = await import("./client_vij6I9Jq.mjs");
  ...
  const [hasGemini, hasClaude, hasOpenai, hasGrok, hasGithubModels] = await Promise.all([hasApiKey(env, "gemini").catch(() => false), hasApiKey(env, "claude").catch(() => false), hasApiKey(env, "openai").catch(() => false), hasApiKey(env, "grok").catch(() => false), hasApiKey(env, "github_models").catch(() => false)]);
  const hasLocal = !!env.AI;
  const aiReady = hasGemini || hasClaude || hasOpenai || hasGrok || hasGithubModels || hasLocal;
  const agentName = await (await import("./settings_BY28V0AV.mjs")).getAgentName(env).catch(() => "相棒");
  const { GEMINI_MODELS, CLAUDE_MODELS, OPENAI_MODELS, WORKERS_AI_MODELS, GROK_MODELS, GITHUB_MODELS } = await import("./config_DIXTf4RD.mjs");
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    import("./settings_BY28V0AV.mjs"),
    import("./skills_DvB2WseM.mjs"),
    import("./scheduled-tasks_CV_rsv03.mjs")
  ]);
