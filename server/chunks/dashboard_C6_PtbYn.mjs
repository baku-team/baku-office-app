...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Wma9rw14.mjs");
  const { cmpVersion } = await import("./update_DGpzGsvE.mjs");
  await import("./index_B48wf0P0.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BTCX4lRV.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BkVwJgoA.mjs");
  ...
  const { reviewQueue } = await import("./users_hXKtwWvE.mjs");
  const { backupAlert } = await import("./backup_BKJkbUPq.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B2UyOhc0.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B11Nmez-.mjs");
