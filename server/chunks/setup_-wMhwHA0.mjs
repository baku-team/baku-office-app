...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_0Vd7eIvZ.mjs");
  const { googleConfigured } = await import("./google_BjuiNiSH.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_CnYFpXfJ.mjs");
