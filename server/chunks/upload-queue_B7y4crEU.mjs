...
import { getFile } from "./storage_CtLBMcNN.mjs";
import { getDriveSave } from "./drive_im6RZ537.mjs";
import { n as notifyOwnerDirect } from "./ctx_CAci7FIw.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_CAci7FIw.mjs").then((n) => n.U);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_CtLBMcNN.mjs");
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_CAci7FIw.mjs").then((n) => n.P);
