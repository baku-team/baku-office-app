...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CX4OI5qq.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_Cfiu9nCW.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_DSp2oymh.mjs");
  ...
  const { buildCtx } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.L);
