...
import { getSession } from "./auth_DfwCgcjk.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_B0LY-udU.mjs";
import { setMemberModel, toggleFavApp, setAppFolders, setAiEngine, setSmartRouting, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_C4tL9AHR.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_B3HDnUo9.mjs";
import { setStorageLimits } from "./storage-usage_BX-vf23e.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps__zNnisb7.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, h as purgeStaleDrafts, i as installLocalApp, a as activeAppDefinition, j as appVersions, k as activateAppVersion, m as deleteLocalApp } from "./external-apps_J8zU4bt9.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const { canDevelopApps } = await import("./auth_DfwCgcjk.mjs");
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_CZq7PEAR.mjs");
  ...
  if (b._action === "notify_webhook") {
    ...
    if (url) {
      const { isPublicHttpsUrl } = await import("./egress_yKY42dgP.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_DiU6RoPH.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.N);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_BHqm0CfS.mjs").then((n2) => n2.T);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_DlfcJjsU.mjs");
  ...
  if (b._action === "purge_stale_drafts") {
    const n = await purgeStaleDrafts(locals.ctx);
  ...
  if (b._action === "app_dev_log") {
    ...
    const { getMessages } = await import("./chat-sessions_BTO_5scg.mjs").then((n) => n.k);
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.D);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.C);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.D);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.D);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.C);
