...
import { setRetentionDays, setMaxUploadMb, fileBelongsTo, getFile, saveFile, storageMode } from "./storage_B4jFnDvF.mjs";
import { m as makeDocument, w as webSearch, b as summarizeTranscript, c as extractInvoiceData, t as transcribeAudio, i as inferApp } from "./media-ai_DnyJbywR.mjs";
import { googleFetch } from "./google_CS5rxal6.mjs";
import { getApiKey, entitlementForGate } from "./client_CFhfgOdP.mjs";
...
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp } from "./apps_Bm85HqIr.mjs";
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, opCatalogText } from "./appdef_Bz-FCBXl.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_yEhcuGoT.mjs";
import { searchDirectory } from "./directory_DcceVvmc.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_B6cetgpE.mjs";
import { logDiag } from "./diag_Cu0HTcAn.mjs";
import { i as isRunnableDefinition } from "./preflight_BBriIbUe.mjs";
...
import { g as getAppDesign, m as createDraft, n as repairOutputLiterals, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_CND5X7DG.mjs";
...
import "./index_DNEehGc8.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_XCId19SC.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_xFT8akwb.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CjIg_yhG.mjs";
import { overBudget, recordUsage, estimateUsd, recordTokens } from "./usage_DN6pGBs1.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DP4baD-d.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers } from "./users_DRl31INi.mjs";
import { listActions, createAction, deleteAction, updateAction } from "./a2a-actions_CmawPw8e.mjs";
import { p as pushToUser } from "./push_yEi1VGM7.mjs";
...
async function editStep(ctx, row, model) {
  ...
  let { patch, truncated, text } = await makePatch(env, model, design.definition, row.spec ?? "", row.paid === 1);
  const REFUSAL_RE = /大きすぎ|多すぎ|組み直せ|作り直せ|一度に|分けて|小分け|細かく分け|難しいです|できません|対応でき(ない|ませ)|含まれています|文字数|トークン|1つの定義|一つの定義/;
  if (!patch && !truncated && REFUSAL_RE.test((text || "").replace(/```[\s\S]*?```/g, ""))) {
    await logDiag(env, "warn", "build", "edit: 断り文を検出→htmlEdits限定で強制再試行", (text || "").slice(0, 200)).catch(() => {
    });
    const force = `${row.spec ?? ""}

【厳守】説明文・断り文は一切返さず、有効なJSONパッチ1個だけを返してください。定義は作り直さず、render.html への htmlEdits（find/replace）だけで対応します。該当箇所が多くても（絵文字・アイコンを全部消す等）必要な数だけ htmlEdits を列挙してください。サイズ・文字数を理由に断らないこと。`;
    const r2 = await makePatch(env, model, design.definition, force, row.paid === 1);
    patch = r2.patch;
  ...
  if (!patch) {
    ...
    if (assess.length >= 10 && !assess.startsWith("{") && !assess.startsWith("[") && !REFUSAL_RE.test(assess)) {
    ...
    await failBuild(ctx, row, explainStop("ai", "AIが修正案を生成できませんでした（応答が空・解釈不能、またはサイズを理由に断りました）。", "「絵文字を消す」「この画面のこのボタンの色を変える」のように対象を1点に絞って再依頼してください。別のAIモデルへの切替も有効です。"));
    return false;
  }
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CFhfgOdP.mjs");
