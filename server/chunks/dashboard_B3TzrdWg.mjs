...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BT1HjjIF.mjs");
  const { cmpVersion } = await import("./update_jDAUZe-b.mjs");
  await import("./index_BnrRQwqf.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BlQX5b9g.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D-YASewB.mjs");
  ...
  const { reviewQueue } = await import("./users_XXidmb3R.mjs");
  const { backupAlert } = await import("./backup_NY8D3ke9.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_rV9_WCfp.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_M2NJJlqW.mjs");
