...
import { getApproval, decideApproval } from "./approvals_SjDxLk_m.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_BO4LdvYt.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_CIL4AtJU.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CDjorHYh.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BCk4EGYy.mjs";
import { joinWithInvite } from "./users_A_R7zOf9.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_CoDdssK4.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_A_R7zOf9.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_hLSdfu9S.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BLEFNUX4.mjs");
