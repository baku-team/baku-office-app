...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Bf1r67RB.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BN22BQ01.mjs");
  await import("./index_DjDLCqan.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_yNmZnfox.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BX3dm1gE.mjs");
  ...
  const { reviewQueue } = await import("./users_BRJ_vBZf.mjs");
  const { backupAlert } = await import("./backup_DsAyR87I.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DGSF7vRP.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_DZ4X55g1.mjs");
