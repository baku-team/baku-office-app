...
async function notifyHook(env, text) {
  ...
  if (lineTo) {
    const { getApiKey } = await import("./client_7WfEvhS0.mjs");
    const token = await getApiKey(env, "line_token").catch(() => null);
    if (token) {
      const [{ cfEgressGateway }, { linePush }] = await Promise.all([import("./ctx_CzH8Lj8g.mjs").then((n) => n.L), import("./ctx_CzH8Lj8g.mjs").then((n) => n.O)]);
      try {
