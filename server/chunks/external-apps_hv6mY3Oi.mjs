...
import { getToken, hostFetch, getVerifyJwk } from "./client_BCk4EGYy.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_CksBaO21.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, deriveConfigFieldsFromUsage, validateDefinition, isBlockedHost } from "./appdef_X-C7jdsb.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_C1VtaqKi.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_X-C7jdsb.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_SjDxLk_m.mjs");
