...
import { v as validateDefinition } from "./appdef_Ciqz5dKq.mjs";
import { g as getAppDesign } from "./external-apps_DohtNeGn.mjs";
import { p as preflight } from "./preflight_DwLTVcRC.mjs";
import { r as runDraftApp } from "./app-runtime_n-15ik-J.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_DohtNeGn.mjs").then((n) => n.z)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_Cu0HTcAn.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
