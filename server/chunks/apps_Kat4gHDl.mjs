...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DwTr64WL.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_CtcYt0T4.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BFi-aKBP.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CzzimBfS.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_h39FFap9.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_BAeIOAX9.mjs");
