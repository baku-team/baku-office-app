...
import { ALLOWED_PERMISSIONS } from "./apps_Du6RRp8T.mjs";
...
const INPUT_TYPES = ["text", "textarea", "number", "boolean", "select", "file", "signature"];
const IDENT = /^[a-zA-Z][a-zA-Z0-9_]*$/;
const opCatalogText = () => GENERATED_OPS.map((op) => `${op}${OP_PERMISSION[op] ? `（要 ${OP_PERMISSION[op]}）` : "（権限不要）"}`).join("、");
function hostOf(url) {
  try {
...
function validateDefinition(input) {
  ...
  const validateUnit = (inputsArr, stepsArr, output, prefix, optionalOutput = false) => {
    ...
    if (!Array.isArray(inputsArr)) add(`${prefix}inputs`, "inputs は配列である必要があります。");
      ...
      if (inp.type === "select" && (!Array.isArray(inp.options) || inp.options.length === 0)) add(`${prefix}inputs[${i}].options`, "select には options が必要です。");
    });
    ...
    if (!Array.isArray(stepsArr) || stepsArr.length === 0) add(`${prefix}steps`, "steps は1つ以上必要です。");
      ...
      switch (s.op) {
        ...
        case "db.write":
          ...
          break;
        case "db.delete":
          if (!refOk(s.from)) add(`${at}.from`, 'db.delete には削除するレコードの id 参照（from）が必要です。例：from を "$id"。');
          break;
        case "data.create":
          ...
          break;
        case "data.update":
          if (!refOk(s.recordId)) add(`${at}.recordId`, 'data.update には対象レコードの id 参照（recordId）が必要です。例：recordId を "$id"。');
          if (!refOk(s.from)) add(`${at}.from`, 'data.update には更新内容の参照（from）が必要です。例：from を "$rec"。');
          if (s.collection !== void 0 && !IDENT.test(s.collection)) add(`${at}.collection`, "collection は識別子（英字始まり）で指定してください。");
          break;
        case "data.get":
        case "data.remove":
          if (!refOk(s.recordId)) add(`${at}.recordId`, `${s.op} には対象レコードの id 参照（recordId）が必要です。例：recordId を "$id"。`);
          if (s.collection !== void 0 && !IDENT.test(s.collection)) add(`${at}.collection`, "collection は識別子（英字始まり）で指定してください。");
          break;
        case "data.list":
          if (s.collection !== void 0 && !IDENT.test(s.collection)) add(`${at}.collection`, "collection は識別子（英字始まり）で指定してください。");
        ...
        case "knowledge.search":
          ...
          break;
        case "record.status":
          if (!refOk(s.from)) add(`${at}.from`, 'record.status には対象レコードの id 参照（from）が必要です。例：from を "$id"。');
    ...
    if (output === void 0 || output === null) {
      ...
      if (!["text", "file", "table", "chart"].includes(output.type)) add(`${prefix}output.type`, "output.type は text/file/table/chart のいずれか。");
      if (!refOk(output.from)) add(`${prefix}output.from`, `未定義の参照：${output.from}`);
    }
  };
...
function isRenderable(input) {
  ...
  return vr.issues.every((i) => i.path === "render.html" && i.message.includes("screens[] に定義されていません"));
}
...
export {
  APP_SCHEMA as A,
  RENDER_HTML_MAX as R,
  appApprovalEffects as a,
  isRenderable as b,
  isBlockedHost as i,
  normalizeScreens as n,
  opCatalogText as o,
  roleCanOpenScreen as r,
  sanitizeRenderHtml as s,
