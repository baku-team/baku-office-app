...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_AX6IIuyG.mjs").then((n) => n.Y), import("./notifications_Dowm9QUM.mjs")]);
    ...
    await (await import("./ctx_AX6IIuyG.mjs").then((n) => n.U)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_AX6IIuyG.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_Cwa6evce.mjs");
