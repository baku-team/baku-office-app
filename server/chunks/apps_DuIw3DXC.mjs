...
import { r as renderComponent } from "./worker-entry_mjVaTgqi.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DzOc4FRZ.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_bbtIbOVC.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_bbtIbOVC.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_4JC1RMyj.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_BsDvhGkR.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BSxCgorH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Bm8taPXR.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CbHDX8yf.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CR7K0x_f.mjs");
