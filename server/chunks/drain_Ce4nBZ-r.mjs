...
import { getApiKey } from "./client_CSBhOn7n.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_C6H2bPoI.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_BiDkfFSy.mjs";
import { processAgentJobs } from "./agent-jobs_IOjsBvQL.mjs";
import { reclaimStaleApprovals } from "./approvals_CrXkUnnq.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_CFPnG8A1.mjs";
import { guardHeavy } from "./diag_DWCSrGCf.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_9tZRIYuw.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_BKJg2vxb.mjs";
import { flushReports } from "./reports_KSPesTg-.mjs";
import { addNotification, pushWebhook } from "./notifications_Cjh13nrA.mjs";
import { getNotifyWebhook } from "./settings_BKzORwjT.mjs";
import { purgeFiles } from "./storage_DV5PW-A4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_DyvUj1cX.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_OY2k6jzJ.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
