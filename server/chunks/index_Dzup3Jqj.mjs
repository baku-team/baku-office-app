...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BCGuPb9o.mjs");
  const { cmpVersion } = await import("./update_Denmhx96.mjs");
  await import("./index_CwnDzWoS.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CdgXOypi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BxO3e0V_.mjs");
  ...
  const { reviewQueue } = await import("./users_BnINx0hx.mjs");
  const { backupAlert } = await import("./backup_wRM-tz48.mjs");
