...
import { cachedEntitlement, getApiKey } from "./client_Du2JHuMP.mjs";
import { DiscordInbound } from "./discord_QiT67BSn.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { joinWithInvite } from "./users_CZPZCIhG.mjs";
import { F as transcribeAudio, G as verifyLineSignature, H as lineReply, I as lineReplyQuick, t as cfEgressGateway } from "./ctx_DAURz88_.mjs";
import { saveFile } from "./storage_Cc5JIPVQ.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { logDiag, looksLikeLimit, PAID_HINT } from "./diag_ByGiCOpc.mjs";
import { getWorkersPaid } from "./settings_hJXdQlLE.mjs";
...
async function joinViaInvite(env2, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CZPZCIhG.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_DTCA23ow.mjs");
  const bins = [];
...
返信で「読み込み」「保管する」「何もしない」のいずれかを送ってください。` : ack.text;
  ...
  const { getMemberModel, parseRequestModel } = await import("./settings_hJXdQlLE.mjs");
  const { engine: model, modelId } = parseRequestModel(await getMemberModel(env2, acc.uid).catch(() => null) ?? "");
  let answer = "";
  const usedTools = [];
  try {
    answer = await ctx.agent.run({ owner, text: prompt, role: acc.role, baseUrl, history, model, modelId, sessionId: sid, onEvent: (ev) => {
...
class LineInbound {
    ...
    for (const ev of events) {
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS } = await import("./upload-queue_DTCA23ow.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_DTCA23ow.mjs");
