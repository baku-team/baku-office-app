...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, fileBelongsTo, storageMode } from "./storage_5xdAYtmH.mjs";
import { getApiKey, getToken, hostFetch, entitlementForGate, masterKeyCtx } from "./client_BQbdmWVM.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_BlvfQibs.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_Cske0FpT.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CH6qhuCH.mjs";
import "./index_DB87vrf0.mjs";
...
import { googleFetch } from "./google_BGN0-NC-.mjs";
...
import { validateDefinition, APP_SCHEMA, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, RENDER_HTML_MAX, opCatalogText } from "./appdef_Clj7O5Hr.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CCVEZELu.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_Cz3g0z76.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_C6OUyrBU.mjs";
import { logDiag } from "./diag_BjrAYDQX.mjs";
import { i as isRunnableDefinition } from "./preflight_CXMNctSN.mjs";
...
import { g as getAppDesign, m as appsBrief, n as repairOutputLiterals, o as createDraft, q as getAppMeta, t as setAppMeta, v as suggestAppMeta, i as installLocalApp, w as pendingRelaxedHosts, x as setAppAllowedRoles, b as listExternalApps, y as installedAppDefs } from "./external-apps_D-px-Ku8.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_D4C-Jff4.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_DKMa7UNX.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DLagWnee.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DSMHh3XA.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { p as pushToUser } from "./push_B0PVHJYH.mjs";
...
async function finalize(ctx2, row, plan, def, model) {
  ...
  const actions = ready ? [
    ...fixAction,
    ...relaxedAction,
    { label: "プレビュー・動作確認して登録", kind: "navigate", href: `/app/${res.id}?preview=1`, style: fixAction.length || relaxedAction.length ? "ghost" : "primary" },
    { label: "ストア申請", kind: "navigate", href: `/apps?tab=apps#app-${res.id}` }
  ] : [
    ...fixAction,
    ...relaxedAction,
    { label: "アプリ開発で確認", kind: "navigate", href: `/apps?tab=apps#app-${res.id}`, style: fixAction.length || relaxedAction.length ? "ghost" : "primary" }
  ];
  await post(
    ctx2,
    row,
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BQTCDKNV.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BQTCDKNV.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BQTCDKNV.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BQbdmWVM.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BQTCDKNV.mjs")).socialStatus(env)).some(Boolean);
