...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B11Nmez-.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_1CauE0vS.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_Bbx7rdbk.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_Bbx7rdbk.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_B2UyOhc0.mjs").then((n) => n.z);
