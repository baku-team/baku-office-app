...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_DV5PW-A4.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_-0prxDd1.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_C6H2bPoI.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_C6H2bPoI.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CfvnGFDu.mjs").then((n) => n.B);
