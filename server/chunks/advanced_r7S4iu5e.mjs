...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_RgvQ0ags.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_D8PtOPfM.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_D2XGvsLY.mjs");
