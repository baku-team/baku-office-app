...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_AVD7qBz2.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_DVo9lSTr.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_hO_rNBCZ.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CoLC07tv.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_VQTsA6SI.mjs";
import "./index_C6kyOQ9r.mjs";
...
import { googleFetch } from "./google_DffTuEJw.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_B8ThPYFn.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C1SKrqjw.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BR7CjU1C.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_ajaQOzEI.mjs";
import { logDiag } from "./diag_DWCSrGCf.mjs";
import { i as isRunnableDefinition } from "./preflight_D1WtfhAS.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_D0kaNE1S.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CUIq43g5.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CFNy6idE.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_D0IPsrw0.mjs";
import { saveLayoutDraft, listSites } from "./sites_DOspayEd.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_BY8sp0Lc.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_avhu3rGa.mjs";
...
- AI engine for app building: the app builder (create/fix apps) runs ONLY with a Claude or Gemini API key. Other keys (ChatGPT/OpenAI, Grok, Groq, Cerebras, GitHub Models, Workers AI) work for chat, homepage/LP building, and in-app AI (ai.infer), but cannot run the app builder: a single build step needs a large prompt (20-34K chars of system instructions) and large output (12-24K tokens for the UI HTML) — free tiers' per-minute token limits (5-10K) physically cannot carry one step, and the others lack the output ceiling/streaming a step needs to finish. When a user asks to create or fix an app and no Claude/Gemini key is registered, do NOT start the build; explain the reason briefly in plain words and guide them to Settings → API keys /settings/keys (Gemini has a free tier and is the easiest way; Claude gives the highest build quality).
...
async function streamText(key, body) {
  ...
  if (!text && stopReason === "refusal") return { refusal: true, usage: { inputTokens, outputTokens } };
...
${html}` }
    ], [], void 0, { maxTokens: paid ? 24e3 : 12e3, thinking: "disabled" }).catch(() => null);
...
${spec}` }], [], void 0, { maxTokens, thinking: "adaptive" }).catch(() => null);
    let h = (r?.text ?? "").trim();
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DOspayEd.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_DFkd9mmc.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_DFkd9mmc.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_DFkd9mmc.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DVo9lSTr.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_DFkd9mmc.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_AVD7qBz2.mjs")).storageMode(env) === "r2";
