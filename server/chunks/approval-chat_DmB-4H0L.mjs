...
import { getApproval, decideApproval } from "./approvals_w6xNIQ3l.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_iZjiHMy7.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_Ige4T837.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CC00lJRu.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_B17j1uG_.mjs";
import { joinWithInvite } from "./users_BYbdQprm.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BthdaNTE.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_BYbdQprm.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_Dd3X8Oj6.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DO_v758Y.mjs");
