...
async function notifyHook(env, text) {
  ...
  if (lineTo) {
    const { getApiKey } = await import("./client_qGL1F133.mjs");
    const token = await getApiKey(env, "line_token").catch(() => null);
    if (token) {
      const [{ cfEgressGateway }, { linePush }] = await Promise.all([import("./ctx_D1FmXRm9.mjs").then((n) => n.L), import("./ctx_D1FmXRm9.mjs").then((n) => n.O)]);
      try {
