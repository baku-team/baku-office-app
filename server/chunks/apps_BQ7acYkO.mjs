...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BWJTdi0a.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_Dd9FH8gm.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BHmUVA_J.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_tV2l6_F4.mjs");
  const { registeredParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DJIg5n3w.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_CUZVwu_2.mjs");
