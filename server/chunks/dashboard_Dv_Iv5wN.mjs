...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CtDgLyHf.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_QSebruY0.mjs");
  await import("./index_CIVgZ-l3.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CgDeq9BV.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cr11AjC3.mjs");
  ...
  const { reviewQueue } = await import("./users_DRxG0iUA.mjs");
  const { backupAlert } = await import("./backup_CYebcBFL.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CXYObSRd.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DXjZvHw0.mjs");
