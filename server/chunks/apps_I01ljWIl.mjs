...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_D0aobTMW.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D1Qahmtg.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Qqa6Ep4W.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BsZJNGRN.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_Ksj9x-WS.mjs");
