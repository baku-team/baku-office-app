...
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_nBAdPSxh.mjs";
import { APP_SCHEMA, validateDefinition, checkOrphanDataScreens, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections } from "./appdef_D9vMBBjU.mjs";
import { getApiKey } from "./client_Bib4yTGq.mjs";
import { getStorageUsage } from "./storage-usage_DeKuvgbX.mjs";
import { getLimits, monthUsd, estimateUsd } from "./usage_X9PGsLLE.mjs";
...
async function preflight(ctx, d) {
  ...
  const structWarnings = structured ? [checkOrphanDataScreens(d.definition), checkWriteScreenEmptyBoRun(d.definition), checkWriteScreenArgKeyMismatch(d.definition), checkReadOnlyCollections(d.definition)].filter((w) => !!w) : [];
  ...
  else if (structWarnings.length) checks.push({ key: "safety", label: "安全確認", status: "warn", detail: structWarnings.join(" ／ ") });
