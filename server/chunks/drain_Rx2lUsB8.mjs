...
import { getApiKey } from "./client_B_HP_Ex9.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_Drnao1kv.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_DId0R5ou.mjs";
import { processAgentJobs } from "./agent-jobs_1NHsLvve.mjs";
import { reclaimStaleApprovals } from "./approvals_AS34t0Ug.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_B-ToQUJa.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_DO1w0Q9r.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_CkNvxg6q.mjs";
import { flushReports } from "./reports_CGQ9NO8m.mjs";
import { addNotification, pushWebhook } from "./notifications_DjbEJfC2.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_BgjO-NRr.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_DYeVC-A7.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_C13894FS.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
