...
import { b as registerPart } from "./parts_BnninK9G.mjs";
...
async function readForm(ctx, a) {
  ...
  if (!r) return NEED$2;
  if (!r.ok) return r.status === 404 ? `フォームが見つかりません（${r.status}）。編集URL（.../forms/d/{ID}/edit）またはフォームIDを指定してください。公開URL（/viewform）のIDはAPIでは読めません。` : `フォーム取得に失敗しました（${r.status}）。`;
  const d = await r.json();
