...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DM4zCU7e.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DUavRV7D.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CqYezlNs.mjs");
