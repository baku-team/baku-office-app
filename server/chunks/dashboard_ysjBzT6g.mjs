...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DNkcBUdZ.mjs");
  const { cmpVersion } = await import("./update_CvvPItj3.mjs");
  await import("./index_C9K-32fF.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_t7uZkVIe.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DbZwsLne.mjs");
  ...
  const { reviewQueue } = await import("./users_0NnH3aXT.mjs");
  const { backupAlert } = await import("./backup_CJuffloa.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CIfdV394.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_Dvmn3unV.mjs");
