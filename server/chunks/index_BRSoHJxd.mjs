...
import { r as renderComponent } from "./worker-entry_t73sgZ-X.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Cpu3FhD0.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.locals.publicHost) {
    ...
    const { buildPublicFullPage } = await import("./app-frame_Bjytiekf.mjs").then((n) => n.d);
  ...
  const { getSession } = await import("./auth_DrSavdul.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BNXMhaMG.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BL9z7daV.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_JaGMrK7t.mjs"),
    import("./scheduled-tasks_D2ATE_pj.mjs")
  ]);
  ...
  await import("./index_D018HHvS.mjs");
