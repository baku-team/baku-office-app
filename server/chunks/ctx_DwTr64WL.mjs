...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_D2DJGMd8.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CtcYt0T4.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_DbiBZY49.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CvA1-zYY.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CzzimBfS.mjs";
import "./index_BFi-aKBP.mjs";
...
import { googleFetch } from "./google_DKYBuAuX.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, opCatalogText, googleOpCatalogText } from "./appdef_BCZwcMiF.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_mug4D6f4.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BF-gQMgL.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BBbdfb7y.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_CDods-dy.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_BDozGNWZ.mjs";
import { logDiag } from "./diag_NtHEh0X8.mjs";
import { i as isRunnableDefinition } from "./preflight_ClMB1vvD.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_Bl3T0xPp.mjs";
...
import { g as getAppDesign, m as appsBrief, n as createDraft, o as repairOutputLiterals, q as getAppMeta, t as setAppMeta, v as suggestAppMeta, i as installLocalApp, w as pendingRelaxedHosts, x as isRegisteredPart, f as fetchAndInstall, y as setAppAllowedRoles, b as listExternalApps, z as installedAppDefs, A as parseTags } from "./external-apps_h39FFap9.mjs";
import { a as appendMessage } from "./chat-sessions_4ZEuV35m.mjs";
import { explainStop } from "./errors_Cz86HmdL.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DCf21T5d.mjs";
import { storeCatalog } from "./store_BAeIOAX9.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BTk_99GX.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_BYVkcwsF.mjs";
import { saveLayoutDraft, listSites } from "./sites_BWgUT_Yc.mjs";
...
import { p as pushToUser } from "./push_CPxhAbmY.mjs";
...
できる範囲（${feas.feasibleScope || "可能な部分"}）を作ります。作れない部分は、下の「📋 外部AI用プロンプトをコピー」で外部AI（ChatGPT 等）に依頼できます。`;
  ...
  try {
    ...
    await logDiag(env, "warn", "build", `phase err cursor=${row.cursor}: ${em}`).catch(() => {
    });
    phase.status = "failed";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_BWgUT_Yc.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_lRtWoQsO.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_lRtWoQsO.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_lRtWoQsO.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CtcYt0T4.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_lRtWoQsO.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_D2DJGMd8.mjs")).storageMode(env) === "r2";
