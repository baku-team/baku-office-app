...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DEBuc12A.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CX82ikhU.mjs");
  await import("./index_mJzrtm6N.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DALGIijZ.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CeHcqvfB.mjs");
  ...
  const { reviewQueue } = await import("./users_3Hp8b7YO.mjs");
  const { backupAlert } = await import("./backup_BmClU-Td.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_B-P5Zaq2.mjs");
