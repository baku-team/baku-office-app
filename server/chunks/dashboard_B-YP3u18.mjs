...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BsDvhGkR.mjs");
  const { cmpVersion } = await import("./update_Dmd4Prui.mjs");
  await import("./index_BSxCgorH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bm8taPXR.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BJXn6H-e.mjs");
  ...
  const { reviewQueue } = await import("./users_dNzXKLOU.mjs");
  const { backupAlert } = await import("./backup_CZsVZHA8.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CbHDX8yf.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CbXVbpHY.mjs");
