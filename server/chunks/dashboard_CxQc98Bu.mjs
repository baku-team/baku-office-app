...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_0Vd7eIvZ.mjs");
  const { cmpVersion } = await import("./update_NrJBGc3m.mjs");
  await import("./index_Dm3QXHiv.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DwBfCXoi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DVrokG3I.mjs");
  ...
  const { reviewQueue } = await import("./users_CyrBIHr-.mjs");
  const { backupAlert } = await import("./backup_CnYFpXfJ.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CVNZFet_.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_BWA_jk-s.mjs");
