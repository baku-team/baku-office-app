...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BqaGF6MH.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_XKaks_hD.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Bd0_TMjG.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BBnHZMEg.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_Dc5V0d7w.mjs";
import "./index_BsLdtSuX.mjs";
...
import { googleFetch } from "./google_R2jRlvIG.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_B9RUsC0I.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_B9RF1ThM.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_qMqf7cny.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_eqrWQ3kZ.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_FQgJZWOl.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_C0vTgaN_.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_Ckv6W-DW.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_mXoXXg1D.mjs";
import { storeCatalog } from "./store_C32Y8upl.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_h0FlJJpN.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_D3RhLZVX.mjs";
import { saveLayoutDraft, listSites } from "./sites_JgFqkHKB.mjs";
...
import { p as pushToUser } from "./push_BDlYwU9g.mjs";
...
async function finalize$1(ctx2, row, plan, def, model) {
  ...
  try {
    const norm = (c) => typeof c === "string" && c ? c : "__default__";
    const deadCol = collectionsNeedingWriter(def).slice(0, 1);
    for (const rawCol of deadCol) {
      ...
      if (okCov) {
        ...
        def.permissions = [.../* @__PURE__ */ new Set([...def.permissions ?? [], ...validateDefinition(def).requiredPermissions])];
        reconcileRenderScreenRefs(def);
        await logDiag(env, "info", "build", `finalize dead-list repaired: ${col || "(既定)"}`).catch(() => {
        });
      }
    }
  } catch {
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_JgFqkHKB.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_B0f_Jhqj.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_B0f_Jhqj.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_B0f_Jhqj.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_BqaGF6MH.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_XKaks_hD.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_B0f_Jhqj.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BqaGF6MH.mjs")).storageMode(env) === "r2";
