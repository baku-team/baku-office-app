...
import { getApproval, decideApproval } from "./approvals_DmeGlKV0.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_DlJYu5pd.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_CVhIpi7y.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_BypXf69E.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_Bib4yTGq.mjs";
import { joinWithInvite } from "./users_BYPeJHFX.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BanefT0i.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_BYPeJHFX.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_ChSs7fW5.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BpoF2QJ4.mjs");
