...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DcvmK-U8.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_CoJfkQeJ.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CtLBMcNN.mjs");
