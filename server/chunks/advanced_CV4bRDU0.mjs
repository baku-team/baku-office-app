...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CJh_mVEi.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Ci5di3aj.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_cf3JyuGL.mjs");
