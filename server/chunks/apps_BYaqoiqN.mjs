...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_CDHQLTJj.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_BBJC6Bgv.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BR_H-Abo.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DYBxMYOj.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DXYlvzaN.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DnM9W0WO.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_e98nUlti.mjs");
