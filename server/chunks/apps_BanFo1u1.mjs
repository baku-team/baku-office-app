...
import { r as renderComponent } from "./worker-entry_xkoqO1BP.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_d5JSGCRR.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_F_luu1Bu.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_F_luu1Bu.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_CbvIxDRi.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_TWPTdJdo.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_wR0izv-9.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_B3yfw8OL.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Dd7ZbSkI.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_NiODytif.mjs");
