...
import { rateLimited, getPublicPage, honeypotTripped, validateSubmission, createSubmission, setSubmissionPayment, moderate, fireSubmitTriggers } from "./public-pages_kcQ20wlf.mjs";
import { createCheckoutSession } from "./payments_Bs1Z7RAL.mjs";
import { maxUploadMb, saveFile } from "./storage_BF4jK8tm.mjs";
...
const POST = async ({ request, params, locals }) => {
  ...
  if (!v.ok) {
    await (await import("./diag_Ca0PUS5y.mjs")).logDiag(env, "warn", "public", `公開フォーム送信NG slug=${slug}: ${v.error}`, `送信キー=[${Object.keys(values).join(",")}] / 必須項目=[${page2.fields.filter((f) => f.required).map((f) => f.name).join(",")}]`).catch(() => {
    });
