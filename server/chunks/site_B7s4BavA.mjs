...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_Lh-2pHR7.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_CXuQdqf4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_DtWKLUBy.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_BmScp1M7.mjs").then((n) => n.V);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_BmScp1M7.mjs").then((n) => n.V);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_Lh-2pHR7.mjs");
