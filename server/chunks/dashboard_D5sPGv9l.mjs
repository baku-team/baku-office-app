...
import { r as renderTemplate, b as renderSlot, m as maybeRenderHead, a as addAttribute, F as Fragment } from "./sequence_2tuU57vh.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_vij6I9Jq.mjs");
  const { cmpVersion } = await import("./update_DNtMaJXt.mjs");
  await import("./index_B3tnatTI.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BgEl515W.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_ln6OPjny.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_DJFUfwu0.mjs");
  const { scopedWidgets, enabledParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  ...
  const { reviewQueue } = await import("./users_CPDZKemR.mjs");
  const { backupAlert } = await import("./backup_P0Ss-8nj.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dk15ONnL.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_BF4jK8tm.mjs");
  const { getWorkersPaid } = await import("./settings_BY28V0AV.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_Y47VMyUf.mjs");
  const { NEURON_USD } = await import("./config_DIXTf4RD.mjs");
  let cf = null;
  if (isOrgAdmin) {
  ...
  <\/script>`]))) })}`;
}, "/home/runner/work/baku-office/baku-office/apps/client/src/pages/dashboard.astro", void 0);
const $$file = "/home/runner/work/baku-office/baku-office/apps/client/src/pages/dashboard.astro";
const $$url = "/dashboard";
