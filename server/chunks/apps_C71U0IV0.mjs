...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CHjxiU9K.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_D3kajqFf.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Dj0mRxJ1.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CgzlDI-b.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DUrYDXEN.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_LArcC0KF.mjs");
  ...
  const folders = await (await import("./settings_hJXdQlLE.mjs")).getAppFolders(env, ses.uid).catch(() => []);
