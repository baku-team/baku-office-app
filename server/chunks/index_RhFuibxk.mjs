...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_Dc3tZu8D.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BV1rSndT.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BwGLztfK.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_GRB0tedR.mjs"),
    import("./scheduled-tasks_Du-A4izr.mjs")
  ]);
  ...
  await import("./index_Q-RNyiqH.mjs");
  const { enabledParts, enabledPartIds } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { disabledBuiltins } = await import("./client_BV1rSndT.mjs");
