...
import { getApiKey } from "./client_DE3oxZSo.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_D1dXAMUS.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_PVO28pUh.mjs";
import { processAgentJobs } from "./agent-jobs_BUNewBlC.mjs";
import { reclaimStaleApprovals } from "./approvals_DmDxppmK.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_DeHVbK8S.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_CcrYmYno.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_DIpM-7-b.mjs";
import { flushReports } from "./reports_1nmMuzi-.mjs";
import { addNotification, pushWebhook } from "./notifications_C0-7w7UE.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_DJmKJuaK.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_iQagsDyw.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_CzHcRT_f.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
