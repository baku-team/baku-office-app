...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DdrEUTKx.mjs");
  const { cmpVersion } = await import("./update_DRIZRftJ.mjs");
  await import("./index_D7U8yzuf.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_vCtql7Lf.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DlE2NMsF.mjs");
  ...
  const { reviewQueue } = await import("./users_Df4NLF9D.mjs");
  const { backupAlert } = await import("./backup_CbTpaP-Z.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CjQOD0Ds.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_Bdf4380v.mjs");
