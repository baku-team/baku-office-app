...
import { getApiKey } from "./client_CZasnwOK.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_DoAWDsqP.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_D1cpdn3R.mjs";
import { processAgentJobs } from "./agent-jobs_CdNWAxLH.mjs";
import { reclaimStaleApprovals } from "./approvals_CD5TN9TN.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_BZwIxANr.mjs";
import { guardHeavy } from "./diag_DWCSrGCf.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_B_VqkAvO.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_CNfSZOPF.mjs";
import { flushReports } from "./reports_DdyUUQ3F.mjs";
import { addNotification, pushWebhook } from "./notifications_Cm3ihAVM.mjs";
import { getNotifyWebhook } from "./settings_BKzORwjT.mjs";
import { purgeFiles } from "./storage_Czk76zwb.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_qyJwekWb.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_CTBVRIxk.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
