...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DmOpR5Pi.mjs");
  const { cmpVersion } = await import("./update_BAp17JJA.mjs");
  await import("./index_CZ8s6CDY.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Cu1bevuo.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CZXi_Yvi.mjs");
  ...
  const { reviewQueue } = await import("./users_D8wIAa8A.mjs");
  const { backupAlert } = await import("./backup_Bwfp_CTt.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_Bbu5kdRU.mjs");
