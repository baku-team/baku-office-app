...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B9y2EXgA.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_DChgT15J.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_96IoWiWF.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_96IoWiWF.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DohtNeGn.mjs").then((n) => n.z);
