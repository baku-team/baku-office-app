...
import { APP_SCHEMA, validateDefinition } from "./appdef_BU9G7jMa.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_BeNzI-n3.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BRkvvUfW.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
