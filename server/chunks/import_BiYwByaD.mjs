...
const $$Import = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement } = await import("./client_F7ZfSNZF.mjs");
  const hasPlus = atLeast(await cachedEntitlement(env), "plus");
  const { listImported } = await import("./import_Dm8KNF50.mjs");
  const { fmtBytes } = await import("./storage-usage_DDEKWLbc.mjs");
