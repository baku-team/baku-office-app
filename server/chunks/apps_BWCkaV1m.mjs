...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_-0prxDd1.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_C6H2bPoI.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CSBhOn7n.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B0UyH5ZM.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_fVy9d1St.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_CfvnGFDu.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_BS6Fvt6h.mjs");
  ...
  const folders = await (await import("./settings_BKzORwjT.mjs")).getAppFolders(env, ses.uid).catch(() => []);
