...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_D6vmRoWk.mjs");
  const { cmpVersion } = await import("./update_jcnta_zN.mjs");
  await import("./index_BVkqyb6o.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_WMo-71Y0.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DhfSs_7T.mjs");
  ...
  const { reviewQueue } = await import("./users_DpXA8U-f.mjs");
  const { backupAlert } = await import("./backup_Bje2un4E.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B2coGmfm.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_D0LOgaub.mjs");
