...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BCk4EGYy.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DPzYaBU_.mjs");
  await import("./index_De32HSX0.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C1VtaqKi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DQYAHUJA.mjs");
  ...
  const { reviewQueue } = await import("./users_A_R7zOf9.mjs");
  const { backupAlert } = await import("./backup_BIJqM7j0.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_hv6mY3Oi.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_CoDdssK4.mjs");
