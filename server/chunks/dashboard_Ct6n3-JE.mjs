...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BAVQqvb3.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CB9sKgzJ.mjs");
  await import("./index_z7F-7bMo.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B5Vd2zbs.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DgZD3W3V.mjs");
  ...
  const { reviewQueue } = await import("./users_DqOcrHOp.mjs");
  const { backupAlert } = await import("./backup_C_gnqQbe.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_YQVzQqzP.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BVOr0lJw.mjs");
