...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BPZJrufk.mjs");
  const { cmpVersion } = await import("./update_B1cpotl7.mjs");
  await import("./index_D_QQxh5_.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_0DZIpoB3.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DGxAdYHE.mjs");
  ...
  const { reviewQueue } = await import("./users_BQ0CNk0d.mjs");
  const { backupAlert } = await import("./backup_DcyEdN5d.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dj_vTif6.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_BNzLQbDF.mjs");
