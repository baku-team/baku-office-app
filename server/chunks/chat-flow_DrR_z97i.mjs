...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_HzCeCgFJ.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_DaDf7n3s.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_CzH8Lj8g.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_CzH8Lj8g.mjs").then((n) => n.Q);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CFGtMTKs.mjs").then((n) => n.z);
