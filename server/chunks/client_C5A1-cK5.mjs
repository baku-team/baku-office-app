...
import { AppError, INFRA } from "./errors_Bw4GoHz5.mjs";
...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    const prevEnt = await env.LICENSE.get(KV_ENTITLEMENT).catch(() => null);
    if (prevEnt && prevEnt !== data.entitlement && (data.entitlement === "nonprofit" || data.entitlement === "enterprise")) {
      try {
        const label = data.entitlement === "nonprofit" ? "非営利団体（全機能無料）" : "エンタープライズ（全機能解放）";
        const [{ buildCtx }, notif] = await Promise.all([import("./ctx_B914Xe2h.mjs").then((n) => n.R), import("./notifications_UsPQ46xf.mjs")]);
    ...
    await (await import("./ctx_B914Xe2h.mjs").then((n) => n.N)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_B914Xe2h.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_CtlKN0ZS.mjs");
