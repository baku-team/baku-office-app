...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BpHW5W0y.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DXZLzw9T.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_CQaGQZPz.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_K8_PuiPo.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DxBrf3Eu.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Dn5Vnq79.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DiUdhpqa.mjs");
