...
import { r as renderComponent } from "./worker-entry_FeoeMa7E.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_7jW0WcLd.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DxYPrzcS.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DxYPrzcS.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_FkRAwPPW.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_D6vmRoWk.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BVkqyb6o.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_WMo-71Y0.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B2coGmfm.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DszIld3w.mjs");
