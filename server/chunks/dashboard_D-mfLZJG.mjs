...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Dd9FH8gm.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_S3AXnUOw.mjs");
  await import("./index_BHmUVA_J.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_tV2l6_F4.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BlM0-ean.mjs");
  ...
  const { reviewQueue } = await import("./users_MRHM8AL0.mjs");
  const { backupAlert } = await import("./backup_czQKQiqR.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DJIg5n3w.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_dk4WezxQ.mjs");
