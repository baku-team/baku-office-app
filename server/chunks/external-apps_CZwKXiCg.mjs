...
import { getToken, hostFetch, getVerifyJwk } from "./client_DcvmK-U8.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_BJNGhFGZ.mjs";
import { sanitizeRenderHtml, validateDefinition, isBlockedHost } from "./appdef_DxTm0WhQ.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_CpqQ2B4g.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_DxTm0WhQ.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_BQ3wJX86.mjs");
