...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C30hHFwA.mjs");
  const { cmpVersion } = await import("./update_CL63Cq30.mjs");
  await import("./index_Cq3Vgwa_.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Dc0l4YqP.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dq8MpX_H.mjs");
  ...
  const { reviewQueue } = await import("./users_Cow1m_q6.mjs");
  const { backupAlert } = await import("./backup_DMTdJwBz.mjs");
