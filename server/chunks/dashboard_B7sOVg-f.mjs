...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C6i3Usi3.mjs");
  const { cmpVersion } = await import("./update_QvvuX7SB.mjs");
  await import("./index_BwDnm4vX.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BLO8JKYO.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_a9HkgK52.mjs");
  ...
  const { reviewQueue } = await import("./users_BGGEwO0x.mjs");
  const { backupAlert } = await import("./backup_Bmu7Qw9u.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_isqg69Qv.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DUcQGvLc.mjs");
