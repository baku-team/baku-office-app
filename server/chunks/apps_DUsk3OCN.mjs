...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_BKYtLCd6.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B4Tk4Zwd.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DMCgfXnA.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_msu7Jdjm.mjs");
