...
import { $ as $$App } from "./App_RkqCbfwP.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_Dun_Y6hX.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_Dun_Y6hX.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_kh_TzdXt.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_D1hw6KSJ.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B0yPowPV.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_2TjohByF.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_2G5fL5LC.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BEZbQKla.mjs");
