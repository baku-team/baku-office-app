...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CXuQdqf4.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BTiBrVBU.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CBlFPHDk.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BGx-Ag0G.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_BtHypiSY.mjs";
import "./index_lNlZmhzH.mjs";
...
import { googleFetch } from "./google_BvZ7tCLO.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, opCatalogText, googleOpCatalogText } from "./appdef_CatMxJnh.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DsmEhxq5.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DUHz2SSi.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_GytoKYml.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_Clfy7jU-.mjs";
import { logDiag } from "./diag_NtHEh0X8.mjs";
import { i as isRunnableDefinition } from "./preflight_DETLFSB4.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_SmfSRBYI.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_CDedU4Ou.mjs";
import { storeCatalog } from "./store_D84zbozf.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_C7j9aJHH.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DqjH7are.mjs";
import { saveLayoutDraft, listSites } from "./sites_Lh-2pHR7.mjs";
...
import { p as pushToUser } from "./push_DV9e1k_w.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_Lh-2pHR7.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BZBO1OKm.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BZBO1OKm.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BZBO1OKm.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BTiBrVBU.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BZBO1OKm.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CXuQdqf4.mjs")).storageMode(env) === "r2";
