...
import { saveChatAttachment, isTextAttachmentMime } from "./storage_Ch3C4uCb.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_BV30PACS.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_DiPzP2bH.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_DiPzP2bH.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CyLGJDop.mjs").then((n) => n.y);
