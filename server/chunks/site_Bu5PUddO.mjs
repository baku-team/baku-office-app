...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_ChkjzB_-.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_DAEZsEe4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_DwQH-VUL.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_-pDHDD3Z.mjs").then((n) => n.W);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_-pDHDD3Z.mjs").then((n) => n.W);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_ChkjzB_-.mjs");
