...
import { r as renderComponent } from "./worker-entry_CqY2mD1V.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D-qcS1UL.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_D3m2l5sl.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_D3m2l5sl.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_CFIfihMj.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_BXVeeS3h.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CL76Ttcr.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_omiWNHKX.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CSNxZelj.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BMbK3h4Y.mjs");
