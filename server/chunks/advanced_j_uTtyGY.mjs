...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DuhUR81K.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DDctJAsU.mjs");
  ...
  const { hasApiKey } = await import("./client_DuhUR81K.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DNiFHm67.mjs");
