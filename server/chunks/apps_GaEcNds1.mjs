...
import { r as renderComponent } from "./worker-entry_DswCW-Nv.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_2Sb0AWRu.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_1CauE0vS.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_1CauE0vS.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_Bbx7rdbk.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_Wma9rw14.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B48wf0P0.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BTCX4lRV.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B2UyOhc0.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BgHpR4lJ.mjs");
