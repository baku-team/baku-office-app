...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BZYsk0AP.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BSmliX-9.mjs");
  await import("./index_Dmbvw9m0.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CW76H0U6.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BZUygzop.mjs");
  ...
  const { reviewQueue } = await import("./users_jU8ZLh2I.mjs");
  const { backupAlert } = await import("./backup_Cdm7Nzh3.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.A);
  ...
  const { storageMode } = await import("./storage_TBpzvJRs.mjs");
