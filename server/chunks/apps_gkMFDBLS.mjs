...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_B58Crkjn.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_DVo9lSTr.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C6kyOQ9r.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_VQTsA6SI.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_qax98Mxc.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_BY8sp0Lc.mjs");
