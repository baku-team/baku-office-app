...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BsuDyowk.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_ClXjVEHs.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CqwoO7zq.mjs");
