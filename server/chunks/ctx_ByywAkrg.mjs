...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_Cvzvo6pu.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CB_YAuNH.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_0VPJXKcx.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_C32ZSfGd.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CCYyWIXk.mjs";
import "./index_CVpv3OCE.mjs";
...
import { googleFetch } from "./google_DiEBjPne.mjs";
...
import { APP_SCHEMA, validateDefinition, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_WH4fyH_W.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_1K009vFk.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DJIJWcpd.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BREWMKHz.mjs";
import { logDiag } from "./diag_CY3PJnOM.mjs";
import { i as isRunnableDefinition } from "./preflight_BfW2iS7j.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_DQgptPRn.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_DvPmNXY7.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_CdeyKsnB.mjs";
import { storeCatalog } from "./store_BpsZcsJX.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_C1TyftTU.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DfVHaJ5K.mjs";
import { saveLayoutDraft, listSites } from "./sites_C23sZAiO.mjs";
...
import { p as pushToUser } from "./push_BOQGpnnG.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_C23sZAiO.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_sQXORFZA.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_sQXORFZA.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_sQXORFZA.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CB_YAuNH.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_sQXORFZA.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_Cvzvo6pu.mjs")).storageMode(env) === "r2";
