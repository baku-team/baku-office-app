...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_o_bCkQmM.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_B_Gr42Mz.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DlJhVZeY.mjs");
