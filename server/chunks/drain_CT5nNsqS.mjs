...
import { getApiKey } from "./client_BTiBrVBU.mjs";
import { x as cfEgressGateway, D as linePush, E as processSummaryJobs, F as processAppBuilds, f as processAppBuild, G as processSiteBuilds } from "./ctx_BmScp1M7.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_C7j9aJHH.mjs";
import { processAgentJobs } from "./agent-jobs_CKP6h5qb.mjs";
import { reclaimStaleApprovals } from "./approvals_DqjH7are.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_YSHkorhG.mjs";
import { guardHeavy } from "./diag_NtHEh0X8.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_BNaQQYSo.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_7yxiF4OV.mjs";
import { flushReports } from "./reports_CkTZLV9J.mjs";
import { addNotification, pushWebhook } from "./notifications_CBlFPHDk.mjs";
import { getNotifyWebhook } from "./settings_DI_y7gTJ.mjs";
import { purgeFiles } from "./storage_CXuQdqf4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_DVPDu8yJ.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  ...
  const at = await guardHeavy(env, "app triggers", () => import("./integration-polling_DCZw_jOB.mjs").then((m) => m.pollAppTriggers(env, locals.ctx)));
  const appTriggers = at.ok ? at.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_DFcLHmTB.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_CGkGx7qL.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_Bi89rK3N.mjs").then((m) => m.flushUsageDigest(env)));
