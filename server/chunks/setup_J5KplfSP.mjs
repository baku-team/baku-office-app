...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_CET1td35.mjs");
  const { googleConfigured } = await import("./google_DsMMCf3F.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_CYqfeXVd.mjs");
