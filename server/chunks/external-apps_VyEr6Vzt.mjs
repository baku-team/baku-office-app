...
import { getToken, hostFetch, getVerifyJwk } from "./client_o_bCkQmM.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_DvmaIa0w.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, validateDefinition, isBlockedHost } from "./appdef_ei1n0r-3.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_BKW0vRzv.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_ei1n0r-3.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_Bee2ijMS.mjs");
