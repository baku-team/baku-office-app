...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_qGL1F133.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BIwCOGmX.mjs");
  await import("./index_BZgMzI_G.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DlKVNdA9.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dl4WCKKC.mjs");
  ...
  const { reviewQueue } = await import("./users_K9vQwd3e.mjs");
  const { backupAlert } = await import("./backup_CXEr48ah.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BTUHKH0-.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CsdZqwDR.mjs");
