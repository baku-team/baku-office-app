...
import { i as inferApp, p as parseJsonObject } from "./ctx_C0kTytCf.mjs";
import { v as validateLayout, m as makeBlock } from "./site_DvSikWAd.mjs";
...
【再生成】前回の問題：${lastErr}。JSONオブジェクト1個だけを、hero と末尾の cta を必ず含め、各ブロックの主要テキストに具体的な中身を入れて出力してください（プレースホルダ禁止）。`;
    let raw = "";
    try {
      raw = await inferApp(env, user, { system: SYS + extra, maxTokens: 6e3 });
    } catch {
      lastErr = "AI呼び出し失敗";
      continue;
    }
