...
import { getApproval, decideApproval } from "./approvals_BYVkcwsF.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CiMMQ0Dd.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_DwTr64WL.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_BDozGNWZ.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_CtcYt0T4.mjs";
import { joinWithInvite } from "./users_CvA1-zYY.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_D2DJGMd8.mjs";
import { getDriveSave } from "./drive_Z4TKh5FF.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { logDiag, looksLikeLimit, PAID_HINT } from "./diag_NtHEh0X8.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CvA1-zYY.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_BhbspOQy.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DycVwT4X.mjs");
