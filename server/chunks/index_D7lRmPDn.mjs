...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C23diL6K.mjs");
  const { cmpVersion } = await import("./update_CqlVYQVB.mjs");
  await import("./index_ByOZSNRu.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BVCI0C7p.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BPsYJ0kN.mjs");
  ...
  const { reviewQueue } = await import("./users_DtIPLfad.mjs");
  const { backupAlert } = await import("./backup_BXZvuf1u.mjs");
