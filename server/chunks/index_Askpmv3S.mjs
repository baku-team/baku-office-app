...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$Index;
  const { getSession } = await import("./auth_DvN_P60r.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DPTfaqTP.mjs");
  const { cmpVersion } = await import("./update_DvoSy1Jy.mjs");
  await import("./index_BGQhwwqy.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BWa2L1Fq.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CCQRMVv0.mjs");
  ...
  const { reviewQueue } = await import("./users_MH4ZLneS.mjs");
  const { backupAlert } = await import("./backup_DnvuItq9.mjs");
