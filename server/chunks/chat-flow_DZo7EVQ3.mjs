...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_D6LLPvBw.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_DJA_I8xR.mjs";
import { canDevelopApps } from "./auth_CoFUCzQz.mjs";
import { cachedEntitlement } from "./client_CY40n_Ph.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DxLzC322.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_DxLzC322.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CDC9FeHL.mjs").then((n) => n.B);
...
async function tryPreAgentRouting(ctx, cfContext, args) {
  ...
  const priorAssistant = [...prior].reverse().find((m) => m.role === "assistant")?.content ?? "";
  const { looksLikeBuildConfirmation, looksLikeUiModeChoice, UI_MODE_QUESTION, UI_MODE_ACTIONS } = await import("./ctx_DxLzC322.mjs").then((n) => n.T);
  if (notPlan && canDevelopApps(role) && looksLikeBuildConfirmation(message, priorAssistant)) {
    const { startAppBuild, processAppBuild, buildModelGuide } = await import("./ctx_DxLzC322.mjs").then((n) => n.R);
  ...
  if (notPlan && canDevelopApps(role) && atLeast(await cachedEntitlement(env), "pro")) {
    const { looksLikeAppEdit } = await import("./ctx_DxLzC322.mjs").then((n) => n.T);
    if (looksLikeAppEdit(message)) {
      const { latestSessionApp, resolveAppByName, startAppEdit, processAppBuild, buildModelGuide } = await import("./ctx_DxLzC322.mjs").then((n) => n.R);
...
async function tryProHopsContinuation(ctx, cfContext, args) {
  ...
  if (!atLeast(await cachedEntitlement(env), "pro")) return null;
  const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_CKfNylmk.mjs");
