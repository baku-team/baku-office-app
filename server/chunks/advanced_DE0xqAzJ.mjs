...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BTiBrVBU.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_GytoKYml.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CXuQdqf4.mjs");
