...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CTSTpEVI.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DEsPVk-8.mjs");
  ...
  const { hasApiKey } = await import("./client_CTSTpEVI.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DAcrnttd.mjs");
