...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_peeVfBbE.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_CsqODza9.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DAEZsEe4.mjs");
