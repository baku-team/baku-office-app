...
import { getApiKey } from "./client_BwbWPyEb.mjs";
import { g as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, k as processSiteBuilds } from "./ctx_DAgrQ03M.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_CrHf1MAm.mjs";
import { processAgentJobs } from "./agent-jobs_XBc9kxiD.mjs";
import { reclaimStaleApprovals } from "./approvals_4iywgTuz.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_VwtiR2PL.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_Ds8fQI-4.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_BEaVdQSG.mjs";
import { flushReports } from "./reports_BMV48_Dt.mjs";
import { addNotification, pushWebhook } from "./notifications_GZWOBxbG.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_CKfFiifC.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_CPO6CNVr.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_iQON6brF.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
