...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CM436dbQ.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_DSXOrOmp.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_QCL1pbe9.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_PX595n4G.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CjLxCdtu.mjs";
import "./index_DM4gS8c2.mjs";
...
import { googleFetch } from "./google_CzEtNdGJ.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_Co82XXX4.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C5I-5cps.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DFi9-Q53.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_I7yGFDAg.mjs";
import { logDiag } from "./diag_ByGiCOpc.mjs";
import { i as isRunnableDefinition } from "./preflight_Cu2wTZ9a.mjs";
import { c as catalogPromptHint } from "./cdn-allowlist_Ck6a6kXM.mjs";
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BkiRi5z4.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_hAUInqJI.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_BoOTsRbP.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DzfOEU8z.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_BIvYTcUT.mjs";
import { saveLayoutDraft, listSites } from "./sites_o_dU3p1m.mjs";
...
import { storeCatalog } from "./store_CdMunyy5.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_DI9orpW2.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_o_dU3p1m.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BW-qxgfP.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BW-qxgfP.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BW-qxgfP.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DSXOrOmp.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BW-qxgfP.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CM436dbQ.mjs")).storageMode(env) === "r2";
