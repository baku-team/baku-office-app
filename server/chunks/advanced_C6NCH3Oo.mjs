...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CQSsPpWQ.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BFdCCvDY.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_C19jFPcM.mjs");
