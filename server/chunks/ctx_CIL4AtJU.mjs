...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CoDdssK4.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BCk4EGYy.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CC8SxGFO.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_A_R7zOf9.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_C1VtaqKi.mjs";
import "./index_De32HSX0.mjs";
...
import { googleFetch } from "./google_C5HMqpnO.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_X-C7jdsb.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BdosPRcV.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BXIhHaRu.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_CQDnhpKq.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_CDjorHYh.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_CksBaO21.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_C0iGshev.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_5T6IOeGu.mjs";
import { storeCatalog } from "./store_CgZFOLfr.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CU61v_Mn.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_SjDxLk_m.mjs";
import { saveLayoutDraft, listSites } from "./sites_B9sGnvPQ.mjs";
...
import { p as pushToUser } from "./push_fQlDgFEE.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_B9sGnvPQ.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_DPCwT9Xl.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_DPCwT9Xl.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_DPCwT9Xl.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_CoDdssK4.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BCk4EGYy.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_DPCwT9Xl.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CoDdssK4.mjs")).storageMode(env) === "r2";
