...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_B0LtoEmz.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BITEyxP2.mjs");
  await import("./index_DnpuPih6.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_7tMaKadi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Ct17D-vs.mjs");
  ...
  const { reviewQueue } = await import("./users_B4GVyeir.mjs");
  const { backupAlert } = await import("./backup_CW2pCKCh.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BP0r-k3z.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_B8-hnGVx.mjs");
