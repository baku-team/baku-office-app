...
import { validateDefinition, appApprovalEffects, roleCanOpenScreen, isBlockedHost, stepNeedsApproval } from "./appdef_DL_Q9-YV.mjs";
import { a as scopeCtx } from "./parts_CYwgYHWx.mjs";
import { g as getAppDesign, a as activeAppDefinition, r as roleCanUseApp } from "./external-apps_CRZxmueG.mjs";
import { i as isGoogleOp, a as GOOGLE_OPS, r as runGoogleOp } from "./google-bridge_CxaeMFe4.mjs";
import { googleStatus, SCOPE_GROUPS } from "./google_D2mb2lvi.mjs";
import { entitlementForGate } from "./client_BHT8hMec.mjs";
...
import { enqueueReport } from "./reports_B7FTJ_Kw.mjs";
...
async function runApp(ctx, def, inputs, owner, screenId, appId, opts) {
  ...
  if (!("rec" in bind) && !defSteps.some((st) => st.as === "rec")) {
    const rec = {};
    for (const inp of defInputs) rec[inp.name] = bind[inp.name];
    bind.rec = rec;
  }
  ...
  try {
    for (const s of defSteps) {
      curStep = s;
      const out = await runStep(ctx, def, s, { ref, interp, jsonTemplate, asText, bind, owner, dryRun: phaseEffDry(s, opts?.phase, opts?.approvalOn ?? true, !!opts?.dryRun), role: opts?.role });
...
async function runInstalledApp(ctx, appId, inputs, owner, screenId, role, opts) {
  ...
  const res = await runApp(scopeCtx(ctx, def.permissions), def, inputs, owner, screenId, appId, { role, ...opts });
...
async function runDataOp(ctx, def, s, h) {
  ...
  switch (s.op) {
    ...
    case "data.update": {
      ...
      const statusVal = s.status === void 0 ? null : typeof s.status === "string" && s.status.startsWith("$") ? h.asText(h.ref(s.status)).slice(0, 40) : h.interp(String(s.status)).slice(0, 40);
