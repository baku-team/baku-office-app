...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_rF0xK8SJ.mjs");
  const { cmpVersion } = await import("./update_CN4BsWAs.mjs");
  await import("./index_DFmNUVmQ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Fv0mgUHT.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Ds1D7Hll.mjs");
  ...
  const { reviewQueue } = await import("./users_De6KMQo9.mjs");
  const { backupAlert } = await import("./backup_DMmHMzfk.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CbPM17XJ.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_lxsY0y5P.mjs");
