...
import { getToken, hostFetch, getVerifyJwk } from "./client_Bib4yTGq.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_ChR00g-2.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, validateDefinition, isBlockedHost } from "./appdef_D9vMBBjU.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_nBAdPSxh.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_D9vMBBjU.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_DmeGlKV0.mjs");
