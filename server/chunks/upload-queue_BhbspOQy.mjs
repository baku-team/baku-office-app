...
import { getFile } from "./storage_D2DJGMd8.mjs";
import { getDriveSave } from "./drive_Z4TKh5FF.mjs";
import { n as notifyOwnerDirect } from "./ctx_DwTr64WL.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_DwTr64WL.mjs").then((n) => n.U);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_D2DJGMd8.mjs");
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_DwTr64WL.mjs").then((n) => n.P);
