...
import { APP_SCHEMA, validateDefinition } from "./appdef_CjtXKAi2.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_C7rooffR.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BS4K6Q4H.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
