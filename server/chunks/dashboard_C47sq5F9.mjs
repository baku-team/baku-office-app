...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BHT8hMec.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BGnwi0i5.mjs");
  await import("./index_11AnePHL.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DB2YNaub.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DMmo2mEs.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_ryUreqJI.mjs");
  ...
  const { reviewQueue } = await import("./users_CV8Vqaki.mjs");
  const { backupAlert } = await import("./backup_DtbbuDCy.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CRZxmueG.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_Bflf1Se6.mjs");
  const { getWorkersPaid } = await import("./settings_C4tL9AHR.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_X9PGsLLE.mjs");
