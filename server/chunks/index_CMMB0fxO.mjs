...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, getApiKey } = await import("./client_DlpdsadF.mjs");
  ...
  if (isAdminOrg) {
    const { googleConfigured } = await import("./google_Ca1jl-Ew.mjs");
  ...
  const groups = [
    ...
    { title: "外部サービス設定", items: [
      { href: "/settings/integrations", label: "外部サービス設定（AI／LINE・Discord・Slack／Google／Notion）", desc: "外部サービスとの接続設定を1か所で", show: isAdminOrg }
    ] },
