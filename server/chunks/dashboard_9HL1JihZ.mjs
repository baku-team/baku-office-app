...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Cti--qkw.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_ZZktGbSy.mjs");
  await import("./index_UJbeOQUy.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_m8OOu-NR.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Ctng9dk8.mjs");
  ...
  const { reviewQueue } = await import("./users_CqQvwEcj.mjs");
  const { backupAlert } = await import("./backup_bl2fAwfM.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_xRQxQ_Fm.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_Bm-CmJY_.mjs");
