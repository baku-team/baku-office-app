...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_yS3Z_a8H.mjs");
  const { cmpVersion } = await import("./update_CL14mXKL.mjs");
  await import("./index_CpFO7isQ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Cem46T0w.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BqT-ddm8.mjs");
  ...
  const { reviewQueue } = await import("./users_DkMhM7L4.mjs");
  const { backupAlert } = await import("./backup_BYKX13l0.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_60xKlRS4.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_K1qefTQL.mjs");
