...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DkJb1xQq.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Cp6RIs0n.mjs");
  await import("./index_DPUrRbwk.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DX3YoKbL.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D6vc-PVH.mjs");
  ...
  const { reviewQueue } = await import("./users_DPntK0eN.mjs");
  const { backupAlert } = await import("./backup_CGdB7tT3.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BSfdE0OQ.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_BpQ232ZA.mjs");
