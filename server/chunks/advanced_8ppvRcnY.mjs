...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BaLSnevk.mjs")).cachedEntitlement(env);
  const hasPlus = atLeast(entitlement, "plus");
  const hasPro = atLeast(entitlement, "pro");
  const { getAiEngine, getCustomPrompt, getWorkersPaid, getNotifyWebhook } = await import("./settings_BmjoORVR.mjs");
  const workersPaid = hasPlus ? await getWorkersPaid(env) : false;
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BWOJJ7c9.mjs");
  ...
  const { hasApiKey } = await import("./client_BaLSnevk.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BABOdLp1.mjs");
