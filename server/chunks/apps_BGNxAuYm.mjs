...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BV30PACS.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_DiPzP2bH.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_Cmhn0ct_.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DDhRaaxU.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CSXPq6XS.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CyLGJDop.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_DwWffkbb.mjs");
