...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_BCrexcwr.mjs");
  const { googleConfigured } = await import("./google_SQBWdaD4.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_QVWqwlx2.mjs");
