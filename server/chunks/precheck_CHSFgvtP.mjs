...
import { validateDefinition } from "./appdef_BJHimmJt.mjs";
import { g as getAppDesign } from "./external-apps_DmeO_5kU.mjs";
import { p as preflight } from "./preflight_BueUwfqI.mjs";
import { r as runDraftApp, p as platformInvariantSuspected, f as escalatePlatformInvariant } from "./app-runtime_BMnoiTJJ.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_DmeO_5kU.mjs").then((n) => n.B)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_D9aXYLsc.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
