...
import { setRetentionDays, setMaxUploadMb, fileBelongsTo, getFile, saveFile, storageMode } from "./storage_DhH-wMJZ.mjs";
import { m as makeDocument, w as webSearch, b as summarizeTranscript, c as extractInvoiceData, t as transcribeAudio, i as inferApp } from "./media-ai_ByaGvXL-.mjs";
import { googleFetch } from "./google_D6pEN1NA.mjs";
import { getApiKey, entitlementForGate } from "./client_kiIL6iv0.mjs";
...
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp } from "./apps_GjR2Wx6H.mjs";
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, opCatalogText, checkRenderScripts, checkRenderHandlers } from "./appdef_Co7mLGGh.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BejmFBJ_.mjs";
import { searchDirectory } from "./directory_CvZORtO6.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_CRU99806.mjs";
import { logDiag } from "./diag_Cu0HTcAn.mjs";
import { i as isRunnableDefinition } from "./preflight_CAdpD9_o.mjs";
...
import { g as getAppDesign, m as createDraft, n as repairOutputLiterals, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_B8q51KTm.mjs";
...
import "./index_BMK2adnV.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_nV6-0Rrb.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_ClhTyJFR.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_B4dTtRx4.mjs";
import { overBudget, recordUsage, estimateUsd, recordTokens } from "./usage_DN6pGBs1.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_Dn0Ys7Zn.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers } from "./users_DYcPALfj.mjs";
import { listActions, createAction, deleteAction, updateAction } from "./a2a-actions_Ib8P7EBR.mjs";
import { p as pushToUser } from "./push_Xy1sh-mg.mjs";
...
- 音声認識・カメラ：利用可（iframe に microphone/camera/autoplay 委譲済み）。ユーザー操作起点でブラウザの許可プロンプトが出る。音声の文字起こしは Web Speech API（SpeechRecognition||webkitSpeechRecognition・lang='ja-JP'・サーバ不要）、カメラは navigator.mediaDevices.getUserMedia({video:true})→<video autoplay playsinline muted>でプレビュー＋canvasで撮影(toDataURL)、録音は MediaRecorder。非対応・許可拒否時は手入力等にフォールバックし、使い終えたら stream の track を stop。取得メディアは fetch 不可なので、AI処理が要るときは dataURL(base64)やテキストを bo.run の入力に載せて ai.infer へ渡す。
...
async function editStep(ctx, row, model) {
  ...
  if (!design || !design.definition || typeof design.definition !== "object") {
    await failBuild(ctx, row, explainStop("system", "修正対象のアプリが見つからないか、定義が壊れています。", "アプリ画面（/apps）で対象アプリが存在するか確認し、無ければ作り直してください。"));
    return false;
  }
...
検証エラー：${issueText}`;
  ...
  if (!vr.ok) {
    await failBuild(ctx, row, explainStop("ai", "修正後の定義が検証に通りませんでした（" + vr.issues.slice(0, 2).map((i) => i.message).join(" / ") + "）。", "対象を「この画面のこの項目」のように絞って再依頼してください。別のAIモデルへの切替も有効です。"));
    return false;
  }
  ...
  const finalMsg = `修正案を作成しました（版 ${def.version}・下書き・${phases.length > 1 ? `${doneCount}/${phases.length}工程・` : ""}）。` + (uiCheck.note ? `
${uiCheck.note}` : "\nUI動作チェック：ボタン等の呼び出しに問題はありませんでした。") + (failed.length ? `
※ 未適用の工程：${failed.join("・")}（もう一度同じ依頼で続けて直せます）` : "") + (res.gate === "ready" ? "\nプレビューで確認し、問題なければ「アプリ」画面の「アプリ開発」で登録すると反映されます（登録するまで現在の公開版のままです）。" : "\n※ 事前確認に課題があります。内容をご確認ください。");
...
async function stepBuild(ctx, row, model, paid) {
  ...
  if (row.status === "planning") {
    ...
    if (!plan2) {
      await failBuild(ctx, row, explainStop("ai", "AIが実装プランを生成できませんでした。", "要件をもう少し具体化して再度ご依頼ください。別のAIモデルへの切替も有効です。"));
      return false;
    }
...
async function verifyAndRepairUi(ctx, model, def, paid, onStep) {
  const env = ctx.env;
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    const issue = checkRenderScripts(h2) || checkRenderHandlers(h2);
...
onclick 等のイベントが呼ぶ関数を <script> 内に正しく実装する、または呼び出し名を既存の関数に合わせる。`;
    const r = await makePatch(env, model, cur, fix, paid).catch(() => ({ patch: null, truncated: false, text: "" }));
    if (!r.patch) break;
  ...
  const remain = checkRenderScripts(h) || checkRenderHandlers(h);
...
async function finalize(ctx, row, plan, def, model) {
  ...
  if (!isRunnableDefinition(def)) {
    await failBuild(ctx, row, explainStop("ai", "各工程は実装しましたが、AIが実行可能な形にまとめきれませんでした。", "要件を分けて（機能・画面ごとに）再度ご依頼ください。"));
    return false;
  }
  const uiCheck = await verifyAndRepairUi(ctx, model, def, row.paid === 1, (m) => post(ctx, row, m));
  def = uiCheck.def;
...
` + (ready ? "下のボタンからプレビューで動作を確認し、問題なければ登録できます。" : "一部の事前確認に課題があります。下のボタンから内容をご確認ください。") + (uiCheck.note ? `
${uiCheck.note}` : "\nUI動作チェック：ボタン等の呼び出しに問題はありませんでした。") + (publicSlug ? `
...
async function runBatch(ctx, row, paid, baseUrl) {
  ...
  if (nowSec() - row.created_at > MAX_BUILD_AGE) {
    await failBuild(ctx, row, explainStop("ai", "実装が長引き、制限時間に達したため中断しました（これまでの工程は草案に保存済み）。", "要件を分けて（機能・画面ごとに）再度ご依頼ください。Workers Paid 有効化で一度に長く処理できます（設定→高度なオプション /settings/advanced）。"));
    return false;
  }
  if (row.attempts >= MAX_BUILD_ATTEMPTS) {
    await failBuild(ctx, row, explainStop("system", "内部処理が繰り返し失敗したため中断しました（これまでの工程は草案に保存済み）。", "要件を分けて再度ご依頼ください。続く場合は管理者へご連絡ください。"));
    return false;
  }
  const model = await buildChatModel(ctx.env, row.model ?? void 0);
  if (!model) {
    await failBuild(ctx, row, explainStop("system", "AIモデルが未設定のため実装できません。", "管理者が『設定→連携 /settings/messaging』で Gemini / Claude / ChatGPT のAPIキーを登録してください。"));
    return false;
  }
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_kiIL6iv0.mjs");
