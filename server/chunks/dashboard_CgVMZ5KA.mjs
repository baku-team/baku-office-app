...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DPM1hhZa.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_ugDPm0Am.mjs");
  await import("./index_D_uNiYtv.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C0JAOYV7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_ONvV-GIo.mjs");
  ...
  const { reviewQueue } = await import("./users_C7cAYI7K.mjs");
  const { backupAlert } = await import("./backup_BUVThEk_.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_C2915SQL.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_D1aDtodm.mjs");
