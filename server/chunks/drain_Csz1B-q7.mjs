...
import { getApiKey } from "./client_BsuDyowk.mjs";
import { g as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, k as processSiteBuilds } from "./ctx_D0PzYluR.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_XkRS78br.mjs";
import { processAgentJobs } from "./agent-jobs_DC3jcjA-.mjs";
import { reclaimStaleApprovals } from "./approvals_DNSANYcE.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_DpYoyyG6.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_C-1DtKp-.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_BrE9XhLt.mjs";
import { flushReports } from "./reports_D-cocfxj.mjs";
import { addNotification, pushWebhook } from "./notifications_DJb4GBxI.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_CqwoO7zq.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const au = await guardHeavy(env, "auto update", () => import("./update_qNLFDUUW.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  return json({ ok: true, sent, notified, summarized, scheduled, video, agentJobs, appBuilds, appBuildsMore, siteBuilds, siteBuildsMore, autoUpdate, approvalsReclaimed, driveBackup, fullBackup, reportsSent, purged });
};
