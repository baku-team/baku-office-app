...
import { APP_SCHEMA, validateDefinition } from "./appdef_glfdKzNP.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_Fv0mgUHT.mjs";
import { p as preflight } from "./preflight_B6xvoL8Q.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_DUN9A_L9.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
