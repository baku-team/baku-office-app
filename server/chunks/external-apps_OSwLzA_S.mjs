...
import { getToken, hostFetch, getVerifyJwk } from "./client_BvxaQHJg.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_Oyt6lT9S.mjs";
import { sanitizeRenderHtml, validateDefinition } from "./appdef_Dtsrvmbq.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_Dtsrvmbq.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (!okApproved) {
          ...
          const { createApproval } = await import("./approvals_CFAZoyFw.mjs");
