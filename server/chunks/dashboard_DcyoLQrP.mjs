...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DsktjJVP.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_bogNddSR.mjs");
  await import("./index_CIn9PEgV.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bo1OmsPF.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DsqMfTvr.mjs");
  ...
  const { reviewQueue } = await import("./users_aSqZYt_j.mjs");
  const { backupAlert } = await import("./backup_875s5RAk.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DtyjwiHH.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_Cknt-1Jr.mjs");
