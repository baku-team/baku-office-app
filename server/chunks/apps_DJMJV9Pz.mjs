...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DoAWDsqP.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CZasnwOK.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_6IcBz0fb.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DJ7cZUGv.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BJqp2VVv.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_Bh6hTC8W.mjs");
