...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_D2mb2lvi.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_BhoCcFTw.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_D2hNWsGE.mjs");
  ...
  const { buildCtx } = await import("./ctx_BM9R08qO.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_BM9R08qO.mjs").then((n) => n.M);
