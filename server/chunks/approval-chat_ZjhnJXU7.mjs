...
import { getApproval, decideApproval } from "./approvals_DNYUp_f4.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CATBeV5o.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_BM9R08qO.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_DbDGp-7Y.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BHT8hMec.mjs";
import { joinWithInvite } from "./users_CV8Vqaki.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_Bflf1Se6.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CV8Vqaki.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_TxlupWiC.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DEKtzgR5.mjs");
