...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BRlB-70J.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_o_bCkQmM.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_f2x1sCW8.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BKW0vRzv.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_VyEr6Vzt.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_574Mnee4.mjs");
