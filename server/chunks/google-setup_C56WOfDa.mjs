...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CJNQCNCS.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_dQx1gP2c.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_BcMltRnA.mjs");
  ...
  const { buildCtx } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.U);
  const { enabledPartIds } = await import("./parts_CVGBl5kS.mjs").then((n) => n.f);
  ...
  const { randomId } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.J);
