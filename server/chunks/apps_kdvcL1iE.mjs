...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CAci7FIw.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_DcvmK-U8.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_J60yRFFB.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CpqQ2B4g.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_CZwKXiCg.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_CsnMQ0nm.mjs");
