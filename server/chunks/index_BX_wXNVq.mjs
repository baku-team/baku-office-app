...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$Index;
  const { getSession } = await import("./auth_CGC5sf0h.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_F7ZfSNZF.mjs");
  const { cmpVersion } = await import("./update_BKvhqxW_.mjs");
  await import("./index_C_5HGQfJ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DlWFE1gi.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DDEKWLbc.mjs");
  ...
  const { reviewQueue } = await import("./users_B5JeV6xv.mjs");
  const { backupAlert } = await import("./backup_Cr5Pv5Qf.mjs");
