...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_-0prxDd1.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CSBhOn7n.mjs");
  ...
  const agentName = await (await import("./settings_BKzORwjT.mjs")).getAgentName(env).catch(() => "相棒");
  const ttsCloudReady = await (await import("./capabilities_BiDkfFSy.mjs")).ttsConfigured(env).catch(() => false);
  const { GEMINI_MODELS, CLAUDE_MODELS, OPENAI_MODELS, WORKERS_AI_MODELS, GROK_MODELS, GITHUB_MODELS, GROQ_MODELS, CEREBRAS_MODELS } = await import("./config_Bb0eO8Ma.mjs");
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    import("./settings_BKzORwjT.mjs"),
    import("./skills_C_09CGOq.mjs"),
    import("./scheduled-tasks_CFPnG8A1.mjs")
  ]);
  ...
  await import("./index_B0UyH5ZM.mjs");
