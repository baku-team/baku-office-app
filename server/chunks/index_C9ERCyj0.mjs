...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CTSTpEVI.mjs");
  const { cmpVersion } = await import("./update_DHYtdEGL.mjs");
  await import("./index_DGUEyazM.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_D2wGK6QL.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Bxct6EX_.mjs");
  ...
  const { reviewQueue } = await import("./users_C4Eo3ed-.mjs");
  const { backupAlert } = await import("./backup_DysPmhw8.mjs");
