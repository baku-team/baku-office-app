...
import { getApiKey } from "./client_Bf1r67RB.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_CSZaRoVj.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_BcGgaiPh.mjs";
import { processAgentJobs } from "./agent-jobs_CLT3tfsh.mjs";
import { reclaimStaleApprovals } from "./approvals_6LeGYTL9.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_B8xl56ZF.mjs";
import { guardHeavy } from "./diag_CY3PJnOM.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_BNi-UTlk.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_DsAyR87I.mjs";
import { flushReports } from "./reports_ZWFolRXJ.mjs";
import { addNotification, pushWebhook } from "./notifications_Yp4r_6gu.mjs";
import { getNotifyWebhook } from "./settings_dTyIbTCR.mjs";
import { purgeFiles } from "./storage_DZ4X55g1.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_BqzkYksd.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_BN22BQ01.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_BIUupLpr.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_Dl7Qzqsc.mjs").then((m) => m.flushUsageDigest(env)));
  ...
  return json({ ok: true, sent, notified, summarized, scheduled, video, agentJobs, appBuilds, appBuildsMore, siteBuilds, siteBuildsMore, uploadBatches, autoUpdate, approvalsReclaimed, driveBackup, fullBackup, reportsSent, weeklyReport, usageDigest, purged });
};
