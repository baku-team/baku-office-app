...
import { getToken, hostFetch, getVerifyJwk } from "./client_RgvQ0ags.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_C1s3LiGN.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, deriveConfigFieldsFromUsage, validateDefinition, isBlockedHost } from "./appdef_B3uQ458a.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_AVdbmGFq.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_B3uQ458a.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_BZa_3z-V.mjs");
