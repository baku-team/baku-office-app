...
import { getApiKey } from "./client_o_bCkQmM.mjs";
import { c as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, m as processAppBuild, o as processSiteBuilds } from "./ctx_BRlB-70J.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_Bn_fgYPT.mjs";
import { processAgentJobs } from "./agent-jobs_Barhz2Di.mjs";
import { reclaimStaleApprovals } from "./approvals_Bee2ijMS.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_CBAQ1-g5.mjs";
import { guardHeavy } from "./diag_-P_u6Fn4.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_DJsdgGT7.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_BKYO9M7j.mjs";
import { flushReports } from "./reports_DfbreC5L.mjs";
import { addNotification, pushWebhook } from "./notifications_DbEuK3I-.mjs";
import { getBuildDriver, getNotifyWebhook } from "./settings_C4tL9AHR.mjs";
import { purgeFiles } from "./storage_DlJhVZeY.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_CsBZEIjQ.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  ...
  const at = await guardHeavy(env, "app triggers", () => import("./integration-polling_B5uoTlyv.mjs").then((m) => m.pollAppTriggers(env, locals.ctx)));
  const appTriggers = at.ok ? at.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_UHCqn2Pz.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_C70aYylG.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_mRHlfB46.mjs").then((m) => m.flushUsageDigest(env)));
