...
import { r as renderComponent } from "./worker-entry_BIRaG6a2.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CKpm4DNM.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CPa2Fh_W.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CPa2Fh_W.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_BtyRFUPY.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_DIsDv6Hl.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_6IGpA5sE.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CgPAAqz7.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CE1vvWJb.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_D-Ss6Oxl.mjs");
