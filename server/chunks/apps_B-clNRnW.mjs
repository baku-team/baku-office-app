...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_-8wQqFQR.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_DLbTjLG9.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_B-lkqngg.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DsS66e1p.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Bx2onJKl.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BJqjrgQW.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_rU7SKhNc.mjs");
