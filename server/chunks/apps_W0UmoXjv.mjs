...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BzuZOfxt.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_C8LO9Go2.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_C1ICAElS.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B7RAaLcs.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DQMqrpyH.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_ppKOQgJW.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_B79cwq38.mjs");
