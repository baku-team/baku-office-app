...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BNXMhaMG.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DppbQ30s.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BcmxKHWi.mjs");
