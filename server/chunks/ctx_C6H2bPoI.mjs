...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_DV5PW-A4.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CSBhOn7n.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Cjh13nrA.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BQwRbui4.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_fVy9d1St.mjs";
import "./index_B0UyH5ZM.mjs";
...
import { googleFetch } from "./google_CGR1ugZH.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_BexAdeyk.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CRCjI2bI.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_W3OLgHnB.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_C40apYhL.mjs";
import { logDiag } from "./diag_DWCSrGCf.mjs";
import { i as isRunnableDefinition } from "./preflight_0kD82fK8.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_C_09CGOq.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_D5f_9wYJ.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BiDkfFSy.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_CrXkUnnq.mjs";
import { saveLayoutDraft, listSites } from "./sites_DqRV3il6.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_BS6Fvt6h.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_C6F8kDnB.mjs";
...
async function buildChatModel(env, modelId) {
  ...
  if (claudeKey) return claudeModel(claudeKey, modelId && isValidClaudeModel(modelId) ? modelId : "claude-sonnet-5");
...
検証エラー：${issueText}`;
  ...
  const review = failed.length || !!uiCheck.note ? await reviewApp(model, row.spec ?? design.spec ?? "", editGoal, def, row.paid === 1).catch(() => ({ issues: [], summary: "" })) : { issues: [] };
...
async function finalize$1(ctx2, row, plan, def, model) {
  ...
  const review = partial ? await reviewApp(model, row.spec ?? plan.description, plan.goal, def, row.paid === 1).catch(() => ({ issues: [], summary: "" })) : { issues: [] };
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DqRV3il6.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_CttyxCCy.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_CttyxCCy.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_CttyxCCy.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CSBhOn7n.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_CttyxCCy.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_DV5PW-A4.mjs")).storageMode(env) === "r2";
