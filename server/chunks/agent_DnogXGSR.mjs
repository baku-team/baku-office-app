...
import { entitlementForGate, getApiKey } from "./client_CyzvxOBO.mjs";
...
import { callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DUTp50JF.mjs";
import { searchDirectory } from "./directory_BNnvwp-t.mjs";
import { autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_CYD_zrxV.mjs";
import { DEFAULT_MODELS, isValidWorkersAiModel, workersAiModelId, claudeModelId, geminiModelId } from "./config_BOdbJfMp.mjs";
import "./index_CpMSUaak.mjs";
import { m as makeDocument, w as webSearch } from "./media-ai_yvXyejFD.mjs";
import { runSkill, generateSkill, listSkills } from "./skills_waeWRO07.mjs";
import { createDraft } from "./external-apps_BfM6GdIz.mjs";
import { videoStatusText, invokeCapability, listCapabilities, capabilitySummary } from "./capabilities_CTlZDY7A.mjs";
import { overBudget, recordUsage, recordTokens, estimateUsd } from "./usage__IUbXlIy.mjs";
import { getApprovalMode, previewFor, createApproval, A2A_OUTWARD } from "./approvals_DL4ycFLy.mjs";
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CyzvxOBO.mjs");
...
${WORKERS_AI_NOTE}` : sys;
  const out = await runToolLoop(model, sysForRun, first, decls, exec, hops, history, onUsage, abort);
