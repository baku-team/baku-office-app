...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DkS04vDB.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_Cti--qkw.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_UJbeOQUy.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_m8OOu-NR.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_xRQxQ_Fm.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_CoEo_yWT.mjs");
