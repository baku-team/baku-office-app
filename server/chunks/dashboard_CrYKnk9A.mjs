...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_B17j1uG_.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DW4YJI28.mjs");
  await import("./index_BCH4lAnd.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CVjeTeB8.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CW0Kmt-j.mjs");
  ...
  const { reviewQueue } = await import("./users_BYbdQprm.mjs");
  const { backupAlert } = await import("./backup_Q51GEanX.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DdRsAqqg.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BthdaNTE.mjs");
