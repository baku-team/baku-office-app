...
import { getAutoUpdate, hasDeployHook, clearDeployHook, setAutoUpdate, getDeployHook, isValidHookUrl, saveDeployHook } from "./update_C7zudgPi.mjs";
...
const GET = async ({ locals, request }) => {
  ...
  return json({ configured: await hasDeployHook(env), auto: await getAutoUpdate(env) });
};
