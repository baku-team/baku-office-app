...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_B_HP_Ex9.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_C13894FS.mjs");
  await import("./index_Bj0rITH8.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CCYEJicX.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BY2W4YRQ.mjs");
  ...
  const { reviewQueue } = await import("./users_Bxtx825g.mjs");
  const { backupAlert } = await import("./backup_CkNvxg6q.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_BgjO-NRr.mjs");
