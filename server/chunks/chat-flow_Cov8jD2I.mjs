...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B6I9yBIu.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_CPa2Fh_W.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_BtyRFUPY.mjs").then((n) => n.P);
  ...
  const { latestSessionApp } = await import("./ctx_BtyRFUPY.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CE1vvWJb.mjs").then((n) => n.z);
