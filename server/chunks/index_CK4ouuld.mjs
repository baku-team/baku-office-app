...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C9EMiQbG.mjs");
  const { cmpVersion } = await import("./update_CeFuxKOE.mjs");
  await import("./index_GEt0WlwX.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Br_IjkaX.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Ba3wilzZ.mjs");
  ...
  const { reviewQueue } = await import("./users_HUdabi85.mjs");
  const { backupAlert } = await import("./backup_BXHnmGCH.mjs");
