...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_TPDi1U5M.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_Bv3SAITG.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_v-7HzW5v.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DwFb6RgQ.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CnHsjCoE.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.A);
  const { storeCatalog, myApps } = await import("./store_DII1RAZO.mjs");
