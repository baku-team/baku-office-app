...
import { getSession } from "./auth_clq7fc2y.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_CqwoO7zq.mjs";
import { setMemberModel, toggleFavApp, setAppFolders, setAiEngine, setSmartRouting, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_C_1axsCz.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_ClXjVEHs.mjs";
import { setStorageLimits } from "./storage-usage_C7DrL7II.mjs";
...
import { setCustomDomain, setSiteConfig } from "./custom-domain_BlEKJqw-.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_ZgdMwRke.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_BLW-izfW.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const { canDevelopApps } = await import("./auth_clq7fc2y.mjs");
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DNSANYcE.mjs");
  ...
  if (b._action === "notify_webhook") {
    ...
    if (url) {
      const { isPublicHttpsUrl } = await import("./egress_BjyQB9oJ.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_DJb4GBxI.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_D0PzYluR.mjs").then((n) => n.L);
      try {
  ...
  if (b._action === "notify_line") {
    const bb = b;
    if (bb.capture) {
      const last = await env.LICENSE.get("line_last_sender").catch(() => "") ?? "";
      if (!last) return json({ ok: true, to: "", captured: false });
      await env.LICENSE.put("notify_line_to", last);
      await env.LICENSE.delete("line_last_sender").catch(() => {
      });
      return json({ ok: true, to: last, captured: true });
    }
    const to = String(bb.to ?? "").trim();
    if (to) await env.LICENSE.put("notify_line_to", to);
    else await env.LICENSE.delete("notify_line_to");
  ...
  if (b._action === "cf_provision") {
    const { getCustomDomain, siteHostsOf } = await import("./custom-domain_BlEKJqw-.mjs");
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_L7PvOe2W.mjs");
  ...
  if (b._action === "app_dev_log") {
    ...
    const { getMessages } = await import("./chat-sessions_BLqLinNd.mjs").then((n) => n.i);
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.A);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.A);
