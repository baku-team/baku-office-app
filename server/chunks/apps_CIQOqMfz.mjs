...
import { r as renderComponent } from "./worker-entry_DBjmFfEO.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_R7lxPNGT.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_Dv895O2a.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_Dv895O2a.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_BPZJrufk.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D_QQxh5_.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_0DZIpoB3.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Dj_vTif6.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_EqUw7sow.mjs");
