...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_B17j1uG_.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_koKo0H-8.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BthdaNTE.mjs");
