...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_DIsDv6Hl.mjs");
  const { googleConfigured } = await import("./google_CulnzWdd.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_D3fZa9hR.mjs");
