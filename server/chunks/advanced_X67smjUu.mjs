...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client__0Sedoue.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BvIeCoUM.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_yFW5rSlV.mjs");
