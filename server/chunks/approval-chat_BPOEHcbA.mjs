...
import { getApproval, decideApproval } from "./approvals_I51IqE6Z.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CliC3F6V.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_Dy72pGSg.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Dm9l3J_i.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BAVQqvb3.mjs";
import { joinWithInvite } from "./users_DqOcrHOp.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BVOr0lJw.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_DqOcrHOp.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_C_HjAFHV.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_CdTTMA5j.mjs");
