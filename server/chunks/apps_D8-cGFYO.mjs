...
import { r as renderComponent } from "./worker-entry_DJ-x0Ut5.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DZYOdo_1.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CiP0tFu8.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CiP0tFu8.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_Pbf3tzM1.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_CDsrEXRz.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Cx5zbqbt.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_C3ejEHDJ.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B8UBajsg.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CNu_M4KU.mjs");
