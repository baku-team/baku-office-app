...
import { getSession } from "./auth_Bh_tJP0x.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_C0xk2SHi.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_Djx5eoR2.mjs";
import { setStorageLimits } from "./storage-usage_9uI8ycKM.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_DMCgfXnA.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_ehcQ_Xp6.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const { canDevelopApps } = await import("./auth_Bh_tJP0x.mjs");
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_CpyhgkQw.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_5KkK0xTP.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.M);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_C3hyS9pN.mjs").then((n2) => n2.S);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_CyvjIqLX.mjs");
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.B);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.B);
