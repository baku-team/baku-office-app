...
import { getApiKey } from "./client_B17j1uG_.mjs";
import { c as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, m as processAppBuild, o as processSiteBuilds } from "./ctx_Ige4T837.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_DpaSPqPu.mjs";
import { processAgentJobs } from "./agent-jobs_D1mo_Bd7.mjs";
import { reclaimStaleApprovals } from "./approvals_w6xNIQ3l.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_69WgPwEr.mjs";
import { guardHeavy } from "./diag_-P_u6Fn4.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_ARKOgIQO.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_Q51GEanX.mjs";
import { flushReports } from "./reports_CSgE-XRk.mjs";
import { addNotification, pushWebhook } from "./notifications_CduOQvTk.mjs";
import { getBuildDriver, getNotifyWebhook } from "./settings_C4tL9AHR.mjs";
import { purgeFiles } from "./storage_BthdaNTE.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const buildDriver = await getBuildDriver(env);
  let appBuilds = 0;
  let appBuildsMore = false;
  const activeIds = new Set(buildDriver === "do" ? [] : seedIds);
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_Dd3X8Oj6.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  ...
  if (buildDriver !== "do") {
    ...
    while (activeIds.size && Date.now() - contStart < 12e4) {
      ...
      for (const id of [...activeIds]) {
        const g2 = await guardHeavy(env, "app build cont", () => processAppBuild(locals.ctx, id, origin, { maxSteps: 4, deadlineMs: 25e3, src: "drain-cont" }));
        if (!g2.ok) {
          activeIds.clear();
          appBuildsMore = true;
          break;
  ...
  const at = await guardHeavy(env, "app triggers", () => import("./integration-polling_Dj_VpHuS.mjs").then((m) => m.pollAppTriggers(env, locals.ctx)));
  const appTriggers = at.ok ? at.value : 0;
  const ib = await guardHeavy(env, "inbound events", () => import("./inbound-durable_C63tN5W3.mjs").then((m) => m.drainStuckInboundEvents(locals.ctx)));
  const inboundStuck = ib.ok ? ib.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_DW4YJI28.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_CzUQCPVf.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_DseepYiF.mjs").then((m) => m.flushUsageDigest(env)));
  ...
  await env.LICENSE.put("drain_last_ok", String(Math.floor(Date.now() / 1e3)), { expirationTtl: 7 * 86400 }).catch(() => {
  });
  return json({ ok: true, sent, notified, summarized, scheduled, video, agentJobs, appBuilds, appBuildsMore, activeBuildIds: [...activeIds], siteBuilds, siteBuildsMore, uploadBatches, appTriggers, inboundStuck, autoUpdate, approvalsReclaimed, driveBackup, fullBackup, reportsSent, weeklyReport, usageDigest, purged });
};
