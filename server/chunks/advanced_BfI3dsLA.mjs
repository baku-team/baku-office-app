...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BR_H-Abo.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Kx7cBb_m.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_hKiHcy_A.mjs");
