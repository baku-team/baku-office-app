...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BZPpVhKD.mjs");
  const { cmpVersion } = await import("./update_B1NgvTaK.mjs");
  await import("./index_BSErZPDT.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_D3EAGfGS.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_fOWEZwYt.mjs");
  ...
  const { reviewQueue } = await import("./users_ByWMp_tr.mjs");
  const { backupAlert } = await import("./backup_BDaKrgNY.mjs");
