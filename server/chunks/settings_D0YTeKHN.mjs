...
import { getSession } from "./auth_DCMxpdwM.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_DWaRDBDI.mjs";
import { setMemberModel, toggleFavApp, setAppFolders, setAiEngine, setSmartRouting, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_DHOyCLdr.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_DNS2-UZq.mjs";
import { setStorageLimits } from "./storage-usage_BkQX1JZw.mjs";
import { s as setEnabledPartIds, p as partCatalog, e as enabledPartIds } from "./parts_CVGBl5kS.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_DQ67RXU1.mjs";
import { f as fetchAndInstall, u as uninstallExternal, a as listExternalApps, b as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, c as applyUpstreamMerge, d as deleteDraft, i as installLocalApp, e as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_ja9a7AEm.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const { canDevelopApps } = await import("./auth_DCMxpdwM.mjs");
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_1nREIbT_.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_CYoNJfzE.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_BWY7KuVP.mjs").then((n) => n.K);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_BWY7KuVP.mjs").then((n2) => n2.P);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_DINuyaOP.mjs");
  ...
  if (b._action === "app_dev_log") {
    ...
    const { getMessages } = await import("./chat-sessions_DJA_I8xR.mjs").then((n) => n.i);
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.B);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.B);
