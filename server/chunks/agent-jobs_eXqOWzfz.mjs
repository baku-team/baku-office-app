...
import { r as runAgent, n as notifyOwnerDirect } from "./cf-adapter_C1pggy-8.mjs";
import { a as appendMessage, g as getMessages, t as toTurns } from "./chat-sessions_Bk4GZRxh.mjs";
...
async function processAgentJobs(ctx, baseUrl = "", limit = 2) {
  ...
  for (const j of results) {
    ...
    try {
      ...
      const upd = await ctx.db.run("UPDATE agent_jobs SET status=?, result=?, updated_at=? WHERE id=? AND status='running'", [exhausted ? "error" : "done", reply, nowSec(), j.id]);
