...
import { getSession } from "./auth_D_CrX1Mh.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_BgjO-NRr.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_CZWqctVS.mjs";
import { setStorageLimits } from "./storage-usage_BY2W4YRQ.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_CCYEJicX.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_JB9CsP9c.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_AS34t0Ug.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_DjbEJfC2.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_Drnao1kv.mjs").then((n) => n.K);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_Drnao1kv.mjs").then((n2) => n2.P);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_DPsNWvCG.mjs");
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.B);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.A);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.B);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.B);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.A);
