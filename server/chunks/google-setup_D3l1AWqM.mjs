...
import { r as renderComponent } from "./worker-entry_CjXejKsA.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Rv_jhNgy.mjs";
...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_BAkPbG1h.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_DFGiAMu3.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_DvpDfdnx.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured, driveStatus } = await import("./drive_ybXUlQJn.mjs");
  ...
  const { randomId } = await import("./ctx_DI-uUz7k.mjs").then((n) => n.K);
