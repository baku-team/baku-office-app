...
import { r as renderComponent } from "./worker-entry_CnA5Rooj.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CTkiTZw2.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BlKVKIq1.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BlKVKIq1.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_CJCsDXd1.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_CFhfgOdP.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DNEehGc8.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Bm85HqIr.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CND5X7DG.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DvIuH694.mjs");
