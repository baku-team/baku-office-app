...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BQbdmWVM.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_C6OUyrBU.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_5xdAYtmH.mjs");
