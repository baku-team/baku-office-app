...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_fwjkKF34.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CuoW304l.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CMMN1v0i.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_DhxWhP3M.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CtVp9LjS.mjs";
import "./index_DSEOIWls.mjs";
import { recordUsage, recordTokens, overBudget, estimateUsd } from "./usage_KVKE99-A.mjs";
...
import { googleFetch } from "./google_BQhgr3sk.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_D0-eULfO.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DG6THdmS.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DOehWqZR.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BEmUCfGQ.mjs";
import { logDiag } from "./diag_D9aXYLsc.mjs";
import { i as isRunnableDefinition } from "./preflight_D12Q3rxJ.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_ZnJTAJrH.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CZdHJJLR.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CJ_aE4sG.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_D-05v0Pw.mjs";
import { saveLayoutDraft, listSites } from "./sites_DHqMhYef.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_DwR_Hikq.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_Deupfqov.mjs";
...
検証エラー：${issueText}`;
  ...
  const finalMsg = `修正しました（版 ${liveVersion}${phases.length > 1 ? `・${doneCount}/${phases.length}工程` : ""}）。` + (uiCheck.note ? `
...
⚠️ 自動反映できませんでした${installErr ? `：${installErr}` : ""}。内容をご確認のうえ、もう一度お試しください。` : "\n※ 事前確認に課題があります。内容をご確認ください。");
  const fmtConcern = (i) => `${i.target ? i.target + "：" : ""}${i.comment}${i.suggestion ? `（${i.suggestion}）` : ""}`;
  const mustConcerns = review.issues.filter((i) => i.severity === "error").map(fmtConcern);
  const editConcerns = review.issues.map(fmtConcern);
  const editActions = [];
  if (mustConcerns.length) editActions.push({ label: "🔧 必須の修正を直す", kind: "reply", text: `次の点を修正してください：${mustConcerns.join(" / ")}`, style: "primary" });
  if (editConcerns.length && editConcerns.length > mustConcerns.length) editActions.push({ label: mustConcerns.length ? "任意の提案も反映する" : "🔧 指摘を修正する", kind: "reply", text: `次の点を修正してください：${editConcerns.join(" / ")}`, style: mustConcerns.length ? "ghost" : "primary" });
...
できる範囲（${feas.feasibleScope || "可能な部分"}）を作ります。作れない部分は、下の「📋 外部AI用プロンプトをコピー」で外部AI（ChatGPT 等）に依頼できます。`;
    ...
    const plan2 = await makePlan(model, row.spec ?? "", paid, row.ui_mode === "simple" || row.ui_mode === "rich" ? row.ui_mode : void 0);
    if (!plan2) {
...
async function makePlan(model, spec, paid, uiMode) {
...
function reviewMessage(rv) {
  ...
  const must = rv.issues.filter((i) => i.severity === "error");
  const opt = rv.issues.filter((i) => i.severity !== "error");
  const fmt = (i) => `・${i.comment}${i.suggestion ? `（${i.suggestion}）` : ""}`;
  const parts = [];
  if (must.length) parts.push(`【必須の修正】このままだと正しく動きません。
${must.map(fmt).join("\n")}`);
  if (opt.length) parts.push(`【任意の改善】より良くする提案です（そのままでも使えます）。
${opt.map(fmt).join("\n")}`);
  return `🔎 チェック結果
${parts.join("\n\n")}`;
}
...
${spec}` }], [], void 0, { maxTokens }).catch(() => null);
  ...
  if (html && /<\/html\s*>/i.test(html) && !looksBakuStyled(html)) {
    ...
    ({ html, capped } = await gen(sys + "【再厳守】主ボタンは background:#1B1D22・color:#fff、アクセントは金 #C9A86A（強調 #946F2C）、背景 #F2F1F4 を基調にする。これらの色値を style に実値で必ず埋め込むこと（補助色は自由）。"));
  }
...
方針：先頭は必ず hero。末尾は cta か contact。一般に5〜9セクション。各セクションに「何を伝えるか」を1文。
出力は JSON 配列1個のみ（前置き・説明・コードフェンス無し）：[{"type":"hero","goal":"..."},...]。type は上の型のみ。`;
  const user = `【説明】
${opts.prompt.trim() || "（おまかせ。一般的で説得力ある構成に）"}${baseHint}`;
  ...
  let sections = arr2.filter((x) => !!x && typeof x === "object" && ALLOWED_TYPES.includes(x.type ?? "")).map((x) => ({ type: String(x.type), goal: String(x.goal ?? "").slice(0, 120) })).slice(0, 12);
  if (!sections.length) return { ok: false, error: "有効なセクションが計画されませんでした" };
  if (sections[0].type !== "hero" && ALLOWED_TYPES.includes("hero")) sections.unshift({ type: "hero", goal: "第一印象・主役の魅力を一目で伝える" });
  const last = sections[sections.length - 1].type;
  if (last !== "cta" && last !== "contact" && ALLOWED_TYPES.includes("cta")) sections.push({ type: "cta", goal: "行動を促す締めくくり" });
...
async function siteBuildStatus(ctx2, slug) {
  ...
  const phaseLabel = row.status === "planning" ? "構成を計画中…" : row.status === "building" ? `${Math.min(row.cursor + 1, total || 1)}/${total || "?"}：${cur?.goal || cur?.type || "セクション"} を作成中…` : row.status === "done" ? "完成しました" : "停止しました";
...
async function processSiteBuild(ctx2, id, opts) {
  ...
  for (let s = 0; s < steps; s++) {
    ...
    const g = await generateOneBlock(env, { type: sec.type, goal: sec.goal, prompt: row.prompt || "", baseBlock, modelId: model });
    const blocks = arr(row.blocks);
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DHqMhYef.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
const UI_MODE_QUESTION = "アプリの作り方を選んでください。\n\n【簡素・確実】シンプルな標準画面で、素早く確実に作ります。操作は分かりやすく、生成の失敗（ボタンが動かない・一度で完成しない等）がほぼ起きません。凝った見た目や複雑な画面には向きません。\n\n【リッチ】作り込んだ見た目・高い操作性の画面（1ページ完結のSPA）にします。表現力は高いですが生成が重く、まれに一度で完成せず修正が必要になることがあります。\n\n下のボタンで選んでください。";
const UI_MODE_ACTIONS = [
  { label: "簡素・確実で作る", kind: "reply", text: "簡素なUIで作成してください", style: "primary" },
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_Boqanm14.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_Boqanm14.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_Boqanm14.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CuoW304l.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_Boqanm14.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_fwjkKF34.mjs")).storageMode(env) === "r2";
