...
import { r as renderComponent } from "./worker-entry_Czw6uTKe.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_jvkkdtpD.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_B-WwEpGI.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_B-WwEpGI.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_B2KW-m1q.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_CET1td35.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DrpQtySy.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_B9MfXM_G.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Bh_N8LI2.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_E9q_L1AB.mjs");
