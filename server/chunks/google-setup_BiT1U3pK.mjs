...
import { r as renderComponent } from "./worker-entry_D24No6WS.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_B7585yHj.mjs";
...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CjorSwtb.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_Z7RbyoZm.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_Cbus0d-q.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured, driveStatus } = await import("./drive_CNqR3rXX.mjs");
  ...
  const { randomId } = await import("./ctx_D6q0G9CB.mjs").then((n) => n.K);
