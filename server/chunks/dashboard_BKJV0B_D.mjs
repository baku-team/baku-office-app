...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_TWPTdJdo.mjs");
  const { cmpVersion } = await import("./update_D172vf7n.mjs");
  await import("./index_wR0izv-9.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B3yfw8OL.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BRqd6sSE.mjs");
  ...
  const { reviewQueue } = await import("./users_E4kYmseI.mjs");
  const { backupAlert } = await import("./backup_CkMEo0bD.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dd7ZbSkI.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_DHYPZETu.mjs");
