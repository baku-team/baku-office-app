...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_M2NJJlqW.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_BzmGqf3D.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_8UIY9kKI.mjs").then((n) => n.P);
  ...
  const { latestSessionApp } = await import("./ctx_8UIY9kKI.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_rV9_WCfp.mjs").then((n) => n.z);
