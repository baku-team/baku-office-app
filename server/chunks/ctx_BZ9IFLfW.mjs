...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BcmxKHWi.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BNXMhaMG.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CsNrJCw7.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CavXFK4T.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_Bh0BoI_b.mjs";
import "./index_D018HHvS.mjs";
...
import { googleFetch } from "./google_3FbN9iX7.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_Bg6y08G0.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DZ6p8OO5.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_Cd1J34nH.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_DppbQ30s.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_CanD1w9d.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_C0cN2tKz.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_o7-0vzyx.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_JaGMrK7t.mjs";
import { storeCatalog } from "./store_CtYb8tYj.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BL9z7daV.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_kjrS3dQ0.mjs";
import { saveLayoutDraft, listSites } from "./sites_CO1H2-4p.mjs";
...
import { p as pushToUser } from "./push_B9i2Sgam.mjs";
...
async function startAppBuild(ctx2, a) {
  if (a.sessionId) {
    const dup = await ctx2.db.first(
      "SELECT id FROM app_builds WHERE owner=? AND session_id=? AND kind IS NULL AND status IN ('planning','building') AND created_at > ? ORDER BY created_at DESC LIMIT 1",
...
async function runBatch(ctx2, row, paid, baseUrl, opts) {
  ...
  await logBuild(ctx2.env, row.id, claim.rowsWritten ? "claim 占有" : "claim 敗北(譲)", `src=${opts?.src ?? "?"} readUpd=${row.updated_at} status=${row.status} cur=${row.cursor} att=${row.attempts}`);
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_CO1H2-4p.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BPARKFZH.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BPARKFZH.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BPARKFZH.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_BcmxKHWi.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BNXMhaMG.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BPARKFZH.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BcmxKHWi.mjs")).storageMode(env) === "r2";
