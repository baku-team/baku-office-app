...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BJ88O0X2.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CtbtonES.mjs");
  await import("./index_DM39iHq9.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B1MvUC7S.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BsNqt4NS.mjs");
  ...
  const { reviewQueue } = await import("./users_C3chIHtA.mjs");
  const { backupAlert } = await import("./backup_gF0mZxnF.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_C3f4hWKe.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_CQY6ET0s.mjs");
