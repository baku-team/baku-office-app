...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BwbWPyEb.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_OqtVDAbG.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CKfFiifC.mjs");
