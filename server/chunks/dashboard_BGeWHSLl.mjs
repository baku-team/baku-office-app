...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DpZoJ4KJ.mjs");
  const { cmpVersion } = await import("./update_DccUsJ_e.mjs");
  await import("./index_Sjqf1K7w.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BGyNvlpv.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_uYd4q_WZ.mjs");
  ...
  const { reviewQueue } = await import("./users_DZrmZRzu.mjs");
  const { backupAlert } = await import("./backup_BCC-mzOk.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DohtNeGn.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B9y2EXgA.mjs");
