...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Bp7iwF3-.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DFVT2Efn.mjs");
  await import("./index_C3Al9_ge.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B9x0OXHg.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BEEBknaa.mjs");
  ...
  const { reviewQueue } = await import("./users_Bxt_vm30.mjs");
  const { backupAlert } = await import("./backup_Cqt8478g.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_3q9Cmcyt.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_Bhujx0BT.mjs");
