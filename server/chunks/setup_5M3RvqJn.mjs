...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_BT1HjjIF.mjs");
  const { googleConfigured } = await import("./google_Dap_ymUM.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_NY8D3ke9.mjs");
