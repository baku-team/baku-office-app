...
import { APP_SCHEMA, validateDefinition } from "./appdef_ByA9vXJX.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_BgEl515W.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_Ca0PUS5y.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
