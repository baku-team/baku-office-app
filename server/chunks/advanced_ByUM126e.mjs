...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BZPpVhKD.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Dst-rRxs.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_-SHcSCdW.mjs");
