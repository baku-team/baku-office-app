...
import { validateDefinition } from "./appdef_ByA9vXJX.mjs";
import { g as getAppDesign } from "./external-apps_Dk15ONnL.mjs";
import { p as preflight } from "./preflight_BpEyu-un.mjs";
import { r as runDraftApp } from "./app-runtime_DGZDs5Yw.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_Dk15ONnL.mjs").then((n) => n.z)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_Ca0PUS5y.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
