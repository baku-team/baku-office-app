...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_8aQWAfs5.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DPaLCj7k.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_DkJb1xQq.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DPUrRbwk.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DX3YoKbL.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BSfdE0OQ.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_CLW1ozyH.mjs");
