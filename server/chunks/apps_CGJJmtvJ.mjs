...
import { r as renderComponent } from "./worker-entry_BBZ7Llo-.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_B6EKPWZb.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CIgfBw05.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CIgfBw05.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_Coysp-id.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_yS3Z_a8H.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CpFO7isQ.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Cem46T0w.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_60xKlRS4.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_f9Ul4VAp.mjs");
