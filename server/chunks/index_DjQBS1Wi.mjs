...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BbGjOLTN.mjs");
  const { cmpVersion } = await import("./update_CYykSgWU.mjs");
  await import("./index_BvPbBcDJ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DV1esmHZ.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DfoLw8bZ.mjs");
  ...
  const { reviewQueue } = await import("./users_DsrUvgx3.mjs");
  const { backupAlert } = await import("./backup_B9-hJaWS.mjs");
