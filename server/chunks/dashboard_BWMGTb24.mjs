...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_XKaks_hD.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DNSn5Qh7.mjs");
  await import("./index_BsLdtSuX.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Dc5V0d7w.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C_QhTyeK.mjs");
  ...
  const { reviewQueue } = await import("./users_BBnHZMEg.mjs");
  const { backupAlert } = await import("./backup_CBN4ERQq.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BZsna514.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BqaGF6MH.mjs");
