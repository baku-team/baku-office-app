...
import { getSession, canDevelopApps } from "./auth_4L0ewUEU.mjs";
import { l as loadDraft, c as checkDefinition, a as checkTransitions, b as checkSandbox, d as checkFabrication, e as checkRun, f as finalizePrecheck } from "./precheck_cbzsjDEB.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_BsZJNGRN.mjs").then((n) => n.C)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_NtHEh0X8.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
