...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_92mF-n-g.mjs");
  const { googleConfigured } = await import("./google_qWLExI_e.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_NU6GHmBC.mjs");
