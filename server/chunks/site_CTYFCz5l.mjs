...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_DB2Dyxn6.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_DZ4X55g1.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_DgX2ERXA.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_CSZaRoVj.mjs").then((n) => n.S);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_CSZaRoVj.mjs").then((n) => n.S);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_DB2Dyxn6.mjs");
