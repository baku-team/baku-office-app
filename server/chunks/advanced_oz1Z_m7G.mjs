...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BAAgke-T.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Ox_ebKS-.mjs");
  ...
  const { hasApiKey } = await import("./client_BAAgke-T.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CLCFzk9N.mjs");
