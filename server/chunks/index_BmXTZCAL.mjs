...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_mUx62QP1.mjs");
  const { cmpVersion } = await import("./update_u04QvfCa.mjs");
  await import("./index_BEVH8lA1.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Dm5RA95F.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CCr9muu4.mjs");
  ...
  const { reviewQueue } = await import("./users_3hHzqJbj.mjs");
  const { backupAlert } = await import("./backup_Dk6n6g5U.mjs");
