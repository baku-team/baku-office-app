...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_B06zZZvw.mjs");
  const { cmpVersion } = await import("./update_B_K9lUer.mjs");
  await import("./index_CeUyuBtY.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_3gQVojvb.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CCHasFVt.mjs");
  ...
  const { reviewQueue } = await import("./users_WlkVrW4p.mjs");
  const { backupAlert } = await import("./backup_CeIQkGDd.mjs");
