...
import { r as renderComponent } from "./worker-entry_wMwr6uXu.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_C0Qy2Xpa.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DfwCgcjk.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CCzD9Z58.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BXtQV7M3.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_C_LrQGXj.mjs"),
    import("./scheduled-tasks_C5KAkfSk.mjs")
  ]);
  ...
  await import("./index_Cw9M4CD3.mjs");
