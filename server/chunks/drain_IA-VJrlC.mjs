...
import { getApiKey } from "./client_0pHY0CaD.mjs";
import { g as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, k as processSiteBuilds } from "./ctx_Cu1Zes9a.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_B4Wt0CUx.mjs";
import { processAgentJobs } from "./agent-jobs_p6wE7gZq.mjs";
import { reclaimStaleApprovals } from "./approvals_DqaVOEOl.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_DvgJR7r0.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_oVNycdSB.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_Ndxo0-kJ.mjs";
import { flushReports } from "./reports_B3bHBj07.mjs";
import { addNotification, pushWebhook } from "./notifications_CiURuo9b.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_CPYUMyFX.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_EOeX1JCI.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_omQ7RKES.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
