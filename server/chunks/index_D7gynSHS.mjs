...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, getApiKey } = await import("./client_CdHBXZZx.mjs");
  ...
  if (isAdminOrg) {
    const { googleConfigured } = await import("./google_DDa6Z_Jd.mjs");
  ...
  const groups = [
    ...
    { title: "外部サービス設定", items: [
      { href: "/settings/integrations", label: "外部サービス設定（AI／LINE・Discord・Slack／Google／クラウド保存）", desc: "外部サービスとの接続設定を1か所で", show: isAdminOrg }
    ] },
