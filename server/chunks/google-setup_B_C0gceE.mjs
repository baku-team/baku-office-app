...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_D-DeurZ_.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_BFi-17hg.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_D58gIbWV.mjs");
  ...
  const { buildCtx } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.L);
