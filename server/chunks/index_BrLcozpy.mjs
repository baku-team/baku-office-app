...
import { r as renderTemplate, b as renderSlot, m as maybeRenderHead, a as addAttribute } from "./sequence_CKjIrPcu.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$Index;
  const { getSession } = await import("./auth_B043h97J.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DpHFYvTj.mjs");
  const { cmpVersion } = await import("./update_DUP6REc_.mjs");
  await import("./index_CNa1iP31.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DNFPezsI.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_ENs9MHUo.mjs");
  ...
  const { reviewQueue } = await import("./users_CuAUdWRX.mjs");
  const { backupAlert } = await import("./backup_CU0q1_JL.mjs");
