...
import { getApproval, decideApproval } from "./approvals_DsktlpIV.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_D3ppHoJV.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_DZmKLyOO.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Uckkqsoj.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BJ88O0X2.mjs";
import { joinWithInvite } from "./users_C3chIHtA.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_CQY6ET0s.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_C3chIHtA.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_XMDK0v_C.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DQuJ-qbQ.mjs");
