...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_Cx_nXCZA.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_B914Xe2h.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_C5A1-cK5.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CFKAHzaY.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Cp-oPbDk.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CPnhLqpU.mjs");
