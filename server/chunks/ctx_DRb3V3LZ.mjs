...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_DMuH0rYg.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_sRscislI.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_BSrFJylT.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CS2WTucu.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DEzXoTbR.mjs";
import "./index_BJcJsswH.mjs";
...
import { googleFetch } from "./google_wcCBu7Mn.mjs";
...
import { APP_SCHEMA, validateDefinition, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_CFiwoKLN.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_acVnMsie.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_6uJBdiA1.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_DUnAdmMv.mjs";
import { logDiag } from "./diag_CY3PJnOM.mjs";
import { i as isRunnableDefinition } from "./preflight_CPDw4YhS.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_B_2DTV2W.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_BDQRcoFz.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_Bfwp1C8W.mjs";
import { storeCatalog } from "./store_DGfBcH1H.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CsMC_79r.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_Dm_6lKAc.mjs";
import { saveLayoutDraft, listSites } from "./sites_BH-HYK_K.mjs";
...
import { p as pushToUser } from "./push_FF6cIKhX.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_BH-HYK_K.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BKPb_-en.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BKPb_-en.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BKPb_-en.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_sRscislI.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BKPb_-en.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_DMuH0rYg.mjs")).storageMode(env) === "r2";
