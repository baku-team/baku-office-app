...
import { getApproval, decideApproval } from "./approvals_CpyhgkQw.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_Bh_tJP0x.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_C3hyS9pN.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Cie779Te.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BKYtLCd6.mjs";
import { joinWithInvite } from "./users_CAsNPlru.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_C0xk2SHi.mjs";
import { getDriveSave } from "./drive_DSp2oymh.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CAsNPlru.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_COShl9LN.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_HBsVqq3c.mjs");
