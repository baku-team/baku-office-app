...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_BmX3boNt.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_DE3oxZSo.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_PVO28pUh.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_BNALeAnc.mjs"),
    import("./scheduled-tasks_DeHVbK8S.mjs")
  ]);
  ...
  await import("./index_CXDHY44j.mjs");
