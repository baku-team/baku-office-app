...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DhfR4ryc.mjs");
  const { cmpVersion } = await import("./update_PSErDPkW.mjs");
  await import("./index_DAI5Xutz.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CkjQoz3l.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cuflftj9.mjs");
  ...
  const { reviewQueue } = await import("./users_RaszYQTy.mjs");
  const { backupAlert } = await import("./backup_B-QlPGrH.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DahLZ_Ik.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_C2XNcZ5b.mjs");
