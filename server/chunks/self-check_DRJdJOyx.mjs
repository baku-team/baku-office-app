...
import { APP_SCHEMA, validateDefinition } from "./appdef_Cmp4V_Cj.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_CVjeTeB8.mjs";
import { p as preflight } from "./preflight_Bmz9f5Zb.mjs";
...
async function gatePublishApp(ctx, appId) {
  const input = await buildAppCheckInput(ctx, appId).catch(() => null);
  if (!input) return { ok: false, concerns: ["アプリ定義を読み取れず公開前の安全検査を実行できませんでした（blocked_check_unavailable）。定義を保存し直してから再度公開してください。"] };
  const det = await deterministicSelfChecks(ctx, input).catch(() => null);
  if (!det) return { ok: false, concerns: ["公開前の安全検査を完了できませんでした（blocked_check_unavailable）。時間をおいて再度お試しください。"] };
  let leaked;
  try {
    leaked = await scanLeakedSecrets(ctx, scanText(input));
  } catch {
    leaked = null;
  }
  if (leaked === null) det.push({ key: "secret", label: "秘密情報", status: "fail", detail: "保存済みの秘密との照合を実行できませんでした（blocked_check_unavailable）。時間をおいて再度公開してください。", ai: false });
  else if (leaked.length) det.push({ key: "secret", label: "秘密情報", status: "fail", detail: `保存済みの秘密（${leaked.join("、")}）の実値が公開物に含まれています。キーはサーバ側保管のまま http.fetch op 経由で使い、定義/HTMLに値を埋め込まないでください。`, ai: false });
...
async function scanLeakedSecrets(ctx, text) {
  const { listApiKeyNames, getApiKey } = await import("./client_B17j1uG_.mjs");
