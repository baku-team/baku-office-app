...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CZ0BwZwu.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_BjQkw6mR.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Cw432j-u.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DrWbSOGs.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BGP-5Wom.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_CsVRrCon.mjs");
