...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DQhV0ghx.mjs");
  const { cmpVersion } = await import("./update_DFTGMJaP.mjs");
  await import("./index_BqlS6zMg.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_B4dUPQYx.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cf8x-KCR.mjs");
  ...
  const { reviewQueue } = await import("./users_DuWAT47M.mjs");
  const { backupAlert } = await import("./backup_DOMbqFUV.mjs");
