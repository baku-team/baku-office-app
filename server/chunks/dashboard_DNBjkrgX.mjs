...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Bib4yTGq.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_C5wTxFxT.mjs");
  await import("./index_BFUyWbe8.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_nBAdPSxh.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DeKuvgbX.mjs");
  ...
  const { reviewQueue } = await import("./users_BYPeJHFX.mjs");
  const { backupAlert } = await import("./backup_jEMcl_dX.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BIy17cIt.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BanefT0i.mjs");
