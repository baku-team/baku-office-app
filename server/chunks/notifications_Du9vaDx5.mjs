...
async function notifyHook(env, text) {
  ...
  if (lineTo) {
    const { getApiKey } = await import("./client_yx4APBBm.mjs");
    const token = await getApiKey(env, "line_token").catch(() => null);
    if (token) {
      const [{ cfEgressGateway }, { linePush }] = await Promise.all([import("./ctx_BFAO0OWB.mjs").then((n) => n.L), import("./ctx_BFAO0OWB.mjs").then((n) => n.N)]);
      try {
