...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BanefT0i.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Bib4yTGq.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_qE0riaEI.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BYPeJHFX.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_nBAdPSxh.mjs";
import "./index_BFUyWbe8.mjs";
...
import { googleFetch } from "./google_f2KqkJh0.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_D9vMBBjU.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C1VDVo4n.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BqGofNU7.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_DYA9iNvW.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_BypXf69E.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_ChR00g-2.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_Riih4RHz.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DZjSSTuf.mjs";
import { storeCatalog } from "./store_DajNP6oI.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BbRloX4B.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DmeGlKV0.mjs";
import { saveLayoutDraft, listSites } from "./sites_CbJYGFQQ.mjs";
...
import { p as pushToUser } from "./push_Dt7js7m8.mjs";
...
検証エラー：${issueText}`;
  ...
  const editUnmet = failed.length > 0 || !!uiCheck.note || editUnwired.length > 0;
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_CbJYGFQQ.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_C0LdvcUx.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_C0LdvcUx.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_C0LdvcUx.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_BanefT0i.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Bib4yTGq.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_C0LdvcUx.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BanefT0i.mjs")).storageMode(env) === "r2";
