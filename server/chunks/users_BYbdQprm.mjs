...
import { revokeSessions } from "./auth_iZjiHMy7.mjs";
...
async function linkIdentity(env, userId, type, externalId) {
  const r = await env.DB.prepare(
    "INSERT INTO identities (id,user_id,type,external_id,password_hash,created_at) SELECT ?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM identities WHERE type=? AND external_id=?)"
  ).bind(randomId(), userId, type, externalId, null, nowSec(), type, externalId).run();
  ...
  return existing?.user_id === userId ? { ok: true } : { ok: false, error: "この外部アカウントは別の会員に連携済みです。" };
}
async function joinWithInvite(env, code, name, identity) {
  ...
  const consume = await env.DB.prepare(
    "UPDATE invites SET used_count=used_count+1, status=CASE WHEN used_count+1>=max_uses THEN 'revoked' ELSE status END WHERE id=? AND status='active' AND used_count < max_uses AND expires_at > ?"
  ).bind(inv.id, nowSec()).run();
  if ((consume.meta?.changes ?? 0) === 0) return { ok: false, error: "招待コードは使用済みまたは期限切れです。" };
  const uid = randomId();
  ...
  const extId = identity.externalId ?? name;
  const idRes = await env.DB.prepare(
    "INSERT INTO identities (id,user_id,type,external_id,password_hash,created_at) SELECT ?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM identities WHERE type=? AND external_id=?)"
  ).bind(randomId(), uid, identity.type, extId, passwordHash, nowSec(), identity.type, extId).run();
  if ((idRes.meta?.changes ?? 0) === 0) {
    await env.DB.prepare("DELETE FROM users WHERE id=?").bind(uid).run().catch(() => {
    });
    return { ok: false, error: "この外部アカウントは既に登録されています。既存のアカウントでログインしてください。" };
  }
  return { ok: true };
}
