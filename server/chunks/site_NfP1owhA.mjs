...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_BnCPgr1A.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_CtLBMcNN.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_DfS-lMTW.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_CAci7FIw.mjs").then((n) => n.V);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_CAci7FIw.mjs").then((n) => n.V);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_BnCPgr1A.mjs");
