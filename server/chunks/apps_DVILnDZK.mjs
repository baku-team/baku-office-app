...
import { r as renderComponent } from "./worker-entry_V9sNtBU8.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D0x0_jut.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BhfKTuGW.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BhfKTuGW.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_DyhUJ_If.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_ZR_ELpXg.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CYlUKgSo.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CSzWS8_7.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CynURPH_.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_CrJqobDy.mjs");
