...
const $$Import = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement } = await import("./client_DpHFYvTj.mjs");
  const hasPlus = atLeast(await cachedEntitlement(env), "plus");
  const { listImported } = await import("./import_BAlh3kcT.mjs");
  const { fmtBytes } = await import("./storage-usage_ENs9MHUo.mjs");
  ...
  return renderTemplate`${renderComponent($$result, "App", $$App, { "title": "書類の取り込み", "active": "/import" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<h1>書類の取り込み</h1> ${!hasPlus && renderTemplate`<div class="card"> <div class="banner banner-warn">この機能は <strong>Plus 以上</strong>のプランで利用できます。</div> <p class="muted">Googleドライブなどの書類を取り込みます（既定はファイル名などの情報だけ）。</p> <a class="btn btn-primary" href="/billing">プラン・お支払いへ</a> </div>`}${hasPlus && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate` <p class="muted">既定は<strong>ファイル名などの情報だけ</strong>を取り込みます。<strong>ファイルデータすべて</strong>を取り込むには、ファイルの保存先（大容量ストレージ）が有効になっている必要があります。実行前に容量と費用を見積もります。</p> ${!isAdmin && renderTemplate`<div class="banner banner-warn">取り込みの実行は管理者のみ可能です。</div>`}<div class="card"> <div class="row"> <label style="flex:1">取り込み元
