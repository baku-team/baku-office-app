...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CtLBMcNN.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_4ZEuV35m.mjs";
import { canDevelopApps } from "./auth_CNSCg6Zs.mjs";
import { cachedEntitlement } from "./client_DcvmK-U8.mjs";
...
${txt}` };
  ...
  if (mime.startsWith("image/") || mime === "application/pdf") {
    ...
    const { inferApp } = await import("./ctx_CAci7FIw.mjs").then((n) => n.P);
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_CAci7FIw.mjs").then((n) => n.W);
  ...
  const { latestSessionApp } = await import("./ctx_CAci7FIw.mjs").then((n) => n.U);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CZwKXiCg.mjs").then((n) => n.C);
...
async function tryPreAgentRouting(ctx, cfContext, args) {
  ...
  const priorAssistant = [...prior].reverse().find((m) => m.role === "assistant")?.content ?? "";
  const { looksLikeBuildConfirmation, looksLikeUiModeChoice } = await import("./ctx_CAci7FIw.mjs").then((n) => n.W);
  if (notPlan && canDevelopApps(role) && looksLikeBuildConfirmation(message, priorAssistant)) {
    const { startAppBuild, processAppBuild, buildModelGuide } = await import("./ctx_CAci7FIw.mjs").then((n) => n.U);
  ...
  if (notPlan && canDevelopApps(role) && atLeast(await cachedEntitlement(env), "pro")) {
    const { looksLikeAppEdit } = await import("./ctx_CAci7FIw.mjs").then((n) => n.W);
    if (looksLikeAppEdit(message)) {
      const { latestSessionApp, resolveAppByName, startAppEdit, processAppBuild, buildModelGuide } = await import("./ctx_CAci7FIw.mjs").then((n) => n.U);
...
async function tryProHopsContinuation(ctx, cfContext, args) {
  const { HOPS_EXCEEDED } = await import("./ai_mug4D6f4.mjs");
  if (args.reply !== HOPS_EXCEEDED) return null;
  if (!atLeast(await cachedEntitlement(env), "pro")) return null;
  const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_BcOH-guF.mjs");
...
async function finalizeAssistantReply(ctx, args) {
  ...
  const { recordTaskFromReply, linkTaskMessage } = await import("./task-log_DFVySHMg.mjs");
  ...
  text = task.reply;
  const { extractActions } = await import("./chat-sessions_4ZEuV35m.mjs").then((n) => n.h);
