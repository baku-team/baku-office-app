...
import { getApiKey, getToken, hostFetch, getVerifyJwk } from "./client_K_W-4z9g.mjs";
...
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_Dd4rnlyx.mjs";
import { getStorageUsage } from "./storage-usage_C7VQSZdL.mjs";
...
async function installedAppLaunchers(ctx) {
  const apps = await installedAppDefs(ctx);
  const items = apps.map((a) => {
    const d = a.definition;
    const name = a.name || d.name || a.id;
    return { id: a.id, name, href: `/app/${a.id}`, category: (d.category || "アプリ").trim(), icon: (d.icon || [...name][0] || "□").trim() };
  });
