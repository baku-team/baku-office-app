...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_DKYBuAuX.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_Cc3dcDAs.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_Z4TKh5FF.mjs");
  ...
  const { buildCtx } = await import("./ctx_DwTr64WL.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_DwTr64WL.mjs").then((n) => n.L);
