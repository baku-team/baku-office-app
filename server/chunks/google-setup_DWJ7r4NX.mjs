...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_B3DwQ4ws.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C1Nj7n9U.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_CoI0Yc4D.mjs");
  ...
  const { buildCtx } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.M);
