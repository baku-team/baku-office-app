...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_ZR_ELpXg.mjs");
  const { cmpVersion } = await import("./update_CknhqMQb.mjs");
  await import("./index_CYlUKgSo.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CSzWS8_7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_QvWtinB2.mjs");
  ...
  const { reviewQueue } = await import("./users_BCkIAx7_.mjs");
  const { backupAlert } = await import("./backup_tu3GkAy9.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CynURPH_.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_BAZ0fe2z.mjs");
