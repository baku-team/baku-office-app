...
import { r as renderComponent } from "./worker-entry_DQHChAwX.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_B2PH-r0f.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.locals.publicHost) {
    ...
    const { buildPublicFullPage } = await import("./app-frame_BfH94Zac.mjs").then((n) => n.d);
  ...
  const { getSession } = await import("./auth_BgvE5w_L.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_XKaks_hD.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_h0FlJJpN.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_mXoXXg1D.mjs"),
    import("./scheduled-tasks_DCVGBQMZ.mjs")
  ]);
  ...
  await import("./index_BsLdtSuX.mjs");
