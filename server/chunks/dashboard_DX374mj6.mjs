...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BsuDyowk.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_qNLFDUUW.mjs");
  await import("./index_D2wnbrnI.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_ZgdMwRke.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_C7DrL7II.mjs");
  ...
  const { reviewQueue } = await import("./users_BdDsp2o8.mjs");
  const { backupAlert } = await import("./backup_BrE9XhLt.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CqwoO7zq.mjs");
