...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_D8olaOjQ.mjs");
  const { cmpVersion } = await import("./update_Dwpkgpzg.mjs");
  await import("./index_B6CIgG92.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C5VHx1eS.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BpC9xa1t.mjs");
  ...
  const { reviewQueue } = await import("./users_j4YR6msW.mjs");
  const { backupAlert } = await import("./backup_DOWVGYWZ.mjs");
