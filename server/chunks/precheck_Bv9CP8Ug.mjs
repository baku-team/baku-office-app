...
import { validateDefinition } from "./appdef_Bi9EyADK.mjs";
import { g as getAppDesign } from "./external-apps_CEIQmGS5.mjs";
import { p as preflight } from "./preflight_D18wzMMp.mjs";
import { r as runDraftApp, p as platformInvariantSuspected, f as escalatePlatformInvariant } from "./app-runtime_DBtJNOSS.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_CEIQmGS5.mjs").then((n) => n.A)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BjrAYDQX.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
