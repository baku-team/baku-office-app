...
import { r as renderComponent } from "./worker-entry_Cxr6c57V.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DTG8utpL.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BxT-nkKO.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BxT-nkKO.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_BBDFUyqe.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_C1dOp0Cn.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_3wN2fDas.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CNr3eCe2.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CK29yI9A.mjs");
