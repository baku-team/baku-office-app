...
import { getApiKey } from "./client_BjKlgqy9.mjs";
import { g as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, k as processSiteBuilds } from "./ctx_DWnRrE-m.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_D7p_P15A.mjs";
import { processAgentJobs } from "./agent-jobs_DEa01_jb.mjs";
import { reclaimStaleApprovals } from "./approvals_ClSMmMgm.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_DFBKLagv.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_C7ouTMRu.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_oUcOtZSh.mjs";
import { flushReports } from "./reports_D3Qw7oqJ.mjs";
import { addNotification, pushWebhook } from "./notifications_BozZHqe1.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_BlqFC5PS.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_KoRHzeZX.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_QQysPA0e.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
