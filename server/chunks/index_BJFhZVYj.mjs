...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_FZErHGWA.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_DBc4AdQ5.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_7tDntmWn.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DN7dUuWz.mjs"),
    import("./scheduled-tasks_BN-f2i-w.mjs")
  ]);
  ...
  await import("./index_DaPtUtWl.mjs");
