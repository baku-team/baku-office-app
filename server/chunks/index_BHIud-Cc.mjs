...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client__0Sedoue.mjs");
  const { cmpVersion } = await import("./update_BXZ6b0jz.mjs");
  await import("./index_BUEso0gu.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CompEXry.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cvwo98fW.mjs");
  ...
  const { reviewQueue } = await import("./users_Dk8dlZ-I.mjs");
  const { backupAlert } = await import("./backup_DczHn6Yd.mjs");
