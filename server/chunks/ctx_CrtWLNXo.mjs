...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CzJPn-cz.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_D0aobTMW.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_bYk0eYQ-.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CI6JXwVH.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_Qqa6Ep4W.mjs";
import "./index_D1Qahmtg.mjs";
...
import { googleFetch } from "./google_D-DeurZ_.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, opCatalogText, googleOpCatalogText } from "./appdef_C-OxwFNF.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BbnkwD24.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_C9Jy2uDQ.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_xZkz6mwM.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_Dfr4Wlvv.mjs";
import { logDiag } from "./diag_NtHEh0X8.mjs";
import { i as isRunnableDefinition } from "./preflight_DTyWkDkI.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_cbzsjDEB.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_m7q6dBg5.mjs";
import { storeCatalog } from "./store_Ksj9x-WS.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_BisWdx2f.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DzwE9jZU.mjs";
import { saveLayoutDraft, listSites } from "./sites_DLxGrjcM.mjs";
...
import { p as pushToUser } from "./push_Dc_z_8aD.mjs";
...
async function finalizeEdit(ctx2, row, model, design, def, phases) {
  ...
  if (reconE.length) await logDiag(env, "info", "build", `editFinalize reconciled: ${reconE.join(" / ")}`, `app=${row.app_id}`).catch(() => {
  });
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DLxGrjcM.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
function looksInternalFileRequest(text) {
  const t = (text || "").trim();
  if (!t) return false;
  if (/google\s*drive|Google\s*Drive|グーグル\s*ドライブ|ドライブ/i.test(t)) return false;
  return /(baku-office|bakuoffice|アプリ内|内部|このシステム|ストレージ|資料|ファイル|PDF|pdf|アップロード|保存済み|保管).*(一覧|表示|見せて|出して|探して|検索|ある|どれ|確認)/.test(t) || /(一覧|表示|見せて|出して|探して|検索|確認).*(baku-office|bakuoffice|アプリ内|内部|このシステム|ストレージ|資料|ファイル|PDF|pdf|アップロード|保存済み|保管)/.test(t);
}
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_ihPE-2hi.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_ihPE-2hi.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_ihPE-2hi.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      if (kind === "all" && /pdf|ｐｄｆ/i.test(rawQuery)) kind = "pdf";
      if (kind === "all" && /画像|写真|image/i.test(rawQuery)) kind = "image";
      const q = rawQuery.toLowerCase().replace(/pdf|ｐｄｆ|画像|写真|一覧|表示|見せて|出して|探して|検索|確認|baku-office|bakuoffice|アプリ内|内部|ストレージ|ファイル|資料/g, "").trim();
      const { listFilesForOwner } = await import("./storage_CzJPn-cz.mjs");
      ...
      const byKind = (r) => {
        ...
        if (kind === "document") return mime === "application/pdf" || mime.startsWith("text/") || /\.(pdf|txt|md|csv|json|docx?|xlsx?)$/i.test(name2);
        return true;
      };
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_D0aobTMW.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_ihPE-2hi.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CzJPn-cz.mjs")).storageMode(env) === "r2";
...
${formatA2aResult(r.result)}` : `公開連絡に失敗：${r.error ?? ""}`;
  ...
  if (looksInternalFileRequest(latest)) runDecls = runDecls.filter((d) => !DRIVE_FILE_TOOLS.has(d.name));
