...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BQbdmWVM.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CEOkBbZo.mjs");
  await import("./index_DB87vrf0.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CH6qhuCH.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BFpXbBnk.mjs");
  ...
  const { reviewQueue } = await import("./users_Cske0FpT.mjs");
  const { backupAlert } = await import("./backup_DGJKv3CX.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_D-px-Ku8.mjs").then((n) => n.A);
  ...
  const { storageMode } = await import("./storage_5xdAYtmH.mjs");
