...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_8aQWAfs5.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_DkJb1xQq.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_D7yFgcIz.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DpWwHMpZ.mjs"),
    import("./scheduled-tasks_CA6X_3bd.mjs")
  ]);
  ...
  await import("./index_DPUrRbwk.mjs");
