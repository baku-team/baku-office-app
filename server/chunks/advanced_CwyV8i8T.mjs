...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DSXOrOmp.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_I7yGFDAg.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CM436dbQ.mjs");
