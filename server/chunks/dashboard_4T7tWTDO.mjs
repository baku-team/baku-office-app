...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_D3kajqFf.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DCcD8SPg.mjs");
  await import("./index_Dj0mRxJ1.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CgzlDI-b.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BE7hLxwS.mjs");
  ...
  const { reviewQueue } = await import("./users_Bbj5MCVQ.mjs");
  const { backupAlert } = await import("./backup_DG0fyPpn.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DUrYDXEN.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_SJF4hL77.mjs");
