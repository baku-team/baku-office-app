...
const POST = async ({ request, locals }) => {
  ...
  if (b.action === "rebuild") {
    ...
    const { rebuildPublicPage } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.R);
  ...
  if (b.action === "toggle") {
    ...
    if (enable) {
      ...
      if (page2) {
        const { gatePublishApp } = await import("./self-check_DixJgty_.mjs");
        const { appendMessage } = await import("./chat-sessions_BLqLinNd.mjs").then((n) => n.i);
