...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_mgVrvX2Z.mjs");
  const { cmpVersion } = await import("./update_57o-2eO5.mjs");
  await import("./index_DtI5QjU2.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BwLwnmlD.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DC1xngr6.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_HFVc6CJO.mjs");
  const { scopedWidgets, enabledParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  ...
  const { reviewQueue } = await import("./users_TBwo4v8p.mjs");
  const { backupAlert } = await import("./backup_CU96tif8.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_WS9MhPJy.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_DpdH5p5l.mjs");
  const { getWorkersPaid } = await import("./settings_BGnJ4d3n.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_D3RuFGmp.mjs");
