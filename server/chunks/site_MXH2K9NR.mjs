...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_o_dU3p1m.mjs";
import { v as validateLayout } from "./site-layout_KPAYKPQC.mjs";
import { audit } from "./storage_CM436dbQ.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_3shkK2dl.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_DCqlLMUd.mjs").then((n) => n.S);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_DCqlLMUd.mjs").then((n) => n.S);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_o_dU3p1m.mjs");
