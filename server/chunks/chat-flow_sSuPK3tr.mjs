...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_DD2sf3yv.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_PY8RFBfq.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DugSuSUS.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_DugSuSUS.mjs").then((n) => n.P);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Bb659Sv7.mjs").then((n) => n.z);
