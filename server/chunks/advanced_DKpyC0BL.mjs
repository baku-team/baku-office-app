...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_XKaks_hD.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_eqrWQ3kZ.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BqaGF6MH.mjs");
