...
import { cancelAgentJob } from "./agent-jobs_Barhz2Di.mjs";
import { b as cancelAppBuild } from "./ctx_BRlB-70J.mjs";
...
const GET = async ({ request, locals }) => {
  ...
  try {
    const ab = (await env.DB.prepare("SELECT id,owner,session_id,name,status,plan,cursor,created_at,kind,app_id,progress_detail FROM app_builds WHERE status IN ('planning','building','finalizing') ORDER BY created_at DESC LIMIT 8").all()).results;
    for (const j of ab) {
      const mine = j.owner === ses.uid;
      ...
      let phaseTitle = "";
      try {
        const phs = JSON.parse(j.plan ?? "{}").phases ?? [];
        total = phs.length;
        const ph = phs[j.cursor];
        if (ph && typeof ph.title === "string") phaseTitle = ph.title;
      } catch {
      }
      ...
      tasks.push({ kind: isEdit ? "edit" : "build", status: j.status, label, mine, phase: isEdit ? 0 : phase, total: isEdit ? 0 : total, elapsedSec: Math.max(0, nowSec() - j.created_at), ...mine ? { ...j.status !== "finalizing" ? { id: j.id } : {}, sessionId: j.session_id ?? void 0, appId: j.app_id ?? void 0, detail: j.progress_detail ?? void 0, phaseTitle: phaseTitle || void 0 } : {} });
    }
  } catch {
  }
