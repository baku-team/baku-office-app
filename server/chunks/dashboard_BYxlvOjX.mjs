...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DcubbLWL.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DNYbkXH4.mjs");
  await import("./index_BQNK-3Dt.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DGSaVCvE.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_qMjszICO.mjs");
  ...
  const { reviewQueue } = await import("./users_F4AaUNpy.mjs");
  const { backupAlert } = await import("./backup_BdFXWJbZ.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dr0mvakg.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_D75KjaIu.mjs");
