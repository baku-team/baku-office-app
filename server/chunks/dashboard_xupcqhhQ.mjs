...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_7WfEvhS0.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CpqcI4Nr.mjs");
  await import("./index_CrfElIO3.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DfgyJWKg.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D9czFsBe.mjs");
  ...
  const { reviewQueue } = await import("./users_B2aN_rhx.mjs");
  const { backupAlert } = await import("./backup_CZqXhkPf.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CFGtMTKs.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_HzCeCgFJ.mjs");
