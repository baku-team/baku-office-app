...
import { r as renderComponent } from "./worker-entry_BIa5Sh9h.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DmlZZbOX.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BPIWY2GB.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BPIWY2GB.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_B5MA7F1b.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_B1mWWadE.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Cz_s9PCZ.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BfOqoIdF.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BHSE11sT.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Cq4zs4-j.mjs");
