...
import { r as renderComponent } from "./worker-entry_Cvknvqqr.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_C6c_U9sE.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_BO4LdvYt.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BCk4EGYy.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CU61v_Mn.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_5T6IOeGu.mjs"),
    import("./scheduled-tasks_CJmSAQov.mjs")
  ]);
  ...
  await import("./index_De32HSX0.mjs");
