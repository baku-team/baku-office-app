...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_Chs1lLNh.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_D4ixuJrX.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_DqgOAE5i.mjs");
  ...
  const { buildCtx } = await import("./ctx_D-gPzIuz.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_D-gPzIuz.mjs").then((n) => n.M);
