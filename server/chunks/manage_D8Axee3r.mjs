...
import { moderate, getPublicPageAny, setPublicEnabled, setRegisterMode, setCapacity, seatsTaken, listSubmissions, deletePublicPage } from "./public-pages_Bg1BWixA.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b.action === "rebuild") {
    ...
    const { rebuildPublicPage } = await import("./ctx_Ige4T837.mjs").then((n) => n.V);
  ...
  if (b.action === "toggle") {
    ...
    if (enable) {
      ...
      if (page2) {
        const { gatePublishApp } = await import("./self-check_DRJdJOyx.mjs");
        const { appendMessage } = await import("./chat-sessions_BTO_5scg.mjs").then((n) => n.k);
