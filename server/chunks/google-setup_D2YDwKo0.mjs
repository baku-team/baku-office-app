...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_R2jRlvIG.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_tl0ys28U.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_CaNyLA0n.mjs");
  ...
  const { buildCtx } = await import("./ctx_Cpc8zRmY.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_Cpc8zRmY.mjs").then((n) => n.M);
