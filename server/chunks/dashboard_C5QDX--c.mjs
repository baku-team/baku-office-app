...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Dkd4XO8R.mjs");
  const { cmpVersion } = await import("./update_CNAZzFfE.mjs");
  await import("./index_DHfqgO_N.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BgIyC1x0.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B9GcqNBI.mjs");
  ...
  const { reviewQueue } = await import("./users_BiQVVDnj.mjs");
  const { backupAlert } = await import("./backup_C-XkxOee.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_C08IdFiZ.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B3AGfRzZ.mjs");
