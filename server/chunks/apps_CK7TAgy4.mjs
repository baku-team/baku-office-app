...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_CCzD9Z58.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Cw9M4CD3.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps__zNnisb7.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_J8zU4bt9.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_W-HTLKWp.mjs");
