...
import { validateDefinition } from "./appdef_DY3OkbZ6.mjs";
import { g as getAppDesign } from "./external-apps_CE1vvWJb.mjs";
import { p as preflight } from "./preflight_SvbOxNHW.mjs";
import { r as runDraftApp } from "./app-runtime_CybrJXd9.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_CE1vvWJb.mjs").then((n) => n.z)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_BS4K6Q4H.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
