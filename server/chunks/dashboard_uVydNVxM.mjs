...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CdHBXZZx.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Du8CTcYz.mjs");
  await import("./index_07W0wUoP.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DWmuHuDh.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_hghleF8B.mjs");
  ...
  const { reviewQueue } = await import("./users_DvMO3Ala.mjs");
  const { backupAlert } = await import("./backup_CT1LamH2.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DB-xHIgj.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CTBMIurh.mjs");
