...
import { r as renderComponent } from "./worker-entry_BIJ9ZTht.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DdOVGU44.mjs";
...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DZSpVNky.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_D_wgy0NO.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_BFhzSwlO.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_BICd75pv.mjs");
  ...
  const { buildCtx } = await import("./ctx_BWJTdi0a.mjs").then((n) => n.U);
  const { enabledPartIds } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  ...
  const { randomId } = await import("./ctx_BWJTdi0a.mjs").then((n) => n.J);
