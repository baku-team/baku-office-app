...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BbGjOLTN.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_EmlmN_zB.mjs");
  ...
  const { hasApiKey } = await import("./client_BbGjOLTN.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_S4P08Enk.mjs");
