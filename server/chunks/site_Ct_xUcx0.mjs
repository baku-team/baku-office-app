...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_COMLR7A8.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { audit } from "./storage_CzB15Zqw.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_ONToWqzD.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.S);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.S);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_COMLR7A8.mjs");
