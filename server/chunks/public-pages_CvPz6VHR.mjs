...
function publicFieldsForDefinition(def) {
  ...
  return hasCustom ? fields.map((f) => ({ ...f, required: false })) : fields;
}
...
const PAGE_COLS = "slug,app_id,title,html,fields,allow_files,enabled,created_by,notify_admin,confirm_email,email_field,confirm_subject,confirm_body,price,currency,pay_label,register_mode";
...
async function upsertPublicPage(env, p) {
  ...
  await env.DB.prepare(
    `INSERT INTO public_pages (slug,app_id,title,html,fields,allow_files,enabled,created_by,notify_admin,confirm_email,email_field,confirm_subject,confirm_body,price,currency,pay_label,created_at,updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
     ON CONFLICT(slug) DO UPDATE SET title=excluded.title, html=excluded.html, fields=excluded.fields, allow_files=excluded.allow_files,
       ...
       pay_label=excluded.pay_label, updated_at=excluded.updated_at`
  ).bind(
    slug,
    p.appId,
    p.title.slice(0, 200),
...
async function rateLimited(env, ip) {
  const key = `pubrl:${ip}`;
  const cur = Number(await env.LICENSE.get(key) ?? "0");
  if (cur >= RL_CAP) return true;
  await kvPut(env, key, String(cur + 1), { expirationTtl: 3600 });
  return false;
}
...
async function createSubmission(env, page, data, files, ip) {
  const id = randomId();
  await env.DB.prepare(
    "INSERT INTO public_submissions (id,slug,app_id,data,files,ip_hash,status,created_at) VALUES (?,?,?,?,?,?,'pending',?)"
  ).bind(id, page.slug, page.app_id, JSON.stringify(data).slice(0, 64 * 1024), files.length ? JSON.stringify(files) : null, ipHash(ip), nowSec()).run();
  return { id };
}
...
async function moderate(env, id, action, reviewer) {
  ...
  if (row.status !== "pending") return { ok: false, error: "この送信は既に処理済みです。" };
  if (action === "approve") {
    ...
    await env.DB.prepare("INSERT INTO app_records (id,app_id,owner,data,created_at) VALUES (?,?,?,?,?)").bind(randomId(), row.app_id, owner, data, nowSec()).run();
  }
