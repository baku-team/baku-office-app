...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BR_H-Abo.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BCSUj1NZ.mjs");
  await import("./index_DYBxMYOj.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DXYlvzaN.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B-fn0x00.mjs");
  ...
  const { reviewQueue } = await import("./users_BV2CrEPZ.mjs");
  const { backupAlert } = await import("./backup_CNX-8Bhc.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DnM9W0WO.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_hKiHcy_A.mjs");
