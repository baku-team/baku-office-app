...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_SzFl8Xp8.mjs");
  const { cmpVersion } = await import("./update_CL1vGhXx.mjs");
  await import("./index_CK9rtFUm.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CVbPdOBU.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B9EX6Bnq.mjs");
  ...
  const { reviewQueue } = await import("./users_CNVeOpbc.mjs");
  const { backupAlert } = await import("./backup_ZA12cWCo.mjs");
