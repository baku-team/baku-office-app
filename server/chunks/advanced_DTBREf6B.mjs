...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_C9EMiQbG.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_3J2h8SOe.mjs");
  ...
  const { hasApiKey } = await import("./client_C9EMiQbG.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_IXeHfBaF.mjs");
