...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_BWY7KuVP.mjs").then((n) => n.U), import("./notifications_CYoNJfzE.mjs")]);
    ...
    await (await import("./ctx_BWY7KuVP.mjs").then((n) => n.Q)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_BWY7KuVP.mjs").then((n) => n.L)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_Ds0bq2JW.mjs");
