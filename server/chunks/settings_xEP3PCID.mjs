...
import { setMaxUploadMb, setRetentionDays } from "./storage_Ik0IyZy_.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_DDGy_-sb.mjs";
import { setStorageLimits } from "./storage-usage_dc8_ScnM.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_CNr3eCe2.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_Czv7yIer.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_B8efPIrw.mjs");
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.y);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.z);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.z);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.y);
