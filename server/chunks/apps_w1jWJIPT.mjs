...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_DmR_wS5Q.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CkzMAmmV.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DtjctWvj.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Ur7P7bey.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DmeO_5kU.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_CGpwMANX.mjs");
