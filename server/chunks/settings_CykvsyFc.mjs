...
import { getSession } from "./auth_B8CX3HBm.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_CSR2WvVx.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_DUUxPCqA.mjs";
import { setStorageLimits } from "./storage-usage_CO7PQfLB.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_BJv6y5C1.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_B45zdqU6.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_C7-VO_NU.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_D77zflm8.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_ODKWvbUB.mjs").then((n) => n.L);
      try {
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.y);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.z);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.z);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.y);
