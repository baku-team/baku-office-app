...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BV1rSndT.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DZgvkCbI.mjs");
  await import("./index_Q-RNyiqH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BzN5Ly8A.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BQQ1Bz39.mjs");
  ...
  const { reviewQueue } = await import("./users_BEPQ0WqN.mjs");
  const { backupAlert } = await import("./backup_DfaJB7As.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_2eZp4hU_.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CT4gbX4d.mjs");
