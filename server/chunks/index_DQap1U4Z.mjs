...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CDCAJlBm.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_B0LtoEmz.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_xoQnZ-Ug.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_Cxrpj4bG.mjs"),
    import("./scheduled-tasks_DkOe59qc.mjs")
  ]);
  ...
  await import("./index_DnpuPih6.mjs");
