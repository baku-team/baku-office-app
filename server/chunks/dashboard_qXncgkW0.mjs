...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Cmhn0ct_.mjs");
  const { cmpVersion } = await import("./update_CrILhcuM.mjs");
  await import("./index_DDhRaaxU.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CSXPq6XS.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BmNx6SDn.mjs");
  ...
  const { reviewQueue } = await import("./users_C8cbQeLC.mjs");
  const { backupAlert } = await import("./backup_6aOuM56H.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CyLGJDop.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_Ch3C4uCb.mjs");
