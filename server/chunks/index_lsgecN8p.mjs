...
import { r as renderComponent } from "./worker-entry_BKO1XHQV.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D-wQHZDr.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_kImFTa8b.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BjQkw6mR.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CRQPwndu.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DDUITG1t.mjs"),
    import("./scheduled-tasks_B0uqOOcA.mjs")
  ]);
  ...
  await import("./index_Cw432j-u.mjs");
