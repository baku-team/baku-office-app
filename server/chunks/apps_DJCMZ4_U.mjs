...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_C48CCjoS.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_Cu1Zes9a.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_0pHY0CaD.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DF6Mutvv.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DF9qKz49.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DzHldYd8.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_BEXCEr29.mjs");
