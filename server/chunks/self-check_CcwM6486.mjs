...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_DixJgty_.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_D9aXYLsc.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
