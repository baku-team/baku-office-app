...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CyRmMA6L.mjs");
  const { cmpVersion } = await import("./update_B9g8m8cE.mjs");
  await import("./index_DaxO1ug-.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BT_VA6pC.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D5P-Ka9J.mjs");
  ...
  const { reviewQueue } = await import("./users_At9DIjir.mjs");
  const { backupAlert } = await import("./backup_Dn5s7PWO.mjs");
