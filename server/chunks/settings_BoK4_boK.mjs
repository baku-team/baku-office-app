...
import { getSession } from "./auth_CwvcOiF1.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_Dvmn3unV.mjs";
import { setMemberModel, toggleFavApp, setAppFolders, setAiEngine, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_BGnJ4d3n.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_CF68La6g.mjs";
import { setStorageLimits } from "./storage-usage_DbZwsLne.mjs";
import { s as setEnabledPartIds, p as partCatalog, e as enabledPartIds } from "./parts_DlPNdOxI.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_t7uZkVIe.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_CIfdV394.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_BkEgDZ_T.mjs");
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_CIfdV394.mjs").then((n) => n.x);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_CIfdV394.mjs").then((n) => n.y);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_CIfdV394.mjs").then((n) => n.y);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_CIfdV394.mjs").then((n) => n.x);
