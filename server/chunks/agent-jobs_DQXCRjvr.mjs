...
import { r as runAgent, n as notifyOwnerDirect } from "./ctx_DkS04vDB.mjs";
...
async function runOneAgentJob(ctx, j, baseUrl, lease) {
  const hb = setInterval(() => {
    ctx.db.run("UPDATE agent_jobs SET updated_at=? WHERE id=? AND status='running' AND lease_owner=?", [nowSec(), j.id, lease]).catch(() => {
    });
  }, 3e4);
  try {
    let history;
