...
import { getApproval, decideApproval } from "./approvals_kjrS3dQ0.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_DrSavdul.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_BZ9IFLfW.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CanD1w9d.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BNXMhaMG.mjs";
import { joinWithInvite } from "./users_CavXFK4T.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BcmxKHWi.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CavXFK4T.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_BLtLjVKO.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_ggkdTLg-.mjs");
