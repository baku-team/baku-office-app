...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BNfwKBey.mjs");
  const { cmpVersion } = await import("./update_DsXJHqJH.mjs");
  await import("./index_B1Ym4BTc.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CED2ECDA.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BGJBsuIr.mjs");
  ...
  const { reviewQueue } = await import("./users_EBnj0HQw.mjs");
  const { backupAlert } = await import("./backup_Bvlaimr7.mjs");
