...
import { r as renderComponent } from "./worker-entry_CVIgym3A.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BA2UOlTJ.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DcLP1ieF.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DcLP1ieF.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_BFAO0OWB.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_yx4APBBm.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C_ej8KZQ.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BeNzI-n3.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BEfZxNjt.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DRlasx4Z.mjs");
  ...
  const folders = await (await import("./settings_FobH8uUC.mjs")).getAppFolders(env, ses.uid).catch(() => []);
