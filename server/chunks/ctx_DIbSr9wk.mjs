...
import { kvPut } from "./kv_CSS355mB.mjs";
import { EgressGateway, DEFAULT_EGRESS_ALLOWLIST } from "./egress_BjyQB9oJ.mjs";
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, fileBelongsTo, storageMode } from "./storage_iUFCTMib.mjs";
import { getApiKey, getToken, hostFetch, entitlementForGate, masterKeyCtx } from "./client_DlpdsadF.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_yYgLGQvf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_hcWwRf5D.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CqmHVZPm.mjs";
import "./index_B2V0XL0x.mjs";
import { recordUsage, recordTokens, overBudget, estimateUsd } from "./usage_Bqk7Mxtc.mjs";
...
import { googleFetch } from "./google_Ca1jl-Ew.mjs";
...
import { validateDefinition, APP_SCHEMA, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, RENDER_HTML_MAX, opCatalogText } from "./appdef_0geZcejn.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, resolveModelSelection, getMemberModel, getSmartRouting, getWorkersAiModel, maxParallelAgents, agentMaxHops } from "./settings_FobH8uUC.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DCACyF4S.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BPfTpqz1.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_Dqozij6P.mjs";
import { logDiag } from "./diag_BRkvvUfW.mjs";
import { i as isRunnableDefinition } from "./preflight_rNRXjiKE.mjs";
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage, collectionFromDefinition } from "./public-pages_CORq3-xY.mjs";
import { g as getAppDesign, m as repairOutputLiterals, n as createDraft, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, i as installLocalApp, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_BTgdbc5l.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_D-_59EQd.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_D0ttewuz.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_B9wOpFrA.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_B41Oi6I1.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { p as pushToUser } from "./push_BYg56hOk.mjs";
...
function makeProviderChain(chain, workersAi, workersAiNote) {
    ...
    async turn(system, history, tools, force, opts) {
      for (; ; ) {
        ...
        if (workersAi) {
          ...
          return workersAi.turn(workersAiNote + "\n\n" + system, history, tools, force, opts);
        }
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DlpdsadF.mjs");
...
${custom}`].filter(Boolean).join("\n");
  ...
  if (wantLocal && env.AI) {
    ...
    for (const id of ordered) {
      ...
      chain.push({ model: b.build(), provider: id, label: labelOf(id) });
    }
    if (!chain.length && image) return "画像を読めるAI（Gemini/Claude）が未設定または利用上限です。クレジット補充、または Gemini（無料枠）/Claude のAPIキーを登録してください（設定 → 連携設定）。";
    if (chain.length) {
...
`;
      const chained = makeProviderChain(chain, env.AI ? workersAiChatModel(env.AI, waModel) : null, WORKERS_AI_NOTE);
      model = chained.model;
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。");
    ...
    try {
      ...
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_FobH8uUC.mjs");
