...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BPJeqCVB.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_BeVszgRW.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_ARKOgIQO.mjs");
  ...
  const { buildCtx } = await import("./ctx_Ige4T837.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_Ige4T837.mjs").then((n) => n.M);
