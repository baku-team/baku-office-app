...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_v-7HzW5v.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BxFk2Le_.mjs");
  await import("./index_DwFb6RgQ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CnHsjCoE.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B5o6RDLs.mjs");
  ...
  const { reviewQueue } = await import("./users_BT4mrZN2.mjs");
  const { backupAlert } = await import("./backup_Bdy2IQDl.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.A);
  ...
  const { storageMode } = await import("./storage_4iye1Zw4.mjs");
