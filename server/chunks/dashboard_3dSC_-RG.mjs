...
import { r as renderComponent } from "./worker-entry_DWUuTmud.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BoTOJnlm.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_Cki9oGIJ.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BTiBrVBU.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DFcLHmTB.mjs");
  await import("./index_lNlZmhzH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BtHypiSY.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_7-DjygCb.mjs");
  ...
  const { reviewQueue } = await import("./users_BGx-Ag0G.mjs");
  const { backupAlert } = await import("./backup_7yxiF4OV.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Dav2SgUe.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_CXuQdqf4.mjs");
  const { getWorkersPaid } = await import("./settings_DI_y7gTJ.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_CAOmc-4O.mjs");
