...
import { x as cfEgressGateway, D as linePush, E as processSummaryJobs, F as processAppBuilds, f as processAppBuild, G as processSiteBuilds } from "./ctx_C3hyS9pN.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_DvWZteUY.mjs";
import { processAgentJobs } from "./agent-jobs_C1guhANw.mjs";
import { reclaimStaleApprovals } from "./approvals_CpyhgkQw.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_D30vSrvO.mjs";
import { guardHeavy } from "./diag_CsI0yNfw.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_DSp2oymh.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_C1ZaPmSf.mjs";
import { flushReports } from "./reports_Ceo89kju.mjs";
import { addNotification, pushWebhook } from "./notifications_5KkK0xTP.mjs";
import { getNotifyWebhook } from "./settings_DI_y7gTJ.mjs";
import { purgeFiles } from "./storage_C0xk2SHi.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_COShl9LN.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  ...
  const at = await guardHeavy(env, "app triggers", () => import("./integration-polling_C1WrKnGQ.mjs").then((m) => m.pollAppTriggers(env, locals.ctx)));
  const appTriggers = at.ok ? at.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_BL1ENf5s.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_Ds0LyxKl.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_C5gpRWgz.mjs").then((m) => m.flushUsageDigest(env)));
