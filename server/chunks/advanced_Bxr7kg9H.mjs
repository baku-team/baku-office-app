...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CB_YAuNH.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BREWMKHz.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Cvzvo6pu.mjs");
