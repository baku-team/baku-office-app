...
import { r as renderComponent } from "./worker-entry_DOgLvlrL.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_MkyRpmK1.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_B7kuZSwA.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_B7kuZSwA.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DmYZEEDZ.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_DsktjJVP.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CIn9PEgV.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Bo1OmsPF.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DtyjwiHH.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_CNqi7plN.mjs");
  ...
  const folders = await (await import("./settings_CGAf54Rj.mjs")).getAppFolders(env, ses.uid).catch(() => []);
