...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_D_CrX1Mh.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_Drnao1kv.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_B_HP_Ex9.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Bj0rITH8.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CCYEJicX.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_JB9CsP9c.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_mE1mqivC.mjs");
