...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_Bm-CmJY_.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Cti--qkw.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_C9FmrfSe.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CqQvwEcj.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_m8OOu-NR.mjs";
import "./index_UJbeOQUy.mjs";
...
import { googleFetch } from "./google_D0Z9obGO.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_DKzuns4-.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CC3evYif.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_D7JP3TLS.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_g8HcmSo2.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_BiC1Bc-8.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_BcniWYMM.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BwRadYch.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DQykcWLS.mjs";
import { storeCatalog } from "./store_CoEo_yWT.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CPTo5Zi4.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_CYnoEgWp.mjs";
import { saveLayoutDraft, listSites } from "./sites_Cbgrxt0R.mjs";
...
import { p as pushToUser } from "./push_BFsUY5f3.mjs";
...
const REQUESTED_FEATURES = [
  ...
  {
    intent: /((外部|お客様|顧客|利用者|一般|ユーザー|WEB|Web|ウェブ)\s*(から)?\s*(の)?\s*(問い?合わせ|お問合せ|申込|申し込み|応募|エントリー|受付|依頼|予約))|(問い?合わせ|申込|応募|予約|受付)\s*(を)?\s*(受(け|付)|受信|一覧|管理|取り?込|フォーム)/i,
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_Cbgrxt0R.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
function estimateBuildScale(text) {
  ...
  const large = explicit >= 5 || distinct >= 6 || features >= 8;
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_DlUb4VID.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_DlUb4VID.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_DlUb4VID.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_Bm-CmJY_.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Cti--qkw.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_DlUb4VID.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_Bm-CmJY_.mjs")).storageMode(env) === "r2";
