...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_DwTr64WL.mjs").then((n) => n.X), import("./notifications_DbiBZY49.mjs")]);
    ...
    await (await import("./ctx_DwTr64WL.mjs").then((n) => n.T)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_DwTr64WL.mjs").then((n) => n.O)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_IXodE4Vt.mjs");
