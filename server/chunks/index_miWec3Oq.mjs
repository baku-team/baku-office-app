...
import { r as renderComponent } from "./worker-entry_D5KFfC0e.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BKz5eCMM.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.locals.publicHost) {
    ...
    const { buildPublicFullPage } = await import("./app-frame_C-ahuMog.mjs").then((n) => n.d);
  ...
  const { getSession } = await import("./auth_CliC3F6V.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BAVQqvb3.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_D3Iq-OUA.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DmzjyEFN.mjs"),
    import("./scheduled-tasks_BnBm2gu4.mjs")
  ]);
  ...
  await import("./index_z7F-7bMo.mjs");
