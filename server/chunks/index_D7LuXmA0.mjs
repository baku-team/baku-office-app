...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.locals.publicHost) {
    ...
    const { buildPublicFullPage } = await import("./app-frame_lWmhgLXk.mjs").then((n) => n.d);
  ...
  const { getSession } = await import("./auth_g5P0sTgC.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_D3kajqFf.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_DYtEz2tN.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_0835KCOB.mjs"),
    import("./scheduled-tasks_D6wQyD48.mjs")
  ]);
  ...
  await import("./index_Dj0mRxJ1.mjs");
