...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_SJF4hL77.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_DJA_I8xR.mjs";
import { canDevelopApps } from "./auth_g5P0sTgC.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_CHjxiU9K.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_CHjxiU9K.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DUrYDXEN.mjs").then((n) => n.B);
