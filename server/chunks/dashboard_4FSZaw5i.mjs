...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CY40n_Ph.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Bs3ks682.mjs");
  await import("./index_aE8388t6.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_f89fKOqu.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dbvnn5LR.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_JIFmI05h.mjs");
  const { scopedWidgets, enabledParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  ...
  const { reviewQueue } = await import("./users_DZE1G0eK.mjs");
  const { backupAlert } = await import("./backup_C05Nvqv1.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CDC9FeHL.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_D6LLPvBw.mjs");
  const { getWorkersPaid } = await import("./settings_dTyIbTCR.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_CfukLLAj.mjs");
