...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B2HA1A74.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_BifgPhj3.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_C0kTytCf.mjs").then((n) => n.Q);
  ...
  const { latestSessionApp } = await import("./ctx_C0kTytCf.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_OSwLzA_S.mjs").then((n) => n.z);
