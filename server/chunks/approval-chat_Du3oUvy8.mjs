...
import { getApproval, decideApproval } from "./approvals_Ce-1kAjv.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_C5jSoDoM.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_D-gPzIuz.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_3h9S-qAu.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_CQSsPpWQ.mjs";
import { joinWithInvite } from "./users_aF0JOfhF.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_C19jFPcM.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_aF0JOfhF.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_WllYctg9.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DLAFFzSR.mjs");
