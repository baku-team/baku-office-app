...
import { cachedEntitlement } from "./client_BOITous_.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { saveChatAttachment } from "./storage_BW2B_Uds.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      const { looksLikeBuildConfirmation } = await import("./cf-adapter_Ds_ZQ-Hh.mjs").then((n) => n.B);
      ...
      if (b.mode !== "plan" && !b.image?.dataB64 && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant) && atLeast(await cachedEntitlement(env), "pro")) {
        const { startAppBuild, processAppBuild } = await import("./cf-adapter_Ds_ZQ-Hh.mjs").then((n) => n.A);
      ...
      if (b.mode !== "plan" && !b.image?.dataB64 && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
        const { looksLikeAppEdit } = await import("./cf-adapter_Ds_ZQ-Hh.mjs").then((n) => n.B);
        if (looksLikeAppEdit(message)) {
          const { latestSessionApp, startAppEdit, processAppBuild } = await import("./cf-adapter_Ds_ZQ-Hh.mjs").then((n) => n.A);
      ...
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_wZLXyjCv.mjs");
