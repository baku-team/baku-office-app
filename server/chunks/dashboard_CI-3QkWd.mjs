...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CkzMAmmV.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_C7zudgPi.mjs");
  await import("./index_DtjctWvj.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Ur7P7bey.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CKAAU9CD.mjs");
  ...
  const { reviewQueue } = await import("./users_DhISMd7C.mjs");
  const { backupAlert } = await import("./backup_BDFBxfKv.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DmeO_5kU.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CzB15Zqw.mjs");
  const { getWorkersPaid } = await import("./settings_C_1axsCz.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_KVKE99-A.mjs");
