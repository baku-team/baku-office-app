...
const $$EventsBlock = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { block } = Astro2.props;
  const p = block.props;
  const { listPublishedEvents, getPublishedEventBySlug } = await import("./events_DrIrLCRE.mjs");
  let events = [];
