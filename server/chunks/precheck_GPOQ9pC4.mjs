...
import { validateDefinition } from "./appdef_B8ThPYFn.mjs";
import { g as getAppDesign } from "./external-apps_qax98Mxc.mjs";
import { p as preflight } from "./preflight_D1WtfhAS.mjs";
import { r as runDraftApp, p as platformInvariantSuspected, f as escalatePlatformInvariant } from "./app-runtime_CUIq43g5.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_qax98Mxc.mjs").then((n) => n.B)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_DWCSrGCf.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
