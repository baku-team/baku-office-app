...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BNfwKBey.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Bttdvwv3.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Ck-RJgTo.mjs");
