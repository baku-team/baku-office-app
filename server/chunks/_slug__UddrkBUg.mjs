...
import { rateLimited, getPublicPage, honeypotTripped, validateSubmission, wouldWaitlist, createSubmission, fireSubmitTriggers, setSubmissionPayment, moderate } from "./public-pages_Bg1BWixA.mjs";
import { createCheckoutSession } from "./payments_C00SuSsV.mjs";
import { maxUploadMb, saveFile } from "./storage_BthdaNTE.mjs";
...
const POST = async ({ request, params, locals }) => {
  ...
  if (!v.ok) {
    await (await import("./diag_-P_u6Fn4.mjs")).logDiag(env, "warn", "public", `公開フォーム送信NG slug=${slug}: ${v.error}`, `送信キー=[${Object.keys(values).join(",")}] / 必須項目=[${page2.fields.filter((f) => f.required).map((f) => f.name).join(",")}]`).catch(() => {
    });
