...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_BJ3WDVbz.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_NtHEh0X8.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
