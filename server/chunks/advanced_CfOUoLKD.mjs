...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DQhV0ghx.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DoEqeUTS.mjs");
  ...
  const { hasApiKey } = await import("./client_DQhV0ghx.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Dr1tD8-3.mjs");
