...
import { getSession } from "./auth_DV5hnO_2.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_Bbu5kdRU.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_BX6sALYq.mjs";
import { setStorageLimits } from "./storage-usage_CZXi_Yvi.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_Cu1bevuo.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_Bm2WRPbZ.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DzgKDhXg.mjs");
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.x);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.y);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.y);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.x);
