...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DBc4AdQ5.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Bd8pBHFN.mjs");
  await import("./index_DaPtUtWl.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_WWbnZ3SD.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CvOL2aXv.mjs");
  ...
  const { reviewQueue } = await import("./users_DYTaJK75.mjs");
  const { backupAlert } = await import("./backup_DXsawawk.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_H2tcQp6t.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_B8-WCIen.mjs");
  const { getWorkersPaid } = await import("./settings_hJXdQlLE.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_Br94sshr.mjs");
