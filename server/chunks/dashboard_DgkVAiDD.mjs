...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CDsrEXRz.mjs");
  const { cmpVersion } = await import("./update_Cm6tgikG.mjs");
  await import("./index_Cx5zbqbt.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C3ejEHDJ.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dg9watpb.mjs");
  ...
  const { reviewQueue } = await import("./users_C8FDAgNl.mjs");
  const { backupAlert } = await import("./backup_DKSm8T-1.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B8UBajsg.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B9Y_ZULI.mjs");
