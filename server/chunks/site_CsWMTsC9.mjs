...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_FMRQ21Fs.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_B0LY-udU.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_Cwau8_F6.mjs");
      ...
      const { getMemberModel, parseRequestModel } = await import("./settings_C4tL9AHR.mjs");
      const { modelId: siteModelId } = parseRequestModel(await getMemberModel(env, ses.uid).catch(() => null) ?? "");
      const { startSiteBuild, processSiteBuild } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.W);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.W);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_FMRQ21Fs.mjs");
