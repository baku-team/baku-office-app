...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_bsCKg9zi.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin" && ses.ctx === "org";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CAyVckvj.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_BxX-ApCe.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConnected } = await import("./drive_BX7_lCnh.mjs");
  ...
  return renderTemplate`${renderComponent($$result, "App", $$App, { "title": "Google連携セットアップ", "active": "/settings/keys", "data-astro-cid-v63rqogn": true }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<h1 data-astro-cid-v63rqogn>Google 連携セットアップ（Google Workspace・WIF）</h1> ${!isAdmin && renderTemplate`<div class="card" data-astro-cid-v63rqogn><div class="banner banner-warn" data-astro-cid-v63rqogn>この画面は管理者のみ利用できます。</div></div>`}${isAdmin && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate` <div class="banner banner-info" style="font-size:.85rem" data-astro-cid-v63rqogn>
