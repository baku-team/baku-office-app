...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_Bav91uYw.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { audit } from "./storage_CKfFiifC.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_Bi0lwxfA.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_DAgrQ03M.mjs").then((n) => n.S);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_DAgrQ03M.mjs").then((n) => n.S);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_Bav91uYw.mjs");
