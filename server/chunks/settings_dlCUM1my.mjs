...
import { getSession } from "./auth_CO6Q4ahN.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_XAs8LXvN.mjs";
import { setMemberModel, setAiEngine, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_BgFAT_Wp.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_CdESEoWA.mjs";
import { setStorageLimits } from "./storage-usage_BPsYJ0kN.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_BVCI0C7p.mjs";
import { f as fetchAndInstall, u as uninstallExternal, l as listExternalApps, b as listDrafts, s as submitDraft, d as deleteDraft, c as installLocalApp, e as appVersions, g as activateAppVersion, h as deleteLocalApp } from "./external-apps_D4vbFip2.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_BR5C_fTn.mjs");
  ...
  if (b._action === "app_versions") {
    return json({ ok: true, versions: await appVersions(locals.ctx, String(b.appId ?? "")) });
  }
  ...
  if (b._action === "delete_local_app") {
    await deleteLocalApp(locals.ctx, String(b.appId ?? ""));
    return json({ ok: true });
  }
