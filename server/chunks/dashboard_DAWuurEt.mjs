...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DSXOrOmp.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BpqrJR44.mjs");
  await import("./index_DM4gS8c2.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CjLxCdtu.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_zaV173Tc.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_BKafWGCx.mjs");
  ...
  const { reviewQueue } = await import("./users_PX595n4G.mjs");
  const { backupAlert } = await import("./backup_BB65yU-z.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DszGd1gL.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CM436dbQ.mjs");
  const { getWorkersPaid } = await import("./settings_hJXdQlLE.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_BDQhcX3s.mjs");
