...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CSBhOn7n.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_OY2k6jzJ.mjs");
  await import("./index_B0UyH5ZM.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_fVy9d1St.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DEme6YUl.mjs");
  ...
  const { reviewQueue } = await import("./users_BQwRbui4.mjs");
  const { backupAlert } = await import("./backup_BKJg2vxb.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CfvnGFDu.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_DV5PW-A4.mjs");
