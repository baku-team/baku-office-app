...
import { getFile } from "./storage_CzJPn-cz.mjs";
import { getDriveSave } from "./drive_D58gIbWV.mjs";
import { n as notifyOwnerDirect } from "./ctx_CrtWLNXo.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.U);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_CzJPn-cz.mjs");
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_CrtWLNXo.mjs").then((n) => n.P);
