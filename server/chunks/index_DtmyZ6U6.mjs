...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_CyzvxOBO.mjs");
  const { cmpVersion } = await import("./update_CVKUiWFp.mjs");
  await import("./index_B5zhrqAU.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_HNyx-taB.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CpG1WxYW.mjs");
  ...
  const { reviewQueue } = await import("./users_BznQBGZY.mjs");
  const { backupAlert } = await import("./backup_DemgGweY.mjs");
