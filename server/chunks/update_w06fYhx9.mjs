...
import { r as renderComponent } from "./worker-entry_WJDWDAoy.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D3Bt7Wog.mjs";
...
const $$Update = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Update;
  const { getSession } = await import("./auth_Hlv2_UQG.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  if (ses.role !== "admin") return Astro2.redirect("/forbidden", 302);
  const { pollHost, APP_VERSION } = await import("./client_sRscislI.mjs");
  const { hasDeployHook, cmpVersion, getAutoUpdate, getAutoUpdateStatus } = await import("./update_CAdJB7AV.mjs");
  ...
  )}<details class="card" id="sync-section"${addAttribute(needsRecovery, "open")} style="margin-top:1rem" data-astro-cid-curm2vkq> <summary style="font-weight:600;cursor:pointer" data-astro-cid-curm2vkq>更新が反映されないとき：リポジトリを最新化（1回だけ）</summary> <p class="muted small" style="margin:.5rem 0" data-astro-cid-curm2vkq>
このアプリは配布リポ（<code data-astro-cid-curm2vkq>${UPSTREAM}</code>）を<strong data-astro-cid-curm2vkq>あなたの GitHub に複製</strong>して動いています。複製が古いと更新の取り込み部分が旧版のままになります。<strong data-astro-cid-curm2vkq>フォークを1回同期</strong>すれば修正版が入り、以降は自動で反映されるようになります。<strong data-astro-cid-curm2vkq>同じ Worker の再ビルドのためデータ・設定は保持</strong>されます（Deploy ボタンの押し直しはしないでください）。
</p> <div class="field" style="margin:.4rem 0 .3rem" data-astro-cid-curm2vkq> <input id="gh-owner" placeholder="GitHub の ユーザー名 または 組織名（例：your-org）" data-astro-cid-curm2vkq> </div> <div style="display:flex;gap:.5rem;flex-wrap:wrap" data-astro-cid-curm2vkq> <button class="btn btn-primary" id="sync-fork" data-astro-cid-curm2vkq>リポジトリ（${REPO_NAME}）を開く ↗</button> <button class="btn" id="list-repos" data-astro-cid-curm2vkq>見つからない：リポジトリ一覧を表示 ↗</button> </div> <ol class="muted small" style="padding-left:1.2rem;margin:.6rem 0 0" data-astro-cid-curm2vkq> <li data-astro-cid-curm2vkq>開いたリポジトリで「<strong data-astro-cid-curm2vkq>Sync fork</strong>」→「<strong data-astro-cid-curm2vkq>Update branch</strong>」を押す（フォークが遅れているときだけ表示されます）。</li> <li data-astro-cid-curm2vkq>同期すると連携中の Cloudflare が<strong data-astro-cid-curm2vkq>自動で再ビルド</strong>します。数分後にこのページ上部のバージョンが上がれば完了です。</li> <li data-astro-cid-curm2vkq>ページが開けない/リポが違う場合は「リポジトリ一覧を表示」から <code data-astro-cid-curm2vkq>${REPO_NAME}</code> を含むものを開いてください（リポ名は <a${addAttribute(SETTINGS, "href")} target="_blank" rel="noreferrer" data-astro-cid-curm2vkq>Cloudflare の Worker 設定 → Build</a> でも確認できます）。</li> <li data-astro-cid-curm2vkq>「Sync fork」が無い場合は当社サポートへ（<code data-astro-cid-curm2vkq>prebuild-update.mjs</code>・<code data-astro-cid-curm2vkq>wrangler.jsonc</code> の2ファイルパッチで対応します）。</li> </ol> </details> <div class="banner banner-info" style="font-size:.84rem;margin-top:1rem" data-astro-cid-curm2vkq>更新は<strong data-astro-cid-curm2vkq>同じプロジェクトの再ビルド</strong>です。Deploy ボタンの押し直し（最初からやり直し）は行わないでください（データが切り離されます）。</div>   `, "scripts": async ($$result2) => renderTemplate(_a || (_a = __template([`<script data-astro-rerun>
    ...
    const ghOwner = () => (document.getElementById("gh-owner").value || "").trim().replace(/^@/, "").replace(/^https?:\\/\\/github\\.com\\//i, "").replace(/\\/+$/, "");
    // ユーザー名/組織名 → そのアカウントの baku-office リポを直接開く（owner/repo・フルパスもそのまま尊重）。
    const syncF = document.getElementById("sync-fork");
    if (syncF) syncF.addEventListener("click", () => {
      const v = ghOwner();
      if (!v) { window.bo.toast("GitHub のユーザー名または組織名を入力してください", "err"); return; }
      const path = v.includes("/") ? v : (encodeURIComponent(v) + "/baku-office");
      window.open("https://github.com/" + path, "_blank", "noopener");
    });
    // フォールバック：リポ名が違う/見つからない場合、そのアカウントの baku-office 絞り込みリポ一覧を開く。
    const listB = document.getElementById("list-repos");
    if (listB) listB.addEventListener("click", () => {
      const v = ghOwner();
      if (!v) { window.bo.toast("GitHub のユーザー名または組織名を入力してください", "err"); return; }
      const owner = v.split("/")[0];
      window.open("https://github.com/" + encodeURIComponent(owner) + "?tab=repositories&q=baku-office", "_blank", "noopener");
    });
