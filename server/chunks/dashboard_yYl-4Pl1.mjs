...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_peeVfBbE.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DZEsTyH6.mjs");
  await import("./index_DyMvLdWz.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_XmBumVGr.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_77e5_QtS.mjs");
  ...
  const { reviewQueue } = await import("./users_Dt8Au497.mjs");
  const { backupAlert } = await import("./backup_C5D8WI6y.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CHDLP6OC.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_DAEZsEe4.mjs");
