...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B4jFnDvF.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_BlKVKIq1.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_CJCsDXd1.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_CJCsDXd1.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CND5X7DG.mjs").then((n) => n.z);
