...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_AVD7qBz2.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_CVltyN7d.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_B58Crkjn.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_B58Crkjn.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_qax98Mxc.mjs").then((n) => n.B);
