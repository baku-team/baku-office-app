...
import { r as renderComponent } from "./worker-entry_Cz-xBtzt.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D_54qtLm.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DED0iuNZ.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_DEBuc12A.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CRcexuT4.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_CnB0e96h.mjs"),
    import("./scheduled-tasks_CG3wESE-.mjs")
  ]);
  ...
  await import("./index_mJzrtm6N.mjs");
