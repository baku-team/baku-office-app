...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_rKb6N0iG.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DAgrQ03M.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_BwbWPyEb.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_P1oP4Wh_.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DGZitt3L.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_D8PNCsk4.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_YD1YsL3h.mjs");
