...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CzB15Zqw.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_CkzMAmmV.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_BF4NlyCu.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_DhISMd7C.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_Ur7P7bey.mjs";
import "./index_DtjctWvj.mjs";
...
import { googleFetch } from "./google_CDJJUUzX.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_BJHimmJt.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_DzBIQjTt.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BEzBZDwk.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_B-bgkPuX.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_YYoCxNfB.mjs";
import { logDiag } from "./diag_D9aXYLsc.mjs";
import { i as isRunnableDefinition } from "./preflight_BueUwfqI.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_Cgcyhwit.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_BMnoiTJJ.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_GcjRF9RO.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DmWkKqQd.mjs";
import { saveLayoutDraft, listSites } from "./sites_COMLR7A8.mjs";
import { v as validateLayout } from "./site-layout_C_DGypSo.mjs";
import { ICON_OPTIONS, BLOCK_DEFS } from "./defs_C7dN86-U.mjs";
import { storeCatalog } from "./store_CGpwMANX.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_Nz15Coiv.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_COMLR7A8.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_Drz2_LeN.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_Drz2_LeN.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_Drz2_LeN.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_CkzMAmmV.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_Drz2_LeN.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CzB15Zqw.mjs")).storageMode(env) === "r2";
