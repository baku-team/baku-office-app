...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_AxuQz5dA.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_SsbS55Wf.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DEGbNk-F.mjs");
