...
`));
  ...
  (async () => {
    try {
      ...
      const reply = await ctx.agent.run({ owner: ses.uid, text: prompt, image: b.image, role: ses.role, baseUrl: origin, history: toTurns(prior), model, modelId, onEvent, signal: request.signal, mode: b.mode });
      ...
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        ...
        const { extractActions } = await import("./chat-sessions_D5dA8xPs.mjs").then((n) => n.b);
