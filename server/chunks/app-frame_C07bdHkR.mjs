...
function frameCsp(isolation = "sandboxed", allowHosts = []) {
  if (isolation !== "relaxed") return FRAME_CSP;
  const hosts = (allowHosts || []).map((h) => String(h).trim().toLowerCase()).filter((h) => /^[a-z0-9.-]+(?::\d+)?$/.test(h)).map((h) => "https://" + h);
  const src = hosts.join(" ");
  return [
    "default-src 'none'",
    "img-src data: https: blob:",
    "media-src data: https: blob: mediastream:",
    "style-src 'unsafe-inline'",
    "font-src data: https: blob:",
    `script-src 'unsafe-inline'${src ? " " + src : ""}`,
    "object-src data: https: blob:",
    "frame-src data: https: blob:",
    `connect-src ${src || "'none'"}`,
    "form-action 'none'",
    "base-uri 'none'"
  ].join("; ");
}
function buildFrameSrcdoc(html, sdk = BRIDGE_SDK, params = {}, viewer = {}, frame = {}) {
  ...
  return [
    ...
    '<meta charset="utf-8">',
    `<meta http-equiv="Content-Security-Policy" content="${frameCsp(frame.isolation, frame.allowHosts)}">`,
    '<meta name="viewport" content="width=device-width,initial-scale=1">',
    `<style>${BAKU_APP_BASE_CSS}</style>`,
    `<script>window.__BO_PARAMS=${paramsJson};window.__BO_VIEWER=${viewerJson};${STORAGE_SHIM}<\/script>`,
    "</head><body>",
    html,
    `<script>${sdk}<\/script>`,
    "</body></html>"
