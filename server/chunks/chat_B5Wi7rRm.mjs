...
import { getSession } from "./auth_Dv895O2a.mjs";
import { cachedEntitlement } from "./client_BPZJrufk.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { saveChatAttachment } from "./storage_BNzLQbDF.mjs";
import { o as ownedSession, c as createSession, g as getMessages, a as appendMessage, e as ensureTitle, t as toTurns } from "./chat-sessions_D5dA8xPs.mjs";
...
（添付ファイルを保存しました: file_id=${att.id}。請求書/領収書なら register_invoice に file_id を渡して登録してください。）`;
  ...
  if (b.background) {
    ...
    const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_47LVBj6D.mjs");
  ...
  }
  {
    ...
    const { looksLikeBuildConfirmation } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.C);
    ...
    if (b.mode !== "plan" && !b.image?.dataB64 && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant) && atLeast(await cachedEntitlement(env), "pro")) {
      const { startAppBuild, processAppBuild } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.B);
  ...
  }
  {
    ...
    if (b.mode !== "plan" && !b.image?.dataB64 && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
      const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.C);
      const priorAssistant = [...prior].reverse().find((m) => m.role === "assistant")?.content ?? "";
      const wantsDelete = looksLikeAppDelete(message);
      const confirmsDelete = looksLikeDeleteConfirmation(message, priorAssistant);
      if (wantsDelete || confirmsDelete) {
        const { latestSessionApp } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.B);
        const appId = await latestSessionApp(ctx, sessionId);
        if (appId) {
          const { getAppDesign, deleteGenApp } = await import("./external-apps_Dj_vTif6.mjs").then((n) => n.y);
          const design = await getAppDesign(ctx, appId).catch(() => null);
          const appName = design?.name ?? appId;
          if (confirmsDelete) {
            await deleteGenApp(ctx, appId).catch(async (e) => {
              await (await import("./diag_B7sefss8.mjs")).logDiag(env, "error", "chat", `deleteGenApp失敗(app=${appId}): ${e?.message ?? e}`);
              throw e;
            });
            const reply3 = `「${appName}」を削除しました。下書き・導入版・公開ページ・蓄積データをまとめて削除しました（元に戻せません）。`;
            await appendMessage(ctx, sessionId, "assistant", reply3);
            return json({ ok: true, reply: reply3, sessionId });
          }
          const reply2 = `「${appName}」を削除しますか？
アプリ本体に加え、下書き・導入版・公開ページ・蓄積データもまとめて削除され、元に戻せません。よろしければ「削除する」を押してください。`;
          ...
          await appendMessage(ctx, sessionId, "assistant", reply2, actions2);
          return json({ ok: true, reply: reply2, actions: actions2, sessionId });
        }
      }
    }
  }
  {
    const { canDevelopApps } = await import("./auth_Dv895O2a.mjs");
    if (b.mode !== "plan" && !b.image?.dataB64 && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
      const { looksLikeAppEdit } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.C);
      if (looksLikeAppEdit(message)) {
        const { latestSessionApp, startAppEdit, processAppBuild } = await import("./cf-adapter_B8hYIOOm.mjs").then((n) => n.B);
        const appId = await latestSessionApp(ctx, sessionId);
        if (appId) {
          const { getWorkersPaid } = await import("./settings_BGnJ4d3n.mjs");
          const origin2 = new URL(request.url).origin;
  ...
  const { extractActions, navGuidance } = await import("./chat-sessions_D5dA8xPs.mjs").then((n) => n.b);
  const ex = extractActions(reply);
  const replyActions = ex.actions.filter((a) => a.kind === "reply");
  const actions = [...replyActions, ...navGuidance(ex.content, ses.role)].slice(0, 6);
