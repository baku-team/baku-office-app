...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BvxaQHJg.mjs");
  const { cmpVersion } = await import("./update_BwZvJQvU.mjs");
  await import("./index_Dj-b6uSu.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DG8Orboc.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BNUHPPwb.mjs");
  ...
  const { reviewQueue } = await import("./users_D3g3e0Ug.mjs");
  const { backupAlert } = await import("./backup_CicElj7l.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_OSwLzA_S.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B2HA1A74.mjs");
