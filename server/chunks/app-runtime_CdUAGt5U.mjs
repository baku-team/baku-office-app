...
import { v as validateDefinition, r as roleCanOpenScreen, a as appApprovalEffects, i as isBlockedHost } from "./appdef_FywQemou.mjs";
import { a as scopeCtx } from "./parts_DlPNdOxI.mjs";
import { g as getAppDesign, a as activeAppDefinition, r as roleCanUseApp } from "./external-apps_WS9MhPJy.mjs";
import { a as APP, A as AppError } from "./errors_DHxjqjD7.mjs";
const MAX_STEPS = 50;
...
async function runInstalledApp(ctx, appId, inputs, owner, screenId, role) {
  ...
  return runApp(scopeCtx(ctx, def.permissions), def, inputs, owner, screenId, appId, { role });
}
...
async function authorizeAppRun(ctx, appId, screenId, role) {
  ...
  return { ok: true, appVersion: app.version };
}
async function appRunNeedsApproval(ctx, appId, screenId, approvalOn) {
  const app = await activeAppDefinition(ctx, appId);
  if (!app || !app.definition) return null;
  return approvalNeedFor(app.definition, screenId, approvalOn);
}
async function draftRunNeedsApproval(ctx, draftId, screenId, approvalOn) {
  const d = await getAppDesign(ctx, draftId);
  if (!d || d.source !== "draft" || !d.definition) return null;
  return approvalNeedFor(d.definition, screenId, approvalOn);
}
function approvalNeedFor(def, screenId, approvalOn) {
  ...
  const lines = [
    ...
    eff.hosts.length ? `送信先ホスト：${eff.hosts.join("・")}` : null,
    eff.emailTo.length ? `メール宛先：${eff.emailTo.join("・")}` : null
  ].filter(Boolean);
...
export {
  ...
  appRunNeedsApproval as b,
  runInstalledApp as c,
