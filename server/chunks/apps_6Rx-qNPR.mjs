...
import { r as renderComponent } from "./worker-entry_BBVyzEQd.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_88atV4VS.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CPPkK4pl.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CPPkK4pl.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_C-HGOQve.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_vij6I9Jq.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B3tnatTI.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BgEl515W.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Dk15ONnL.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_pK3-WoB3.mjs");
  ...
  const folders = await (await import("./settings_BY28V0AV.mjs")).getAppFolders(env, ses.uid).catch(() => []);
