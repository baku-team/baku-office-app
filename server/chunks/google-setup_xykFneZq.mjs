...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_f2KqkJh0.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C_U4U2I2.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_CsLxKJkj.mjs");
  ...
  const { buildCtx } = await import("./ctx_CVhIpi7y.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_CVhIpi7y.mjs").then((n) => n.M);
