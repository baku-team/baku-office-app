...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_DhWdwsPc.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DiLiyXJK.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_CuoW304l.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DSEOIWls.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CtVp9LjS.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_C9jYx4MB.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_DwR_Hikq.mjs");
