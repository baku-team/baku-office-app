...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Cl9NskZP.mjs");
  const { cmpVersion } = await import("./update_fD1oz18L.mjs");
  await import("./index_C6eAEUan.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CNzxAd3z.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BVWLHlLO.mjs");
  ...
  const { reviewQueue } = await import("./users_PJWKMIit.mjs");
  const { backupAlert } = await import("./backup_CNw9_pq1.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BFESc2lw.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_dCjt0lxN.mjs");
