...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DPTfaqTP.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Bow6uBXh.mjs");
  ...
  const { hasApiKey } = await import("./client_DPTfaqTP.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Ckp1uovh.mjs");
