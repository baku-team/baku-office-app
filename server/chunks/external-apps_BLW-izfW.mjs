...
import { getToken, hostFetch, getVerifyJwk } from "./client_BsuDyowk.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_5uXD-icA.mjs";
import { sanitizeRenderHtml, validateDefinition, isBlockedHost } from "./appdef_BrFbYECV.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_Ck6a6kXM.mjs";
import { r as registeredParts } from "./parts_D92YfUNk.mjs";
import { installApp } from "./apps_ZgdMwRke.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_BrFbYECV.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_DNSANYcE.mjs");
