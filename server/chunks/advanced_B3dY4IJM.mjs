...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_DcubbLWL.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_iHfghs73.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_D75KjaIu.mjs");
