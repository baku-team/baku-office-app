...
import { r as renderComponent } from "./worker-entry_BWORIuLp.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D5oD8cJr.mjs";
...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_C1XRtU1e.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin" && ses.ctx === "org";
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_wBp1yDst.mjs");
  const { hasApiKey } = await import("./client__0Sedoue.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C1_NSXvK.mjs");
  ...
  return renderTemplate`${renderComponent($$result, "App", $$App, { "title": "Google連携セットアップ", "active": "/settings/keys", "data-astro-cid-v63rqogn": true }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<h1 data-astro-cid-v63rqogn>Google 連携セットアップ</h1> ${!isAdmin && renderTemplate`<div class="card" data-astro-cid-v63rqogn><div class="banner banner-warn" data-astro-cid-v63rqogn>この画面は管理者のみ利用できます。</div></div>`}${isAdmin && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate` <div class="banner banner-info" data-astro-cid-v63rqogn>
連携方法は2つあります。<strong data-astro-cid-v63rqogn>Google Workspace をお使いなら「サービスアカウント」方式がおすすめ</strong>です（下のボタンの <strong data-astro-cid-v63rqogn>Cloud Shell でほぼ自動</strong>・残る手動は管理コンソールでの委任承認1回のみ）。個人 Gmail など Workspace 以外は「OAuth」方式（クライアントID/シークレットの作成だけ手動）。<strong data-astro-cid-v63rqogn>下のタブで方法を選んでください。</strong> </div> ${configured ? renderTemplate`<div class="banner banner-ok" data-astro-cid-v63rqogn>✅ すでに Google 連携は設定済みです（方式：${gstatus?.mode === "sa" ? "サービスアカウント" : gstatus?.mode === "oauth" ? "OAuth" : "—"}）。各機能の連携は <a href="/calendar" data-astro-cid-v63rqogn>カレンダー</a> / <a href="/gmail" data-astro-cid-v63rqogn>Gmail</a> / <a href="/meet" data-astro-cid-v63rqogn>Meet</a> の画面から行えます。</div>` : renderTemplate`<div class="banner banner-warn" data-astro-cid-v63rqogn>未設定です。下のいずれかの方法で設定してください。</div>`}<div class="seg" id="gmethod" role="group" aria-label="連携方法の選択" style="margin:.6rem 0" data-astro-cid-v63rqogn> <button class="seg-opt on" type="button" data-m="sa" data-astro-cid-v63rqogn>サービスアカウント（Workspace・おすすめ）</button> <button class="seg-opt" type="button" data-m="oauth" data-astro-cid-v63rqogn>OAuthクライアント（個人Gmail等）</button> </div> <div class="gpanel" data-panel="sa" data-astro-cid-v63rqogn> <div class="banner banner-info" data-astro-cid-v63rqogn>Google Workspace を使う組織向け。<strong data-astro-cid-v63rqogn>OAuth クライアントID/シークレットも、サービスアカウント鍵(JSON)も不要</strong>です（<strong data-astro-cid-v63rqogn>キーレス＝鍵を発行しません</strong>。組織ポリシーで鍵作成が禁止されていても使えます）。<strong data-astro-cid-v63rqogn>下のステップを上から順に進めてください。</strong></div> <div id="sa-wizard"${addAttribute(saInfo ? "1" : "0", "data-configured")} data-astro-cid-v63rqogn> <section class="wstep" data-step="1" data-astro-cid-v63rqogn> <header class="wstep-h" data-astro-cid-v63rqogn><span class="wstep-n" data-astro-cid-v63rqogn>1</span><span class="wstep-t" data-astro-cid-v63rqogn>Google Cloud で準備（Cloud Shell）</span><span class="wstep-badge" data-astro-cid-v63rqogn></span></header> <div class="wstep-b" data-astro-cid-v63rqogn> <p class="muted" data-astro-cid-v63rqogn>下のボタンで <strong data-astro-cid-v63rqogn>Cloud Shell（ブラウザ内ターミナル）</strong> が開きます。ログイン済み・インストール不要で、プロジェクト作成・API有効化・サービスアカウント作成・WIF 設定までを自動で行います。</p> <a class="btn btn-primary"${addAttribute(cloudshellUrl, "href")} target="_blank" rel="noopener" data-astro-cid-v63rqogn>Cloud Shell で開く ↗</a> <p class="muted small" style="margin-top:6px" data-astro-cid-v63rqogn>開いたら ①「Authorize（承認）」→ ②ターミナルに<strong data-astro-cid-v63rqogn>下の実行コマンド</strong>を貼り付け → ③<strong data-astro-cid-v63rqogn>「Enter」キーを押す</strong>（⚠️ 貼り付けただけでは実行されません）。完了すると <strong data-astro-cid-v63rqogn>WIF設定(JSON) が自動でクリップボードにコピー</strong>されるので、このタブに戻って手順2に貼り付けます。</p> <div class="field" style="margin-top:8px" data-astro-cid-v63rqogn><label data-astro-cid-v63rqogn>Cloud Shell に貼り付ける実行コマンド</label><div class="row" data-astro-cid-v63rqogn><input id="wif-cmd" type="text" readonly${addAttribute(wifCommand, "value")} style="flex:1;font-family:monospace;font-size:.78rem" data-astro-cid-v63rqogn><button class="btn" id="copy-wif-cmd" type="button" data-astro-cid-v63rqogn>コピー</button></div><div class="muted small" data-astro-cid-v63rqogn>先頭の <code data-astro-cid-v63rqogn>WORKER_URL</code> はこの画面のURLから自動設定されています。</div></div> <details class="adv" style="margin-top:6px" data-astro-cid-v63rqogn><summary data-astro-cid-v63rqogn>自分の端末の gcloud で実行する場合</summary> <pre style="background:#0f172a;color:#e2e8f0;padding:.8rem;border-radius:8px;overflow:auto;font-size:.8rem" data-astro-cid-v63rqogn><code data-astro-cid-v63rqogn>gcloud auth login
${wifCommand.replace("bash ", "bash scripts/")}</code></pre> </details> <div class="wstep-nav" data-astro-cid-v63rqogn><button class="btn btn-primary" type="button" data-next="2" data-astro-cid-v63rqogn>準備ができた → 次へ</button></div> </div> </section> <section class="wstep" data-step="2" data-astro-cid-v63rqogn> <header class="wstep-h" data-astro-cid-v63rqogn><span class="wstep-n" data-astro-cid-v63rqogn>2</span><span class="wstep-t" data-astro-cid-v63rqogn>WIF設定を登録</span><span class="wstep-badge" data-astro-cid-v63rqogn></span></header> <div class="wstep-b" data-astro-cid-v63rqogn> <p class="muted" data-astro-cid-v63rqogn>Cloud Shell の完了時に <strong data-astro-cid-v63rqogn>WIF設定が自動コピー</strong>されています。下の欄をクリックして貼り付け（Ctrl+V / Cmd+V）してください。</p> <div class="field" data-astro-cid-v63rqogn><label for="sa-wif" data-astro-cid-v63rqogn>WIF設定（JSON）</label><textarea id="sa-wif" rows="3"${addAttribute('{ "sa_email": "...", "client_id": "...", "project_number": "...", "pool": "...", "provider": "..." }', "placeholder")} style="width:100%;font-family:monospace;font-size:.78rem" data-astro-cid-v63rqogn></textarea><div class="muted small" data-astro-cid-v63rqogn>うまく貼れない場合は Cloud Shell 出力の「┌─ … └─」の間の 1 行をコピーしてください。</div></div> <div class="field" data-astro-cid-v63rqogn><label for="sa-subject" data-astro-cid-v63rqogn>代理するユーザーのメール（Workspace）</label><input id="sa-subject" type="email" placeholder="admin@yourdomain.co.jp"${addAttribute(saInfo?.subject ?? orgEmail, "value")} autocomplete="off" data-astro-cid-v63rqogn><div class="muted small" data-astro-cid-v63rqogn>${orgEmail && !saInfo ? `ログイン中の団体アカウント（${orgEmail}）を既定表示しています。別ユーザーを代理させる場合は変更してください。` : "予定/メールを操作する Workspace ユーザーのメール。"}</div></div> <fieldset style="border:1px solid #dde3ea;border-radius:6px;padding:.5rem .8rem" data-astro-cid-v63rqogn> <legend class="muted" style="font-size:.85rem" data-astro-cid-v63rqogn>利用する機能（委任するスコープ）</legend> ${groupList.map((g) => renderTemplate`<label style="display:block;margin:.2rem 0" data-astro-cid-v63rqogn><input type="checkbox" class="sa-grp"${addAttribute(g.id, "value")}${addAttribute(g.scopes, "data-scopes")}${addAttribute(grantedSet.has(g.id) || g.id === "calendar", "checked")} data-astro-cid-v63rqogn> ${g.label}${g.restricted ? renderTemplate`<span class="pill" style="background:#fee2e2;color:#b91c1c;margin-left:4px" data-astro-cid-v63rqogn>審査対象</span>` : ""}</label>`)} </fieldset> <div class="wstep-nav" data-astro-cid-v63rqogn><button class="btn btn-primary" id="sa-go" data-astro-cid-v63rqogn>① 設定を登録</button></div> <div id="sa-result" style="margin-top:.5rem" data-astro-cid-v63rqogn></div> </div> </section> <section class="wstep" data-step="3" data-astro-cid-v63rqogn> <header class="wstep-h" data-astro-cid-v63rqogn><span class="wstep-n" data-astro-cid-v63rqogn>3</span><span class="wstep-t" data-astro-cid-v63rqogn>ドメイン全体の委任を承認</span><span class="wstep-badge" data-astro-cid-v63rqogn></span></header> <div class="wstep-b" data-astro-cid-v63rqogn> <div class="banner banner-warn small" data-astro-cid-v63rqogn>この手順だけは <strong data-astro-cid-v63rqogn>Google Workspace の超管理者</strong> の操作が必要です（Google がセキュリティ上、人による承認を必須にしているため省略できません・反映に1〜2分かかります）。</div> <p class="muted" data-astro-cid-v63rqogn>下の2つの値を使います（右の「コピー」で取得）。</p> <div class="field" data-astro-cid-v63rqogn><label data-astro-cid-v63rqogn>クライアントID</label><div class="row" data-astro-cid-v63rqogn><input id="sa-cid" type="text" readonly${addAttribute(saInfo?.clientId ?? "（手順2を登録すると表示されます）", "value")} style="flex:1;font-family:monospace;font-size:.82rem" data-astro-cid-v63rqogn><button class="btn" id="sa-copy-cid" type="button"${addAttribute(!saInfo, "disabled")} data-astro-cid-v63rqogn>コピー</button></div></div> <div class="field" data-astro-cid-v63rqogn><label data-astro-cid-v63rqogn>OAuth スコープ（カンマ区切り）</label><div class="row" data-astro-cid-v63rqogn><input id="sa-scopes" type="text" readonly value="" style="flex:1;font-family:monospace;font-size:.78rem" data-astro-cid-v63rqogn><button class="btn" id="sa-copy-scopes" type="button" data-astro-cid-v63rqogn>コピー</button></div></div> <a class="btn btn-primary" href="https://admin.google.com/ac/owl/domainwidedelegation" target="_blank" rel="noopener" data-astro-cid-v63rqogn>ドメイン全体の委任の設定を開く ↗</a> <ol class="dwd-steps" data-astro-cid-v63rqogn> <li data-astro-cid-v63rqogn>開いた画面（「<strong data-astro-cid-v63rqogn>API クライアント</strong>」のドメイン全体の委任）で <strong data-astro-cid-v63rqogn>「新しく追加」</strong> をクリック</li> <li data-astro-cid-v63rqogn><strong data-astro-cid-v63rqogn>「クライアント ID」</strong> 欄に、上の<strong data-astro-cid-v63rqogn>クライアントID</strong>を貼り付け</li> <li data-astro-cid-v63rqogn><strong data-astro-cid-v63rqogn>「OAuth スコープ（カンマ区切り）」</strong> 欄に、上の<strong data-astro-cid-v63rqogn>スコープ</strong>を貼り付け</li> <li data-astro-cid-v63rqogn><strong data-astro-cid-v63rqogn>「承認」</strong>（または「Authorize」）をクリック</li> <li data-astro-cid-v63rqogn>一覧に追加されれば完了（反映まで1〜2分待つことがあります）</li> </ol> <div class="wstep-nav" data-astro-cid-v63rqogn><button class="btn btn-primary" type="button" data-next="4" data-astro-cid-v63rqogn>承認した → 次へ</button></div> </div> </section> <section class="wstep" data-step="4" data-astro-cid-v63rqogn> <header class="wstep-h" data-astro-cid-v63rqogn><span class="wstep-n" data-astro-cid-v63rqogn>4</span><span class="wstep-t" data-astro-cid-v63rqogn>連携を確認</span><span class="wstep-badge" data-astro-cid-v63rqogn></span></header> <div class="wstep-b" data-astro-cid-v63rqogn> <p class="muted" data-astro-cid-v63rqogn>承認が反映されたら、下のボタンで実際に接続できるかを確認します（手順3の承認直後は1〜2分かかることがあります）。</p> <button class="btn btn-primary" id="sa-test" data-astro-cid-v63rqogn>② 連携を確認</button> <div id="sa-test-result" style="margin-top:.5rem" data-astro-cid-v63rqogn></div> <div class="row" style="margin-top:1rem" data-astro-cid-v63rqogn><button class="btn btn-ghost" id="sa-disconnect" type="button"${addAttribute(!saInfo, "hidden")} data-astro-cid-v63rqogn>連携を解除</button><button class="btn btn-ghost" id="sa-restart" type="button" data-astro-cid-v63rqogn>最初から設定し直す</button></div> </div> </section> </div> </div> <div class="gpanel" data-panel="oauth" hidden data-astro-cid-v63rqogn> <ol class="steps" data-astro-cid-v63rqogn> <li data-astro-cid-v63rqogn> <h3 data-astro-cid-v63rqogn>1. プロジェクトを作成（または選択）</h3> <p class="muted" data-astro-cid-v63rqogn>Google Cloud に未作成ならプロジェクトを1つ作成します。</p> <a class="btn btn-primary" href="https://console.cloud.google.com/projectcreate" target="_blank" rel="noopener" data-astro-cid-v63rqogn>プロジェクト作成を開く ↗</a> </li> <li data-astro-cid-v63rqogn> <h3 data-astro-cid-v63rqogn>2. 必要な API を有効化</h3> <p class="muted" data-astro-cid-v63rqogn>使う機能の API だけで構いません（上のプロジェクトを選んでから各リンクで「有効にする」）。</p> <div class="row" data-astro-cid-v63rqogn> <a class="btn" href="https://console.cloud.google.com/apis/library/calendar-json.googleapis.com" target="_blank" rel="noopener" data-astro-cid-v63rqogn>Calendar API ↗</a> <a class="btn" href="https://console.cloud.google.com/apis/library/gmail.googleapis.com" target="_blank" rel="noopener" data-astro-cid-v63rqogn>Gmail API ↗</a> <a class="btn" href="https://console.cloud.google.com/apis/library/meet.googleapis.com" target="_blank" rel="noopener" data-astro-cid-v63rqogn>Meet API ↗</a> </div> </li> <li data-astro-cid-v63rqogn> <h3 data-astro-cid-v63rqogn>3. OAuth 同意画面を設定</h3> <p class="muted" data-astro-cid-v63rqogn>User Type は組織内利用なら「内部」、外部利用なら「外部」。アプリ名・サポートメールを入力し、下表のスコープを追加します。テスト段階は「テストユーザー」に連携する Google アカウントを追加してください。</p> <a class="btn btn-primary" href="https://console.cloud.google.com/apis/credentials/consent" target="_blank" rel="noopener" data-astro-cid-v63rqogn>同意画面を開く ↗</a> <div class="table-wrap" style="margin-top:.6rem" data-astro-cid-v63rqogn><table data-astro-cid-v63rqogn> <thead data-astro-cid-v63rqogn><tr data-astro-cid-v63rqogn><th data-astro-cid-v63rqogn>スコープ</th><th data-astro-cid-v63rqogn>用途</th><th data-astro-cid-v63rqogn>区分</th></tr></thead> <tbody data-astro-cid-v63rqogn> ${allScopes.map((s) => renderTemplate`<tr data-astro-cid-v63rqogn><td data-astro-cid-v63rqogn><code style="font-size:.78rem" data-astro-cid-v63rqogn>${s.scope}</code></td><td data-astro-cid-v63rqogn>${s.label}</td><td data-astro-cid-v63rqogn>${s.restricted ? renderTemplate`<span class="pill" style="background:#fee2e2;color:#b91c1c" aria-label="Restricted（Google審査対象）" data-astro-cid-v63rqogn>⚠ Restricted（審査対象）</span>` : "通常"}</td></tr>`)} </tbody> </table></div> <p class="muted" data-astro-cid-v63rqogn>※ Gmail の閲覧/送信（Restricted）を外部公開アプリで使う場合、Google の審査が必要になることがあります。まず Calendar/Meet だけで始めるのが簡単です。</p> </li> <li data-astro-cid-v63rqogn> <h3 data-astro-cid-v63rqogn>4. OAuth クライアント ID を作成</h3> <p class="muted" data-astro-cid-v63rqogn>「アプリケーションの種類＝<strong data-astro-cid-v63rqogn>ウェブ アプリケーション</strong>」を選び、<strong data-astro-cid-v63rqogn>承認済みのリダイレクト URI</strong> に下の値をそのまま貼り付けます。</p> <div class="field" data-astro-cid-v63rqogn> <label data-astro-cid-v63rqogn>承認済みのリダイレクト URI（コピーして貼り付け）</label> <div class="row" data-astro-cid-v63rqogn><input id="redir" type="text" readonly${addAttribute(redirectUri, "value")} style="flex:1;font-family:monospace;font-size:.82rem" data-astro-cid-v63rqogn><button class="btn" id="copyRedir" data-astro-cid-v63rqogn>コピー</button></div> </div> <a class="btn btn-primary" href="https://console.cloud.google.com/apis/credentials/oauthclient" target="_blank" rel="noopener" data-astro-cid-v63rqogn>クライアント作成を開く ↗</a> </li> <li data-astro-cid-v63rqogn> <h3 data-astro-cid-v63rqogn>5. 発行された ID / シークレットを登録</h3> <p class="muted" data-astro-cid-v63rqogn>作成後に表示される<strong data-astro-cid-v63rqogn>クライアントID</strong>と<strong data-astro-cid-v63rqogn>クライアントシークレット</strong>を貼り付けて保存します（暗号化保存・再デプロイ不要）。</p> <div class="card" data-astro-cid-v63rqogn> <div class="field" data-astro-cid-v63rqogn><label data-astro-cid-v63rqogn>クライアントID <span class="muted" data-astro-cid-v63rqogn>（${status.id ? "✓ 設定済み" : "未設定"}${bySecret.id ? "・Worker Secret" : ""}）</span></label><input id="gcid" name="google_client_id" type="text" placeholder="xxxxx.apps.googleusercontent.com" autocomplete="off"${addAttribute(bySecret.id, "disabled")} data-astro-cid-v63rqogn></div> <div class="field" data-astro-cid-v63rqogn><label data-astro-cid-v63rqogn>クライアントシークレット <span class="muted" data-astro-cid-v63rqogn>（${status.secret ? "✓ 設定済み" : "未設定"}${bySecret.secret ? "・Worker Secret" : ""}）</span></label><input id="gcsec" name="google_client_secret" type="password" placeholder="GOCSPX-..." autocomplete="off"${addAttribute(bySecret.secret, "disabled")} data-astro-cid-v63rqogn></div> ${(bySecret.id || bySecret.secret) && renderTemplate`<p class="muted" data-astro-cid-v63rqogn>Worker Secret で投入済みの項目は画面から変更できません（Secret が優先されます）。</p>`} <button class="btn btn-primary" id="saveGoogle"${addAttribute(allGoogleBySecret, "disabled")}${addAttribute(allGoogleBySecret ? "ID・シークレットは Worker Secret で設定済みのため、画面からは変更できません" : "", "title")} data-astro-cid-v63rqogn>保存</button> </div> </li> </ol> <p class="muted" data-astro-cid-v63rqogn>うまくいかない場合：連携時に <code data-astro-cid-v63rqogn>error=token</code> 等が出るときは、ステップ4のリダイレクト URI 不一致（末尾スラッシュ・http/https・ドメイン違い）が最有力です。</p> <details data-astro-cid-v63rqogn> <summary data-astro-cid-v63rqogn>技術者向け：ステップ1〜2を gcloud で自動化</summary> <p class="muted" data-astro-cid-v63rqogn>gcloud CLI が使える場合、プロジェクト作成と API 有効化はスクリプトで自動化できます（OAuth クライアント作成＝ステップ3〜5 は引き続き手動）。</p> <pre style="background:#0f172a;color:#e2e8f0;padding:.8rem;border-radius:8px;overflow:auto;font-size:.8rem" data-astro-cid-v63rqogn><code data-astro-cid-v63rqogn>gcloud auth login
scripts/google-oauth-setup.sh</code></pre> <p class="muted" data-astro-cid-v63rqogn>完了後、残りの手動ステップへのリンクと貼り付け用リダイレクト URI が出力されます。</p> </details> </div>` })}`} `, "scripts": async ($$result2) => renderTemplate(_a || (_a = __template([`<script>
