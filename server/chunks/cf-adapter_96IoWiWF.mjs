...
import { setRetentionDays, setMaxUploadMb, fileBelongsTo, getFile, saveFile, storageMode } from "./storage_B9y2EXgA.mjs";
import { m as makeDocument, w as webSearch, b as summarizeTranscript, c as extractInvoiceData, t as transcribeAudio, i as inferApp } from "./media-ai_DKqrKXrX.mjs";
import { googleFetch } from "./google_DKcryrKj.mjs";
import { getApiKey, entitlementForGate } from "./client_DpZoJ4KJ.mjs";
...
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp } from "./apps_BGyNvlpv.mjs";
import { v as validateDefinition, A as APP_SCHEMA, R as RENDER_HTML_MAX, o as opCatalogText } from "./appdef_Ciqz5dKq.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CGhAZnSU.mjs";
import { searchDirectory } from "./directory_DNaG9wzI.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_346zBxru.mjs";
import { logDiag } from "./diag_Cu0HTcAn.mjs";
import { i as isRunnableDefinition } from "./preflight_DwLTVcRC.mjs";
...
import { renderFormHtml, upsertPublicPage } from "./public-pages_nVpmJrxF.mjs";
import { g as getAppDesign, m as createDraft, n as repairOutputLiterals, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_DohtNeGn.mjs";
...
import "./index_Sjqf1K7w.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DNpe-luC.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_n-15ik-J.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_ByGt33xj.mjs";
import { overBudget, recordUsage, estimateUsd, recordTokens } from "./usage_DN6pGBs1.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_BiAr3eZE.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers } from "./users_DZrmZRzu.mjs";
import { listActions, createAction, deleteAction, updateAction } from "./a2a-actions_C_ZzUsYo.mjs";
import { p as pushToUser } from "./push__RKkFMHT.mjs";
...
async function rebuildPublicPage(ctx, slug) {
  const { getPublicPageAny } = await import("./public-pages_nVpmJrxF.mjs");
  const page = await getPublicPageAny(ctx.env, slug);
  if (!page) return { ok: false, error: "公開ページが見つかりません。" };
  const design = await getAppDesign(ctx, page.app_id);
  ...
  const fields = page.fields && page.fields.length ? page.fields : fieldsFromDefinition(def);
  const customHtml = def.render && typeof def.render.html === "string" ? String(def.render.html).trim() : "";
  const html = customHtml || renderFormHtml({ title: design.name || page.title, intro: design.spec ?? void 0, fields, allowFiles: page.allow_files, submitLabel: "送信する" });
  const r = await upsertPublicPage(ctx.env, {
    slug,
    appId: page.app_id,
    title: design.name || page.title,
    html,
    fields,
  ...
  return r.ok ? { ok: true } : { ok: false, error: r.error ?? "再生成に失敗しました。" };
}
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DpZoJ4KJ.mjs");
