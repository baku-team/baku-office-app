...
import { getSession } from "./auth_D3m2l5sl.mjs";
import { cachedEntitlement } from "./client_BXVeeS3h.mjs";
...
import { parseRequestModel } from "./settings_D2rjJD-A.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  let visionImage;
  if (b.image?.dataB64) {
    const { prepareAttachment } = await import("./chat-flow_XK4Ko55x.mjs");
...
`));
  ...
  (async () => {
    try {
      ...
      const { looksLikeBuildConfirmation } = await import("./cf-adapter_CFIfihMj.mjs").then((n) => n.D);
      const { canDevelopApps } = await import("./auth_D3m2l5sl.mjs");
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant) && atLeast(await cachedEntitlement(env), "pro")) {
        const { startAppBuild, processAppBuild } = await import("./cf-adapter_CFIfihMj.mjs").then((n) => n.C);
      ...
      if (b.mode !== "plan" && !visionImage) {
        const { tryHandleAppDelete } = await import("./chat-flow_XK4Ko55x.mjs");
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
        const { looksLikeAppEdit } = await import("./cf-adapter_CFIfihMj.mjs").then((n) => n.D);
        if (looksLikeAppEdit(message)) {
          const { latestSessionApp, startAppEdit, processAppBuild } = await import("./cf-adapter_CFIfihMj.mjs").then((n) => n.C);
      ...
      const reply = await ctx.agent.run({ owner: ses.uid, text: prompt, image: visionImage, role: ses.role, baseUrl: origin, history: toTurns(prior), model, modelId, onEvent, signal: request.signal, mode: b.mode });
      const { HOPS_EXCEEDED } = await import("./ai_CCPaNrnh.mjs");
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_DAE_3UzW.mjs");
        ...
        const { buildReplyActions } = await import("./chat-flow_XK4Ko55x.mjs");
      ...
      await (await import("./diag_Cu0HTcAn.mjs")).logDiag(env, "error", "chat", `stream失敗(model=${b.model ?? "auto"}): ${msg}`).catch(() => {
      });
      const { explainStop } = await import("./errors_Bw4GoHz5.mjs");
