...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Du2JHuMP.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_C8oeXWQv.mjs");
  await import("./index_C12A0H0f.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CdXzec5b.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_uMzW_E-X.mjs");
  ...
  const { reviewQueue } = await import("./users_CZPZCIhG.mjs");
  const { backupAlert } = await import("./backup_DbCD7Fbp.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DI2SUbWs.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_Cc5JIPVQ.mjs");
