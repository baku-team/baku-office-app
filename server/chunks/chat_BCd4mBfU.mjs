...
const $$Chat = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Chat;
  const { getSession } = await import("./auth_CzLHri08.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_BaLSnevk.mjs");
  ...
  const { listSkills } = await import("./skills_xHdUMve6.mjs");
