...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BmScp1M7.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_BTiBrVBU.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_lNlZmhzH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BtHypiSY.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_Dav2SgUe.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_D84zbozf.mjs");
