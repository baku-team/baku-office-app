...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CfcRI8wU.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_BV1rSndT.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_Q-RNyiqH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BzN5Ly8A.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_2eZp4hU_.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_DOMMn1Ws.mjs");
