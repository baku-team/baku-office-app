...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BvZ7tCLO.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C0NsU8Rq.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_BNaQQYSo.mjs");
  ...
  const { buildCtx } = await import("./ctx_BmScp1M7.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_BmScp1M7.mjs").then((n) => n.L);
