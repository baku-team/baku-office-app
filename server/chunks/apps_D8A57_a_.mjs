...
import { r as renderComponent } from "./worker-entry_BkXQJrCj.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_XFbJ31la.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CwvcOiF1.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CwvcOiF1.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_EVoG7B4q.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_DNkcBUdZ.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C9K-32fF.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_t7uZkVIe.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CIfdV394.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_DjYP7bZb.mjs");
