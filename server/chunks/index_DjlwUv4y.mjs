...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$Index;
  const { getSession } = await import("./auth_COnj03-x.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_D9LOZLIb.mjs");
  const { cmpVersion } = await import("./update_CiOaJgBw.mjs");
  await import("./index_Dwg_RGlI.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DXHaPAC7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BUInF2zJ.mjs");
  ...
  const { reviewQueue } = await import("./users_hcpqXQgI.mjs");
  const { backupAlert } = await import("./backup_Cr_QdPkb.mjs");
