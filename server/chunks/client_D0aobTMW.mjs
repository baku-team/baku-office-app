...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_CrtWLNXo.mjs").then((n) => n.X), import("./notifications_bYk0eYQ-.mjs")]);
    ...
    await (await import("./ctx_CrtWLNXo.mjs").then((n) => n.T)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_CrtWLNXo.mjs").then((n) => n.O)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_DmcrW5jV.mjs");
