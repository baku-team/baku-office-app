...
import { getApproval, decideApproval } from "./approvals_BZa_3z-V.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CDgvn5Qk.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_X9wJ7pe3.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Ctf-lfxw.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_RgvQ0ags.mjs";
import { joinWithInvite } from "./users_pLJUzq-G.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_D2XGvsLY.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_pLJUzq-G.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_CJgt8MaF.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_DRWzOBHp.mjs");
