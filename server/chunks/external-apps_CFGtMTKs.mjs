...
import { getToken, hostFetch, getVerifyJwk } from "./client_7WfEvhS0.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_7oh8SOHY.mjs";
import { sanitizeRenderHtml, validateDefinition } from "./appdef_CasaQNdK.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_CasaQNdK.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (!okApproved) {
          ...
          const { createApproval } = await import("./approvals_BphIQP1l.mjs");
  ...
  try {
    if (d.definition) {
      const { rebuildPublicPageFromDef } = await import("./public-pages_CgImd4LL.mjs");
...
async function deleteLocalApp(ctx, appId, deleteData = false) {
  ...
  await (await import("./public-pages_CgImd4LL.mjs")).deletePublicPagesByApp(ctx.env, appId).catch(() => {
  });
