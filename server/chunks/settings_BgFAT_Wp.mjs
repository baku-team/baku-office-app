...
import { isValidGeminiModel, isValidClaudeModel, isValidWorkersAiModel, DEFAULT_MODELS } from "./config_dJ2QM2XC.mjs";
...
function isMemberModel(v) {
  ...
  return v === "gemini" || v === "claude" || v === "local" || isValidGeminiModel(v) || isValidClaudeModel(v) || isValidWorkersAiModel(v);
}
...
function parseRequestModel(raw) {
  if (!raw) return {};
  if (raw === "gemini" || raw === "claude" || raw === "local") return { engine: raw };
