...
import { getApproval, decideApproval } from "./approvals_DrZ344zp.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_kImFTa8b.mjs";
import { t as transcribeAudio, d as runApprovedTool, p as parseApprovalAction, n as notifyOwnerDirect } from "./ctx_CZ0BwZwu.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_BtIyOUbU.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BjQkw6mR.mjs";
import { joinWithInvite } from "./users_Dfaoq2IJ.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_B7Xn3k8N.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_Dfaoq2IJ.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_Dkadey5u.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BgP3m0ei.mjs");
