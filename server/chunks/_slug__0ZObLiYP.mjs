...
import { b as buildPublicFullPage, a as buildFrameSrcdoc, P as PUBLIC_BRIDGE_SDK, c as FRAME_ALLOW, F as FRAME_SANDBOX } from "./app-frame_DrRzrUqG.mjs";
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (!page2 && Astro2.url.searchParams.get("preview") === "1") {
    const { getSession, canDevelopApps } = await import("./auth_DmR_wS5Q.mjs");
  ...
  if (!preview && Astro2.url.searchParams.get("preview") !== "1" && !Astro2.locals.publicHost) {
    const { getSiteHosts, getPublicHomeSlug } = await import("./custom-domain_BlEKJqw-.mjs");
    const hosts = await getSiteHosts(Astro2.locals.ctx);
    if (hosts && hosts.active) {
      const { publicUrlFor } = await import("./site-routing_BYRi73Ou.mjs");
  ...
  if (Astro2.locals.publicHost) {
    const { isReservedSlug } = await import("./site-routing_BYRi73Ou.mjs");
