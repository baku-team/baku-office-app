...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_DEBuc12A.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_mJzrtm6N.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DALGIijZ.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_DlBDHOBj.mjs");
