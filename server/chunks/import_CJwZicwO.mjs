...
const $$Import = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Import;
  const { getSession } = await import("./auth_B8OlNtyH.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const isAdmin = ses.role === "admin" && ses.ctx === "org";
  const { cachedEntitlement } = await import("./client_CTSTpEVI.mjs");
  const hasPlus = atLeast(await cachedEntitlement(env), "plus");
  const { listImported } = await import("./import_Bu70O-j9.mjs");
  const { fmtBytes } = await import("./storage-usage_Bxct6EX_.mjs");
