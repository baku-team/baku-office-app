...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BkMBHzOm.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_Clyp4VjA.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_DQtqtGe4.mjs");
  ...
  const { buildCtx } = await import("./ctx_X9wJ7pe3.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_X9wJ7pe3.mjs").then((n) => n.M);
