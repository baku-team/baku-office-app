...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_-pDHDD3Z.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_peeVfBbE.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DyMvLdWz.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_XmBumVGr.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_CHDLP6OC.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_BfcOtcrm.mjs");
