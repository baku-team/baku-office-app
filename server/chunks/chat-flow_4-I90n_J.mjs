...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CPYUMyFX.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_C48CCjoS.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_Cu1Zes9a.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_Cu1Zes9a.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DzHldYd8.mjs").then((n) => n.B);
