...
import { getSession } from "./auth_BhfKTuGW.mjs";
import { cachedEntitlement } from "./client_ZR_ELpXg.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  let visionImage;
  if (b.image?.dataB64) {
    const { prepareAttachment } = await import("./chat-flow_DRA50WId.mjs");
...
`));
  ...
  (async () => {
    try {
      ...
      const { looksLikeBuildConfirmation } = await import("./cf-adapter_DyhUJ_If.mjs").then((n) => n.D);
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant) && atLeast(await cachedEntitlement(env), "pro")) {
        const { startAppBuild, processAppBuild } = await import("./cf-adapter_DyhUJ_If.mjs").then((n) => n.C);
      ...
      if (b.mode !== "plan" && !visionImage) {
        const { tryHandleAppDelete } = await import("./chat-flow_DRA50WId.mjs");
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
        const { looksLikeAppEdit } = await import("./cf-adapter_DyhUJ_If.mjs").then((n) => n.D);
        if (looksLikeAppEdit(message)) {
          const { latestSessionApp, startAppEdit, processAppBuild } = await import("./cf-adapter_DyhUJ_If.mjs").then((n) => n.C);
      ...
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_Celf0rSv.mjs");
        ...
        const { buildReplyActions } = await import("./chat-flow_DRA50WId.mjs");
