...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_D_CrX1Mh.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_B_HP_Ex9.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_DId0R5ou.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_ByDxgDAw.mjs"),
    import("./scheduled-tasks_B-ToQUJa.mjs")
  ]);
  ...
  await import("./index_Bj0rITH8.mjs");
