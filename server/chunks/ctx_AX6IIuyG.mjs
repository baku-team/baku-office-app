...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_B-P5Zaq2.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_DEBuc12A.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Dowm9QUM.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_3Hp8b7YO.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DALGIijZ.mjs";
import "./index_mJzrtm6N.mjs";
...
import { googleFetch } from "./google_B3DwQ4ws.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_BWLv0u_y.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_Ctiq9-xA.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BCfWmupP.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_JWy-y-aT.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_hJdQx8dJ.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_vq2lbu4C.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_Tdcaw_FT.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_CnB0e96h.mjs";
import { storeCatalog } from "./store_DlBDHOBj.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CRcexuT4.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DPodwbLT.mjs";
import { saveLayoutDraft, listSites } from "./sites_ui_TKTNM.mjs";
...
import { p as pushToUser } from "./push_BPljrKR7.mjs";
...
function sectionShell(title, sectionFragments) {
  const bus = `<script>window.__app=(function(){var m={},last={};return{on:function(e,f){(m[e]=m[e]||[]).push(f);if(Object.prototype.hasOwnProperty.call(last,e)){try{f(last[e]);}catch(_){}}},emit:function(e,d){last[e]=d;(m[e]||[]).slice().forEach(function(f){try{f(d);}catch(_){}});}};})();window.addEventListener("load",function(){window.__app.emit("ready");});<\/script>`;
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_ui_TKTNM.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BbcnGh82.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BbcnGh82.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BbcnGh82.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_B-P5Zaq2.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DEBuc12A.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BbcnGh82.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_B-P5Zaq2.mjs")).storageMode(env) === "r2";
