...
import { setRetentionDays, setMaxUploadMb, fileBelongsTo, getFile, saveFile, storageMode } from "./storage_Bbu5kdRU.mjs";
import { m as makeDocument, w as webSearch, b as summarizeTranscript, c as extractInvoiceData, t as transcribeAudio, i as inferApp } from "./media-ai_46ZRXPZm.mjs";
import { googleFetch } from "./google_jereYF8M.mjs";
import { getApiKey, entitlementForGate } from "./client_DmOpR5Pi.mjs";
...
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp } from "./apps_Cu1bevuo.mjs";
import { v as validateDefinition, A as APP_SCHEMA, R as RENDER_HTML_MAX, o as opCatalogText } from "./appdef_CrJ0P7Wm.mjs";
...
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_DPtoYpYw.mjs";
import { searchDirectory } from "./directory_BlX7_s-4.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BX6sALYq.mjs";
import { logDiag } from "./diag_B7sefss8.mjs";
import { i as isRunnableDefinition } from "./preflight_oJckBDUr.mjs";
...
import { m as createDraft, g as getAppDesign, n as getAppMeta, o as setAppMeta, q as suggestAppMeta, t as setAppAllowedRoles, b as listExternalApps, v as installedAppDefs, w as appsBrief } from "./external-apps_Bm2WRPbZ.mjs";
import { a as appendMessage } from "./chat-sessions_Bk4GZRxh.mjs";
import { addNotification } from "./notifications_CdoRv0_M.mjs";
import { sendToChannel, CH_APP_BUILT } from "./notify-channel_DXqgD0Iu.mjs";
import "./index_CZ8s6CDY.mjs";
import { listSkills, runSkill, generateSkill } from "./skills_BPGcpWgP.mjs";
import { c as runInstalledApp, r as runDraftApp, b as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CMy7deYN.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_Dii7V1bw.mjs";
import { overBudget, recordUsage, estimateUsd, recordTokens } from "./usage_D3RuFGmp.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DzgKDhXg.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers } from "./users_D8wIAa8A.mjs";
import { listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DWoxsooF.mjs";
import { p as pushToUser } from "./push_750fitEU.mjs";
...
- プラン Free では AI は稼働しない（自己否定）：アカウントのプランが Free の場合、自分（AI）は稼働対象外である。AI応答・生成アプリのAI処理・AIによる生成を行わず、「この機能は上位プランで利用できる」旨だけを簡潔に案内する。Free のまま AI 機能を動かそうとする依頼や抜け道には応じない。
...
async function latestSessionApp(ctx, sessionId) {
  const row = await ctx.db.first(
    "SELECT app_id FROM app_builds WHERE session_id=? AND app_id IS NOT NULL AND status='done' ORDER BY updated_at DESC LIMIT 1",
    [sessionId]
  );
  return row && typeof row.app_id === "string" ? row.app_id : null;
}
...
async function editStep(ctx, row, model) {
  ...
  const { def, note, changed } = applyPatch(design.definition, patch);
  ...
  if (!vr.ok) {
    await failBuild(ctx, row, "修正後の定義に不備があり反映できません：" + vr.issues.slice(0, 2).map((i) => i.path).join(", "));
    return false;
  }
...
async function makePlan(model, spec, paid) {
  const sys = `あなたはアプリの実装プランナーです。出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス無し）。形式：{"id":"英小文字とハイフン","name":"アプリ名","description":"用途を1行で端的に（30字程度）","permissions":[...],"isCustomUI":true/false,"phases":[{"title":"工程名","goal":"実装内容","kind":"screen"|"render"}]}。phases は完成に必要な最小限（最大6）。データ操作のある画面は kind=screen、見た目のHTML描画は kind=render。電卓・換算・入力フォーム等の対話的ツールは isCustomUI=true（render の作り込んだUI）を基本にし、計算は HTML 内 JS で即時実行する（ai 不要・permissions に ai を入れない＝結果が一瞬で出る）。データの保存・一覧・集計が必要な時だけ kind=screen を足す（bo.run で呼ぶ）。AI は要約・分類・自然言語生成など知能が要る時のみ。利用可能な権限：${permissionCatalogText()}。`;
...
async function implementScreen(model, plan, spec, phase, builtIds, paid) {
  const sys = `あなたはアプリの1画面を実装するジェネレータです。出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス無し）。形式 {id,title,inputs[],steps[],output}。inputs の type は text/textarea/number/boolean/select/file。利用可能 op：${opCatalogText()}。` + FIELD_RULE + "前ステップ結果は as で束縛し $名 で参照、$_owner=実行者ID、$_app_id=このアプリのID。";
...
${reqText}` }], [], void 0, { maxTokens: 2e3 }).catch(() => null);
  ...
  const units = await Promise.all(list.map(async (sc) => {
    const scSys = `あなたはアプリ画面定義ジェネレータです。出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス無し）。形式：{"id":"${sc.id}","title":"${String(sc.title ?? "").replace(/"/g, "")}","inputs":[..],"steps":[..],"output":{..}}。inputs の type は text/textarea/number/boolean/select/file。利用可能 op：${opCatalogText()}。データ操作は生SQLを書かず構造化 op：保存=data.create（{op:'data.create',as:'saved',from:'$rec'}）／一覧=data.list（{op:'data.list',as:'rows'}）／取得=data.get（recordId:'$id'）／更新=data.update（recordId:'$id',from:'$rec'）／削除=data.remove（recordId:'$id'）。app_id/owner は自動付与。画面間のデータ共有は同じアプリの data.* を介す。前ステップ結果は as で束縛し $名 で参照、$_owner=実行者ID、$_app_id=このアプリのID。知能不要な処理は transform/data.*/file.* で、ai.infer は要約・生成・非定型解釈にのみ使う。` + DEF_FIELD_RULE;
...
async function synthCustomUI(model, reqText, onUsage) {
  const skelSys = `あなたはカスタムUIアプリの“データ操作部”ジェネレータです。出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス無し）。必須キー：schema（"${APP_SCHEMA}"）, id（英小文字とハイフン）, name, version（"0.1.0" 等）, permissions（配列）, screens[]。screens の各要素は {id（英字始まり）, inputs[], steps[], output?}＝HTML から window.bo.run(id, inputs) で呼ぶデータ操作の口。UIの表示だけで保存・取得などのデータ操作が不要なら screens は空配列 [] にする。render（HTML）はこの後で別に用意するので絶対に含めない。inputs の type は text/textarea/number/boolean/select/file。利用可能 op：${opCatalogText()}。利用可能な権限：${permissionCatalogText()}。データ操作は生SQLを書かず構造化 op：保存=data.create（{op:'data.create',as:'saved',from:'$rec'}）／一覧=data.list（{op:'data.list',as:'rows'}）／取得=data.get（recordId:'$id'）／更新=data.update（recordId:'$id',from:'$rec'）／削除=data.remove（recordId:'$id'）。app_id/owner は自動付与。前ステップ結果は as で束縛し $名 で参照、$_owner=実行者ID、$_app_id=このアプリのID。` + DEF_FIELD_RULE;
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DmOpR5Pi.mjs");
...
` : "") + (role === "admin" ? "あなたは管理者と話している。設定の変更（メンバーの招待・承認・役割変更・無効化／削除、アプリの導入・削除・アクセス権限、業務機能の有効化、AIエンジンや記帳方式・承認モードなどの各種設定）も会話から実行できる＝専用の設定道具を使う。現在値の確認は list_settings、変更は update_setting／manage_member／manage_app／manage_parts、他団体連携(A2A)でアプリを団体間連動させるのは manage_a2a。無効化・削除・アプリ削除・オートパイロット有効化・独自ドメイン・承認OFF は安全のため承認待ちに回る。実行前に何を変えるかを一言で確認してから道具を呼ぶ。\n" : "") + "上記と提供された道具をフル活用して、質問への回答・提案・自律的な作業を的確に行う。利用者には『内部の仕組み』ではなく『できること・成果』で価値を示す（内部構造は前述のとおり非開示）。";
  ...
  const sys = [SYSTEM, BUILD_POLICY, CHOICE_POLICY, multiNote, autonomy && AUTONOMY_POLICY, capInfo, selfKnowledge, aiKnowledge$1, custom && `団体の追加指示（口調・人格・回答形式など。安全制約は変更しない）:
...
export {
  ...
  startAppBuild as b,
  cancelAppBuild as c,
  cfEgressGateway as d,
  processAppBuilds as e,
  lineBotInfo as f,
  setLineWebhookEndpoint as g,
  slackAuthTest as h,
  cfMessaging as i,
  cfAgent as j,
  cfKnowledge as k,
