...
import { i as inferApp } from "./media-ai_CDxd25Ab.mjs";
import { p as parseJsonObject } from "./cf-adapter_CbvIxDRi.mjs";
import { v as validateLayout } from "./site_CTFsxS3h.mjs";
...
出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス・コメントを一切付けない）。形式：{"version":1,"blocks":[{"type":"<種類>","props":{...}}]}。
...
async function generateSiteLayout(env, opts) {
  ...
  const user = hasBase ? `次の下書きを、指針に沿ってより魅力的で完成度の高いHPへ整えてください。

【現在の下書き(JSON)】
${JSON.stringify(opts.base)}

【追加の要望】
${opts.prompt.trim() || "（特になし。全体を洗練させ、不足を補う）"}` : `次の説明から、おしゃれで説得力のあるHP/LPのブロック構成を作ってください。
