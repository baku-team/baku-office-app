...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_B-lkqngg.mjs");
  const { cmpVersion } = await import("./update_BXvdLhN2.mjs");
  await import("./index_DsS66e1p.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bx2onJKl.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DT725Tyd.mjs");
  ...
  const { reviewQueue } = await import("./users_DX4-aRfk.mjs");
  const { backupAlert } = await import("./backup_BT9mTern.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BJqjrgQW.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_BBucxay0.mjs");
