...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CZasnwOK.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CTBVRIxk.mjs");
  await import("./index_6IcBz0fb.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DJ7cZUGv.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_niaZDX1m.mjs");
  ...
  const { reviewQueue } = await import("./users_DUD3EVsR.mjs");
  const { backupAlert } = await import("./backup_CNfSZOPF.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BJqp2VVv.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_Czk76zwb.mjs");
