...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CSZaRoVj.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_Bf1r67RB.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DjDLCqan.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_yNmZnfox.mjs");
  const { registeredParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DGSF7vRP.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_Dqumkj6n.mjs");
