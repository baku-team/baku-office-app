...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BO4LdvYt.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CIL4AtJU.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_BCk4EGYy.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_De32HSX0.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_C1VtaqKi.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_hv6mY3Oi.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_CgZFOLfr.mjs");
