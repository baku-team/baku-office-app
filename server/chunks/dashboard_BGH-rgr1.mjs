...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BNXMhaMG.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BYsZK7_T.mjs");
  await import("./index_D018HHvS.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Bh0BoI_b.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Bbwycmod.mjs");
  ...
  const { reviewQueue } = await import("./users_CavXFK4T.mjs");
  const { backupAlert } = await import("./backup_BJOqWQh8.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CKPNCc45.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BcmxKHWi.mjs");
