...
import { l as loadDraft, c as checkDefinition, a as checkTransitions, b as checkSandbox, d as checkFabrication, e as checkRun, f as finalizePrecheck } from "./precheck_BkiRi5z4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_DszGd1gL.mjs").then((n) => n.B)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_ByGiCOpc.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
