...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BXVeeS3h.mjs");
  const { cmpVersion } = await import("./update_DfIaBO_d.mjs");
  await import("./index_CL76Ttcr.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_omiWNHKX.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DsLYDMIL.mjs");
  ...
  const { reviewQueue } = await import("./users_nVitkZLK.mjs");
  const { backupAlert } = await import("./backup_aWt9p5CI.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CSNxZelj.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CpD6nnHA.mjs");
