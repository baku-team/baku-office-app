...
import { r as renderComponent } from "./worker-entry_D-n8S5gW.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_k24DtfnL.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_5TQEU5l1.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_Bf1r67RB.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BcGgaiPh.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_iW6a4cRN.mjs"),
    import("./scheduled-tasks_B8xl56ZF.mjs")
  ]);
  ...
  await import("./index_DjDLCqan.mjs");
