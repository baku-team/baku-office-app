...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_C0xk2SHi.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BKYtLCd6.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_5KkK0xTP.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CAsNPlru.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DMCgfXnA.mjs";
import "./index_B4Tk4Zwd.mjs";
...
import { googleFetch } from "./google_CX4OI5qq.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, opCatalogText, googleOpCatalogText } from "./appdef_CFwTTtdH.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_7oVkgfdK.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_llIVk3pP.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_Djx5eoR2.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_Cie779Te.mjs";
import { logDiag, logBuild } from "./diag_CsI0yNfw.mjs";
import { i as isRunnableDefinition } from "./preflight_D_-sbB7j.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_CqMtin6Q.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_D3h2vx12.mjs";
import { storeCatalog } from "./store_msu7Jdjm.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DvWZteUY.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_CpyhgkQw.mjs";
import { saveLayoutDraft, listSites } from "./sites_DlUd7hWn.mjs";
...
import { p as pushToUser } from "./push_BUMsvTlA.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DlUd7hWn.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_D1BhkDaA.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_D1BhkDaA.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_D1BhkDaA.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_C0xk2SHi.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BKYtLCd6.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_D1BhkDaA.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_C0xk2SHi.mjs")).storageMode(env) === "r2";
