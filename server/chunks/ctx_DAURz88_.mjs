...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_Cc5JIPVQ.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Du2JHuMP.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_Bo6AdxJp.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CZPZCIhG.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CdXzec5b.mjs";
import "./index_C12A0H0f.mjs";
...
import { googleFetch } from "./google_OENwjbIy.mjs";
...
import { validateDefinition, APP_SCHEMA, RENDER_HTML_MAX, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, opCatalogText } from "./appdef_BHERKvDH.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dn536Rzr.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_B5UDwcrZ.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_COWToytw.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_DrbxvjIl.mjs";
import { logDiag } from "./diag_ByGiCOpc.mjs";
import { i as isRunnableDefinition } from "./preflight_D3jaJ790.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BW1Veluf.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_CZ1hRghc.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_CJuuAfa1.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DzDhNcts.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_CuYBSsAU.mjs";
import { saveLayoutDraft, listSites } from "./sites_6YZgoxTL.mjs";
...
import { storeCatalog } from "./store_C1MoMs01.mjs";
import { setCustomDomain } from "./custom-domain_BlEKJqw-.mjs";
import { p as pushToUser } from "./push_Bh8S4bo8.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_6YZgoxTL.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_Cna_en9q.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_Cna_en9q.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_Cna_en9q.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Du2JHuMP.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_Cna_en9q.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_Cc5JIPVQ.mjs")).storageMode(env) === "r2";
