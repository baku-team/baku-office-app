...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_Du2JHuMP.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DrbxvjIl.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Cc5JIPVQ.mjs");
