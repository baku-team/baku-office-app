...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DRb3V3LZ.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_sRscislI.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BJcJsswH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DEzXoTbR.mjs");
  const { registeredParts } = await import("./parts_BnninK9G.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DAQnWVVG.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_DGfBcH1H.mjs");
