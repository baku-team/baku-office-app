...
import { A as APP_SCHEMA, v as validateDefinition } from "./appdef_hDA8H9NL.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_BfOqoIdF.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_Cu0HTcAn.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
