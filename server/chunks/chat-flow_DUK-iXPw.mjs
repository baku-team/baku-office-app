...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CSR2WvVx.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_B8CX3HBm.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_ODKWvbUB.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_ODKWvbUB.mjs").then((n) => n.Q);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.z);
