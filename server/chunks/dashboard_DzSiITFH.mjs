...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DIsDv6Hl.mjs");
  const { cmpVersion } = await import("./update_C9USRGH0.mjs");
  await import("./index_6IGpA5sE.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CgPAAqz7.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_bVVPWNdb.mjs");
  ...
  const { reviewQueue } = await import("./users_J5oAir2y.mjs");
  const { backupAlert } = await import("./backup_D3fZa9hR.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CE1vvWJb.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_B6I9yBIu.mjs");
