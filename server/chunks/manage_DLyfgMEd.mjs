...
const POST = async ({ request, locals }) => {
  ...
  if (b.action === "rebuild") {
    ...
    const { rebuildPublicPage } = await import("./ctx_CAci7FIw.mjs").then((n) => n.U);
  ...
  if (b.action === "toggle") {
    ...
    if (enable) {
      ...
      if (page2) {
        const { gatePublishApp } = await import("./self-check_D6u4HSSY.mjs");
        const { appendMessage } = await import("./chat-sessions_4ZEuV35m.mjs").then((n) => n.i);
