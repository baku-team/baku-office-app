...
import { getFile } from "./storage_B-P5Zaq2.mjs";
import { getDriveSave } from "./drive_CoI0Yc4D.mjs";
import { n as notifyOwnerDirect } from "./ctx_AX6IIuyG.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.V);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_B-P5Zaq2.mjs");
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.Q);
