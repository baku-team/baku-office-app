...
const $$Domain = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const pubPages = cfg ? await (await import("./public-pages_Bg1BWixA.mjs")).listPublicPages(env) : [];
  const defPublic = cfg?.publicHost || cfg?.domain || "";
  const defAdmin = cfg?.adminHost || (cfg?.domain ? "app." + cfg.domain : "");
  const cfTokenReady = cfg ? await (await import("./autonomy_koKo0H-8.mjs")).getAutonomyConfig(env).then((a) => a.cfToken).catch(() => false) : false;
