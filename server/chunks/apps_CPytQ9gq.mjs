...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_O88cMZw-.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_O88cMZw-.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CnCLUt7x.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BQbdmWVM.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DB87vrf0.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CH6qhuCH.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_D-px-Ku8.mjs").then((n) => n.A);
  const { storeCatalog, myApps } = await import("./store_CfJe66nF.mjs");
