...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DlpdsadF.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_D99W7elw.mjs");
  await import("./index_B2V0XL0x.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CqmHVZPm.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DOqGG1Kj.mjs");
  ...
  const { reviewQueue } = await import("./users_hcWwRf5D.mjs");
  const { backupAlert } = await import("./backup_nqZGzUoF.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BTgdbc5l.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_iUFCTMib.mjs");
