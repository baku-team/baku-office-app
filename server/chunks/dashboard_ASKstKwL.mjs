...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BOITous_.mjs");
  const { cmpVersion } = await import("./update_CC7LsJ6W.mjs");
  await import("./index_4iE78nsz.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Du6RRp8T.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cw0PMSjQ.mjs");
  ...
  const { reviewQueue } = await import("./users_QFiAkptP.mjs");
  const { backupAlert } = await import("./backup_IP7N6KOl.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_89S1Hixh.mjs").then((n) => n.y);
  ...
  const { storageMode } = await import("./storage_BW2B_Uds.mjs");
