...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CqhnQISs.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_CHfKjn8b.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_6AnNEDFc.mjs");
  ...
  const { buildCtx } = await import("./ctx_Dy72pGSg.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_Dy72pGSg.mjs").then((n) => n.M);
