...
import { o as ownedSession, c as createSession, g as getMessages, a as appendMessage, e as ensureTitle, t as toTurns } from "./chat-sessions_4ZEuV35m.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (inboundFiles.length) {
    const { prepareAttachment } = await import("./chat-flow_DfS-lMTW.mjs");
...
`));
  ...
  (async () => {
    try {
      ...
      const { tryPreAgentRouting, tryProHopsContinuation, finalizeAssistantReply } = await import("./chat-flow_DfS-lMTW.mjs");
      ...
      const reply = await ctx.agent.run({ owner: ses.uid, text: prompt, attachments: visionAttachments, role: ses.role, baseUrl: origin, history: toTurns(prior), model, modelId, sessionId, onEvent, signal: request.signal, mode: b.mode });
      ...
      await (await import("./diag_NtHEh0X8.mjs")).logDiag(env, "error", "chat", `stream失敗(model=${b.model ?? "auto"}): ${msg}`).catch(() => {
      });
