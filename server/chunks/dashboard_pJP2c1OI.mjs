...
import { r as renderComponent } from "./worker-entry_-xdkkFpg.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_mWhJRXe1.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CiMMQ0Dd.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CtcYt0T4.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_C0lZZli1.mjs");
  await import("./index_BFi-aKBP.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CzzimBfS.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_B329IR1J.mjs");
  ...
  const { reviewQueue } = await import("./users_CvA1-zYY.mjs");
  const { backupAlert } = await import("./backup_DXzKwEAF.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_h39FFap9.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_D2DJGMd8.mjs");
