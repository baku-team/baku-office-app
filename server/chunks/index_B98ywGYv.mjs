...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.locals.publicHost) {
    const { getPublicHomeSlug } = await import("./custom-domain_BlEKJqw-.mjs");
    const { getPublicPage } = await import("./public-pages_DHQdIiIX.mjs");
    const { buildPublicFullPage } = await import("./app-frame_DrRzrUqG.mjs").then((n) => n.d);
  ...
  const { getSession } = await import("./auth_DmR_wS5Q.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CkzMAmmV.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_GcjRF9RO.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_Cgcyhwit.mjs"),
    import("./scheduled-tasks_ZGMfkpHW.mjs")
  ]);
  ...
  await import("./index_DtjctWvj.mjs");
