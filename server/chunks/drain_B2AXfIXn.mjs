...
import { getApiKey } from "./client_Dd9FH8gm.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, p as processAppBuild, E as processSiteBuilds } from "./ctx_BWJTdi0a.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_B5cgUMAK.mjs";
import { processAgentJobs } from "./agent-jobs_CebeQ0aB.mjs";
import { reclaimStaleApprovals } from "./approvals_wlxweYCT.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_C61t2yz_.mjs";
import { guardHeavy } from "./diag_CY3PJnOM.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_BICd75pv.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_czQKQiqR.mjs";
import { flushReports } from "./reports_D_ujh2fg.mjs";
import { addNotification, pushWebhook } from "./notifications_CIyssDNw.mjs";
import { getNotifyWebhook } from "./settings_dTyIbTCR.mjs";
import { purgeFiles } from "./storage_dk4WezxQ.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_sj9-l518.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_S3AXnUOw.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_D30T9yfL.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_BQQFt2Ai.mjs").then((m) => m.flushUsageDigest(env)));
  ...
  return json({ ok: true, sent, notified, summarized, scheduled, video, agentJobs, appBuilds, appBuildsMore, activeBuildIds: [...activeIds], siteBuilds, siteBuildsMore, uploadBatches, autoUpdate, approvalsReclaimed, driveBackup, fullBackup, reportsSent, weeklyReport, usageDigest, purged });
};
