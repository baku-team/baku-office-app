...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CMcttt0q.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CMcttt0q.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_D1FmXRm9.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_qGL1F133.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BZgMzI_G.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DlKVNdA9.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BTUHKH0-.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_Dl3HN3kw.mjs");
