...
import { n as nowSec, e as ensureSeed, c as currentPeriod, b as listWallets, f as findOrCreateCategory, a as createTx, s as softDeleteTx } from "./accounting_CtvChRxg.mjs";
import { createMember } from "./membership_CHJIZtTa.mjs";
import { registerLocalUser } from "./users_B8aVK5B7.mjs";
...
async function saveEvent(env, a) {
  ...
  if (a.id) {
    await env.DB.prepare(
      "UPDATE events SET slug=?,title=?,lead=?,body=?,location=?,event_date=?,capacity=?,plans=?,published=?,updated_at=? WHERE id=?"
    ).bind(slug, a.title.trim(), a.lead ?? null, a.body ?? null, a.location ?? null, a.event_date ?? null, cap, plansJson, a.published ? 1 : 0, now, a.id).run();
    return { ok: true, id: a.id };
  }
  const id = randomId();
  await env.DB.prepare(
    "INSERT INTO events (id,slug,title,lead,body,location,event_date,capacity,plans,published,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
  ).bind(id, slug, a.title.trim(), a.lead ?? null, a.body ?? null, a.location ?? null, a.event_date ?? null, cap, plansJson, a.published ? 1 : 0, now, now).run();
  return { ok: true, id };
}
...
async function registerForEvent(env, a) {
  ...
  const acct = await registerLocalUser(env, { name, loginId: a.loginId, password: a.password, role: "member", status: "active" });
...
const CRAFT_LP_DEFAULT = {
  ...
  body: [
    '<p class="lead">醸造のように、キャリアもゆっくり発酵する。一杯のクラフトビールを起点に、畑違いの人と本音で未来を語り合う——そんな夜をつくっています。</p>',
    "<h2>🍺 醸造に学ぶ</h2><p>発酵は「待つ技術」。焦らず、混ぜ合わせ、時を読む。キャリアの育て方と驚くほど似ています。</p>",
    "<h2>🤝 越境してつながる</h2><p>醸造家・人事・エンジニア・経営者。畑違いの人と一杯交わすほど、視界が開けます。</p>",
    "<h2>🧭 次の一歩を持ち帰る</h2><p>ただ飲むだけでは終わらせない。対話のなかで、明日の小さな行動を見つけて帰る場です。</p>"
  ].join("\n")
};
async function seedDemoEvent(env) {
  const { getSite, upsertSite } = await import("./sites_xKNJha3X.mjs");
