...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_D-gPzIuz.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_CQSsPpWQ.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BNv26dye.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_C_lv8bXm.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_RUDRQx3S.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_C33XoSbp.mjs");
