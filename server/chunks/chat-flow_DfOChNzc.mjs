...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_UGqxcZD2.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_DOVO9a8c.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_D1XapsiS.mjs").then((n) => n.Q);
  ...
  const { latestSessionApp } = await import("./ctx_D1XapsiS.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_BIgCZPwg.mjs").then((n) => n.z);
