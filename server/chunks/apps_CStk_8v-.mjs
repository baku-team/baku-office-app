...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_Cpc8zRmY.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_XKaks_hD.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BsLdtSuX.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Dc5V0d7w.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BZsna514.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_C32Y8upl.mjs");
