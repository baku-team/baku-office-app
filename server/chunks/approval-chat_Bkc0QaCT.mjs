...
import { getApproval, decideApproval } from "./approvals_DqjH7are.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_Cki9oGIJ.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_BmScp1M7.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_Clfy7jU-.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BTiBrVBU.mjs";
import { joinWithInvite } from "./users_BGx-Ag0G.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_CXuQdqf4.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_BGx-Ag0G.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_DVPDu8yJ.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BS821qA_.mjs");
