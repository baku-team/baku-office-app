...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_mfWxKLIy.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DWnRrE-m.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_BjKlgqy9.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_NXtqd6gx.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_X5x_Eyo3.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_xzpbaRmZ.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_CZzW_Q4A.mjs");
