...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CqwoO7zq.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_clq7fc2y.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_D0PzYluR.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_D0PzYluR.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_BLW-izfW.mjs").then((n) => n.B);
