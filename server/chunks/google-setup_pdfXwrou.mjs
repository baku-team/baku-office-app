...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CSArKuHH.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_JUEoXe-2.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_im6RZ537.mjs");
  ...
  const { buildCtx } = await import("./ctx_CAci7FIw.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_CAci7FIw.mjs").then((n) => n.L);
