...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_yx4APBBm.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BR-wxBFT.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_eLhCSUGc.mjs");
