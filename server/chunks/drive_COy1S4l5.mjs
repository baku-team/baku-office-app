...
import { getApiKey, saveApiKey } from "./client_BAAgke-T.mjs";
import { listFiles, getFile } from "./storage_CLCFzk9N.mjs";
import { n as nowSec } from "./accounting_CelDuPQg.mjs";
import { serviceAccountConfigured, serviceAccountAccessToken } from "./google-sa_xG-HCT_c.mjs";
...
async function driveAvailable(env) {
  return await serviceAccountConfigured(env) || await driveConnected(env);
}
...
async function driveStatus(env) {
  ...
  const oauth = await driveConnected(env);
