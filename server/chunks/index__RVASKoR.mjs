...
import { r as renderComponent } from "./worker-entry_Cq91UoJg.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Q8WrOfZ2.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_BDZXx1aq.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_peeVfBbE.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_t2W-o4hP.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DpQQvlnT.mjs"),
    import("./scheduled-tasks_DaP_ABtd.mjs")
  ]);
  ...
  await import("./index_DyMvLdWz.mjs");
