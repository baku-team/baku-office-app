...
import { kvPut } from "./kv_H5bDbyIA.mjs";
import { EgressGateway, DEFAULT_EGRESS_ALLOWLIST } from "./egress_5_OuCaR0.mjs";
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_DWaRDBDI.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_C_RhHGpz.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CYoNJfzE.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BSfpZ42G.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_DQ67RXU1.mjs";
import "./index_DQSrHVYT.mjs";
import { recordUsage, recordTokens, overBudget, recordCallLog, estimateUsd } from "./usage_Bcla-e0G.mjs";
...
import { getAiEngine, getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, setPaidSwitchOk, resolveModelSelection, getMemberModel, getSmartRouting, getWorkersAiModel, maxParallelAgents, agentMaxHops, getPaidSwitchOk } from "./settings_DHOyCLdr.mjs";
import { googleFetch } from "./google_CJNQCNCS.mjs";
import { t as toolsOf, a as scopeCtx, p as partCatalog, e as enabledPartIds, s as setEnabledPartIds, c as enabledParts, d as partOfTool } from "./parts_CVGBl5kS.mjs";
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, opCatalogText, googleOpCatalogText } from "./appdef_BEoNJ34p.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_mug4D6f4.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_Dk9FiQdf.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_jNwgifi1.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_DNS2-UZq.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, f as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_BzH8SuQy.mjs";
import { logDiag } from "./diag_DWKsGqE6.mjs";
import { i as isRunnableDefinition } from "./preflight_CM72wj6v.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BHeKCxgA.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_BSxu6ge_.mjs";
import { storeCatalog } from "./store_BYZkU0fI.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_dOBDgNOm.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_1nREIbT_.mjs";
import { saveLayoutDraft, listSites } from "./sites_C9_qjgRP.mjs";
...
import { p as pushToUser } from "./push_BnXsMAga.mjs";
...
async function implementScreen(model, plan, spec, phase, builtIds, paid) {
  const sys = `あなたはアプリの1画面を実装するジェネレータです。出力は JSON オブジェクト1個のみ（前置き・説明・コードフェンス無し）。形式 {id,title,inputs[],steps[],output}。inputs の type は text(短答)/textarea(段落)/number(数値)/boolean(はい・いいえ)/select(プルダウン)/radio(単一選択)/checkboxes(複数選択)/scale(評価スケール)/date(日付)/time(時刻)/email(メール)/tel(電話)/file(ファイル)/signature(署名)。select/radio/checkboxes/scale は options（選択肢の配列）必須。利用可能 op：${opCatalogText()}。${googleOpCatalogText()}。` + FIELD_RULE + "前ステップ結果は as で束縛し $名 で参照、$_owner=実行者ID、$_app_id=このアプリのID。";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_C9_qjgRP.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_BeUqSri_.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_BeUqSri_.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_BeUqSri_.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_C_RhHGpz.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_BeUqSri_.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_DWaRDBDI.mjs")).storageMode(env) === "r2";
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。"));
    ...
    try {
      ...
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_DHOyCLdr.mjs");
