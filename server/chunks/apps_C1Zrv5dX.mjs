...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DZmKLyOO.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_BJ88O0X2.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DM39iHq9.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_B1MvUC7S.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_C3f4hWKe.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_DsLKTr3i.mjs");
