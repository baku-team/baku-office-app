...
import { r as renderComponent } from "./worker-entry_CMwbooUk.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_FherttgJ.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DV5hnO_2.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DV5hnO_2.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_DZb-oM1L.mjs").then((n) => n.y);
  const entitlement = await (await import("./client_DmOpR5Pi.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CZ8s6CDY.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_Cu1bevuo.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Bm2WRPbZ.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_D-MOicdc.mjs");
