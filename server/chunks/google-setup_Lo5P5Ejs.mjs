...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_CshPVe2i.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C0GTqIkc.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_DJsdgGT7.mjs");
  ...
  const { buildCtx } = await import("./ctx_BRlB-70J.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_BRlB-70J.mjs").then((n) => n.M);
