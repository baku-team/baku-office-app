...
import { getApproval, decideApproval } from "./approvals_BQ3wJX86.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_CNSCg6Zs.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_CAci7FIw.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CaMCQb2i.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_DcvmK-U8.mjs";
import { joinWithInvite } from "./users_plxkvjND.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_CtLBMcNN.mjs";
import { getDriveSave } from "./drive_im6RZ537.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_plxkvjND.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_B7y4crEU.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_lg6R25PH.mjs");
