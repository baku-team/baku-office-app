...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CuoW304l.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_D-ttyOpV.mjs");
  await import("./index_DSEOIWls.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CtVp9LjS.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DPXTwEic.mjs");
  ...
  const { reviewQueue } = await import("./users_DhxWhP3M.mjs");
  const { backupAlert } = await import("./backup_Co22Tg80.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_C9jYx4MB.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_fwjkKF34.mjs");
