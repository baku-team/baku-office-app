...
import { getApiKey } from "./client_BV1rSndT.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_CfcRI8wU.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_BwGLztfK.mjs";
import { processAgentJobs } from "./agent-jobs_C3lEsTfq.mjs";
import { reclaimStaleApprovals } from "./approvals_W-wuF5ql.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_Du-A4izr.mjs";
import { guardHeavy } from "./diag_DWCSrGCf.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_QOdhKyAo.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_DfaJB7As.mjs";
import { flushReports } from "./reports_D3J_jazP.mjs";
import { addNotification, pushWebhook } from "./notifications_qIVK1Fm-.mjs";
import { getNotifyWebhook } from "./settings_BKzORwjT.mjs";
import { purgeFiles } from "./storage_CT4gbX4d.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_BNto5Wwv.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_DZgvkCbI.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
