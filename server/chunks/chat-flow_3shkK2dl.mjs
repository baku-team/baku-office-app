...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CM436dbQ.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_DJA_I8xR.mjs";
import { canDevelopApps } from "./auth_CG1mOonk.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DCqlLMUd.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_DCqlLMUd.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DszGd1gL.mjs").then((n) => n.B);
