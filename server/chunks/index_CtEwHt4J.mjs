...
import { r as renderComponent } from "./worker-entry_DoxxdHpx.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CPgDF8_j.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DlJYu5pd.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_Bib4yTGq.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BbRloX4B.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DZjSSTuf.mjs"),
    import("./scheduled-tasks_BBUcnwuC.mjs")
  ]);
  ...
  await import("./index_BFUyWbe8.mjs");
