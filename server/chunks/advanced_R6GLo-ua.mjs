...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_B_HP_Ex9.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_CZWqctVS.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BgjO-NRr.mjs");
