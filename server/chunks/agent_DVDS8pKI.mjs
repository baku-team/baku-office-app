...
import { entitlementForGate, getApiKey } from "./client_BaLSnevk.mjs";
...
import { getAiEngine, getCustomPrompt, resolveModelSelection, getMemberModel, getWorkersAiModel, maxParallelAgents, agentMaxHops } from "./settings_BmjoORVR.mjs";
import { callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BKcsLqrV.mjs";
import { searchDirectory } from "./directory_D7Bl4LmL.mjs";
import { autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BWOJJ7c9.mjs";
import { DEFAULT_MODELS, isValidWorkersAiModel, workersAiModelId, claudeModelId, geminiModelId } from "./config_BOdbJfMp.mjs";
import "./index_BhxwzBes.mjs";
import { m as makeDocument, w as webSearch } from "./media-ai_BNSopcp2.mjs";
import { runSkill, generateSkill, listSkills } from "./skills_xHdUMve6.mjs";
import { createDraft } from "./external-apps_DeVDLtJQ.mjs";
import { videoStatusText, invokeCapability, listCapabilities, capabilitySummary } from "./capabilities_BQac8816.mjs";
import { overBudget, recordUsage, recordTokens, estimateUsd } from "./usage__IUbXlIy.mjs";
import { getApprovalMode, previewFor, createApproval, A2A_OUTWARD } from "./approvals_CLIMKLyN.mjs";
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BaLSnevk.mjs");
...
${custom}`].filter(Boolean).join("\n");
  ...
  let want = opts.model;
  let reqWa = opts.waModel && isValidWorkersAiModel(opts.waModel) ? opts.waModel : void 0;
  if (!want) {
    const sel = resolveModelSelection(await getMemberModel(env, owner).catch(() => null));
    if (sel.engine === "gemini") want = void 0;
    else {
      want = sel.engine;
      if (sel.waModel) reqWa = reqWa ?? sel.waModel;
    }
  }
  let model = null;
  let provider = "gemini";
  const wantLocal = want === "local" || !geminiKey && !claudeKey;
  const waModel = reqWa ?? (env.AI ? await getWorkersAiModel(env) : workersAiModelId(env));
  if (wantLocal && env.AI) {
