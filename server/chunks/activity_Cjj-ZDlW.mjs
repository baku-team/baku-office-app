...
import { cancelAgentJob } from "./agent-jobs_b6uEPOAz.mjs";
import { c as cancelAppBuild } from "./cf-adapter_EVoG7B4q.mjs";
...
const GET = async ({ request, locals }) => {
  ...
  const tasks = [];
  try {
    const a = (await env.DB.prepare("SELECT id,owner,prompt,status FROM agent_jobs WHERE status IN ('pending','running') ORDER BY created_at DESC LIMIT 8").all()).results;
    for (const j of a) {
      const mine = j.owner === ses.uid;
      tasks.push({ kind: "agent", status: j.status, label: mine ? trunc(j.prompt) : "AIエージェント", mine, ...mine ? { id: j.id } : {} });
    }
  } catch {
  }
  try {
  ...
  try {
    const ab = (await env.DB.prepare("SELECT id,owner,name,status,plan,cursor,created_at,kind,app_id FROM app_builds WHERE status IN ('planning','building') ORDER BY created_at DESC LIMIT 8").all()).results;
    for (const j of ab) {
      const mine = j.owner === ses.uid;
      ...
      tasks.push({ kind: isEdit ? "edit" : "build", status: j.status, label, mine, phase: isEdit ? 0 : phase, total: isEdit ? 0 : total, elapsedSec: Math.max(0, nowSec() - j.created_at), ...mine ? { id: j.id } : {} });
    }
  } catch {
  }
