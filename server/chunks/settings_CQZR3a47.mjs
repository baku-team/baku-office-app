...
import { getSession } from "./auth_DED0iuNZ.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_B-P5Zaq2.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_JWy-y-aT.mjs";
import { setStorageLimits } from "./storage-usage_CeHcqvfB.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_DALGIijZ.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, h as purgeStaleDrafts, i as installLocalApp, a as activeAppDefinition, j as appVersions, k as activateAppVersion, m as deleteLocalApp } from "./external-apps_2vXyEMVW.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const { canDevelopApps } = await import("./auth_DED0iuNZ.mjs");
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DPodwbLT.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_Dowm9QUM.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_AX6IIuyG.mjs").then((n) => n.N);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_AX6IIuyG.mjs").then((n2) => n2.T);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_f1Kfcpw-.mjs");
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.D);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.C);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.D);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.D);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_2vXyEMVW.mjs").then((n) => n.C);
