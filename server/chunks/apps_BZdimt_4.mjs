...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_CS3Dw_M5.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_CS3Dw_M5.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_lLyugAHU.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_CtDgLyHf.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CIVgZ-l3.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CgDeq9BV.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CXYObSRd.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_yAvbHxmG.mjs");
