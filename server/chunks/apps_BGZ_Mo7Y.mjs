...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_CjorSwtb.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_D6q0G9CB.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_dimZBE3u.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_mSn5WQnS.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_C5DRyoDI.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_DS8tDlHa.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BYz_sEoU.mjs");
