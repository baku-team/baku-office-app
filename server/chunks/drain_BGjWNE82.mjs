...
import { getApiKey } from "./client_DVo9lSTr.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_B58Crkjn.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_CFNy6idE.mjs";
import { processAgentJobs } from "./agent-jobs_oqj7QHyz.mjs";
import { reclaimStaleApprovals } from "./approvals_D0IPsrw0.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_9RRCMur5.mjs";
import { guardHeavy } from "./diag_DWCSrGCf.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_D8cguug1.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_Cj_NX3HB.mjs";
import { flushReports } from "./reports_rw1PjLJB.mjs";
import { addNotification, pushWebhook } from "./notifications_hO_rNBCZ.mjs";
import { getNotifyWebhook } from "./settings_BKzORwjT.mjs";
import { purgeFiles } from "./storage_AVD7qBz2.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_CS_jgIMO.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_CZFj0yh_.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
