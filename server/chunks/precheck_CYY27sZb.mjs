...
import { l as loadDraft, c as checkDefinition, a as checkTransitions, b as checkSandbox, d as checkFabrication, e as checkRun, f as finalizePrecheck } from "./precheck_DNNW7xYN.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    await (await import("./external-apps_VyEr6Vzt.mjs").then((n) => n.D)).recheckDraft(ctx, draftId);
  } catch {
  }
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_-P_u6Fn4.mjs")).logDiag(env, "error", "precheck", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
