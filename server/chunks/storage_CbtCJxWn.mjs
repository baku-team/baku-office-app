...
async function saveChatAttachment(env, img, createdBy, ctx) {
  const mime = (img.mimeType ?? "").toLowerCase();
  if (!mime || !(mime.startsWith("image/") || mime === "application/pdf")) {
    return { ok: false, status: 400, error: "添付は画像または PDF のみ対応しています。" };
  }
  let view;
  try {
  ...
  const file = new File([view], `upload-${nowSec()}.${ext}`, { type: mime });
  try {
