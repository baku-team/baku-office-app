...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BTLpH8fW.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DKs0ZAVL.mjs");
  ...
  const { hasApiKey } = await import("./client_BTLpH8fW.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_BocuNd5z.mjs");
