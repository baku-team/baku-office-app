...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_PSBt_dAs.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BGjNNron.mjs");
  await import("./index_D1fW0wtH.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BJv6y5C1.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CO7PQfLB.mjs");
  ...
  const { reviewQueue } = await import("./users_DGeJCVlm.mjs");
  const { backupAlert } = await import("./backup_wdx9Kmti.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_B45zdqU6.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CSR2WvVx.mjs");
