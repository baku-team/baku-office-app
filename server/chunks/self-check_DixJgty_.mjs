...
import { APP_SCHEMA, validateDefinition } from "./appdef_BJHimmJt.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_Ur7P7bey.mjs";
...
async function deterministicSelfChecks(ctx, input) {
  ...
  if (badSql) {
    ...
    checks.push({ key: "destructive", label: "破壊的動作", status: "fail", detail: "危険なJS（eval/Function/cookie/localStorage/外部送出 等）を検出。サンドボックス描画では不要なため申請できません（外部ライブラリが必要ならリッチ描画(relaxed)で宣言してください）。", ai: false });
  } else {
...
async function scanLeakedSecrets(ctx, text) {
  const { listApiKeyNames, getApiKey } = await import("./client_CkzMAmmV.mjs");
