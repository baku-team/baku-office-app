...
import { r as runAgent, n as notifyOwnerDirect } from "./ctx_Ige4T837.mjs";
import { a as appendMessage, g as getMessages, t as toTurns } from "./chat-sessions_BTO_5scg.mjs";
...
async function runOneAgentJob(ctx, j, baseUrl, lease) {
  ...
  try {
    ...
    const raw = await runAgent(ctx, j.owner, j.prompt, void 0, baseUrl, j.role ?? "member", { unattended: true, signal: canceller.signal, history: history ? toTurns(history) : void 0, feature: "agent_job", onEvent: (ev) => {
    ...
    const upd = await ctx.db.run("UPDATE agent_jobs SET status='error', result=?, updated_at=? WHERE id=? AND status='running' AND lease_owner=?", [String(e.message ?? e), nowSec(), j.id, lease]);
    if (upd.rowsWritten) await addNotification(ctx, { owner: j.owner, kind: "agent", body: "AIの作業でエラーが発生しました。", link: j.session_id ? `/?ses=${j.session_id}` : "/" }).catch(() => {
    });
    return false;
  } finally {
    clearInterval(hb);
...
async function processAgentJobById(ctx, jobId, baseUrl = "") {
  const now = nowSec();
  const stale = now - AGENT_JOB_STALE;
  const j = await ctx.db.first("SELECT id,owner,session_id,prompt,role,created_at,status FROM agent_jobs WHERE id=?", [jobId]);
  if (!j) return "done";
  ...
  await runOneAgentJob(ctx, { id: j.id, owner: j.owner, session_id: j.session_id, prompt: j.prompt, role: j.role, created_at: j.created_at }, baseUrl, lease);
  return "done";
}
