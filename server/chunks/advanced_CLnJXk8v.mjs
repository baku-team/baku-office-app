...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CkzMAmmV.mjs")).cachedEntitlement(env);
  const hasPlus = atLeast(entitlement, "plus");
  const hasPro = atLeast(entitlement, "pro");
  const { getAiEngine, getCustomPrompt, getWorkersPaid, getSmartRouting } = await import("./settings_C_1axsCz.mjs");
  const workersPaid = hasPlus ? await getWorkersPaid(env) : false;
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_YYoCxNfB.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CzB15Zqw.mjs");
