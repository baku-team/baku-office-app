...
import { getFile } from "./storage_CzB15Zqw.mjs";
import { n as notifyOwnerDirect } from "./ctx_BYlP_hKo.mjs";
const WAIT_SEC = 30;
const MODE_TTL = 3600;
...
async function setOwnerMode(env, owner, mode, context) {
  const { kvPut } = await import("./kv_DEN7iKIl.mjs");
...
async function interpretInstruction(env, text) {
  ...
  const { inferApp } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.M);
  ...
  let out = "";
  try {
    out = await inferApp(env, t, { system: sys, maxTokens: 200 });
  } catch {
    return { action: "none", persist: false, context: "" };
  }
  try {
    const { parseJsonObject } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.R);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_CzB15Zqw.mjs");
    for (const r of rows) await softDeleteFile(ctx.env, r.file_id).catch(() => {
    });
    header = `🗑 ${total}件のファイルを破棄しました。`;
  } else if (mode === "store") {
    header = `🗂 ${total}件のファイルを『資料』に保管しました（一覧：${baseUrl}/files）。`;
  } else {
    header = `📄 読み込み結果（${total}件${context ? `・${context}` : ""}）`;
    for (const r of rows) {
      const out = await readOne(ctx, r, context, baseUrl).catch((e) => `読み取りに失敗しました（${e.message}）。`);
      parts.push(`【${label(r)}】
${out}`);
    }
  }
...
async function readOne(ctx, r, context, baseUrl) {
  ...
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_BYlP_hKo.mjs").then((n) => n.M);
  ...
  return await inferApp(ctx.env, "添付の資料を読み取って要約してください。", { system: sys, attachments: [{ mime: f.mime, buf: f.buf, name: f.name }], maxTokens: 2e3 }).catch(() => "") || "資料を読み取れませんでした（対応AIキーが必要な場合があります）。";
}
