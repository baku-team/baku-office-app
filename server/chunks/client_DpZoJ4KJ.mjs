...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await (await import("./cf-adapter_96IoWiWF.mjs").then((n) => n.B)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_Bfh8eXer.mjs");
