...
import { getSession } from "./auth_Cx_nXCZA.mjs";
import { setMaxUploadMb, setRetentionDays } from "./storage_D9WoeNDX.mjs";
import { setMemberModel, toggleFavApp, setAppFolders, setAiEngine, setSmartRouting, setWorkersAiModel, setBookkeepingMode, setCustomPrompt, setNotifyWebhook, setWorkersPaid } from "./settings_CGAf54Rj.mjs";
import { setAutonomy, saveAutonomyConfig } from "./autonomy_CSZFFkEz.mjs";
import { setStorageLimits } from "./storage-usage_BH2RbdbL.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_Cp-oPbDk.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_BeTYyH2S.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_lH8PyLcm.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_UsPQ46xf.mjs");
    let sent = false;
    if (/^https:\/\/(discord\.com|discordapp\.com|hooks\.slack\.com)\//.test(typed)) {
      const { cfEgressGateway } = await import("./ctx_B914Xe2h.mjs").then((n) => n.L);
      try {
  ...
  if (b._action === "onboarding_dismiss") {
    if (b.show) await env.LICENSE.delete("onboarding_dismissed");
    else await env.LICENSE.put("onboarding_dismissed", "1");
    return json({ ok: true });
  }
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.y);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.z);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.z);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.y);
