...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_sMs6A3qN.mjs");
  const { cmpVersion } = await import("./update_Geji4Pec.mjs");
  await import("./index_BlHS1PcQ.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_xFLd2b3e.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D0x6iHeS.mjs");
  ...
  const { brandName, getTheme } = await import("./theme_DPHiRbLD.mjs");
  const { reviewQueue } = await import("./users_Cb57KvWx.mjs");
  const { backupAlert } = await import("./backup_BeaU_72G.mjs");
