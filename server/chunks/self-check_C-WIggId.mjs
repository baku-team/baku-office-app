...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_DVsclqXs.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_-P_u6Fn4.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
