...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_Ik0IyZy_.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_BxT-nkKO.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_BBDFUyqe.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_BBDFUyqe.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.z);
