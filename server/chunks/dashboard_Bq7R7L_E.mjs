...
import { r as renderComponent } from "./worker-entry_CX9xnnEN.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Bie5zV6F.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_4L0ewUEU.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_D0aobTMW.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DXjdfLkT.mjs");
  await import("./index_D1Qahmtg.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Qqa6Ep4W.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D_FZDQ6D.mjs");
  ...
  const { reviewQueue } = await import("./users_CI6JXwVH.mjs");
  const { backupAlert } = await import("./backup_BBQKUebC.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BsZJNGRN.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_CzJPn-cz.mjs");
