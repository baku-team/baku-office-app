...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_D1hw6KSJ.mjs");
  const { cmpVersion } = await import("./update_y1CUnZO1.mjs");
  await import("./index_B0yPowPV.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_2TjohByF.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cy5ncKBQ.mjs");
  ...
  const { reviewQueue } = await import("./users_Ce6-ijKe.mjs");
  const { backupAlert } = await import("./backup_6bdefGda.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_2G5fL5LC.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CFjJVPOM.mjs");
