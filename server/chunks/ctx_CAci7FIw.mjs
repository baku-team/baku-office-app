...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_CtLBMcNN.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_DcvmK-U8.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CB0G4NNQ.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_plxkvjND.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CpqQ2B4g.mjs";
import "./index_J60yRFFB.mjs";
import { recordUsage, recordTokens, overBudget, recordCallLog, estimateUsd } from "./usage_CAOmc-4O.mjs";
...
import { googleFetch } from "./google_CSArKuHH.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, opCatalogText, googleOpCatalogText } from "./appdef_DxTm0WhQ.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_mug4D6f4.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_BQHgLGd7.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_B1kLe2i-.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_CoJfkQeJ.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_CaMCQb2i.mjs";
import { logDiag } from "./diag_NtHEh0X8.mjs";
import { i as isRunnableDefinition } from "./preflight_BJNGhFGZ.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_D1fpXD0w.mjs";
...
import { g as getAppDesign, m as appsBrief, n as createDraft, o as repairOutputLiterals, q as getAppMeta, t as setAppMeta, v as suggestAppMeta, i as installLocalApp, w as pendingRelaxedHosts, x as isRegisteredPart, f as fetchAndInstall, y as setAppAllowedRoles, b as listExternalApps, z as installedAppDefs, A as parseTags } from "./external-apps_CZwKXiCg.mjs";
import { a as appendMessage } from "./chat-sessions_4ZEuV35m.mjs";
import { explainStop } from "./errors_Cz86HmdL.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DO7mIsrT.mjs";
import { storeCatalog } from "./store_CsnMQ0nm.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CKLelfj1.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_BQ3wJX86.mjs";
import { saveLayoutDraft, listSites } from "./sites_BnCPgr1A.mjs";
...
import { p as pushToUser } from "./push_DNouAJfP.mjs";
...
function prunePlanToSpec(plan, spec) {
  ...
  if (followUps && followUps.length) next.followUps = followUps;
  ...
  try {
    if (phase.kind === "render") {
      ...
      if (added.length) {
        ...
        await logDiag(env, "warn", "build", `screen failed cursor=${row.cursor} phase="${phase.title}" reason=${reason}`).catch(() => {
        });
      }
    }
  } catch (e) {
...
async function addGeneratedTriggers(model, spec, def) {
  if (!INTAKE_HINT.test(spec || "")) return { def, added: 0 };
  const raw = await generateTriggers(model, spec, def).catch(() => []);
  if (!raw.length) return { def, added: 0 };
  ...
  if (!valid.length) return { def, added: 0 };
  return { def: { ...def, permissions: [...perms], triggers: valid }, added: valid.length };
}
async function finalize$1(ctx2, row, plan, def, model) {
  ...
  if (dataScreenPending && screenCount() === 0) {
    try {
      const rhtml = String(def.render?.html ?? "") || null;
      const ids = [...new Set([...(rhtml ?? "").matchAll(/\bbo\s*\.\s*run\s*\(\s*['"`]([a-zA-Z0-9_]+)/g)].map((m) => m[1]))].slice(0, 6);
      const recovered = [];
      const built = [];
      ...
      if (recovered.length) {
        def.screens = recovered;
        def.permissions = [.../* @__PURE__ */ new Set([...def.permissions ?? [], ...validateDefinition(def).requiredPermissions])];
        await logDiag(env, "info", "build", `finalize recovered ${recovered.length} data screen(s): ${built.join(",")}`).catch(() => {
        });
      }
    } catch {
    }
  ...
  const trig = await addGeneratedTriggers(model, row.spec ?? plan.description, def).catch(() => ({ def, added: 0 }));
  def = trig.def;
  ...
  if (noDataLayer) plan.unmetCriteria = [.../* @__PURE__ */ new Set([...plan.unmetCriteria ?? [], "データの保存・一覧（画面定義）が未実装＝入力しても保存されません。"])];
  const partial = failed.length > 0 || !!uiCheck.note || !!gate.note || noDataLayer;
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_BnCPgr1A.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_Doe1X5zf.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_Doe1X5zf.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_Doe1X5zf.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_DcvmK-U8.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_Doe1X5zf.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_CtLBMcNN.mjs")).storageMode(env) === "r2";
...
export {
  setLineWebhookActive as A,
  slackAuthTest as B,
  buildCtx as C,
  linePush as D,
  processSummaryJobs as E,
  processAppBuilds as F,
  processSiteBuilds as G,
  ...
  startAppBuild as g,
  getPublicProfile as h,
  inferApp as i,
  setPublicProfile as j,
  generateOrgProfile as k,
  publishDirectory as l,
  myDirectory as m,
  notifyOwnerDirect as n,
  orgDisplayName as o,
  parseApprovalAction as p,
  searchDirectory as q,
  runAgent as r,
  suggestAccountItem as s,
  transcribeAudio as t,
  unpublishDirectory as u,
  verifyOrgExistence as v,
  reportDirectory as w,
  cfEgressGateway as x,
  lineBotInfo as y,
  setLineWebhookEndpoint as z
};
