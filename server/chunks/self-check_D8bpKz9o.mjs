...
import { loadDraftForCheck, deterministicSelfChecks, aiSelfChecks, mergeChecks, finalize, persistSelfCheck } from "./self-check_Ci8jWS9N.mjs";
...
`));
  ...
  (async () => {
    try {
      ...
      await (await import("./diag_ByGiCOpc.mjs")).logDiag(env, "error", "self-check", `失敗(draft=${draftId}): ${msg}`).catch(() => {
      });
