...
import { r as renderComponent } from "./worker-entry_Cl9T0jH-.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DrGjDLp1.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DQFw2eZX.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DQFw2eZX.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_BK8gsTKF.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_AxuQz5dA.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BvR2HJE_.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BsdZaID6.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_TTFEFEgh.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_B9QFooCD.mjs");
