...
import { parseRequestModel, getWorkersPaid } from "./settings_BGnJ4d3n.mjs";
import { s as startAppEdit, p as processAppBuild } from "./cf-adapter_C1pggy-8.mjs";
...
const GET = async ({ request, locals }) => {
  ...
  const row = await locals.ctx.db.first("SELECT status,error FROM app_builds WHERE id=? AND owner=?", [id, ses.uid]);
  if (!row) return json({ ok: false, status: "unknown" });
  return json({ ok: true, status: row.status, message: row.error ?? "" });
};
const POST = async ({ request, locals }) => {
  ...
  const instruction = b.debug ? "このアプリの動作確認（デバッグ）を行う。計算式・ボタンのイベント・画面遷移・入力検証・推論などに不具合がないか点検し、見つかれば最小差分で直す。問題が無ければ変更せず確認結果を述べる。" : String(b.instruction ?? "").trim();
