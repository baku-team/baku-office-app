...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_BF4jK8tm.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_CPPkK4pl.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_C-HGOQve.mjs").then((n) => n.P);
  ...
  const { latestSessionApp } = await import("./ctx_C-HGOQve.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Dk15ONnL.mjs").then((n) => n.z);
