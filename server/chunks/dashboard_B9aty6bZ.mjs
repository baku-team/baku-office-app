...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_C5A1-cK5.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DYx7pJ-_.mjs");
  await import("./index_CFKAHzaY.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Cp-oPbDk.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BH2RbdbL.mjs");
  ...
  const { reviewQueue } = await import("./users_wYijm9MC.mjs");
  const { backupAlert } = await import("./backup_BN5OQsqi.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BeTYyH2S.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_D9WoeNDX.mjs");
