...
async function markSubmissionPaidBySession(env, submissionId, session) {
  const row = await env.DB.prepare("SELECT slug,pay_status,amount,stripe_session FROM public_submissions WHERE id=?").bind(submissionId).first();
  if (!row) return { ok: false, reason: "not_found" };
  if (session) {
    if (session.payment_status && session.payment_status !== "paid") return { ok: false, slug: row.slug, reason: "not_paid" };
    if (row.stripe_session && session.id && session.id !== row.stripe_session) return { ok: false, slug: row.slug, reason: "session_mismatch" };
    if ((row.amount ?? 0) > 0 && typeof session.amount_total === "number" && session.amount_total !== Math.round(row.amount ?? 0)) {
      return { ok: false, slug: row.slug, reason: "amount_mismatch" };
    }
  }
