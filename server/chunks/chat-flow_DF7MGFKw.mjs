...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_2nH8kXF8.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_B-WwEpGI.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_B2KW-m1q.mjs").then((n) => n.P);
  ...
  const { latestSessionApp } = await import("./ctx_B2KW-m1q.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Bh_N8LI2.mjs").then((n) => n.z);
