...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_BmX3boNt.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_D1dXAMUS.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_DE3oxZSo.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CXDHY44j.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CRTeJApz.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_qrcuL6Wt.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_Z8dAny_Z.mjs");
