...
import { r as renderTemplate, m as maybeRenderHead, F as Fragment, a as addAttribute, u as unescapeHTML } from "./sequence_2tuU57vh.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, hasApiKey } = await import("./client_BOITous_.mjs");
  ...
  const [hasGemini, hasClaude, hasOpenai] = await Promise.all([hasApiKey(env, "gemini").catch(() => false), hasApiKey(env, "claude").catch(() => false), hasApiKey(env, "openai").catch(() => false)]);
  const hasLocal = !!env.AI;
  const aiReady = hasGemini || hasClaude || hasOpenai || hasLocal;
  const { GEMINI_MODELS, CLAUDE_MODELS, OPENAI_MODELS, WORKERS_AI_MODELS } = await import("./config_M3rcN2DV.mjs");
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    import("./settings_BGnJ4d3n.mjs"),
    import("./skills_CqIIG0oQ.mjs"),
    import("./scheduled-tasks_Cb3MAGwx.mjs")
  ]);
  ...
  const { enabledParts, enabledPartIds } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
    ...
    (function () {
        ...
        function renderActions(parent, actions) {
          ...
          for (const a of actions) {
            ...
            btn.type = "button";
            btn.className = "btn btn-sm " + (a.style === "primary" ? "btn-primary" : "btn-ghost");
            btn.textContent = a.label;
            btn.addEventListener("click", async () => {
              if (a.kind === "reply" && a.text) { sendText(a.text); }
              else if (a.kind === "navigate" && a.href) { location.href = a.href; }
              else if (a.kind === "link" && a.href) { window.open(a.href, "_blank", "noopener"); }
              else if (a.kind === "api" && a.endpoint) {
                const r = await window.bo.api(a.endpoint, a.payload || {}, { btn, successMsg: a.successMsg || null });
                if (r.ok) { if (a.then) location.href = a.then; else if (a.reload) setTimeout(() => location.reload(), 600); }
              }
            });
        ...
        // actions=メッセージ下に出すボタン配列（任意）。
        function addMsg(role, text, stream, actions) {
          // 最初の発言でヒーロー（空状態）を隠す。
          ...
          if (stream && role === "a" && text) {
            ...
            const step = () => {
              ...
              else { bub.innerHTML = html; renderActions(d.querySelector(".cmsg-body"), actions); log.scrollTop = log.scrollHeight; }
            };
            step();
          } else {
            bub.innerHTML = html;
        ...
        async function openSession(id) {
          ...
          if (msgs.length) {
            msgs.forEach((m) => addMsg(m.role === "assistant" ? "a" : "u", m.content, false, m.actions)); setLastSession(id);
            // 末尾がユーザー発言＝応答待ち（バックグラウンド処理中）。完了を監視して自動反映。
            const last = msgs[msgs.length - 1];
            if (last && last.role !== "assistant") { addMsg("a", "この依頼はバックグラウンドで実行中です。完了するとここに表示されます（画面を離れても続行します）。"); pollBackgroundResult(id); }
        ...
        att?.addEventListener("change", () => {
          ...
          if (!f) return;
          // P1-05：送信前にサイズと種別を検査（大容量でブラウザ/ワーカー上限に当たる前に弾く）。
          const MAX_MB = 8;
          if (f.size > MAX_MB * 1024 * 1024) { window.bo.toast(\`ファイルが大きすぎます（上限 \${MAX_MB}MB）。\`, "err"); att.value = ""; return; }
          if (!/^image\\//.test(f.type) && f.type !== "application/pdf") { window.bo.toast("画像（image/*）または PDF のみ添付できます。", "err"); att.value = ""; return; }
          const reader = new FileReader();
          reader.onload = () => {
            const url = String(reader.result);
            pendingImage = { mimeType: f.type || "application/octet-stream", dataB64: url.split(",")[1] || "" };
            pendingFileName = f.name;
            const nm = document.getElementById("att-name"); if (nm) nm.textContent = "添付: " + f.name;
          };
        ...
        function pollBackgroundResult(sid) {
          ...
          bgPollTimer = _every(async () => {
            ...
            try {
              ...
              if (r.ok) { const n = (r.data.messages || []).length; if (n !== lastCount) { lastCount = n; openSession(sid); window.bo.pollAgent?.(); } }
            } catch (e) { /* retry */ }
            ...
            try {
              ...
              if (tries > 1 && !(a.tasks || []).some((t) => t.mine)) { stop(); openSession(sid); window.bo.pollAgent?.(); }
            } catch (e) { /* retry */ }
            if (tries > 300) stop(); // 約25分で打ち切り
          }, 5000);
        }
        ...
        async function send() {
          ...
          try {
            ...
            if (done) {
              window.bo.pollAgent?.();
              addMsg("a", done.reply, true, done.actions);
