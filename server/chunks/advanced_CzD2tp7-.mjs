...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_C30hHFwA.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_I3hALocb.mjs");
  ...
  const { hasApiKey } = await import("./client_C30hHFwA.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_My0jkMrn.mjs");
