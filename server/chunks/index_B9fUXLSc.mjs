...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, hasApiKey } = await import("./client_CQaGQZPz.mjs");
  ...
  if (ses.role === "admin" && hasPro && !hasExternalAI) return Astro2.redirect("/setup", 302);
  const agentName = await (await import("./settings_CGAf54Rj.mjs")).getAgentName(env).catch(() => "相棒");
  const ttsCloudReady = await (await import("./capabilities_DGPtqc6I.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_s8EjoVze.mjs"),
    import("./scheduled-tasks_BTjY6mXk.mjs")
  ]);
