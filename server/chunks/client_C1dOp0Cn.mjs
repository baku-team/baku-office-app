...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await (await import("./cf-adapter_BBDFUyqe.mjs").then((n) => n.B)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_CCr37Hlj.mjs");
