...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BwbWPyEb.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_iQON6brF.mjs");
  await import("./index_P1oP4Wh_.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DGZitt3L.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Bhf9Jsvw.mjs");
  ...
  const { reviewQueue } = await import("./users_a0-1xmNf.mjs");
  const { backupAlert } = await import("./backup_BEaVdQSG.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_D8PNCsk4.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_CKfFiifC.mjs");
