...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DhWdwsPc.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CuoW304l.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CJ_aE4sG.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_ZnJTAJrH.mjs"),
    import("./scheduled-tasks_C346P2f2.mjs")
  ]);
  ...
  await import("./index_DSEOIWls.mjs");
