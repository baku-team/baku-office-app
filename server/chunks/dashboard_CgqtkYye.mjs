...
import { r as renderComponent } from "./worker-entry_DFn3p-F5.mjs";
import { kvPut } from "./kv_H5bDbyIA.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DU0__uW2.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DCMxpdwM.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_C_RhHGpz.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BBqgx3ry.mjs");
  await import("./index_DQSrHVYT.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DQ67RXU1.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BkQX1JZw.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_H5bDbyIA.mjs");
  const { scopedWidgets, enabledParts } = await import("./parts_CVGBl5kS.mjs").then((n) => n.f);
  ...
  const { reviewQueue } = await import("./users_BSfpZ42G.mjs");
  const { backupAlert } = await import("./backup_Cqgm97p5.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_ja9a7AEm.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_DWaRDBDI.mjs");
  const { getWorkersPaid } = await import("./settings_DHOyCLdr.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_Bcla-e0G.mjs");
