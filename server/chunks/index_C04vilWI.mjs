...
import { r as renderComponent } from "./worker-entry_Cl8UOWz3.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_gP-CDZ5s.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_C5jSoDoM.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CQSsPpWQ.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_DyTev1Pk.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_Bfcll_iq.mjs"),
    import("./scheduled-tasks_MjEv04xx.mjs")
  ]);
  ...
  await import("./index_BNv26dye.mjs");
