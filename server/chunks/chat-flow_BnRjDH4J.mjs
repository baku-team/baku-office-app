...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_aIMHFm6C.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_C5n36hBk.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_CCwYpYbr.mjs").then((n) => n.Q);
  ...
  const { latestSessionApp } = await import("./ctx_CCwYpYbr.mjs").then((n) => n.O);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Ish93S4w.mjs").then((n) => n.z);
