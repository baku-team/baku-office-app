...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_PSBt_dAs.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DUUxPCqA.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_CSR2WvVx.mjs");
