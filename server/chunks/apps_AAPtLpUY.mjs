...
import { r as renderComponent } from "./worker-entry_DcjAKj5O.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_B0dg8gdV.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_DaDf7n3s.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_DaDf7n3s.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CzH8Lj8g.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_7WfEvhS0.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CrfElIO3.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DfgyJWKg.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_CFGtMTKs.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BzBYqI7n.mjs");
