...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_DAURz88_.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_Du2JHuMP.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C12A0H0f.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CdXzec5b.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DI2SUbWs.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_C1MoMs01.mjs");
