...
import { setMaxUploadMb, setRetentionDays } from "./storage__hw20N6r.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_pY8q6gw4.mjs";
import { setStorageLimits } from "./storage-usage_BBN7Q7J1.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_cG6KZTqF.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_BLHrxe4n.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DC1FBp_Z.mjs");
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.y);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.z);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.z);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.y);
