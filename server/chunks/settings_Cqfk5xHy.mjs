...
import { setMaxUploadMb, setRetentionDays } from "./storage_TBpzvJRs.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_BEZoUF8-.mjs";
import { setStorageLimits } from "./storage-usage_BZUygzop.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_CW76H0U6.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_CEIQmGS5.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_CqvpClhP.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_D4_8tfDi.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_DdgMveJQ.mjs").then((n) => n.L);
      try {
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.A);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.z);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.A);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.A);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_CEIQmGS5.mjs").then((n) => n.z);
