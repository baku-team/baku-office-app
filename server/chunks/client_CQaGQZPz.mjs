...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await (await import("./ctx_DXZLzw9T.mjs").then((n) => n.N)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_DXZLzw9T.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_CkC5O1hj.mjs");
