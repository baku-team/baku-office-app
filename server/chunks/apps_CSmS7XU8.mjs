...
import { r as renderComponent } from "./worker-entry_B6lNJQhC.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Ba9MDR0e.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_jflEqZdB.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_jflEqZdB.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_D6vAg_ie.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_BWXt_wx-.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BwPbfhwG.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_cG6KZTqF.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_BF6_U4eO.mjs");
