...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_Bdf4380v.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_FnSEomav.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_DBX5cnQS.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_DBX5cnQS.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_CjQOD0Ds.mjs").then((n) => n.z);
