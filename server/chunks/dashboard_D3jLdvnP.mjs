...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CQSsPpWQ.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_CKlzZ6NV.mjs");
  await import("./index_BNv26dye.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C_lv8bXm.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_iV88NonB.mjs");
  ...
  const { reviewQueue } = await import("./users_aF0JOfhF.mjs");
  const { backupAlert } = await import("./backup_D0ivisFi.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_RUDRQx3S.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_C19jFPcM.mjs");
