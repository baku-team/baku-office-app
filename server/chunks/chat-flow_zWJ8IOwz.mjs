...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_BBucxay0.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_-8wQqFQR.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_DLbTjLG9.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_DLbTjLG9.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_BJqjrgQW.mjs").then((n) => n.z);
