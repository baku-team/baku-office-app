...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_Cm6c2326.mjs");
  const { cmpVersion } = await import("./update_z_x5jXL5.mjs");
  await import("./index_C38zwQKE.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_Be-sZMiJ.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BZdkfaAd.mjs");
  ...
  const { reviewQueue } = await import("./users_BK6Ux9z1.mjs");
  const { backupAlert } = await import("./backup_CKPXonPG.mjs");
