...
const GET = async ({ request, url }) => {
  ...
  const rows = results ?? [];
  const appName = (await env.DB.prepare("SELECT name FROM external_apps WHERE id=?").bind(appId).first().catch(() => null))?.name || appId;
  const base = `export_${appId}${collection ? "_" + collection : ""}`;
