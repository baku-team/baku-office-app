...
import { r as renderComponent } from "./worker-entry_CnGvZ36Z.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_D4L_w06Q.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BjOWYtvE.mjs");
  ...
  const { atLeast, planLabel } = await import("./cf-adapter_C1pggy-8.mjs").then((n) => n.y);
  const entitlement = await (await import("./client_w07XlKo6.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CyuXw7nH.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BsMJJaFf.mjs");
  const { registeredParts } = await import("./parts_DlPNdOxI.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_B-0nE2nO.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_CcTcDbck.mjs");
