...
import { r as renderComponent } from "./worker-entry_7C4vkEZA.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_Bzu-w89w.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BwhEewZt.mjs");
  ...
  const { atLeast, planLabel } = await import("./cf-adapter_C3FZ9rAH.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_Cl9NskZP.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C6eAEUan.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CNzxAd3z.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BFESc2lw.mjs").then((n) => n.y);
  const { storeCatalog, myApps } = await import("./store_7Xuazgbc.mjs");
