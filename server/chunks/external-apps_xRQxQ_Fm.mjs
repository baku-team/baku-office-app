...
import { getToken, hostFetch, getVerifyJwk } from "./client_Cti--qkw.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_BcniWYMM.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, validateDefinition, isBlockedHost } from "./appdef_DKzuns4-.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_m8OOu-NR.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_DKzuns4-.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_CYnoEgWp.mjs");
