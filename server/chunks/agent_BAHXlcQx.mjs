...
import { entitlementForGate, getApiKey } from "./client_BTLpH8fW.mjs";
...
import { permissionCatalogText } from "./apps_B5O5FLC5.mjs";
...
import { callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_VNR7apHV.mjs";
import { searchDirectory } from "./directory_DdyN4P-i.mjs";
import { autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_DKs0ZAVL.mjs";
...
import "./index_DO-Y0Vx2.mjs";
import { m as makeDocument, w as webSearch } from "./media-ai_C-EWPQoK.mjs";
import { runSkill, generateSkill, listSkills } from "./skills_CjvrrlUH.mjs";
import { createDraft } from "./external-apps_CETWOrmK.mjs";
import { videoStatusText, invokeCapability, listCapabilities, capabilitySummary } from "./capabilities_DAjIaJcm.mjs";
import { overBudget, recordUsage, recordTokens, estimateUsd } from "./usage_C0o2RuAe.mjs";
import { getApprovalMode, previewFor, createApproval, A2A_OUTWARD } from "./approvals_BnTnOHbv.mjs";
...
「承認待ち」一覧（/approvals）で管理者が承認すると実行されます。承認ID: ${id}`;
  ...
  switch (name) {
    ...
    case "propose_app": {
      ...
      const res = await createDraft(ctx, { name: name2, description: args.description ? String(args.description) : void 0, spec, permissions: perms, definition: args.definition, estTokens: Number(args.estimated_tokens) || void 0, role }, owner);
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BTLpH8fW.mjs");
