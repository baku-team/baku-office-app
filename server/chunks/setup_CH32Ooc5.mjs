...
const $$Setup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { hasApiKey } = await import("./client_vij6I9Jq.mjs");
  const { googleConfigured } = await import("./google_LDFRfhIk.mjs");
  const { getTheme } = await import("./theme_DFty9gzU.mjs");
  const { getBackupState } = await import("./backup_P0Ss-8nj.mjs");
