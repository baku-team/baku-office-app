...
const POST = async ({ request, locals }) => {
  ...
  if (b.background) {
    const { looksLikeNewAppRequest, looksLikeAppEdit } = await import("./ctx_CIL4AtJU.mjs").then((n) => n.X);
    ...
    if (!autoAppReq) {
      ...
      const jobId = await enqueueAgentJob(ctx, { owner: ses.uid, sessionId, prompt, role: ses.role });
      try {
