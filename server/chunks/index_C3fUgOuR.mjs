...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement, hasApiKey } = await import("./client_C_RhHGpz.mjs");
  ...
  const agentName = await (await import("./settings_DHOyCLdr.mjs")).getAgentName(env).catch(() => "相棒");
  const ttsCloudReady = await (await import("./capabilities_dOBDgNOm.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    import("./settings_DHOyCLdr.mjs"),
    import("./skills_BSxu6ge_.mjs"),
    import("./scheduled-tasks_AbRAQPxb.mjs")
  ]);
  ...
  const { enabledParts, enabledPartIds } = await import("./parts_CVGBl5kS.mjs").then((n) => n.f);
