...
import { EgressGateway, DEFAULT_EGRESS_ALLOWLIST } from "./egress_yKY42dgP.mjs";
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BthdaNTE.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_B17j1uG_.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_CduOQvTk.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_BYbdQprm.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CVjeTeB8.mjs";
import "./index_BCH4lAnd.mjs";
import { recordUsage, recordTokens, overBudget, recordCallLog, estimateUsd } from "./usage_X9PGsLLE.mjs";
...
import { getAiEngine, getBuildDriver, getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, setPaidSwitchOk, resolveModelSelection, getMemberModel, getSmartRouting, getWorkersAiModel, maxParallelAgents, agentMaxHops, getPaidSwitchOk } from "./settings_C4tL9AHR.mjs";
import { googleFetch } from "./google_BPJeqCVB.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_Cmp4V_Cj.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_C13dLMSP.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_DzFpkU47.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_koKo0H-8.mjs";
import { i as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, g as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, j as googleToolPreflight } from "./app-runtime_CC00lJRu.mjs";
import { logBuild, logDiag } from "./diag_-P_u6Fn4.mjs";
import { i as isRunnableDefinition } from "./preflight_Bmz9f5Zb.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_D7IVELmm.mjs";
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage, collectionFromDefinition } from "./public-pages_Bg1BWixA.mjs";
import { g as getAppDesign, n as appsBrief, o as createDraft, q as repairOutputLiterals, t as getAppMeta, v as setAppMeta, w as suggestAppMeta, i as installLocalApp, x as pendingRelaxedHosts, y as isRegisteredPart, f as fetchAndInstall, z as setAppAllowedRoles, b as listExternalApps, A as parseTags, B as installedAppDefs } from "./external-apps_DdRsAqqg.mjs";
import { a as appendMessage, b as estimateTokens, i as estimateToolTokens, p as providerInputCap } from "./chat-sessions_BTO_5scg.mjs";
import { explainStop } from "./errors_Cz86HmdL.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_Bq-8lTbF.mjs";
import { storeCatalog } from "./store_Z-no6Ohb.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DpaSPqPu.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_w6xNIQ3l.mjs";
import { saveLayoutDraft, listSites } from "./sites_DBr2-kMn.mjs";
...
import { p as pushToUser } from "./push_B11okRk1.mjs";
...
Generated apps/public/HP/LP run in a cross-origin sandbox iframe (srcdoc): external comms blocked except curated CDNs.
...
async function recordEgress(db, e) {
  if (!e.blocked && e.ok) {
    const day = new Date(e.at * 1e3).toISOString().slice(0, 10);
    await db.run(
      "INSERT INTO egress_log_daily (day,host,method,count) VALUES (?,?,?,1) ON CONFLICT(day,host,method) DO UPDATE SET count=count+1",
      [day, e.host, e.method]
    ).catch(() => {
    });
    return;
...
function thinkingParam(modelId, mode) {
  ...
  if (mode === "disabled") return /sonnet-5|sonnet-4-6|opus-4-[678]/i.test(modelId) ? { type: "disabled" } : null;
...
const STREAM_IDLE_MS = 6e4;
const STREAM_TOTAL_MS = 9e5;
async function streamText(key, body, onTextDelta) {
  ...
  for (; ; ) {
    ...
    let chunk;
    try {
      const budget = Math.max(1e3, Math.min(STREAM_IDLE_MS, STREAM_TOTAL_MS - (Date.now() - startedAt)));
      chunk = await Promise.race([
        reader.read(),
        new Promise((_, rej) => {
          timer = setTimeout(() => rej(new Error(`claude stream timeout (idle ${Math.round(budget / 1e3)}s)`)), budget);
        })
      ]);
    } catch (e) {
      try {
      ...
      return { error: { message: "claude stream: " + (e.message ?? String(e)) } };
    } finally {
    ...
    async turn(system, history, tools, force, opts) {
      ...
      try {
        r = await fetchWithRateRetry(() => fetch("https://api.anthropic.com/v1/messages", {
          ...
          signal: AbortSignal.timeout(STREAM_TOTAL_MS)
          // 非ストリーム経路の総枠上限＝無応答で永久 await しない
        }));
      } catch (e) {
        return { error: { message: "claude network: " + (e.message ?? String(e)) } };
      }
    ...
    async turn(system, history, tools, force, opts) {
      ...
      try {
        r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(modelId)}:generateContent?key=${encodeURIComponent(key)}`, {
          ...
          signal: AbortSignal.timeout(GEMINI_TOTAL_MS)
          // 総枠上限（無応答で永久 await しない・実機MEOハング対策）
        });
      } catch (e) {
  ...
  return {
    ...
    async turn(system, history, tools, force, opts) {
      ...
      try {
        ...
        await recordCallLog(env, provider, res.usage ?? {}, { feature, model: modelId ?? model.name, plan, durationMs: Date.now() - t0 }).catch(() => {
        });
        return res;
      } catch (e) {
        plan ??= await cachedEntitlement(env).catch(() => void 0);
        await recordCallLog(env, provider, {}, { feature, model: modelId ?? model.name, plan, durationMs: Date.now() - t0 }).catch(() => {
        });
        throw e;
      }
    }
...
function normalizePlan(raw) {
  ...
  let followUps = Array.isArray(r.followUps) ? r.followUps.map((s) => String(s).trim().replace(/\s+/g, " ").slice(0, 120)).filter(Boolean).slice(0, 10) : [];
  ...
  if (overflow.length) followUps = [...followUps, ...overflow].slice(0, 10);
...
function phaseIsComplex(phase) {
  const t = `${phase.title ?? ""} ${phase.goal ?? ""}`;
  if (/(upsert|あれば更新|存在すれ|一意|重複|前月比|前年比|差分|突合|マージ|リレーション|結合|紐づ|集計|合計|平均|算出|計算|採点|条件分岐|複数の.*(種類|データ))/i.test(t)) return true;
  return /(Google|Gmail|カレンダー|スプレッドシート|ドライブ|LINE|Discord|Slack|外部API|外部サイト|Web.*(取得|検索|連携)|スクレイ|webhook|AI.*(分析|要約|分類|判定|抽出|生成)|自動.*(要約|分類|判定|添削)|感情分析|文章生成)/i.test(t);
}
function screenMissingWrite(screen) {
  if (!screen || typeof screen !== "object") return false;
  const s = screen;
  const id = typeof s.id === "string" ? s.id : "";
  const label = `${id} ${typeof s.title === "string" ? s.title : ""}`;
  if (/(一覧|リスト|履歴|表示|閲覧|検索|list|history|view|search|index)/i.test(label)) return false;
  const saveIntent = /(保存|登録|作成|追加|更新|記録|upsert)/i.test(label) || /(^|_)(create|update|upsert|save|insert|regist)/i.test(id) || /upsert/i.test(id);
  if (!saveIntent) return false;
  const hasWrite = Array.isArray(s.steps) && s.steps.some((st) => st != null && typeof st === "object" && /^data\.(create|update|upsert)$/.test(String(st.op ?? "")));
  return !hasWrite;
}
function repairMissingRecFrom(screen) {
  if (!screen || typeof screen !== "object") return false;
  const s = screen;
  if (!Array.isArray(s.steps) || !Array.isArray(s.inputs) || s.inputs.length === 0) return false;
  let changed = false;
  for (const st of s.steps) {
    if (!st || typeof st !== "object") continue;
    const o = st;
    if (o.op !== "data.create" && o.op !== "data.upsert") continue;
    if (typeof o.from === "string" && o.from.trim().length > 0) continue;
    o.from = "$rec";
    changed = true;
  }
  return changed;
}
function salvageScreenDroppingBadIntegrationSteps(plan, screen, html) {
  if (!screen || typeof screen !== "object") return null;
  if (screenIsValid(plan, screen, html)) return null;
  const s = screen;
  if (!Array.isArray(s.steps)) return null;
  const INTEG = /^(google\.|http\.fetch$|message\.send$|notify$)/;
  const kept = s.steps.filter((st) => !(st && typeof st === "object" && INTEG.test(String(st.op ?? ""))));
  if (kept.length === s.steps.length) return null;
  if (kept.length === 0) return null;
  const salvaged = { ...screen, steps: kept };
  return screenIsValid(plan, salvaged, html) ? salvaged : null;
}
function pruneUnmetAgainstDonePhases(unmet, phases) {
  const core = (s) => (s || "").replace(/(画面|ボタン|機能|の実装|を実装|の追加|を追加|の作成|を作成|UI|フォーム|ページ|の設定|する)/gi, "").replace(/[\s・、。（）()【】]/g, "").trim();
  const doneCores = (phases ?? []).filter((p) => p?.status === "done").map((p) => core(p.title ?? "")).filter((c) => c.length >= 3);
  if (!doneCores.length) return unmet;
  return unmet.filter((u) => {
    const uc = core(u);
    return uc.length < 3 || !doneCores.some((dc) => dc.includes(uc));
  });
}
const CAPABILITY_BOUNDARIES = [
  {
    key: "web_search",
    intent: /(web|ウェブ|ネット|インターネット|ネット上|オンライン|検索エンジン)[^。、\n]{0,10}(で|から|を|上)?[^。、\n]{0,6}(検索|巡回|クロール|スクレイ|情報収集|自動[^。、\n]{0,3}(取得|収集|抽出))|スクレイピング|クローリング|(競合|他社|相場|市場|口コミ|評判)[^。、\n]{0,8}(を)?[^。、\n]{0,6}(web|ウェブ|ネット|ネット上|オンライン)[^。、\n]{0,6}(検索|取得|収集|調べ)/i,
    note: "🌐 Webサイトを自動で巡回・検索して情報を集める機能は、アプリには持たせられません（汎用のWeb検索がないため）。決まった公開API（http.fetch）があれば連携でき、無ければAIチャットに調べてもらって手動/代理で登録する形で代替できます。"
  },
  {
    key: "line_individual",
    intent: /((特定|個別|各|指定)[^。、\n]{0,4}(の)?[^。、\n]{0,4}(人|方|お客様|顧客|ユーザー|利用者|会員|メンバー)|本人宛|1対1|一対一)[^。、\n]{0,12}(LINE|ライン)[^。、\n]{0,10}(送|通知|push|プッシュ|連絡|メッセージ|届)|(LINE|ライン)[^。、\n]{0,8}(個人|個別|1対1|一対一|ダイレクト|DM|本人宛)[^。、\n]{0,10}(送|通知|push|連絡|届|メッセージ)/i,
    note: "📱 特定の個人あてにLINEで個別送信（push）することは仕組み上できません。連携済みのLINEグループ／チャンネルへの配信（そこにいる全員に届く形）なら可能です。"
  }
];
function detectCapabilityBoundaries(spec) {
  const t = specUserTurns(spec).slice(0, 4e3);
  return CAPABILITY_BOUNDARIES.filter((b) => b.intent.test(t)).map((b) => b.note);
}
function isCapabilityBoundaryText(s) {
  return CAPABILITY_BOUNDARIES.some((b) => b.intent.test(s));
}
const REQUESTED_FEATURES = [
  {
    intent: /(LINE|ライン|Discord|ディスコード|Slack|スラック)[^。、．\n]{0,10}(通知|送信|配信|投稿|連絡|メッセージ|アラート|お知らせ)|(通知|配信|アラート|お知らせ|メッセージ)[^。、．\n]{0,8}(LINE|ライン|Discord|Slack)/i,
    label: "外部チャット（LINE / Discord / Slack）への通知・送信",
    has: (ops) => ops.has("message.send") || ops.has("notify")
  },
  {
    intent: /(Google|グーグル|Gmail|ジーメール|スプレッドシート|スプシ|グーグルカレンダー|Google\s*(カレンダー|シート|ドライブ|フォーム|ドキュメント|Meet|Drive|Sheets|Calendar))/i,
    label: "Google連携（カレンダー / スプレッドシート / Gmail / ドライブ 等）",
    has: (ops) => [...ops].some((o) => o.startsWith("google."))
  },
  {
    intent: /(AI|人工知能|ＡＩ)[^。、．\n]{0,8}(分析|要約|分類|判定|生成|抽出|添削|評価|採点|翻訳|校正|解析)|(自動)(要約|分類|判定|添削|生成|翻訳)|感情分析|文章生成/i,
    label: "AI による分析・要約・分類・生成 など",
    has: (ops) => ops.has("ai.infer")
  },
  {
    intent: /((外部|お客様|顧客|利用者|一般|ユーザー|WEB|Web|ウェブ)\s*(から)?\s*(の)?\s*(問い?合わせ|お問合せ|申込|申し込み|応募|エントリー|受付|依頼|予約))|(問い?合わせ|申込|応募|予約|受付)\s*(を)?\s*(受付|受信|受け付|取り?込|フォーム)/i,
    label: "外部からの問い合わせ・申込・予約の受付／取り込み",
    has: (ops, def) => ops.has("data.create") || ops.has("data.upsert") || Array.isArray(def.triggers) && def.triggers.length > 0
  }
];
function collectDefOps(def) {
  const ops = /* @__PURE__ */ new Set();
  const scan = (steps) => {
    if (Array.isArray(steps)) {
      for (const st of steps) if (st && typeof st === "object") {
...
const FAKE_INTEGRATION_CHECKS = [
  {
    // 「(スプレッド)シートに/へ 記録/追記/保存/書込」または「スプレッドシート/Google 連携/同期」＝外部シートへの永続化を主張。
    claim: /(スプレッドシート|Google\s*シート|グーグルシート|シート)[^。<>\n]{0,10}[にへ][^。<>\n]{0,6}(記録|追記|保存|書き込|反映)|(スプレッドシート|Google\s*スプレッド|グーグル|Googleドライブ|Googleカレンダー|Gmail)[^。<>\n]{0,8}(連携|同期|に記録|へ記録)/i,
    hasOp: (ops) => [...ops].some((o) => o.startsWith("google.")),
    label: "Googleスプレッドシート等への記録・連携"
  },
  {
    claim: /(LINE|ライン)(グループ|チャンネル)?[^。<>\n]{0,8}[にへ][^。<>\n]{0,6}(送信|通知|連絡)(しました|する|します|完了)?|(LINE|ライン)[^。<>\n]{0,6}(送信|通知)しました/i,
    hasOp: (ops) => ops.has("message.send") || ops.has("notify"),
    label: "LINE/Discord/Slack への送信・通知"
  }
];
function checkFakeIntegrationSuccess(def) {
  const html = String(def.render?.html ?? "");
  if (!html) return [];
  const ops = collectDefOps(def);
  const out = [];
  for (const c of FAKE_INTEGRATION_CHECKS) {
    if (c.claim.test(html) && !c.hasOp(ops)) out.push(`画面は「${c.label}」の成功/連携を示していますが、実際にそれを行う処理がアプリにありません＝操作しても実際には実行・記録されません（見た目だけの成功表示）。`);
  }
  return out;
}
function specUserTurns(spec) {
  const s = spec || "";
  if (!/^(利用者|AI): /m.test(s)) return s;
  const segs = s.split(/\n(?=(?:利用者|AI): )/).filter((seg) => seg.startsWith("利用者: ")).map((seg) => seg.slice("利用者: ".length));
  return segs.length ? segs.join("\n") : s;
}
const HANDLEBARS_BLOCK = /\{\{\s*[#/^]/;
function writerColumnsForCollection(screens, collection) {
  const norm = (c) => typeof c === "string" && c.trim() ? c.trim() : "";
  for (const s of screens) {
    if (!s || typeof s !== "object") continue;
    const sc = s;
    if (!Array.isArray(sc.steps) || !Array.isArray(sc.inputs) || !sc.inputs.length) continue;
    const writes = sc.steps.some((st) => st && typeof st === "object" && st.op === "data.create" && norm(st.collection) === collection);
    if (writes) {
      const cols = sc.inputs.map((i) => i?.name).filter((n) => typeof n === "string" && !!n);
      if (cols.length) return cols;
    }
  }
  return [];
}
function removeBrokenSheetWrites(screens) {
  let removed = false;
  for (const s of screens) {
    if (!s || typeof s !== "object") continue;
    const sc = s;
    if (!Array.isArray(sc.steps)) continue;
    const steps = sc.steps;
    const brokenRemove = /* @__PURE__ */ new Set();
    for (let i = 0; i < steps.length; i++) {
      const st = steps[i];
      if (!st || typeof st !== "object") continue;
      const op = st.op;
      if (op !== "google.sheets.append" && op !== "google.sheets.update") continue;
      const vr = st.google?.values;
      let broken = false;
      let pIdx = -1;
      if (typeof vr === "string" && vr.startsWith("$")) {
        const base = vr.slice(1).split(".")[0];
        pIdx = steps.findIndex((p) => p && typeof p === "object" && p.as === base);
        const prod = pIdx >= 0 ? steps[pIdx] : void 0;
        broken = !!prod && (prod.op === "ai.infer" || prod.op === "transform" && typeof prod.template === "string" && HANDLEBARS_BLOCK.test(prod.template));
      }
      if (!broken) continue;
      brokenRemove.add(i);
      if (pIdx >= 0) {
        const base = vr.slice(1).split(".")[0];
        const pOp = steps[pIdx].op;
        const usedElsewhere = steps.some((p, j) => j !== pIdx && j !== i && p && typeof p === "object" && JSON.stringify(p).includes("$" + base));
        if (!usedElsewhere && (pOp === "transform" || pOp === "ai.infer")) brokenRemove.add(pIdx);
      }
    }
    if (brokenRemove.size) {
      sc.steps = steps.filter((_, j) => !brokenRemove.has(j));
      removed = true;
      const list = sc.steps.find((st) => st && typeof st === "object" && st.op === "data.list");
      const hasAppend = sc.steps.some((st) => st && typeof st === "object" && st.op === "google.sheets.append");
      if (list && typeof list.as === "string" && list.as && !hasAppend) {
        const coll = typeof list.collection === "string" && list.collection.trim() ? list.collection.trim() : "";
        const cols = writerColumnsForCollection(screens, coll);
        if (cols.length) {
          sc.steps.push(
            { op: "transform", as: "gsheet_grid", from: "$" + list.as, columns: cols },
            { op: "google.sheets.append", google: { id: "$config.sheet_id", values: "$gsheet_grid" } }
          );
        }
      }
    }
...
function injectGoogleSheetAppend(def, spec) {
  const us = specUserTurns(spec);
  const wantsSheet = /(スプレッドシート|Google\s*シート|グーグルシート|Googleスプレッド|スプシ)/i.test(us) && /(記録|追記|書き込|保存|反映|記入|入力|集計|蓄積|残す|エクスポート|連携)/.test(us);
  if (!wantsSheet) return false;
  const screens = Array.isArray(def.screens) ? def.screens : [];
  const removed = removeBrokenSheetWrites(screens);
  if ([...collectDefOps(def)].some((o) => o.startsWith("google."))) {
    def.permissions = [.../* @__PURE__ */ new Set([...Array.isArray(def.permissions) ? def.permissions : [], ...validateDefinition(def).requiredPermissions])];
    return removed;
  }
  const target = screens.find((s) => {
    if (!s || typeof s !== "object") return false;
    const sc = s;
    return Array.isArray(sc.inputs) && sc.inputs.length > 0 && Array.isArray(sc.steps) && sc.steps.some((st) => st && typeof st === "object" && st.op === "data.create") && !sc.steps.some((st) => st && typeof st === "object" && st.op === "google.sheets.append");
  });
  if (!target || !Array.isArray(target.steps)) return removed;
  const names = (target.inputs ?? []).map((i) => i?.name).filter((n) => typeof n === "string" && !!n);
  if (!names.length) return removed;
  const rowTemplate = '{"rows":[[' + names.map((n) => `"{{${n}}}"`).join(",") + "]]}";
  const idx = target.steps.findIndex((st) => st && typeof st === "object" && st.op === "data.create");
  target.steps.splice(
    idx + 1,
    0,
    // data.create の直後＝保存に成功してから追記
    { op: "transform", as: "gsheet_row", template: rowTemplate },
    { op: "google.sheets.append", google: { id: "$config.sheet_id", values: "$gsheet_row.rows" } }
  );
  const perms = new Set(Array.isArray(def.permissions) ? def.permissions : []);
  perms.add("google:sheets:write");
  def.permissions = [...perms];
  return true;
}
function checkRequestedFeatureOps(spec, def) {
  const s = specUserTurns(spec);
  if (!s.trim() || !def || typeof def !== "object") return [];
  const ops = collectDefOps(def);
  const missing = [];
  for (const f of REQUESTED_FEATURES) {
    if (f.intent.test(s) && !f.has(ops, def)) missing.push(`要求された「${f.label}」が実装されていません（対応する処理が見当たりません）。`);
  }
  return missing;
}
...
function specForPrompt(spec, maxChars) {
  if (spec.length <= maxChars) return spec;
  const head = Math.floor(maxChars * 0.8);
  const tail = Math.max(0, maxChars - head - 24);
  return spec.slice(0, head) + "\n…（中略：全文は保存済みの要件に格納）…\n" + spec.slice(spec.length - tail);
}
async function startAppBuild(ctx2, a) {
  if (a.sessionId) {
    const dup = await ctx2.db.first(
      "SELECT id FROM app_builds WHERE owner=? AND session_id=? AND (kind='build' OR kind IS NULL) AND status IN ('planning','building') AND created_at > ? ORDER BY created_at DESC LIMIT 1",
      [a.owner, a.sessionId, nowSec() - 180]
    ).catch(() => null);
    if (dup?.id) {
      await logBuild(ctx2.env, dup.id, "二重生成を抑止", "同session の進行中ビルドを再利用（新規生成せず）", "warn");
      return dup.id;
    }
  }
  ...
  await ctx2.db.run(
    ...
    // 要件を保持する（第三者評価P0）が、byte 基準で 384KiB に切る＝D1 行上限(2MB)に安全に収まり、日本語≒12.8万字・
    // 英語≒38万字まで全文保持する。文字数 cap(旧 1,000,000字)だと日本語 UTF-8 で最大3MBとなり INSERT が例外になる
    // ため byte cap にする（Fable5実証）。これを超える1MB級の仕様書は次段の要件台帳(別保存・チャンク)で全文を扱う。
    // 下流の各工程は specForPrompt で入力予算内に抜粋する（全文をそのまま LLM に渡すと 200Kトークン超で全滅する）。
    [id, a.owner, a.sessionId ?? null, a.model ?? null, a.paid ? 1 : 0, capBytes(a.spec, 384 * 1024), a.uiMode ?? null, now, now]
  );
  return id;
}
function hashStr(s) {
...
async function kickBuild(ctx2, id, baseUrl = "", src = "start") {
  const env = ctx2.env;
  const driver = await getBuildDriver(env).catch(() => "cron");
  await logBuild(env, id, "kick", `src=${src} driver=${driver}`);
  if (driver === "do") {
    try {
      const ns = env.BUILD_DO;
      if (ns) {
        await ns.get(ns.idFromName(id)).fetch(new Request("https://internal/kick", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ buildId: id, origin: baseUrl }) }));
        return;
      }
    } catch {
    }
  ...
  if (wu) wu((async () => {
    for (let i = 0; i < 4; i++) {
      if (!await processAppBuild(ctx2, id, baseUrl, { src: `kick-fallback:${src}` }).catch(() => false)) break;
    }
  })());
}
...
async function startAppEdit(ctx2, a) {
  const dup = await ctx2.db.first(
    "SELECT id FROM app_builds WHERE owner=? AND app_id=? AND kind='edit' AND status IN ('planning','building') AND created_at > ? ORDER BY created_at DESC LIMIT 1",
    [a.owner, a.appId, nowSec() - 90]
  ).catch(() => null);
  if (dup?.id) {
    await logBuild(ctx2.env, dup.id, "二重編集を抑止", "同appの進行中編集を再利用（新規生成せず）", "warn");
    return dup.id;
  }
...
async function resolveAppByName(ctx2, message) {
  ...
  if (top.length === 1) return { appId: top[0].id, name: top[0].name };
...
function applyPatch(curDef, patch) {
  ...
  if (patch.definition && typeof patch.definition === "object" && isRunnableDefinition(patch.definition)) {
    const nextDef = patch.definition;
    const curHtml = curDef.render?.html;
    if (typeof curHtml === "string" && curHtml.trim()) {
      nextDef.render = curDef.render;
      const byId = /* @__PURE__ */ new Map();
      for (const sc of Array.isArray(curDef.screens) ? curDef.screens : []) {
        const sid = sc?.id;
        if (typeof sid === "string" && sid) byId.set(sid, sc);
      }
      for (const sc of Array.isArray(nextDef.screens) ? nextDef.screens : []) {
        const sid = sc?.id;
        if (typeof sid === "string" && sid) byId.set(sid, sc);
      }
      if (byId.size) nextDef.screens = [...byId.values()];
    }
    def = { ...curDef, ...nextDef };
    if (curDef.id != null) def.id = curDef.id;
    changed = true;
    ...
    for (const e of patch.htmlEdits) {
      ...
      if (countOccurrences(html, e.find) === 1) {
        html = html.split(e.find).join(String(e.replace ?? ""));
        changed = true;
      }
    }
...
async function editStep(ctx2, row, model) {
  ...
  if (row.status === "planning") {
    ...
    const planJson = JSON.stringify({ name: design.name, phases: phases2 });
    const defJson = JSON.stringify(curDef);
    const wonPlan = await ctx2.db.run(
      "UPDATE app_builds SET status='building', name=?, plan=?, definition=?, cursor=0, attempts=0, updated_at=? WHERE id=? AND status='planning'",
      [design.name, planJson, defJson, nowSec(), row.id]
    );
    if (!wonPlan.rowsWritten) return false;
    row.status = "building";
    row.plan = planJson;
    row.definition = defJson;
    row.cursor = 0;
    row.attempts = 0;
  ...
  if (!plan) {
    await failBuild(ctx2, row, explainStop("system", "編集プランのデータが壊れているため続行できません。", "お手数ですが、対象を絞ってもう一度ご依頼ください。"), "plan-corrupt");
    return false;
  }
  const phases = plan.phases ?? [];
  let def;
  if (row.definition) {
    const parsedDef = parseJsonObject(row.definition);
    if (!parsedDef) {
      await failBuild(ctx2, row, explainStop("system", "アプリ定義のデータが壊れているため続行できません。", "お手数ですが、対象アプリを作り直すか、もう一度ご依頼ください。"), "def-corrupt");
      return false;
    }
    def = parsedDef;
  } else {
...
【前回は出力が長すぎて途中で切れました。今度は必ず1回で収める】(1)定義全体を作り直さない。(2)definition を返すときは render を絶対に含めない（現行 render.html は自動保持される）＝変更/追加する screen だけを screens[] に入れる。(3)見た目・文言・JS だけの変更は htmlEdits（find/replace）の最小差分で返す。(4)有効なJSONパッチ1個だけを返す。`;
    const r3 = await makePatch(env, model, def, tight, row.paid === 1, { maxTokens: row.paid === 1 ? 48e3 : 2e4 });
    patch = r3.patch;
    truncated = r3.truncated;
    text = r3.text;
  }
  ...
  if (patch) {
    ...
    if (applied.changed) {
      ...
      for (const s of Array.isArray(probe.screens) ? probe.screens : []) repairMissingRecFrom(s);
      repairMissingRecFrom(probe);
      probe.permissions = [.../* @__PURE__ */ new Set([...Array.isArray(probe.permissions) ? probe.permissions : [], ...validateDefinition(probe).requiredPermissions])];
      const vr = validateDefinition(probe);
      if (vr.ok) {
        nextDef = probe;
        phaseStatus = "done";
        ...
        phaseStatus = "failed";
        await logBuild(env, row.id, `edit 工程${row.cursor + 1} validateNG`, vr.issues.slice(0, 3).map((i) => `${i.path}: ${i.message}`).join(" / ").slice(0, 220), "warn");
      }
    } else {
...
async function repairDeadListCollections(ctx2, row, def, plan, model) {
  const env = ctx2.env;
  try {
    const norm = (c) => typeof c === "string" && c ? c : "__default__";
    const deadCol = collectionsNeedingWriter(def).slice(0, row.paid === 1 ? 4 : 1);
    for (const rawCol of deadCol) {
      const col = rawCol === "__default__" ? "" : rawCol;
      const rhtml = String(def.render?.html ?? "") || null;
      const existIds = (def.screens ?? []).map((s) => String(s.id ?? "")).filter(Boolean);
      const readerHint = (def.screens ?? []).filter((s) => (s.steps ?? []).some((st) => st && typeof st === "object" && (st.op === "data.list" || st.op === "data.get") && norm(st.collection) === rawCol)).map((s) => {
        const o = s;
        return { id: o.id, inputs: o.inputs, output: o.output };
      }).slice(0, 2);
      const ph = { title: `${col || "データ"}の保存`, goal: `一覧が data.list する collection「${col || "(既定)"}」に登録/更新する保存画面。既存一覧が表示する列に対応する inputs を用意し、data.create（一意キーあれば keyFields 付き data.upsert）で${col ? `collection「${col}」へ` : ""}必ず保存する（data.list だけにしない）。ビルド時なので既存レコードは無い。参照する既存一覧画面：${JSON.stringify(readerHint).slice(0, 800)}`, kind: "screen", status: "todo" };
      const cand = await implementScreen(model, plan, row.spec ?? plan.description, ph, existIds, row.paid === 1, 1);
      const okCov = cand.values.find((s) => screenIsValid(plan, s, rhtml) && !screenMissingWrite(s) && (s.steps ?? []).some((st) => st && typeof st === "object" && (st.op === "data.create" || st.op === "data.upsert")));
      if (okCov) {
        if (col) for (const st of okCov.steps ?? []) {
          const o = st;
          if ((o.op === "data.create" || o.op === "data.upsert") && norm(o.collection) !== rawCol) o.collection = col;
        }
        def.screens = [...def.screens ?? [], okCov];
        def.permissions = [.../* @__PURE__ */ new Set([...def.permissions ?? [], ...validateDefinition(def).requiredPermissions])];
        reconcileRenderScreenRefs(def);
        await logDiag(env, "info", "build", `dead-list repaired: ${col || "(既定)"}`).catch(() => {
        });
      }
    }
  } catch {
...
async function finalizeEdit(ctx2, row, model, design, def, phases) {
  ...
  const reconEColl = reconcileDataCollections(def);
  if (reconEColl.length) await logBuild(env, row.id, "editFinalize collection整合", reconEColl.join(" / "));
...
検証エラー：${issueText}`;
  ...
  const editPlanLite = { id: row.app_id ?? design.name, name: design.name, description: design.definition?.description ?? "", goal: design.definition?.goal ?? row.spec ?? design.spec ?? "", permissions: Array.isArray(def.permissions) ? def.permissions : [], isCustomUI: true, phases: [] };
  await repairUnwiredRenderWires(ctx2, row, def, editPlanLite, model).catch(() => {
  });
  await repairDeadListCollections(ctx2, row, def, editPlanLite, model).catch(() => {
  });
  try {
    const rwE = reconcileWriteFormToReadScreen(def);
    if (rwE.length) await logBuild(env, row.id, "editFinalize 保存フォーム張替", rwE.join(" / "));
  } catch {
  }
  ...
  const editUnwired = unwiredRenderWriteScreens(def);
  const editStructural = [];
  {
    const v = checkReadOnlyCollections(def);
    if (v) editStructural.push(v);
  }
  {
    const v = checkDataCollections(def);
    if (v) editStructural.push(v);
  }
  {
    const v = checkWriteScreenEmptyBoRun(def);
    if (v) editStructural.push(v);
  }
  {
    const v = checkWriteScreenArgKeyMismatch(def);
    if (v) editStructural.push(v);
  }
  for (const mf of checkRequestedFeatureOps(row.spec ?? "", def)) editStructural.push(mf);
  const editUnmet = failed.length > 0 || !!uiCheck.note || editUnwired.length > 0 || editStructural.length > 0;
  ...
  const review = editUnmet ? await reviewApp(model, row.spec ?? design.spec ?? "", editGoal, def, row.paid === 1, editCrit).catch(() => ({ issues: [], summary: "" })) : { issues: [] };
  const editStatus = editUnmet ? "done_partial" : "done";
  const editReason = editUnmet ? "partial" : "completed";
  ...
  const editRuntimeIssue = !!uiCheck.note || failed.length > 0 || editUnwired.length > 0 || editStructural.length > 0;
  ...
  if (editUnwired.length) editSuggestions.unshift(`保存画面「${editUnwired.map((u) => u.id).join("・")}」がUIのどのボタンからも呼ばれていません＝画面から保存できません。render に入力フォームと保存ボタンを置き bo.run("${editUnwired[0].id}", {各入力名: 値}) を呼ぶよう直してください`);
  for (const v of editStructural) if (!editSuggestions.includes(v)) editSuggestions.push(v);
...
const BUILD_COLS = "id,owner,session_id,model,paid,status,spec,plan,definition,cursor,attempts,created_at,updated_at,kind,app_id,ui_mode,render_sections";
...
できる範囲（${feas.feasibleScope || "可能な部分"}）を作ります。作れない部分は、下の「📋 外部AI用プロンプトをコピー」で外部AI（ChatGPT 等）に依頼できます。`;
  ...
  const plan = parseJsonObject(row.plan ?? "{}");
  if (!plan) {
    await failBuild(ctx2, row, explainStop("system", "実装プランのデータが壊れているため続行できません。", "お手数ですが、要件を分けてもう一度ご依頼ください。"), "plan-corrupt");
    return false;
  }
  const def = row.definition ? parseJsonObject(row.definition) : assembleDefinition(plan, [], null);
  if (!def) {
    await failBuild(ctx2, row, explainStop("system", "アプリ定義のデータが壊れているため続行できません。", "お手数ですが、対象アプリを作り直してください。"), "def-corrupt");
    return false;
  }
  const phases = plan.phases ?? [];
  if (row.cursor >= phases.length) return finalize$1(ctx2, row, plan, def, model);
  const phase = phases[row.cursor];
  const canPromote = phase.kind !== "render" && !row.model && phaseIsComplex(phase) && !!await getApiKey(env, "claude").catch(() => null);
  const stepModel = canPromote ? await buildChatModel(env, void 0, "plan").catch(() => null) ?? model : model;
  if (stepModel !== model) await logBuild(env, row.id, `工程${row.cursor + 1} model昇格`, `複雑工程のため Opus で生成（${phase.title}）`).catch(() => {
  });
  const screens = Array.isArray(def.screens) ? def.screens : [];
  const html = def.render?.html ?? null;
  try {
    if (phase.kind === "render") {
      const builtScreens = builtScreensFromDef(def);
      const prior = typeof html === "string" && html.trim() && looksTruncated(html) && html.length <= RENDER_HTML_MAX ? html : null;
      let out;
      if (prior) {
        out = await continueRenderHtml(env, model, prior, paid, 2, false, { builtScreens }).catch(() => prior);
      } else {
        const onSection = async (done, total) => {
          await ctx2.db.run("UPDATE app_builds SET progress_detail=?, updated_at=? WHERE id=? AND status IN ('planning','building')", [`${done}/${total}`, nowSec(), row.id]).catch(() => {
          });
        };
        const priorSplit = parseRenderSections(row.render_sections);
        const onFragments = async (data) => {
          await ctx2.db.run("UPDATE app_builds SET render_sections=?, updated_at=? WHERE id=? AND status IN ('planning','building')", [JSON.stringify(data), nowSec(), row.id]).catch(() => {
          });
        };
        out = plan.minimal ? null : await implementRenderSplit(env, model, plan, row.spec ?? "", builtScreens, paid, onSection, { prior: priorSplit, onFragments }).catch(() => null);
        await ctx2.db.run("UPDATE app_builds SET render_sections=NULL WHERE id=?", [row.id]).catch(() => {
        });
        if (!out) {
          await ctx2.db.run("UPDATE app_builds SET progress_detail=NULL WHERE id=?", [row.id]).catch(() => {
          });
          out = await implementRender(env, model, plan, row.spec ?? "", builtScreens, paid, { rounds: 2, returnPartial: true });
        }
      }
      ...
      if (usable && !looksTruncated(usable)) {
        def.render = renderObjFor(usable);
        phase.status = "done";
      } else if ((!usable || looksTruncated(usable)) && (row.attempts ?? 0) < 2) {
        const bumped = await ctx2.db.run("UPDATE app_builds SET attempts=attempts+1, updated_at=? WHERE id=? AND cursor=? AND status IN ('planning','building')", [nowSec(), row.id, row.cursor]);
        if (!bumped.rowsWritten) return false;
        row.attempts = (row.attempts ?? 0) + 1;
        await logBuild(env, row.id, `工程${row.cursor + 1} render:再試行`, `attempts=${row.attempts}（生成失敗/タイムアウト→再生成）`, "warn");
        return true;
      } else {
        phase.status = "failed";
        phase.failReason = usable ? "UI生成が途中で切れ完成しませんでした（大きすぎ/未収束）" : "UIを生成できませんでした（複数回タイムアウト/空応答）";
      }
      ...
      for (let a = 0; a < 2 && !added.length; a++) {
        ...
        const cand = await implementScreen(stepModel, plan, row.spec ?? "", phase, builtIds, paid, a, fb);
        if (!cand.values.length) {
          reason = cand.truncated ? "empty_or_truncated" : "empty_or_unparsable";
          continue;
        }
        for (const s of cand.values) repairMissingRecFrom(s);
        if (a === 1) for (let i = 0; i < cand.values.length; i++) {
          const sv = salvageScreenDroppingBadIntegrationSteps(plan, cand.values[i], html);
          if (sv) {
            cand.values[i] = sv;
            await logBuild(env, row.id, `工程${row.cursor + 1} screen:salvage`, `統合stepの無効参照で画面ごと失う所を、統合stepを外して保存機能を残しました id=${String(sv.id ?? "")}`, "warn");
          }
        }
        const ok = cand.values.filter((s) => screenIsValid(plan, s, html) && !screenMissingWrite(s));
        if (ok.length) {
          added = ok;
          if (ok.length < cand.values.length) phase.failReason = `partial_screens:${ok.length}/${cand.values.length}`;
        } else if (cand.values.some((s) => screenIsValid(plan, s, html) && screenMissingWrite(s))) reason = "invalid_structure:この画面は保存/登録/更新の役割ですが書き込み操作がありません＝入力しても保存されません。data.create か data.update か、一意保存（同じ組合せで1件）なら keyFields 付き data.upsert を steps に必ず入れてください。";
      ...
      if (added.length) {
        ...
        const saveIntent = /(保存|登録|作成|追加|更新|記録|管理|upsert|save|create)/i.test(`${phase.title} ${phase.goal ?? ""}`);
        if (saveIntent) {
          const READ = /* @__PURE__ */ new Set(["data.list", "data.get"]);
          const norm = (c) => typeof c === "string" && c ? c : "__default__";
          const readInPhase = /* @__PURE__ */ new Set();
          for (const s of added) for (const st of s.steps ?? []) if (st && typeof st === "object" && READ.has(st.op)) readInPhase.add(norm(st.collection));
          const dead = new Set(collectionsNeedingWriter(def, { suppressWhenNoWriters: false }));
          const targetCol = [...readInPhase].find((c) => dead.has(c));
          if (targetCol) {
            const colName = targetCol === "__default__" ? "" : targetCol;
            const covIds = screens.map((s) => String(s.id ?? "")).filter(Boolean);
            const covPhase = { title: `${colName || "データ"}の保存`, goal: `一覧はあるが保存が無い「${colName || "(既定)"}」の保存画面。data.create（一意保存なら keyFields 付き data.upsert）で${colName ? `collection「${colName}」へ` : ""}必ず保存する（data.list だけにしない）。入力は一覧が表示する列に対応させる。`, kind: "screen", status: "todo" };
            const cov = await implementScreen(stepModel, plan, row.spec ?? "", covPhase, covIds, paid, 1).catch(() => ({ values: [], truncated: false }));
            for (const s of cov.values) repairMissingRecFrom(s);
            const okCov = cov.values.find((s) => screenIsValid(plan, s, html) && !screenMissingWrite(s) && (s.steps ?? []).some((st) => st && typeof st === "object" && (st.op === "data.create" || st.op === "data.upsert")));
            if (okCov) {
              if (colName) for (const st of okCov.steps ?? []) {
                const o = st;
                if ((o.op === "data.create" || o.op === "data.upsert") && norm(o.collection) !== targetCol) o.collection = colName;
              }
              screens.push(okCov);
              def.screens = screens;
              await logBuild(env, row.id, `工程${row.cursor + 1} screen:coverage`, `保存画面を補完 collection=${colName || "(既定)"} id=${String(okCov.id ?? "")}`);
            }
          }
        }
      } else {
  ...
  const adv = await ctx2.db.run(
    "UPDATE app_builds SET plan=?, definition=?, cursor=?, attempts=0, progress_detail=NULL, updated_at=? WHERE id=? AND cursor=? AND status IN ('planning','building')",
...
async function verifyAndRepairUi(ctx2, model, def, paid, onStep, maxPass = 3, rowId) {
  ...
  if (looksTruncated(html0)) {
    ...
    const completed = await continueRenderHtml(env, model, html0, paid, 4, false, { builtScreens: builtScreensFromDef(def) }).catch(() => html0);
  ...
  let cur = def;
  if (rowId && typeof html0 === "string" && !checkRenderScripts(html0)) {
    try {
      const pe = checkRenderScriptsParsed(html0);
      if (pe) await logBuild(env, rowId, "uiCheck 実パース警告", pe, "warn");
    } catch {
    }
  }
  if (rowId && typeof html0 === "string") {
    try {
      const be = checkRenderBusReachability(html0);
      if (be) await logBuild(env, rowId, "uiCheck バス到達警告", be, "warn");
    } catch {
    }
  }
  let prevIssue = "";
  for (let pass = 0; pass < maxPass; pass++) {
    ...
    const issue = checkRenderScripts(h2) || checkRenderHandlers(h2) || checkRenderDomRefs(cur) || checkRenderDataKeys(cur) || checkRenderScreens(cur) || checkRenderBusReachability(h2);
    ...
    if (rowId) await logBuild(env, rowId, `uiCheck pass${pass} 検出`, issue.slice(0, 400), "warn");
...
onclick 等のイベントが呼ぶ関数を <script> 内に正しく実装する、または呼び出し名を既存の関数に合わせる。`;
    ...
    if (!r.patch) {
      if (rowId) await logBuild(env, rowId, `uiCheck pass${pass} 修復`, r.truncated ? "patch生成が出力上限で切れ未修復" : "patch生成できず未修復", "warn");
      break;
    }
    const applied = applyPatch(cur, r.patch);
    if (!applied.changed) {
      if (rowId) await logBuild(env, rowId, `uiCheck pass${pass} 修復`, "find不一致で無変更（未修復）", "warn");
      break;
    }
    const probe = JSON.parse(JSON.stringify(applied.def));
    repairOutputLiterals(probe);
    if (!validateDefinition(probe).ok) {
      if (rowId) await logBuild(env, rowId, `uiCheck pass${pass} 修復`, "適用後の検証NGで不採用（未修復）", "warn");
      break;
    }
    cur = probe;
    if (rowId) await logBuild(env, rowId, `uiCheck pass${pass} 修復`, "適用OK", "info");
  }
  let h = cur.render.html ?? "";
  let remain = checkRenderScripts(h) || checkRenderHandlers(h) || checkRenderDomRefs(cur) || checkRenderDataKeys(cur) || checkRenderScreens(cur) || checkRenderBusReachability(h);
  if (remain && h.trim()) {
    await hb();
    if (onStep) await onStep("🛠 ボタンが直り切らないため、カスタムUIを作り直しています…").catch(() => {
    });
    if (rowId) await logBuild(env, rowId, "uiCheck 全書き直し着手", `find/replace で直し切れず残: ${remain.slice(0, 200)}`, "warn");
    const rewritten = await rewriteRenderHtml(env, model, h, remain, paid, builtScreensFromDef(cur)).catch(() => null);
    if (rewritten) {
      const probe = JSON.parse(JSON.stringify(cur));
      probe.render.html = rewritten;
      repairOutputLiterals(probe);
      const stillBad = checkRenderScripts(rewritten) || checkRenderHandlers(rewritten) || checkRenderDomRefs(probe) || checkRenderDataKeys(probe) || checkRenderScreens(probe) || checkRenderBusReachability(rewritten);
      if (!stillBad && validateDefinition(probe).ok) {
        cur = probe;
        h = rewritten;
        remain = "";
      } else if (rowId) await logBuild(env, rowId, "uiCheck 全書き直し", stillBad ? `書き直し後も残る: ${stillBad.slice(0, 120)}` : "書き直し後の検証NGで不採用", "warn");
    } else if (rowId) await logBuild(env, rowId, "uiCheck 全書き直し", "書き直しHTMLを生成できず（出力切れ/失敗）", "warn");
  }
  if (!remain && typeof h === "string" && h.trim() && !checkRenderScripts(h)) {
    try {
      const pe = checkRenderScriptsParsed(h);
      if (pe) {
        remain = pe;
        if (rowId) await logBuild(env, rowId, "uiCheck 実パースで確定SyntaxError昇格", pe.slice(0, 200), "warn");
      }
    } catch {
    }
  }
  if (rowId) await logBuild(env, rowId, "uiCheck 終了", remain ? `未解決＝done_partial 注記: ${remain.slice(0, 160)}` : "UI検査 全通過（ボタン配線OK）", remain ? "warn" : "info");
...
async function dryRunGate(ctx2, row, plan, def, model) {
  ...
  const uiIssue = (x) => {
    ...
    return (h ? checkRenderScripts(h) || checkRenderHandlers(h) || checkRenderDomRefs(x) : null) || checkRenderDataKeys(x) || checkRenderScreens(x) || null;
  };
  ...
  await logBuild(env, row.id, `dryRun 失敗 ${fails.length}件→修復`, fails.map((f) => `${f.unit}: ${f.msg}`).join(" / ").slice(0, 400), "warn");
  ...
  await logBuild(env, row.id, "dryRun 終了", fails.length ? `未解決 ${fails.length}件（done_partial 注記）: ${fails.slice(0, 2).map((f) => f.unit).join(",")}` : "全画面 dry-run OK", fails.length ? "warn" : "info");
  return { def, note };
}
...
async function judgeCoverage(model, spec, plan, def) {
  ...
  const sys = "あなたはアプリの受入検査官です。利用者の要望に対し、実装が『明らかに欠けている要求』だけを列挙してください。実装済みのもの・細かな改善・好み・装飾は挙げない。要望で具体的に求められた画面/機能のうち、実装（画面一覧・カスタムUIの見出し/ボタン）に見当たらないものだけを、利用者の言葉で簡潔に。『誰が使うか・利用者層・閲覧/アクセス範囲・権限の対象（例：社員全員が使う）』は機能ではなく公開範囲の設定で扱うため、未実装として挙げない。各項目には、その要求が要望文に書かれている根拠となる**原文の一部を quote に必ず入れる**（要望文に無い『あると良い』は絶対に挙げない）。JSON のみ・最大5件・無ければ空配列。";
...
async function repairUnwiredRenderWires(ctx2, row, def, plan, model) {
  const env = ctx2.env;
  const targets = unwiredRenderWriteScreens(def);
  if (!targets.length) return;
  const paid = row.paid === 1;
  const cap = paid ? 2 : 1;
  const adopts = async (patchDef, tid) => {
    if (!patchDef) return false;
    if (unwiredRenderWriteScreens(patchDef).some((u) => u.id === tid)) return false;
    if (String(patchDef.render?.html ?? "").length > RENDER_HTML_MAX) return false;
    if (checkWriteScreenEmptyBoRun(patchDef) || checkWriteScreenArgKeyMismatch(patchDef)) return false;
    return validateDefinition(patchDef).ok;
  };
  for (const t of targets.slice(0, cap)) {
    const hb = setInterval(() => {
      ctx2.db.run("UPDATE app_builds SET updated_at=? WHERE id=? AND status IN ('planning','building')", [nowSec(), row.id]).catch(() => {
      });
    }, 3e4);
    try {
      const wanted = t.inputs.filter((n) => n !== "recordId");
      let candidate = null;
      if (t.deadButtonId) {
        const base = `対象アプリの render.html を最小差分で直す。id="${t.deadButtonId}" の保存ボタンを押したとき、同じフォーム内の入力値を集めて window.bo.run("${t.id}", { ${wanted.map((n) => `${n}: <対応する入力欄の値>`).join(", ")} }) を呼び、返り値 res.ok が false ならエラーをトースト表示、true なら window.__app.emit("changed") を発火するよう、そのボタンの click 導線（無ければ addEventListener('click', …) を追加）を結線する。`;
        for (let attempt = 0; attempt < 2 && !candidate; attempt++) {
          const instr = base + (attempt === 0 ? `
【厳守】screen「${t.id}」は既に定義済み。definition を返しても render は現行保持され配線されない＝効果ゼロ。必ず render.html への htmlEdits（find/replace）だけで返すこと。` : `
【再厳守】前回 definition を返したが無効。htmlEdits 以外は一切禁止。definition/screens キーを含めず、render.html の find/replace のみで返せ。`);
          const r = await makePatch(env, model, def, instr, paid).catch(() => ({ patch: null }));
          if (r.patch) {
            const ap = applyPatch(def, r.patch);
            if (ap.changed && await adopts(ap.def, t.id)) candidate = ap.def;
          }
        }
      }
      if (!candidate) {
        const section = { id: `sec_${t.id}`.slice(0, 40), title: `${t.collection || "データ"}の保存`, purpose: `collection「${t.collection || "(既定)"}」への保存フォーム＝入力欄(${wanted.join(", ") || "必要項目"})と保存ボタンを置き、押下で bo.run("${t.id}", {各入力名: 値}) を呼び ok 確認後 __app.emit("changed") する`, wireScreenIds: [t.id] };
        const frag = await generateSection(env, model, plan, row.spec ?? plan.description, section, builtScreensFromDef(def), paid, 1).catch(() => null);
        if (frag && frag.trim()) {
          const probe = JSON.parse(JSON.stringify(def));
          const spliced = spliceSectionIntoHtml(String(probe.render?.html ?? ""), frag);
          if (spliced) {
            probe.render.html = spliced;
            if (await adopts(probe, t.id)) candidate = probe;
          }
        }
      }
      if (candidate) {
        def.render = candidate.render;
        def.screens = candidate.screens ?? def.screens;
        def.permissions = [.../* @__PURE__ */ new Set([...def.permissions ?? [], ...validateDefinition(def).requiredPermissions])];
        reconcileRenderScreenRefs(def);
        await logBuild(env, row.id, "配線修復", `${t.id} を render に結線（${t.deadButtonId ? "死ボタン結線" : "フォーム生成"}）`);
      } else {
        await logBuild(env, row.id, "配線修復スキップ", `${t.id}: 採用ゲート不成立（partial のまま）`, "warn");
      }
    } catch {
    } finally {
      clearInterval(hb);
...
async function finalize$1(ctx2, row, plan, def, model) {
  ...
  const reconColl = reconcileDataCollections(def);
  if (reconColl.length) await logBuild(env, row.id, "finalize collection整合", reconColl.join(" / "));
  const collIssue = checkDataCollections(def);
  if (collIssue) await logBuild(env, row.id, "collection不整合(残)", collIssue.slice(0, 200), "warn");
  ...
  if (plan.isCustomUI) {
    try {
      ...
      for (const sid of missing) {
        const ph = { title: sid, goal: `カスタムUIが bo.run("${sid}") で呼ぶデータ画面。${sid} に対応する操作を data.* で実装する。保存/登録/更新/upsert の画面なら必ず書き込み op（data.create／data.update／一意保存は keyFields 付き data.upsert）を含める（data.list だけにしない）`, kind: "screen", status: "todo" };
        const cand = await implementScreen(model, plan, row.spec ?? plan.description, ph, built, row.paid === 1, 1);
        const okScreen = cand.values.map((s) => ({ ...s, id: sid })).find((s) => screenIsValid(plan, s, rhtml) && !screenMissingWrite(s));
        if (okScreen) {
  ...
  await repairDeadListCollections(ctx2, row, def, plan, model);
  if (plan.isCustomUI) {
    try {
      const rw = reconcileWriteFormToReadScreen(def);
      if (rw.length) await logBuild(env, row.id, "finalize 保存フォーム張替", rw.join(" / "));
    } catch {
    }
  }
  if (plan.isCustomUI) await repairUnwiredRenderWires(ctx2, row, def, plan, model).catch(() => {
  });
  const uiCheck = await verifyAndRepairUi(ctx2, model, def, row.paid === 1, (m) => post(ctx2, row, m), 3, row.id);
  def = uiCheck.def;
  ...
  def = trig.def;
  if (trig.added > 0) {
    const rc2 = reconcileDataCollections(def);
    if (rc2.length) await logBuild(env, row.id, "トリガ後 collection再整合", rc2.join(" / "));
  }
  ...
  if (injectGoogleSheetAppend(def, row.spec ?? plan.description)) await logBuild(env, row.id, "Googleシート追記を注入", "主な保存画面に google.sheets.append(id:$config.sheet_id) を決定的注入＝保存の都度1件をシートへ追記（連携設定でシートID入力が必要）", "info");
  ...
  const emptyWrite = checkWriteScreenEmptyBoRun(def);
  if (emptyWrite) structuralIssues.push(emptyWrite);
  const argMismatch = checkWriteScreenArgKeyMismatch(def);
  if (argMismatch) structuralIssues.push(argMismatch);
  const deadList = checkReadOnlyCollections(def);
  if (deadList) structuralIssues.push(deadList);
  const unwiredWrites = unwiredRenderWriteScreens(def);
  if (unwiredWrites.length) structuralIssues.push(`保存画面「${unwiredWrites.slice(0, 3).map((u) => u.id).join("・")}」がUIのどのボタンからも呼ばれていません（画面からは保存できません）。render に入力フォームと保存ボタンを置き、bo.run("${unwiredWrites[0].id}", {各入力名: 値}) を呼んでください。`);
  for (const mf of checkRequestedFeatureOps(row.spec ?? plan.description, def)) structuralIssues.push(mf);
  for (const fk of checkFakeIntegrationSuccess(def)) structuralIssues.push(fk);
  if (collIssue) structuralIssues.push(collIssue);
  let partial = failed.length > 0 || !!uiCheck.note || !!gate.note || structuralIssues.length > 0;
  if (partial) await logBuild(env, row.id, "partial 内訳", `失敗工程=${failed.length}[${failed.slice(0, 3).join(",")}] UI検査=${uiCheck.note ? "残" : "OK"} dryRun=${gate.note ? "残" : "OK"} 構造=${structuralIssues.length ? structuralIssues.join("／").slice(0, 150) : "OK"}`, "warn");
  const bigBuild = (plan.phases ?? []).length >= 5 || (plan.successCriteria?.length ?? 0) >= 3 || !!plan.isCustomUI && screenCount() >= 5;
  const reviewGate = await ctx2.env.LICENSE.get("builder_review_gate").catch(() => null);
  const review = partial || bigBuild && (reviewGate === "observe" || reviewGate === "downgrade") ? await reviewApp(model, row.spec ?? plan.description, plan.goal, def, row.paid === 1, plan.successCriteria).catch(() => ({ issues: [], summary: "" })) : { issues: [] };
  const reviewErrors = review.issues.filter((i) => i.severity === "error");
  if (reviewErrors.length) await logBuild(env, row.id, "reviewApp error指摘(観測)", reviewErrors.map((i) => i.comment).join(" / ").slice(0, 300), "warn");
  if (reviewGate === "downgrade" && reviewErrors.length) {
    for (const e of reviewErrors) structuralIssues.push(e.comment);
    partial = true;
  }
  ...
  const implOps = collectDefOps(def);
  const implementedIntents = REQUESTED_FEATURES.filter((f) => f.has(implOps, def)).map((f) => f.intent);
  const capabilityNotes = detectCapabilityBoundaries(row.spec ?? plan.description);
  const unmet = pruneUnmetAgainstDonePhases(
    [.../* @__PURE__ */ new Set([...structuralIssues, ...plan.followUps ?? [], ...judged])].filter((u) => !implementedIntents.some((re) => re.test(u))).filter((u) => !isCapabilityBoundaryText(u)),
    plan.phases ?? []
  ).slice(0, 6);
...
※ まだ実装できていない要求：
${unmet.map((u) => `・${u}`).join("\n")}
下の「続けて作って（残りを実装）」で追加できます。` : "") + // #6 P4：能力外は「続けて作って」で作れる風にせず、代替案つきで正直に別掲（status は落とさない）。
    (capabilityNotes.length ? `

⚠️ 仕組み上できないこと（代替案があります）：
${capabilityNotes.map((c) => `・${c}`).join("\n")}` : "") + (plan.planFallback ? `
...
const FIELD_RULE = `inputs[].name は英字始まりの半角識別子（表示名は label）。output は {type∈text/table/file, from は steps の as を "$名" で参照}。データ操作は生SQLを書かず構造化 op（data.*）を使う＝アプリは保存先テーブルや他アプリ/他人のデータに触れない（安全境界）。保存=data.create（{op:'data.create',as:'saved',from:'$rec'}。$rec は画面の入力をまとめたオブジェクト＝複数項目でも transform 不要で from:'$rec' でそのまま保存できる。template で JSON 文字列を手書きしない（入力に含まれる引用符・改行で壊れる。status 等の固定した短い値だけ template 可）。app_id/owner は自動付与＝指定しない）／一覧=data.list（{op:'data.list',as:'rows'}。新しい順に取得し、各行に id が入る＝編集/削除に使える。件数は limit で指定可・既定100）／取得=data.get（{op:'data.get',as:'row',recordId:'$id'}）／更新=data.update（{op:'data.update',recordId:'$id',from:'$rec'}。from はオブジェクトを渡す＝既存データにマージされ、渡したフィールドだけ更新し他は保持する。状態(ステータス)だけ変えるなら status:'$status'（{op:'data.update',recordId:'$id',status:'$status'}＝data列は触らない）か、変更フィールドだけの部分オブジェクトを transform で作って渡す（例 {op:'transform',as:'patch',template:'{"status":"{{status}}"}'}→from:'$patch'）。{{status}} のような単一値を from に直接渡してはいけない＝レコード全体がその文字列に置換され他項目が消える）／削除=data.remove（{op:'data.remove',recordId:'$id'}）／同一の組合せで1件だけ保存（あれば更新・無ければ新規＝upsert）=data.upsert（{op:'data.upsert',from:'$rec',keyFields:['store_id','year_month']}。keyFields が全一致する既存を探し、あれば渡したフィールドをマージ更新・無ければ新規作成する＝『data.list で存在確認してから create/update に分岐』を自分で書かない・これ1つで足りる。「店舗×対象月で1件」のような一意保存の画面は必ず data.upsert を使う。keyFields には業務上の一意な組合せだけを指定し、recordId/id などレコード自身のIDは入れない＝IDは毎回変わるため必ず新規作成になり登録できない）。【保存画面の鉄則】画面の役割が保存/登録/更新/記録/upsert なら、必ず書き込み op（data.create／data.update／data.upsert）を steps に含める＝data.list だけの画面は一覧・検索・表示専用に限る（保存画面が list だけだと入力しても保存されず、一覧も常に空になる）。同じ論理データ（例：月次計測）は保存・一覧・取得・更新の全画面で必ず同一の collection 名にする。一覧→編集/削除のCRUDでは、行ごとに id を持つカスタムUIにし、各ボタンは対象画面に id を渡す。複数画面(screens[])のときは各画面に任意で purpose（その画面が何をするか＝AIがチャット/LINEからいつ呼ぶかの1文）を付ける。UI表示専用・内部データ操作口の画面には aiCallable:false を付ける（AIの直接起動対象から外す・既定は起動可）。アプリ全体に keywords（別名・用途語の配列）を付けると呼び出し精度が上がる。【外部連携op（依頼にある時だけ・無ければ足さない）】外部送信/連携は専用opを使う（生SQL/生fetchは書けない）：外部HTTP=http.fetch（{op:'http.fetch',as:'res',url:'https://実在する既知の公開API',method:'GET'|'POST'}。allowHosts に送信先ホストを宣言。example.com・ダミーURL・存在しないエンドポイントは書かない＝実在する既知APIだけ。汎用的な「Web検索」opは無い＝検索エンジンには接続できない）／メール送信=notify（{op:'notify',channel:'email',to:'$addr',subject:'…',message:'…'}）・アプリ内通知=notify（channel:'inapp'）／LINE・Discord・Slack送信=message.send（{op:'message.send',channel:'連携済みチャンネルの論理id',message:'…'}。特定個人へのLINE pushは不可＝連携済みチャンネルへの配信のみ）／AI分析・要約・分類=ai.infer（{op:'ai.infer',as:'out',prompt:'…{{入力}}…'}。決定的な計算は transform で書きAIに投げない）／Google連携=google.*（カレンダー/スプレッドシート/Gmail/ドライブ。Pro+プランと組織のGoogle連携・同意が前提）。【transformの形】transform ステップには必ず template（{{var}}展開の文字列）か from（値の参照。任意で path で 'a.b.c' 抽出）を付ける＝どちらも無い transform は不正で弾かれる。一覧や取得結果をそのまま画面に返すだけなら transform を挟まず、output を {type:'table',from:'$rows'} のように直前の as を直接参照する（不要な transform を作らない＝これが取得画面の生成失敗の主因）。【複数の種類のデータ（リレーション）】1アプリで顧客と案件のように複数の種類を扱うときは collection で種別を分ける：data.create/data.list/data.get/data.update/data.remove すべてに collection:'customers' のように同じ名前を付ける（例 保存 {op:'data.create',from:'$rec',collection:'customers'}／一覧 {op:'data.list',as:'rows',collection:'customers'}）。関連は data(JSON) に相手の id（例 customer_id）を入れて結合する。【データの共有範囲】各利用者が自分のデータだけ見るアプリは指定不要（既定 personal）。顧客台帳・在庫・シフトなど組織で同じデータを共有する業務アプリは、definition のトップに dataScope:'shared' を付ける＝メンバー全員が同じレコードを参照・更新できる（app_id は常に強制され他アプリには触れない）。通知/メール=notify（権限 notify）。アプリ内通知は{op:'notify',channel:'inapp',to:'$_owner'またはメンバーuid,message:'…'}、メール送信は{op:'notify',channel:'email',to:'$email',subject:'…',message:'…'}（メールは組織のGmail連携が必要）。申込受付→確認通知などに使う。LINE/Discord/Slackへ送信=message.send（権限 messaging:send）。{op:'message.send',channel:'（連携設定で作られたチャンネルの論理id）',message:'本文'}。本文は {{var}} テンプレートか、前段 step の結果を $名（例 $alert_message）で参照する（$参照はそのまま message に入れる＝{{}}で囲まない）。宛先/トークンはサーバ側で解決＝アプリJSには渡らない。閾値超過アラートや申込のチーム共有など、利用者が明示的にLINE/Discord/Slackへの送信を求めた時だけ使う（要望文に無ければ足さない）。社内文書を根拠にAIに答えさせる（RAG）=knowledge.search（権限 knowledge）。{op:'knowledge.search',as:'kb',from:'$question'}で組織ナレッジを検索し、続く ai.infer の prompt に『次の社内資料を根拠に回答：{{kb}}\\n\\n質問：{{question}}』のように {{kb}} を差し込む。グラフ/ダッシュボード=output.type を 'chart' にし、from に [{label,value}] 形式のデータを渡す。集計は data.list で取得した行を transform（テンプレート/JSONパス）や ai.infer で [{label,value}] に変換して作る。棒グラフで描画される。chart に 'bar'/'line'/'pie' を指定可（既定 bar）。電子署名（C8）=inputs に type:'signature' を使うと手書き署名パッド（canvas）が出る。送信時にPNG画像として保存され、申込書/同意書の署名に使える。承認ワークフロー=保存時に data.create で status:'pending' を付ける（{op:'data.create',from:'$rec',status:'pending'}）。状態別の一覧は data.list に status:'pending' を付けて絞る（{op:'data.list',as:'rows',status:'pending'}＝各行に id と status が入る）。承認/却下は record.status で遷移し、必ず権限と遷移元を宣言する：{op:'record.status',from:'$id',to:'approved',fromStatus:'pending',requiredRoles:['admin']}（requiredRoles=この操作を実行できるロール／fromStatus=この状態の時のみ遷移可＝不正遷移を防ぐ）。承認画面（screen）自体にも requiredRoles:['admin'] を付けて承認者以外に開かせない（UI非表示だけでなくサーバ側でも強制される）。【簡潔さの原則】計算・整形・換算など答えが一意に決まる処理は ai.infer ではなく transform（テンプレート/式）で決定的に行う＝余計な解説を出さず結果だけ返す。ai.infer は要約・分類・自然言語生成など知能が要る箇所だけに使い、その prompt には『前置き・解説を付けず、求められた結果だけを簡潔に返す』と明記する。`;
...
選択肢（id｜用途）：
` + libChoices() + "\ngoal は必須＝実装の指針かつ完成判定の基準。『接続できる』『保存できる』『配布できる』のように、実際に成立しないと意味がない達成条件を明確に書く（曖昧な『〜を管理する』だけにしない）。successCriteria は goal を『利用者が実際に操作して確かめられる』測定可能な言葉に分解する（動く/表示される/保存される/接続できる 等）。装飾や好みは書かない。【MVP優先度】phases には priority を付ける：P1＝これ単体で要望の中心価値が成立する最小の工程（＝最優先。必ず先頭に置く）。P2/P3＝あると良い追加。P1 だけ完成すれば『使える』状態にする（無停止で途中まででも価値が残るように）。acceptance は各工程が『動いた』と客観的に言える条件を、利用者の操作と結果で1〜2個書く（Given/When/Then 相当・専門用語なし）。phases は完成に必要な最小限（最大6）。データ操作のある画面は kind=screen、見た目のHTML描画は kind=render。1つの kind=screen 工程には画面を1つだけ書く（保存/一覧/更新/削除はそれぞれ別工程。6工程に収まらない分は followUps へ）。要望が6工程に収まらない場合は、要望文に明示されたのに今回作らない残りの要求（追加の画面・機能）だけを followUps に正直に列挙する（利用者の言葉で簡潔に・空可）。要望文に無い LINE/Discord/Gmail/通知/cron/CSV 等を推測で足さない。【分割ビルド】spec に「【分割ビルド】」の指示があるときは、phases をデータが密に結合する中核機能だけ（最大4工程・priority=P1 中心）に絞り、要望文に明示された残りの機能は推測を足さず全て followUps に列挙する（利用者は完成後『続けて作って』で追加する）。【最小実装】要望の中心機能だけを作る。履歴・エクスポート/DL・共有・テーマ切替・設定など明示的に頼まれていない付加機能は phases に入れない（肥大化＝生成が途中で切れて完成しない主因）。" + uiGuidance + `計算・換算・整形など答えが決まる処理は HTML 内 JS で即時実行（ai 不要・permissions に ai を入れない）。データの保存・一覧・集計が要る時は、操作ごと（保存/一覧/更新/削除 等）に kind=screen の画面を必ず用意し（render はその画面の id を bo.run で正確に呼ぶ＝id を勝手に作らない）、render の JS から bo.run で呼ぶ（画面はSPAのまま再読込しない）。保存/一覧を伴うカスタムUIで screen 画面が0個のプランは不可（bo.run が呼べず失敗する）。【一覧画面を必ず対で作る】保存したデータを利用者が画面で一覧・表示・管理するアプリでは、保存するデータ種（collection）ごとに、それを読み出す data.list の一覧/表示画面を保存画面とセットで必ず用意する＝保存画面だけで終えない（一覧画面が無いと render が保存画面を一覧代用して壊れる）。送信だけで完結するフォームや人が見ない裏方処理には一覧工程を足さない。6工程を超える場合のみ、同一データの保存画面と一覧画面を1つの kind=screen 工程にまとめてよい（それ以外は1工程1画面）。【collection一致】複数種のデータを扱うときは各工程の goal に collection 名（英小文字1語）を明記し、同じデータの保存/一覧/更新の工程で同一名を使う（不一致だと一覧が常に空になる）。【一意保存】『◯◯ごとに1件』の一意保存(upsert)が要る工程は goal に業務上の一意キーを明記する（例：店舗×対象月で1件）＝レコードidを一意キーにしない。AI は要約・分類・自然言語生成など知能が要る時のみ。利用可能な権限：${permissionCatalogText(["messaging:receive"])}。`;
...
${specForPrompt(spec, 6e4)}` }], [], void 0, { maxTokens, thinking: "disabled", jsonSchema: PLAN_SCHEMA }).catch(() => null);
...
async function continueRenderHtml(env, model, html, paid, rounds = 4, forceFirst = false, opts) {
  const isSection = opts?.closeTag === "</section>";
  const unit = isSection ? "<section> フラグメント" : "HTML";
  const closeRule = isSection ? "onclick 属性は使わず addEventListener でこの IIFE 内から配線した関数を全て <script> 内に実装し、最後は </section> で閉じること" : "onclick 等のイベントが呼ぶ関数を全て <script> 内に実装し、最後は </html> で閉じること";
  const contract = opts?.builtScreens?.length ? "\n\n" + renderContractRules(opts.builtScreens) + "\n呼べる口：" + fieldContract(opts.builtScreens) : "";
  const sys = await getAiKnowledgeFor(env, "repair") + `

あなたはカスタムUIの ${unit} を完成させるアシスタントです。与えられた『途中まで生成された ${unit}』の続きだけを出力します。前置き・説明・コードフェンス・重複は一切出さない。<script> は必ず閉じ、${closeRule}。既存の HTML が読み込んでいる外部リソース参照（厳選CDNの <script src>/<link href>）は機能に必要なのでそのまま維持し、新たな外部リソースは追加しない。` + contract;
  const stillOpen = (h) => isSection ? !/<\/section\s*>/i.test(h) || (h.match(/<script\b[^>]*>/gi) ?? []).length > (h.match(/<\/script\s*>/gi) ?? []).length : looksTruncated(h);
  let forceNext = forceFirst;
  for (let i = 0; i < rounds; i++) {
    if (!stillOpen(html) && !forceNext || html.length > RENDER_HTML_MAX) break;
    forceNext = false;
    await logDiag(env, "info", "build", `${isSection ? "section" : "render"} continuation round ${i + 1}: len=${html.length}`).catch(() => {
    });
    const cont = await model.turn(sys, [
      { role: "user", text: `以下は途中まで生成された ${unit} です。この続きだけを（重複・前置き・コードフェンスなしで）そのまま出力してください。<script> を閉じ、${closeRule}。
...
async function rewriteRenderHtml(env, model, brokenHtml, issue, paid, builtScreens) {
  const rewriteLibRecipes = recipesFor(libsInHtml(brokenHtml));
  const sys = await getAiKnowledgeFor(env, "repair") + "\n\n" + (rewriteLibRecipes ? rewriteLibRecipes + "\n\n" : "") + "あなたはカスタムUIの HTML を修正するアシスタントです。渡された『動作しない HTML』を、見た目と機能をできる限り保ったまま、不具合を直した完全な HTML 文書として出力します。前置き・説明・コードフェンス・JSON は一切付けない。onclick 等のイベントが呼ぶ関数を全て <script> 内に実装し、<script> を必ず閉じ、最後は </html> で閉じること。元の HTML が読み込んでいる外部リソース参照（厳選CDNの <script src>/<link href>）は機能に必要なのでそのまま残す（勝手に削除・変更しない）。新たな外部リソースは追加しない。" + DESIGN_BASELINE + (builtScreens?.length ? "\n\n" + renderContractRules(builtScreens) + "\n呼べる口：" + fieldContract(builtScreens) : "");
...
${brokenHtml.slice(0, RENDER_HTML_MAX)}` }
  ...
  if ((looksTruncated(html) || capped) && html.length <= RENDER_HTML_MAX) {
    html = await continueRenderHtml(env, model, html, paid, 4, capped && !looksTruncated(html), { builtScreens });
  }
  if (html.length > RENDER_HTML_MAX || looksTruncated(html)) return null;
  return html;
}
function fieldContract(bs) {
  return bs.map((s) => {
    const fs = (s.fields ?? []).map((f) => `${f.name}:${f.type}${f.options?.length ? `[${f.options.slice(0, 10).join("|")}]` : ""}`).join(", ");
    return `bo.run("${s.id}",{ ${fs || (s.inputs ?? []).join(", ") || "…"} })`;
  }).join(" / ") || "(データ操作なし)";
}
function builtScreensFromDef(def) {
  const screens = Array.isArray(def.screens) ? def.screens : [];
  return screens.map((s) => {
    const o = s;
    const inps = Array.isArray(o.inputs) ? o.inputs : [];
    const fields = inps.map((i) => ({ name: String(i?.name ?? ""), type: String(i?.type ?? "text"), label: i?.label ? String(i.label) : void 0, options: Array.isArray(i?.options) ? i.options.map(String) : void 0 })).filter((f) => f.name);
    const writes = Array.isArray(o.steps) && o.steps.some((st) => st != null && typeof st === "object" && /^data\.(create|update|upsert)$/.test(String(st.op ?? "")));
    return { id: String(o.id ?? ""), writes, inputs: fields.map((f) => f.name), fields };
  }).filter((s) => s.id);
}
function renderContractRules(builtScreens) {
  const ids = builtScreens.map((s) => `"${s.id}"${s.writes ? "【保存】" : ""}${s.inputs?.length ? `(入力:${s.inputs.join(",")})` : ""}`).join(" / ") || "（データ画面は未定義）";
  return "\n【データ操作の鉄則（最重要）】window.bo.run で呼べる画面IDは次の定義済みIDだけ（【保存】印はデータを保存/更新する画面）：" + ids + "。各画面の入力の型と選択肢は次のとおり（この型に合う入力UIを作る＝date は日付入力、time は時刻入力、select/radio は options のドロップダウン/ラジオ、checkboxes は複数選択、number は数値、file は file 入力。名前から型を推測しない）：" + fieldContract(builtScreens) + `。これ以外のID（save/list/update/delete/assetupsert 等の思いつきの名前）を呼んでも実行されず 400 で失敗する＝必ず上の実在IDを、対応する操作に正しく使う。上にデータ画面が無い場合は保存/一覧は行わない。
【書き込み画面を読み取りに使わない（最重要）】上の【保存】印の画面は保存・更新の目的でのみ呼ぶ。一覧・表示・初期ロードの目的で bo.run しない＝空引数 bo.run("保存画面ID",{}) や {__list:true}/{__action:'list'} 等の定義に無いキーを渡しても動作は切り替わらず、空/壊れたレコードが保存されるだけ。一覧・表示は【保存】印の無い data.list の読取画面を呼び、読取画面が上の一覧に無ければ一覧UIは作らない（保存画面で代用しない）。
【引数キーの一致】bo.run の第2引数のキー名は上の各画面の入力名と一字一句一致させる（言い換え・camelCase 変換・省略をしない）。
【配線の被覆（保存画面を必ず呼ぶ・最重要）】上に列挙した画面のうちデータを保存/更新/upsert する画面（保存フォーム系）は、必ずいずれかのボタンかフォーム送信から window.bo.run("その画面ID", {各入力名:値}) を呼ぶUIを用意すること＝どの保存画面もUIから一度も呼ばれない状態にしない。呼ばれない保存画面は『保存できる風で保存されない』殻になり不合格。全ての保存画面に対応する入力フォーム＋保存ボタンを漏れなく作る。
...
【連携op（Googleシート/LINE等）の結果は固定文言で偽装しない（最重要）】Googleシート追記・LINE送信などの連携を呼ぶボタンは、成功文言（『追記しました』『送信しました』）を固定で書かない＝必ず bo.run の戻り値 output をそのまま画面に表示する。連携先が未設定なら output は『（連携設定が未完了のためスキップしました…）』等のスキップ文言が返る＝それを表示する（固定の『追記しました』を出すと、実際には書かれていないのに成功に見える偽成功になり不合格）。
【連携設定フォームを画面内に作らない】GoogleシートのURL/ID などの連携設定値は、アプリ画面内に入力フォームや『未連携です』表示を作らない＝プラットフォームの『アプリ一覧→連携設定』で管理者が設定する（$config.<key> で実行時に解決される）。画面内に独自の設定フォームを作ると二重管理で混乱するので作らない。
【ボタンが必ず動く4原則（最重要・これを外すと全ボタンが無反応になる）】①onclick 等が呼ぶ関数は <script> のトップレベルに定義する。IIFE や DOMContentLoaded の中に隠した関数を onclick から呼ぶと実行時エラーで無反応になる＝隠すなら window.関数名=関数 で公開する。②window.bo は本文の後に注入される＝script のトップレベルで bo に触れない。bo を使う初期化は必ず window.addEventListener('load', …) か DOMContentLoaded の中で行う（トップレベルで bo.run すると ReferenceError で以降の全配線が死ぬ）。③フォームの送信ボタンは type="button" にして addEventListener で処理する（または <form onsubmit> で必ず e.preventDefault() する）。既定の submit はサンドボックスで遮断され無反応になる。④getElementById/querySelector で参照する id は必ず HTML に実在させる（存在しない id を参照すると null エラーで以降の配線が全て止まる）。
...
function parseRenderSections(raw) {
  if (!raw) return void 0;
  try {
    const d = JSON.parse(raw);
    if (!Array.isArray(d.sections) || !Array.isArray(d.frags) || !d.sections.length || d.sections.length !== d.frags.length) return void 0;
    const sections = d.sections.map((s) => {
      const o = s ?? {};
      return { id: String(o.id ?? ""), title: String(o.title ?? ""), purpose: String(o.purpose ?? ""), wireScreenIds: Array.isArray(o.wireScreenIds) ? o.wireScreenIds.map(String) : void 0 };
    });
    if (sections.some((s) => !s.id)) return void 0;
    const frags = d.frags.map((f) => typeof f === "string" && f ? f : null);
    return { sections, frags };
  } catch {
    return void 0;
  }
}
...
async function deriveUiSections(model, plan, spec) {
  const sys = "あなたはUI設計者です。次のアプリを、利用者に見せる機能セクションに分けます。各セクションは1つの明確な機能に絞る（例：入力フォーム / 一覧表示 / 検索・絞り込み / 集計・グラフ / ステータス変更 / 設定）。できるだけ小さく分ける＝1セクション=1機能に絞り、大きな塊を避ける（各セクションが小さいほど速く・確実に・高品質に生成できる）。単純で1機能なら1件、複数機能なら機能ごとに2〜8件へ細かく分ける。id は英小文字とハイフンの短い識別子。JSON のみ出力。";
  const r = await model.turn(sys, [{ role: "user", text: `アプリ名：${plan.name}
目標：${plan.goal}
要望：${spec.slice(0, 1500)}` }], [], void 0, { maxTokens: 1500, thinking: "disabled", jsonSchema: SECTION_SCHEMA }).catch(() => null);
  ...
  }).filter((s) => s.title && !seen.has(s.id) && (seen.add(s.id), true)).slice(0, 8);
}
function sectionShell(title, sectionFragments) {
  const bus = `<script>window.__app=(function(){var m={},last={};return{on:function(e,f){(m[e]=m[e]||[]).push(f);if(Object.prototype.hasOwnProperty.call(last,e)){try{f(last[e]);}catch(_){}}},emit:function(e,d){last[e]=d;(m[e]||[]).slice().forEach(function(f){try{f(d);}catch(_){}});}};})();(function(){var f=false;function r(){if(f)return;f=true;window.__app.emit("ready");}if(document.readyState==="complete")r();else window.addEventListener("load",r);window.addEventListener("pageshow",function(e){if(e.persisted&&!f)r();});})();<\/script>`;
...
function spliceSectionIntoHtml(html, fragment) {
  if (typeof html !== "string" || !html || !fragment || !fragment.trim()) return null;
  const anchor = html.lastIndexOf("</div></body>");
  if (anchor < 0) return null;
  const out = html.slice(0, anchor) + "\n" + fragment.trim() + "\n" + html.slice(anchor);
  return out.length <= RENDER_HTML_MAX ? out : null;
}
async function generateSection(env, model, plan, spec, section, builtScreens, paid, attempt = 0, shrink = false) {
  const opList = fieldContract(builtScreens);
  const sys = await getAiKnowledgeFor(env, "implement") + `

あなたはカスタムUIの1セクション（機能ブロック）を作る HTML ジェネレータです。出力は <section id="…" class="app-sec">…</section> のフラグメントそのものだけ（前置き・説明・コードフェンス・doctype/html/head/body は一切付けない）。サンドボックス iframe（別オリジン・外部通信なし）で描画。<style>（このセクション用）と <script> は section 内に inline 可。【隔離（最重要）】このセクションの JS は必ず1つの即時実行関数 (function(){ … })(); に閉じる（グローバル関数を作らない＝他セクションと衝突しない）。onclick 属性は使わず addEventListener でこの IIFE 内から配線する。【id 名前空間（衝突防止）】このセクション内の全ての要素 id と、for/aria-*/getElementById/querySelector が指す対象は必ず "${section.id}-" で始める（他セクションと id が衝突すると getElementById が別セクションの要素を掴んで誤動作する）。【データ】保存/一覧は window.bo.run(screenId, inputs)（Promise, 戻り {ok,output:{type,value},error?}）。呼べる口：` + opList + '。output.value が table のときは JSON 文字列なので JSON.parse。【起動と連携】window.bo はページ本体の後に注入されるため、初期の bo.run は window.__app.on("ready", …) の中で行う（このバスは sticky ＝ ready が先に発火済みでも on 購読時に必ず再生されるので初期ロードは確実に走る）。データを追加/更新/削除したら window.__app.emit("changed") を呼ぶ。一覧・集計系は window.__app.on("ready", 再読込) と window.__app.on("changed", 再読込) の両方で自動更新する。【イベントの対応（重要）】window.__app.on("E", …) で購読するイベント E は、必ずどこかで window.__app.emit("E", …) して発火させる（対応する emit が無い on は永久に発火せず、その初期化・モード切替・ボタンが動かない）。別セクションのボタン操作で状態が変わるときも emit で通知し、受け側は on で反応する。' + designBaselineFor({ spec: [spec, plan.goal, plan.description].join(" "), hasData: builtScreens.length > 0, libs: plan.libs }) + (plan.libs?.length ? "\n" + recipesFor(plan.libs) + "\n" : "") + renderContractRules(builtScreens);
  const fb = (shrink ? "\n【このセクションが大きすぎてアプリ全体が容量上限(256KB)を超えました。機能は必ず保ったまま、装飾・冗長なマークアップ・重複するCSS/JS・長いインラインデータを大幅に削り、できる限り短い実装で </section> まで出し切ってください。】" : attempt > 0 ? "\n【前回は出力上限で途中で切れました。装飾や冗長なコードを削り、最小実装で必ず </section> まで出し切ってください（機能は保つ）。】" : "") + (attempt > 0 && section.wireScreenIds?.length ? `
【前回、保存画面の配線が漏れた可能性があります】各保存画面 ${section.wireScreenIds.map((id) => `bo.run("${id}")`).join("・")} を呼ぶ入力フォームと保存ボタンを必ず含めること。` : "");
  const maxTokens = shrink ? paid ? 1e4 : 7e3 : attempt === 0 ? paid ? 12e3 : 8e3 : paid ? 24e3 : 12e3;
  const wireNote = section.wireScreenIds?.length ? `
【必須配線（最重要）】このセクションは次の保存画面を必ず配線する（1画面=1入力フォーム=1保存ボタン=1 bo.run・id は文字列リテラル必須）：` + section.wireScreenIds.map((id) => {
    const keys = builtScreens.find((b) => b.id === id)?.inputs ?? [];
    return `bo.run("${id}", {${keys.length ? keys.map((k) => `${k}: 値`).join(", ") : "各入力名: 値"}})`;
  }).join(" / ") + `。各画面に入力欄＋保存ボタン（type="button" + addEventListener）を作り、押下でその bo.run を呼び、戻り値の ok を確認してから成功表示し window.__app.emit("changed") を発火する。一覧だけにしない。` : "";
  const userText = `アプリ「${plan.name}」（目標：${plan.goal}）のうち、このセクションだけを作る。
【セクション】${section.title}
【役割】${section.purpose}${wireNote}
<section id="${section.id}" class="app-sec"> で始め </section> で終える完全なフラグメントを出力。${fb}`;
  const r = await model.turn(sys, [{ role: "user", text: userText }], [], void 0, { maxTokens, thinking: "disabled" }).catch(() => null);
  let h = (r?.text ?? "").trim();
  const fenced = h.match(/```(?:html)?\s*([\s\S]*?)```/i);
  if (fenced) h = fenced[1].trim();
  const m = h.match(/<section[\s\S]*/i);
  if (!m) return null;
  h = m[0];
  const opens0 = (h.match(/<script\b[^>]*>/gi) ?? []).length, closes0 = (h.match(/<\/script\s*>/gi) ?? []).length;
  if (opens0 > closes0 || r?.truncated || !/<\/section\s*>/i.test(h)) {
    h = await continueRenderHtml(env, model, h, paid, 3, true, { closeTag: "</section>", builtScreens }).catch(() => h);
    const mm = h.match(/<section[\s\S]*<\/section>/i);
    if (mm) h = mm[0];
  }
  const trimmed = h.match(/<section[\s\S]*<\/section>/i);
  if (trimmed) h = trimmed[0];
  const opens = (h.match(/<script\b[^>]*>/gi) ?? []).length, closes = (h.match(/<\/script\s*>/gi) ?? []).length;
  if (opens > closes || !/<\/section\s*>/i.test(h)) return null;
  if (section.wireScreenIds?.length) {
    const missing = section.wireScreenIds.filter((id) => !new RegExp(`bo\\s*\\.\\s*run\\s*\\(\\s*['"\`]${id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`).test(h));
    if (missing.length) {
      if (attempt < 1) return null;
      await logDiag(env, "warn", "build", `section ${section.id}: 必須配線 ${missing.join(",")} が生成されず（attempt=${attempt}・採用＝P0/P1 が受け皿）`).catch(() => {
      });
    }
  }
  return h;
...
function assignWriteScreensToSections(sections, builtScreens) {
  const writeIds = builtScreens.filter((s) => s.writes).map((s) => s.id);
  const out = sections.map((s) => ({ ...s, wireScreenIds: [...s.wireScreenIds ?? []] }));
  if (!writeIds.length || !out.length) return out;
  const assigned = /* @__PURE__ */ new Set();
  for (const s of out) for (const id of s.wireScreenIds) assigned.add(id);
  let k = 0;
  for (const id of writeIds) {
    if (assigned.has(id)) continue;
    out[k % out.length].wireScreenIds.push(id);
    assigned.add(id);
    k++;
  }
  return out;
}
function largestFragIndex(frags) {
  let mi = 0;
  for (let i = 1; i < frags.length; i++) if ((frags[i]?.length ?? 0) > (frags[mi]?.length ?? 0)) mi = i;
  return mi;
}
async function implementRenderSplit(env, model, plan, spec, builtScreens, paid, onProgress, opts) {
  if (!builtScreens.length) return null;
  const prior = opts?.prior?.sections?.length && opts.prior.sections.length === opts.prior.frags.length ? opts.prior : void 0;
  let sections = prior ? prior.sections : await deriveUiSections(model, plan, spec);
  if (!prior && sections.length < 2 && builtScreens.length >= 2) {
    const seenSec = /* @__PURE__ */ new Set();
    sections = builtScreens.slice(0, 8).map((s) => {
      let sid = secSlug(s.id);
      if (seenSec.has(sid)) {
        let n = 2;
        while (seenSec.has(`${sid}-${n}`)) n++;
        sid = `${sid}-${n}`;
      }
      seenSec.add(sid);
      return { id: sid, title: s.id, purpose: `画面「${s.id}」の入力/一覧/操作UI（bo.run("${s.id}", …) を必ず呼ぶ保存/表示UI）`, wireScreenIds: s.writes ? [s.id] : [] };
    });
    await logDiag(env, "info", "build", `render split: 画面ベース分割 app=${plan.id} sections=${sections.length}`).catch(() => {
    });
  }
  if (sections.length < 2) return null;
  if (!prior) sections = assignWriteScreensToSections(sections, builtScreens);
  await logDiag(env, "info", "build", `render split: app=${plan.id} sections=${sections.length}${prior ? "（再開）" : ""}`).catch(() => {
  });
  const total = sections.length;
  const frags = prior && prior.frags.length === sections.length ? [...prior.frags] : new Array(sections.length).fill(null);
  let sectionsDone = frags.filter((f) => f).length;
  await onProgress?.(sectionsDone, total);
  await runPool(sections, 3, async (sec, idx) => {
    if (frags[idx]) return null;
    let frag = await generateSection(env, model, plan, spec, sec, builtScreens, paid, 0).catch(() => null);
    if (!frag) frag = await generateSection(env, model, plan, spec, sec, builtScreens, paid, 1).catch(() => null);
    frags[idx] = frag;
    sectionsDone++;
    await onProgress?.(sectionsDone, total);
    await opts?.onFragments?.({ sections, frags }).catch(() => {
    });
    return null;
  });
  for (let i = 0; i < frags.length; i++) {
    if (!frags[i]) {
      frags[i] = await generateSection(env, model, plan, spec, sections[i], builtScreens, paid, 1).catch(() => null);
      await opts?.onFragments?.({ sections, frags }).catch(() => {
      });
    }
  }
  const missingCount = frags.filter((f) => !f).length;
  if (missingCount === frags.length) return null;
  if (missingCount) {
    for (let i = 0; i < frags.length; i++) if (!frags[i]) frags[i] = `<section id="${sections[i].id}" style="padding:16px"><p style="color:#8a8a8a;font-size:.9rem">「${sections[i].title || sections[i].id}」は生成できませんでした。会話で「続けて作って」を押すと追加できます。</p></section>`;
    await logDiag(env, "warn", "build", `render split: ${missingCount}/${frags.length} セクション欠け→成功分は保持しプレースホルダで組み立て（all-or-nothing 回避）app=${plan.id}`).catch(() => {
    });
  }
  let html = sectionShell(plan.name, frags);
  for (let shrink = 0; shrink < 3 && html.length > RENDER_HTML_MAX && !looksTruncated(html); shrink++) {
    const mi = largestFragIndex(frags);
    await logDiag(env, "info", "build", `render split: 256KiB超 → 最大section「${sections[mi].id}」を縮小再生成 round ${shrink + 1}（len=${html.length}）`).catch(() => {
    });
    const shrunk = await generateSection(env, model, plan, spec, sections[mi], builtScreens, paid, 1, true).catch(() => null);
    if (!shrunk || shrunk.length >= (frags[mi]?.length ?? 0)) break;
    frags[mi] = shrunk;
    html = sectionShell(plan.name, frags);
  }
  if (html.length > RENDER_HTML_MAX || looksTruncated(html)) return null;
  return html;
}
...
${specForPrompt(spec, 4e4)}`
  ...
  if (html && (looksTruncated(html) || capped) && html.length <= RENDER_HTML_MAX) {
    ...
    html = await continueRenderHtml(env, model, html, paid, opts?.rounds ?? 4, capped && !looksTruncated(html), { builtScreens });
  }
...
async function processAppBuilds(ctx2, baseUrl = "", limit = 2, opts) {
  ...
  const driver = await getBuildDriver(env).catch(() => "cron");
  const lease = nowSec() - (driver === "do" ? DO_LEASE : LEASE$1);
  ...
  for (const s of stuck) {
    const appId = s.app_id || (() => {
      try {
        return String(JSON.parse(s.plan ?? "{}").id ?? "").trim();
      } catch {
        return "";
      }
    })();
    const artifact = appId ? await ctx2.db.first("SELECT id FROM app_drafts WHERE id=? UNION ALL SELECT id FROM external_apps WHERE id=? LIMIT 1", [appId, appId]).catch(() => null) : null;
    if (!artifact) {
      await ctx2.db.run("UPDATE app_builds SET status='building', updated_at=? WHERE id=? AND status='finalizing'", [nowSec(), s.id]).catch(() => {
      });
      await logBuild(env, s.id, "finalizing 回収: 成果物なし→building へ再駆動", `appId=${appId || "?"}`, "warn").catch(() => {
      });
      continue;
    }
    const nm = (() => {
  ...
  for (const row of rows) {
    if (driver === "do") {
      if (nowSec() - row.created_at > MAX_BUILD_AGE) {
        await runBatch(ctx2, row, paid, baseUrl, { ...opts, src: "drain-age" }).catch(() => {
        });
        processed++;
        continue;
      }
      await kickBuild(ctx2, row.id, baseUrl, "drain").catch(() => {
      });
      await ctx2.db.run("UPDATE app_builds SET updated_at=? WHERE id=? AND status IN ('planning','building')", [nowSec(), row.id]).catch(() => {
      });
      processed++;
      continue;
    }
    const more = await runBatch(ctx2, row, paid, baseUrl, { ...opts, src: "drain" });
    processed++;
    if (more) {
      moreActive = true;
...
async function claimBuild(ctx2, id, readUpdatedAt) {
  const claimTs = Math.max(nowSec(), readUpdatedAt + 1);
  const r = await ctx2.db.run("UPDATE app_builds SET updated_at=? WHERE id=? AND updated_at=? AND status IN ('planning','building')", [claimTs, id, readUpdatedAt]);
  return !!r.rowsWritten;
}
async function runBatch(ctx2, row, paid, baseUrl, opts) {
  const won = await claimBuild(ctx2, row.id, row.updated_at);
  await logBuild(ctx2.env, row.id, won ? "claim 占有" : "claim 敗北(譲)", `src=${opts?.src ?? "?"} readUpd=${row.updated_at} status=${row.status} cur=${row.cursor} att=${row.attempts}`);
  if (!won) return false;
...
const appBuilder = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  applyPatch,
  assembleDefinition,
  assignWriteScreensToSections,
  buildModelGuide,
  builtScreensFromDef,
  cancelAppBuild,
  capBytes,
  checkFakeIntegrationSuccess,
  checkRequestedFeatureOps,
  claimBuild,
  coerceScreens,
  continueRenderHtml,
  designBaselineFor,
  detectCapabilityBoundaries,
  filterSpecGroundedItems,
  injectGoogleSheetAppend,
  isCapabilityBoundaryText,
  isSpecGrounded,
  kickBuild,
  largestFragIndex,
  latestSessionApp,
  ...
  processAppBuilds,
  prunePlanToSpec,
  pruneUnmetAgainstDonePhases,
  rebuildPublicPage,
  renderObjFor,
  repairMissingRecFrom,
  resolveAppByName,
  runPool,
  salvageScreenDroppingBadIntegrationSteps,
  screenIsValid,
  screenIssues,
  screenMissingWrite,
  sectionShell,
...
const COLS = "id,owner,slug,title,status,prompt,base,plan,blocks,cursor,attempts,model,note,error,created_at,updated_at,base_revision";
const arr = (s) => {
  try {
...
async function startSiteBuild(ctx2, a) {
  ...
  const now = nowSec();
  const cur = await ctx2.db.first("SELECT draft_revision FROM sites WHERE slug=?", [a.slug]).catch(() => null);
  const baseRev = cur?.draft_revision ?? 0;
  await ctx2.db.run(
    "INSERT INTO site_builds (id,owner,slug,title,status,prompt,base,blocks,cursor,attempts,model,base_revision,created_at,updated_at) VALUES (?,?,?,?, 'planning', ?,?, '[]', 0, 0, ?, ?, ?, ?)",
    [id, a.owner, a.slug, a.title ?? null, (a.prompt || "").slice(0, 8e3), a.base ? JSON.stringify(a.base) : null, a.model ?? null, baseRev, now, now]
  );
  return id;
}
...
async function finalize(ctx2, row) {
  ...
  const stillActive = await ctx2.db.first("SELECT id FROM site_builds WHERE id=? AND status IN ('planning','building')", [row.id]).catch(() => null);
  if (!stillActive) return;
  const saved = await saveLayoutDraft(ctx2.env, row.slug, JSON.stringify(layout), row.title || void 0, row.base_revision ?? 0);
  const doneNote = saved.conflict ? "生成中にページが編集されたため、自動反映は見送りました。生成結果は編集画面の履歴からご確認ください（既存の編集は保持されています）。" : `${layout.blocks.length}個のセクションでHPを作成しました。${note ? `（${note}）` : ""}`;
  await ctx2.db.run(
    "UPDATE site_builds SET status='done', cursor=?, note=?, updated_at=? WHERE id=? AND status IN ('planning','building')",
    [layout.blocks.length, doneNote, nowSec(), row.id]
  );
}
async function claimSiteBuild(ctx2, id, readUpdatedAt) {
  const claimTs = Math.max(nowSec(), readUpdatedAt + 1);
  const r = await ctx2.db.run("UPDATE site_builds SET updated_at=?, attempts=attempts+1 WHERE id=? AND updated_at=?", [claimTs, id, readUpdatedAt]);
  return !!r.rowsWritten;
}
async function processSiteBuild(ctx2, id, opts) {
  ...
  for (let s = 0; s < steps; s++) {
    ...
    const won = await claimSiteBuild(ctx2, id, row.updated_at);
    if (!won) return true;
...
const BOOL_TRUE = /* @__PURE__ */ new Set(["true", "on", "1", "yes"]);
const BOOL_FALSE = /* @__PURE__ */ new Set(["false", "off", "0", "no"]);
const asBool = (v) => BOOL_TRUE.has(String(v ?? "").trim().toLowerCase());
const parseBool = (v) => {
...
const normSettingKey = (v) => String(v ?? "").trim();
function isDestructiveSettingsCall(name, args) {
  if (name === "manage_member") {
    const action = String(args.action ?? "");
    if (action === "disable" || action === "delete") return true;
    if (action === "set_role" && String(args.role ?? "").trim() === "admin") return true;
    if (action === "invite" && String(args.role ?? "").trim() === "admin") return true;
    return false;
  }
  ...
  if (name === "update_setting") {
    const k = normSettingKey(args.setting);
    if (k === "autonomy") return asBool(args.value);
    if (k === "custom_domain") return String(args.value ?? "").trim() !== "";
    if (k === "approval_mode") return parseBool(args.value) !== true;
    return false;
  }
  return false;
}
  ...
  if (name === "update_setting") {
    ...
    switch (key) {
      ...
      case "approval_mode": {
        ...
        if (b === null) return "承認モードは true（必須）／false（不要）で指定してください。";
        const on = await setApprovalMode(env, b);
      ...
      case "autonomy": {
        const b = parseBool(value);
        if (b === null) return "オートパイロットは true（有効）／false（無効）で指定してください。";
        await setAutonomy(env, b);
        return `オートパイロットを ${b ? "有効" : "無効"} にしました。`;
      }
      case "custom_domain": {
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_DBr2-kMn.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
function looksHugeBuild(text) {
  const t = (text || "").slice(0, 4e3);
  if (/カスタム\s*ui|render\s*\.?\s*html|\brender\b|作り込|凝った(ui|画面|見た目|レイアウト)|独自(の)?(ui|画面|レイアウト)|html(で|を|の)/i.test(t)) return true;
  if (/(複数|いくつ|何|数)\s*(の|個|つ)?\s*(画面|ページ|タブ|フォーム)|(画面|ページ|タブ)\s*(を|は)?\s*(分け|複数|切替|切り替え|たくさん)|ダッシュボード|管理画面/.test(t)) return true;
  const save = /(保存|記録|登録|蓄積|ため(て|る)|溜め|入力(を|して)?(残|貯))/.test(t);
  const list = /(一覧|リスト|履歴|検索|絞り込|フィルタ)/.test(t);
  const agg = /(集計|集約|合計|平均|レポート|月次|週次|前月比|前年比|推移|統計|ランキング|ダッシュボード)/.test(t);
  const complexOp = /(あれば更新|存在すれば|一意|重複を?\s*(更新|排除|チェック|防)|upsert|前月比|前年比|差分|マージ|突合)/i.test(t);
  if (complexOp) return true;
  if (save && (list || agg)) return true;
  if (t.length >= 600) return true;
  return false;
}
function looksLikeNewAppRequest(text) {
  const t = (text || "").slice(0, 400);
  return /(作りた|作って|作成し|作成くださ|作成をお願い|つくって|つくりた|生成し|構築し|開発し|新規(に|で|作成|の|アプリ)|新しく(作|つく))/.test(t);
}
function looksLikeSeparateNewApp(text) {
  const t = (text || "").slice(0, 400);
  if (!looksLikeNewAppRequest(t)) return false;
  return /(新し|別(の|に|で|個)|もう(一|1)つ|新規)/.test(t) && /(アプリ|ツール)/.test(t);
}
function looksLikeStrongEditRequest(text) {
  const t = (text || "").slice(0, 400);
  return /(直して|直したい|直せ|修正して|修正したい|なおして|バグ|不具合|エラー|動かな|おかしい|表示されな|うまくいか|変更して|変えて|追加して|削除して|消して|対応して|反映して|更新して)/.test(t);
}
...
async function targetAppIsLarge(ctx2, appId) {
  try {
    const design = await getAppDesign(ctx2, appId);
    const def = design?.definition;
    if (!def) return false;
    if (def.render && typeof def.render.html === "string" && def.render.html.length > 0) return true;
    if (Array.isArray(def.screens) && def.screens.length >= 3) return true;
    return JSON.stringify(def).length > 2e4;
  } catch {
    return false;
  }
}
...
const DELETE_CONFIRM_MARK = "［削除を実行する";
const DELETE_CONFIRM_RE = /^［削除を実行する(?::([A-Za-z0-9_-]+))?］/;
function parseDeleteConfirm(message) {
  const m = (message || "").trim();
  const dm = m.match(DELETE_CONFIRM_RE);
  if (!dm) return { confirm: false, appId: "", cleanMsg: m };
  return { confirm: true, appId: dm[1] ?? "", cleanMsg: m.slice(dm[0].length).trimStart() };
}
const BUILD_MARKS = { del: "［既存を削除して作り直す", keep: "［残して新しく作る］", split: "［機能ごとに分割して作成］", bulk: "［一括で作成］" };
const BUILD_CONFIRM_MARKER_RE = /^［(既存を削除して作り直す(:[A-Za-z0-9_-]+)?|残して新しく作る|機能ごとに分割して作成|一括で作成)］/;
function parseBuildMarks(message) {
  let m = (message || "").trim();
  const f = { del: false, delAppId: "", keep: false, split: false, bulk: false };
  for (; ; ) {
    const dm = m.match(/^［既存を削除して作り直す(?::([A-Za-z0-9_-]+))?］/);
    if (dm) {
      f.del = true;
      if (dm[1]) f.delAppId = dm[1];
      m = m.slice(dm[0].length).trimStart();
      continue;
    }
    if (m.startsWith(BUILD_MARKS.keep)) {
      f.keep = true;
      m = m.slice(BUILD_MARKS.keep.length).trimStart();
      continue;
    }
    if (m.startsWith(BUILD_MARKS.split)) {
      f.split = true;
      m = m.slice(BUILD_MARKS.split.length).trimStart();
      continue;
    }
    if (m.startsWith(BUILD_MARKS.bulk)) {
      f.bulk = true;
      m = m.slice(BUILD_MARKS.bulk.length).trimStart();
      continue;
    }
    break;
  }
...
function looksLikeBuildConfirmation(text, priorAssistant = "") {
  const raw = (text || "").trim();
  if (BUILD_CONFIRM_MARKER_RE.test(raw)) return true;
...
function estimateBuildScale(text) {
  const t = (text || "").slice(0, 6e3);
  let explicit = 0;
  for (const m of t.matchAll(/(\d{1,2})\s*(?:画面|ページ|タブ|フォーム)/g)) explicit = Math.max(explicit, Number(m[1]) || 0);
  const KINDS = ["一覧", "登録", "詳細", "検索", "絞り込み", "集計", "レポート", "設定", "カレンダー", "ダッシュボード", "入力", "編集", "グラフ", "予約", "在庫", "顧客", "スタッフ", "シフト", "月次", "分析", "承認", "申請"];
  const distinct = new Set(KINDS.filter((k) => t.includes(k))).size;
  const features = extractFeatureBullets(t, 30).length;
  const large = explicit >= 5 || distinct >= 6 || features >= 8 && distinct >= 3;
  const screens = explicit > 0 ? explicit : Math.min(distinct || 3, 8);
  return { screens, features, large };
}
function extractFeatureBullets(text, max = 8) {
  const seen = /* @__PURE__ */ new Set();
  const out = [];
  for (const line of (text || "").split("\n")) {
    if (out.length >= max) break;
    const bl = line.match(/^\s*(?:[・･]|[-*]\s|\d+[.)、])\s*(.+)$/);
    if (!bl) continue;
    const bold = bl[1].match(/\*\*([^*\n]{2,30})\*\*/);
    let s = (bold ? bold[1] : bl[1].split(/[—–\-:：|｜]/)[0]).replace(/[*#`]/g, "").trim();
    if (!s || /^(アプリ名|名称|ゴール|目的|概要|方針|利用者|権限|ロール|データ|保存|画面数?|項目|備考|前提|注意|以下)([:：（(]|$)/.test(s)) continue;
    s = s.slice(0, 40);
...
${reqText}` }], [], void 0, { maxTokens: htmlMax }).catch(() => null);
  ...
  if (env && (looksTruncated(html) || capped) && html.length <= RENDER_HTML_MAX) {
    html = await continueRenderHtml(env, model, html, paid, 4, capped && !looksTruncated(html), { builtScreens: builtScreensFromDef({ screens }) }).catch(() => html);
  }
...
function summarizeAppDefinition(def) {
  ...
  if (render && typeof render.html === "string") {
    render.html = `[render.html 省略：${render.html.length}文字。UIの骨子は screens[] を参照。全文が必要なら編集モード（「対象アプリ id=…」）で取得]`;
  }
  if (Array.isArray(d.screens)) {
    for (const s of d.screens) {
      if (s && typeof s.html === "string" && s.html.length > 500) s.html = s.html.slice(0, 500) + `…[省略 ${s.html.length - 500}文字]`;
    }
  }
  return d;
}
const GOOGLE_SHARED_PART_IDS = /* @__PURE__ */ new Set(["gmail", "calendar", "docs", "sheets", "slides", "forms", "tasks", "contacts", "meet", "drive"]);
async function execTool(ctx2, owner, baseUrl, name, args, role, activeTools, approved = false, forceApproval, summarizeAppDef = false) {
  const tool = activeTools.find((t) => t.name === name);
  if (tool) {
    const gpart = partOfTool(name);
    const effRequiredRole = tool.requiredRole ?? (gpart && GOOGLE_SHARED_PART_IDS.has(gpart.id) ? ["admin", "developer"] : void 0);
    if (effRequiredRole && !effRequiredRole.includes(role)) return `「${name}」を実行する権限がありません（${effRequiredRole.join("・")}のみ）。`;
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_CMwv8Br3.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_CMwv8Br3.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_CMwv8Br3.mjs");
...
${(i.text ?? "").slice(0, 500)}`;
    ...
    case "get_app": {
      ...
      const defOut = summarizeAppDef ? summarizeAppDefinition(design.definition ?? null) : design.definition ?? null;
      let defJson = JSON.stringify(defOut);
      if (summarizeAppDef && defJson.length > 2e4) defJson = defJson.slice(0, 2e4) + "…[以下省略：全文は編集モードで取得]";
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_BthdaNTE.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_B17j1uG_.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_CMwv8Br3.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BthdaNTE.mjs")).storageMode(env) === "r2";
  ...
  const exec = async (n, a) => {
    ...
    if (appMap.has(n)) {
      ...
      if (opts.unattended) {
        ...
        const approvalOn = forceApproval ?? await getApprovalMode(env);
        const need = await appRunNeedsApproval(ctx2, appId, screenId, approvalOn, a);
        if (need) {
          const auth = await authorizeAppRun(ctx2, appId, screenId, role);
          if (!auth.ok) return `この操作は実行できません：${auth.error}`;
          const saveRes = await runInstalledApp(ctx2, appId, a, owner, screenId, role, { phase: "save", approvalOn });
          if (!saveRes.ok) return `この操作は実行できません：${saveRes.error ?? "保存に失敗しました。"}`;
          const id = await createApproval(env, owner, n, { __appId: appId, __screenId: screenId, inputs: a, __phase: "send", __approvalOn: approvalOn }, need.preview, { role, appId, screenId, appVersion: auth.appVersion, subjectType: "installed", defHash: auth.defHash, permsHash: auth.permsHash }, ctx2);
          return `⚠️ この操作の外部送信は承認が必要です（${need.reason}）。保存は完了しました。
${need.preview}
「承認待ち」一覧（/approvals）で管理者が承認すると送信されます。承認ID: ${id}`;
        }
      }
...
${formatA2aResult(r.result)}` : `公開連絡に失敗：${r.error ?? ""}`;
    ...
    return execTool(ctx2, owner, baseUrl, n, a, role, activeTools, autoApproved, forceApproval, summarizeGetApp);
  };
  const hops = opts.maxHops ?? await agentMaxHops(env);
  ...
  summarizeGetApp = newBuildAsk;
  const canDev = isDeveloperRole(role);
  let runDecls = planOnly || provider === "workers_ai" ? [] : decls;
  if (newBuildAsk || !canDev || looksHugeBuild(latest)) runDecls = runDecls.filter((d) => d.name !== "propose_app");
  if (canDev && editMode && !planOnly && provider !== "workers_ai") {
    const idm = latest.match(/対象アプリ\s*id=\s*([A-Za-z0-9_-]+)/);
    const appId = idm?.[1];
    if (appId && (looksHugeBuild(latest) || await targetAppIsLarge(ctx2, appId))) {
      try {
        const { startAppEdit: startAppEdit2, kickBuild: kickBuild2 } = await Promise.resolve().then(() => appBuilder);
        const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_C4tL9AHR.mjs");
        const paid = await getWorkersPaid2(env).catch(() => false);
        const eId = await startAppEdit2(ctx2, { owner, sessionId: opts.sessionId, appId, instruction: latest, model: opts.modelId, paid });
        await kickBuild2(ctx2, eId, baseUrl, "agent-edit");
        opts.onEvent?.({ type: "delegated_build" });
        await logDiag(env, "info", "agent", `editMode→startAppEdit へ委譲 app=${appId}`).catch(() => {
        });
        return "承知しました。ご指定のアプリの修正を、工程ごとに順番にバックグラウンドで実装します。完了するとこの会話に表示し、ベル（通知）でもお知らせします（画面を離れても続行します）。少々お待ちください。";
      } catch (e) {
        await logDiag(env, "warn", "agent", `editMode 背景委譲に失敗（前景ツールループへフォールバック）: ${e?.message ?? e}`).catch(() => {
        });
      }
    }
  }
...
` + budgetNote;
  ...
  if (shouldForce) {
    ...
    const isHugeBuild = looksHugeBuild(reqText);
    if (isHugeBuild) await logDiag(env, "info", "agent", `巨大要件と判定＝同期合成を回避し app-builder へ委譲（confirmBuild=${confirmBuild}）`).catch(() => {
    });
    if (!isHugeBuild && wantsCustomUI) {
    ...
    if (!isHugeBuild && !isRunnableDefinition(def)) {
      opts.onEvent?.({ type: "tool", name: "propose_app" });
...
${reqText}` }], [], void 0, { maxTokens: 8e3 }).catch((e) => ({ error: { message: String(e?.message ?? e) } }));
    ...
    if (isRunnableDefinition(def)) {
      ...
      if (vr.ok) {
        ...
        const synthIssues = [];
        {
          const v = checkReadOnlyCollections(drec);
          if (v) synthIssues.push(v);
        }
        for (const u of unwiredRenderWriteScreens(drec)) synthIssues.push(`保存画面「${u.id}」がUIのどのボタンからも呼ばれていません（画面から保存できません）。`);
        for (const mf of checkRequestedFeatureOps(reqText, drec)) synthIssues.push(mf);
        const scn = Array.isArray(drec.screens) ? drec.screens.length : 0;
        if (scn === 0 && /保存|登録|一覧|管理|受付|申込/.test(reqText)) synthIssues.push("保存・一覧の機能を組み込めませんでした（表示のみの画面になっています）。");
        if (synthIssues.length) {
          await logDiag(env, "info", "agent", `synth正直化: issues=${synthIssues.length}`).catch(() => {
          });
          return withUsedAi(`アプリ「${d.name ?? ""}」の草案を作成しました（草案ID: ${res.id}）。ただ一部が未接続/未実装の可能性があります：
・${synthIssues.slice(0, 4).join("\n・")}

「アプリ」画面の「アプリ開発」で仕上げ・修正できます。`);
        }
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。"));
    ...
    if (!confirmBuild) {
      await logDiag(env, "info", "agent", "未合意のため背景ビルド委譲を保留＝合意を促す").catch(() => {
      });
      const pre = out && out !== HOPS_EXCEEDED ? out + "\n\n" : "";
      return withUsedAi(pre + "ご要望はおおよそ把握しました。この内容で作成してよろしければ「作成して」とお送りください。追加・修正したい点があれば続けてお知らせください。");
    }
    try {
      const { startAppBuild: startAppBuild2, kickBuild: kickBuild2 } = await Promise.resolve().then(() => appBuilder);
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_C4tL9AHR.mjs");
      const paid = await getWorkersPaid2(env).catch(() => false);
      const bId = await startAppBuild2(ctx2, { owner, sessionId: opts.sessionId, spec: reqText, model: opts.modelId, paid });
      await kickBuild2(ctx2, bId, baseUrl, "agent");
      opts.onEvent?.({ type: "delegated_build" });
...
async function runApprovedTool(ctx2, owner, baseUrl, role, tool, args) {
  if (typeof args.__appId === "string") {
    const sendOpts = args.__phase === "send" ? { phase: "send", approvalOn: args.__approvalOn !== false } : void 0;
    const res = await runInstalledApp(ctx2, args.__appId, args.inputs ?? {}, owner, args.__screenId ? String(args.__screenId) : void 0, role, sendOpts);
  ...
  if (typeof args.__draftId === "string") {
    const sendOpts = args.__phase === "send" ? { phase: "send", approvalOn: args.__approvalOn !== false } : {};
    const res = await runDraftApp(ctx2, args.__draftId, args.inputs ?? {}, owner, args.__screenId ? String(args.__screenId) : void 0, { role, ...sendOpts });
    return res.ok ? { ok: true, result: formatRunResult(res, baseUrl) } : { ok: false, error: res.error ?? "アプリ実行に失敗しました。" };
  }
...
const agent = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  looksLikeNewAppRequest,
  looksLikeSeparateNewApp,
  looksLikeStrongEditRequest,
...
function cfAgent(ctx2) {
  return {
    run: (i) => runAgent(ctx2, i.owner, i.text, i.image, i.baseUrl ?? "", i.role ?? "member", { history: i.history, model: i.model, modelId: i.modelId, sessionId: i.sessionId, onEvent: i.onEvent, signal: i.signal, mode: i.mode, feature: i.feature, audience: i.audience, attachments: i.attachments, maxHops: i.maxHops, waitUntil: i.waitUntil })
  };
}
...
export {
  unpublishDirectory as A,
  searchDirectory as B,
  reportDirectory as C,
  lineBotInfo as D,
  setLineWebhookEndpoint as E,
  setLineWebhookActive as F,
  slackAuthTest as G,
  buildCtx as H,
  verifyLineSignature as I,
  lineLoadingStart as J,
  lineReply as K,
  lineReplyQuick as L,
  index as M,
  egressCf as N,
  ...
  cancelAppBuild as b,
  cfEgressGateway as c,
  runApprovedTool as d,
  estimateDiscrepancy as e,
  startAppEdit as f,
  startAppBuild as g,
  processSummaryJobs as h,
  inferApp as i,
  processAppBuilds as j,
  kickBuild as k,
  linePush as l,
  processAppBuild as m,
  notifyOwnerDirect as n,
  processSiteBuilds as o,
  ...
  orgDisplayName as u,
  getPublicProfile as v,
  setPublicProfile as w,
  generateOrgProfile as x,
  verifyOrgExistence as y,
  publishDirectory as z
};
