...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_o_bCkQmM.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_UHCqn2Pz.mjs");
  await import("./index_f2x1sCW8.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BKW0vRzv.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_D5akHrmm.mjs");
  ...
  const { reviewQueue } = await import("./users_Bp2KjBvV.mjs");
  const { backupAlert } = await import("./backup_BKYO9M7j.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_VyEr6Vzt.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_DlJhVZeY.mjs");
