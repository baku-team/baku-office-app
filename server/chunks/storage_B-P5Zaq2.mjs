...
const EXEC_MAGICS = [
  { sig: [77, 90], label: "実行ファイル" },
  { sig: [127, 69, 76, 70], label: "実行ファイル" },
  { sig: [254, 237, 250, 206], label: "実行ファイル" },
  { sig: [254, 237, 250, 207], label: "実行ファイル" },
  { sig: [207, 250, 237, 254], label: "実行ファイル" },
  { sig: [206, 250, 237, 254], label: "実行ファイル" },
  { sig: [202, 254, 186, 190], label: "実行ファイル" },
  { sig: [35, 33], label: "スクリプト" }
];
const CONTENT_MAGICS = [
  { sig: [137, 80, 78, 71], mime: "image/png" },
  { sig: [255, 216, 255], mime: "image/jpeg" },
  { sig: [71, 73, 70, 56], mime: "image/gif" },
  { sig: [37, 80, 68, 70], mime: "application/pdf" }
];
function sigMatch(b, sig) {
  if (b.length < sig.length) return false;
  for (let i = 0; i < sig.length; i++) if (b[i] !== sig[i]) return false;
  return true;
}
function inspectMagic(head) {
  for (const m of EXEC_MAGICS) if (sigMatch(head, m.sig)) return { reject: m.label };
  for (const m of CONTENT_MAGICS) if (sigMatch(head, m.sig)) return { detectedMime: m.mime };
  return {};
}
async function saveFile(env, file, createdBy, ctx = "org", opts) {
  ...
  const insp = inspectMagic(new Uint8Array(plain.slice(0, 16)));
  if (insp.reject) throw new Error(`このファイルは安全のため受け取れません（${insp.reject}の可能性があります）。`);
  const mime = insp.detectedMime ?? file.type ?? null;
  if (opts?.dest === "drive") {
    const { saveBufferToDriveFolder } = await import("./drive_CoI0Yc4D.mjs");
    const up = await saveBufferToDriveFolder(env, file.name || "file", mime || "application/octet-stream", plain, opts.folderName);
    if (!up) throw new Error("Google ドライブへの保存に失敗しました。連携状態（設定→連携）をご確認ください。");
    await env.DB.prepare("INSERT INTO files (id,name,size,mime,ref,created_by,created_at,enc,expires_at,ctx) VALUES (?,?,?,?,?,?,?,0,NULL,?)").bind(id, file.name || "file", size, mime, `drive:${up.id}`, createdBy, nowSec(), ctx).run();
...
async function readBlob(env, ref, enc) {
  ...
  if (ref.startsWith("drive:")) {
    const { readDriveFile } = await import("./drive_CoI0Yc4D.mjs");
...
async function deleteBlob(env, ref) {
  if (ref.startsWith("drive:")) {
    const { deleteDriveFile } = await import("./drive_CoI0Yc4D.mjs");
