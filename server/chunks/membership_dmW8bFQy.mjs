...
import { r as renderComponent, s as spreadAttributes } from "./worker-entry_DIzVSdtb.mjs";
...
const $$Membership = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession, canAccess } = await import("./auth_BbUuA01A.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses || ses.ctx !== "org") return Astro2.redirect("/login", 302);
  const canEdit = canAccess(ses.role, "accounting");
  const { listMembers, memberStats, FEE_STATUSES, FEE_LABEL } = await import("./membership_Dhgs4Lyc.mjs");
