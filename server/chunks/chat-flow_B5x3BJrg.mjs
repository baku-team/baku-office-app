...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_C0xk2SHi.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_qgxfbXK9.mjs";
import { canDevelopApps } from "./auth_Bh_tJP0x.mjs";
import { cachedEntitlement } from "./client_BKYtLCd6.mjs";
...
${txt}` };
  ...
  if (mime.startsWith("image/") || mime === "application/pdf") {
    ...
    const { inferApp } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.P);
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.W);
  ...
  const { latestSessionApp } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.U);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_ehcQ_Xp6.mjs").then((n) => n.C);
...
async function tryPreAgentRouting(ctx, cfContext, args) {
  ...
  const priorAssistant = [...prior].reverse().find((m) => m.role === "assistant")?.content ?? "";
  const { looksLikeBuildConfirmation, looksLikeUiModeChoice } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.W);
  if (notPlan && canDevelopApps(role) && looksLikeBuildConfirmation(message, priorAssistant)) {
    const { startAppBuild, processAppBuild, buildModelGuide } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.U);
  ...
  if (notPlan && canDevelopApps(role) && atLeast(await cachedEntitlement(env), "pro")) {
    const { looksLikeAppEdit } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.W);
    if (looksLikeAppEdit(message)) {
      const { latestSessionApp, resolveAppByName, startAppEdit, processAppBuild, buildModelGuide } = await import("./ctx_C3hyS9pN.mjs").then((n) => n.U);
...
async function tryProHopsContinuation(ctx, cfContext, args) {
  ...
  if (!atLeast(await cachedEntitlement(env), "pro")) return null;
  const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_C1guhANw.mjs");
