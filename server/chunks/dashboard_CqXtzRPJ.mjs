...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BWXt_wx-.mjs");
  const { cmpVersion } = await import("./update_CYPidvd8.mjs");
  await import("./index_BwPbfhwG.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_cG6KZTqF.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BBN7Q7J1.mjs");
  ...
  const { reviewQueue } = await import("./users_CxdV4_xq.mjs");
  const { backupAlert } = await import("./backup_BUxedpdr.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BLHrxe4n.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage__hw20N6r.mjs");
