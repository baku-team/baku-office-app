...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CTBMIurh.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_C0Raud0A.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_DloQDFvv.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_DloQDFvv.mjs").then((n) => n.P);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_DB-xHIgj.mjs").then((n) => n.z);
