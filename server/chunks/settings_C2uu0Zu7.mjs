...
function isMemberEngine(v) {
  return v === "gemini" || v === "claude" || v === "local";
}
async function getMemberEngine(env, uid) {
  if (!uid) return null;
  const v = await env.LICENSE.get("member_engine:" + uid);
  return isMemberEngine(v) ? v : null;
}
async function setMemberEngine(env, uid, e) {
  const v = isMemberEngine(e) ? e : "gemini";
  await kvPut(env, "member_engine:" + uid, v);
  return v;
}
