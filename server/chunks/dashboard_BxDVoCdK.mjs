...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_DM4zCU7e.mjs");
  const { cmpVersion } = await import("./update_Bivej0_M.mjs");
  await import("./index_DuHFU0n5.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_oLPd3RSw.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cupq3nDe.mjs");
  ...
  const { reviewQueue } = await import("./users_DwzuO0m5.mjs");
  const { backupAlert } = await import("./backup_CNZ07UtO.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_D5nJw62F.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_CqYezlNs.mjs");
