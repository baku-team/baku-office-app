...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { canDevelopApps } = await import("./auth_C5n36hBk.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_CCwYpYbr.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_BCrexcwr.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_C75Q_n4c.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_AfFtxawQ.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_Ish93S4w.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_DEwZjiaF.mjs");
