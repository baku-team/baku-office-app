...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_CB_YAuNH.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_B6Gdy7_n.mjs");
  await import("./index_CVpv3OCE.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CCYyWIXk.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_46Iv_8nD.mjs");
  ...
  const { reviewQueue } = await import("./users_C32ZSfGd.mjs");
  const { backupAlert } = await import("./backup_CEC5ZlvD.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DbldLbJu.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_Cvzvo6pu.mjs");
