...
import { getApiKey } from "./client_peeVfBbE.mjs";
import { c as cfEgressGateway, l as linePush, h as processSummaryJobs, j as processAppBuilds, m as processAppBuild, o as processSiteBuilds } from "./ctx_-pDHDD3Z.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_t2W-o4hP.mjs";
import { processAgentJobs } from "./agent-jobs_Bl1hqTFw.mjs";
import { reclaimStaleApprovals } from "./approvals_BRjh5QAu.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_DaP_ABtd.mjs";
import { guardHeavy } from "./diag_-P_u6Fn4.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_Cg3mGgJJ.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_C5D8WI6y.mjs";
import { flushReports } from "./reports_RhxclsfF.mjs";
import { addNotification, pushWebhook } from "./notifications_DmXXQIJl.mjs";
import { getBuildDriver, getNotifyWebhook } from "./settings_C4tL9AHR.mjs";
import { purgeFiles } from "./storage_DAEZsEe4.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_C0Fkqr3v.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  ...
  const at = await guardHeavy(env, "app triggers", () => import("./integration-polling_8Lxn6ntt.mjs").then((m) => m.pollAppTriggers(env, locals.ctx)));
  const appTriggers = at.ok ? at.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_DZEsTyH6.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
  ...
  const wk = await guardHeavy(env, "weekly kpi report", () => import("./kpi_-NRNhxSP.mjs").then((m) => m.maybeSendWeeklyReport(env)));
  const weeklyReport = wk.ok ? wk.value : false;
  const ud = await guardHeavy(env, "usage digest", () => import("./usage-digest_CE4kVWYW.mjs").then((m) => m.flushUsageDigest(env)));
