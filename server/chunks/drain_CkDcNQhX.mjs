...
import { getApiKey } from "./client_CuoW304l.mjs";
import { t as cfEgressGateway, B as linePush, C as processSummaryJobs, D as processAppBuilds, E as processSiteBuilds } from "./ctx_DiLiyXJK.mjs";
import { d as dueReminders, m as markReminderDone } from "./invoices_Cm4Zc-nT.mjs";
import { pollVideoJobs } from "./capabilities_CJ_aE4sG.mjs";
import { processAgentJobs } from "./agent-jobs_Ddsckp_T.mjs";
import { reclaimStaleApprovals } from "./approvals_D-05v0Pw.mjs";
import { runDueScheduledTasks } from "./scheduled-tasks_C346P2f2.mjs";
import { guardHeavy } from "./diag_D9aXYLsc.mjs";
import { getDriveBackup, driveConnected, backupToDrive, uploadBufferToDrive } from "./drive_w3tBkj4X.mjs";
import { getBackupSchedule, backupAlert, buildBackup, backupFileName, recordBackupDone } from "./backup_Co22Tg80.mjs";
import { flushReports } from "./reports_BSDefUoq.mjs";
import { addNotification, pushWebhook } from "./notifications_CMMN1v0i.mjs";
import { getNotifyWebhook } from "./settings_C_1axsCz.mjs";
import { purgeFiles } from "./storage_fwjkKF34.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const ub = await guardHeavy(env, "upload batches", () => import("./upload-queue_Du1qglAI.mjs").then((m) => m.processReadyUploads(locals.ctx, origin, 5)));
  const uploadBatches = ub.ok ? ub.value : 0;
  const au = await guardHeavy(env, "auto update", () => import("./update_D-ttyOpV.mjs").then((m) => m.maybeAutoUpdate(env, origin)));
