...
import { r as renderComponent } from "./worker-entry_CPXAzpHR.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_tqGS462O.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CDgvn5Qk.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_RgvQ0ags.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_BYcOWt22.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_CdbeqiTB.mjs"),
    import("./scheduled-tasks_Cs3vSthK.mjs")
  ]);
  ...
  await import("./index_CT3c9yT8.mjs");
