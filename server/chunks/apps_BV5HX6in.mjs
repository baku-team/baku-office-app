...
import { r as renderComponent } from "./worker-entry_DP3aFonH.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_BUTIA-KI.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_BnN5IZQs.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_BnN5IZQs.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./cf-adapter_D-L40dlj.mjs").then((n) => n.z);
  const entitlement = await (await import("./client_DM4zCU7e.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DuHFU0n5.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_oLPd3RSw.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_D5nJw62F.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_N6ITNHBy.mjs");
  ...
  const folders = await (await import("./settings_D2rjJD-A.mjs")).getAppFolders(env, ses.uid).catch(() => []);
  const projectsList = isAdminOrg ? await import("./projects_GnqKk175.mjs").then((m) => Promise.all([m.listProjects(env), m.projectAppCounts(env)])).catch(() => [[], {}]) : [[], {}];
