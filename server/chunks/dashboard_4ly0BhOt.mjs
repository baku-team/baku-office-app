...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C1dOp0Cn.mjs");
  const { cmpVersion } = await import("./update_CC0Kfql3.mjs");
  await import("./index_3wN2fDas.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CNr3eCe2.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_dc8_ScnM.mjs");
  ...
  const { reviewQueue } = await import("./users_IdlFUeaG.mjs");
  const { backupAlert } = await import("./backup_BwdZ_G3O.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Czv7yIer.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_Ik0IyZy_.mjs");
