...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C1ICAElS.mjs");
  const { cmpVersion } = await import("./update_kaZ9S9yw.mjs");
  await import("./index_B7RAaLcs.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DQMqrpyH.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_DEjww2Hw.mjs");
  ...
  const { reviewQueue } = await import("./users_D0oZu9b6.mjs");
  const { backupAlert } = await import("./backup_B8IRMuqo.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_ppKOQgJW.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_DJiP0NOX.mjs");
