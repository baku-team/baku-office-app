...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_CFjJVPOM.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_Dun_Y6hX.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./cf-adapter_kh_TzdXt.mjs").then((n) => n.D);
  ...
  const { latestSessionApp } = await import("./cf-adapter_kh_TzdXt.mjs").then((n) => n.C);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_2G5fL5LC.mjs").then((n) => n.z);
