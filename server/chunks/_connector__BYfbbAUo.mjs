...
import { cachedEntitlement, getApiKey } from "./client_DBc4AdQ5.mjs";
import { DiscordInbound } from "./discord_QiT67BSn.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { joinWithInvite } from "./users_DYTaJK75.mjs";
import { F as transcribeAudio, G as verifyLineSignature, H as lineReply, I as lineReplyQuick, t as cfEgressGateway } from "./ctx_BPtaGmAl.mjs";
import { saveFile } from "./storage_B8-WCIen.mjs";
...
async function joinViaInvite(env2, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_DYTaJK75.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_CcLg3m-E.mjs");
  const bins = [];
...
class LineInbound {
    ...
    for (const ev of events) {
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS } = await import("./upload-queue_CcLg3m-E.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_CcLg3m-E.mjs");
