...
import { rateLimited, getPublicPage, honeypotTripped, validateSubmission, createSubmission, setSubmissionPayment, moderate, fireSubmitTriggers } from "./public-pages_BaPDC316.mjs";
import { createCheckoutSession } from "./payments_BaSHS40h.mjs";
import { maxUploadMb, saveFile } from "./storage_DJiP0NOX.mjs";
...
const POST = async ({ request, params, locals }) => {
  ...
  if (!v.ok) {
    await (await import("./diag_DUN9A_L9.mjs")).logDiag(env, "warn", "public", `公開フォーム送信NG slug=${slug}: ${v.error}`, `送信キー=[${Object.keys(values).join(",")}] / 必須項目=[${page2.fields.filter((f) => f.required).map((f) => f.name).join(",")}]`).catch(() => {
    });
