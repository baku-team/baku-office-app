...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_BM9R08qO.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_BHT8hMec.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_11AnePHL.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_DB2YNaub.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_CRZxmueG.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_BuJbodmQ.mjs");
