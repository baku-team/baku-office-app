...
import { cachedEntitlement } from "./client_DBc4AdQ5.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  let visionImage;
  if (b.image?.dataB64) {
    const { prepareAttachment } = await import("./chat-flow_DjmjPPXw.mjs");
...
`));
  ...
  (async () => {
    try {
      ...
      const { looksLikeBuildConfirmation, looksLikeUiModeChoice, UI_MODE_QUESTION, UI_MODE_ACTIONS } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.T);
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && looksLikeBuildConfirmation(message, priorAssistant)) {
        const { startAppBuild, processAppBuild, buildModelGuide } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.R);
      ...
      if (b.mode !== "plan" && !visionImage) {
        const { tryHandleAppDelete } = await import("./chat-flow_DjmjPPXw.mjs");
      ...
      if (b.mode !== "plan" && !visionImage && canDevelopApps(ses.role, ses.ctx) && atLeast(await cachedEntitlement(env), "pro")) {
        const { looksLikeAppEdit } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.T);
        if (looksLikeAppEdit(message)) {
          const { latestSessionApp, resolveAppByName, startAppEdit, processAppBuild, buildModelGuide } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.R);
      ...
      if (reply === HOPS_EXCEEDED && atLeast(await cachedEntitlement(env), "pro")) {
        const { enqueueAgentJob, processAgentJobs } = await import("./agent-jobs_BkXvXfUh.mjs");
        ...
        const { buildReplyActions } = await import("./chat-flow_DjmjPPXw.mjs");
