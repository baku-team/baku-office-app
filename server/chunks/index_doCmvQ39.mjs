...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_DJwkpzjq.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_Du2JHuMP.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_DzDhNcts.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_CJuuAfa1.mjs"),
    import("./scheduled-tasks_D2zX51oE.mjs")
  ]);
  ...
  await import("./index_C12A0H0f.mjs");
