...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BHT8hMec.mjs")).cachedEntitlement(env);
  const hasPlus = atLeast(entitlement, "plus");
  const hasPro = atLeast(entitlement, "pro");
  const { getAiEngine, getCustomPrompt, getWorkersPaid, getSmartRouting } = await import("./settings_C4tL9AHR.mjs");
  const workersPaid = hasPlus ? await getWorkersPaid(env) : false;
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DTd_eHEW.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Bflf1Se6.mjs");
