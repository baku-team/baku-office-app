...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_D75KjaIu.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { canDevelopApps } from "./auth_Ddwkb9TX.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_ga_0ayig.mjs").then((n) => n.R);
  ...
  const { latestSessionApp } = await import("./ctx_ga_0ayig.mjs").then((n) => n.P);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_Dr0mvakg.mjs").then((n) => n.z);
