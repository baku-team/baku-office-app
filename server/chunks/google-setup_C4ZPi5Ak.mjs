...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BcZ-G960.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_C8P8uqY3.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_vts0a43M.mjs");
  ...
  const { buildCtx } = await import("./ctx_DZmKLyOO.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_DZmKLyOO.mjs").then((n) => n.M);
