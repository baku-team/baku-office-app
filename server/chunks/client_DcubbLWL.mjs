...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_ga_0ayig.mjs").then((n) => n.S), import("./notifications_CVAwxfIj.mjs")]);
    ...
    await (await import("./ctx_ga_0ayig.mjs").then((n) => n.O)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_ga_0ayig.mjs").then((n) => n.Q)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_DCmK1lHn.mjs");
