...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_CzH8Lj8g.mjs").then((n) => n.S), import("./notifications_CWqQzglb.mjs")]);
    ...
    await (await import("./ctx_CzH8Lj8g.mjs").then((n) => n.P)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_CzH8Lj8g.mjs").then((n) => n.M)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_B_pd6msK.mjs");
