...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CVhIpi7y.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_Bib4yTGq.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_BFUyWbe8.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_nBAdPSxh.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BIy17cIt.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_DajNP6oI.mjs");
