...
import { r as renderComponent } from "./worker-entry_Dgx8xQ02.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_DILFvID6.mjs";
...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Apps;
  const { getSession } = await import("./auth_D0ErBMBg.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const ctx = Astro2.locals.ctx;
  const { canDevelopApps } = await import("./auth_D0ErBMBg.mjs");
  const isAdminOrg = canDevelopApps(ses.role, ses.ctx);
  const { atLeast, planLabel } = await import("./ctx_DIbSr9wk.mjs").then((n) => n.K);
  const entitlement = await (await import("./client_DlpdsadF.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_B2V0XL0x.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CqmHVZPm.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers } = await import("./external-apps_BTgdbc5l.mjs").then((n) => n.z);
  const { storeCatalog, myApps } = await import("./store_awGg7Iqk.mjs");
