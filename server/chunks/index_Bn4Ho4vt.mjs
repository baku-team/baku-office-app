...
import { r as renderComponent } from "./worker-entry_BADHBBiv.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_50HMszLw.mjs";
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_Dxqiw4Ha.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_Cti--qkw.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_CPTo5Zi4.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_DQykcWLS.mjs"),
    import("./scheduled-tasks_DmOGQ7eu.mjs")
  ]);
  ...
  await import("./index_UJbeOQUy.mjs");
