...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_B0LtoEmz.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_DgoYClVo.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_B8-hnGVx.mjs");
