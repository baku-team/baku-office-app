...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_RgvQ0ags.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_Cfb-TADr.mjs");
  await import("./index_CT3c9yT8.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_AVdbmGFq.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Cv5VmzbI.mjs");
  ...
  const { reviewQueue } = await import("./users_pLJUzq-G.mjs");
  const { backupAlert } = await import("./backup_PxgSKzJN.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DLPZncV7.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_D2XGvsLY.mjs");
