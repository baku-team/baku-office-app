...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_l0SzJYis.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_r7aC_zsV.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_CtyKcQMu.mjs");
  ...
  const { buildCtx } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.Y);
  ...
  const { randomId } = await import("./ctx_BHqm0CfS.mjs").then((n) => n.M);
