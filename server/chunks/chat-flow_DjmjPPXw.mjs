...
import { isTextAttachmentMime, saveChatAttachment } from "./storage_B8-WCIen.mjs";
import { d as dedupeActions, f as filterAiActions, n as navGuidance, a as appendMessage } from "./chat-sessions_DJA_I8xR.mjs";
import { canDevelopApps } from "./auth_FZErHGWA.mjs";
...
async function tryHandleAppDelete(ctx, sessionId, role, sesCtx, message, prior) {
  ...
  const { looksLikeAppDelete, looksLikeDeleteConfirmation } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.T);
  ...
  const { latestSessionApp } = await import("./ctx_BPtaGmAl.mjs").then((n) => n.R);
  const appId = await latestSessionApp(ctx, sessionId);
  if (!appId) return null;
  const { getAppDesign, deleteGenApp } = await import("./external-apps_H2tcQp6t.mjs").then((n) => n.B);
