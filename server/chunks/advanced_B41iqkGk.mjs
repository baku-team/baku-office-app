...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_CryjS_8W.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_vuCzOMhh.mjs");
  ...
  const { hasApiKey } = await import("./client_CryjS_8W.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_Bz3EKlWm.mjs");
