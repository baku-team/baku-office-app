...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_Dy72pGSg.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_BAVQqvb3.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_z7F-7bMo.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_B5Vd2zbs.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_YQVzQqzP.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_DQ4a_u1m.mjs");
