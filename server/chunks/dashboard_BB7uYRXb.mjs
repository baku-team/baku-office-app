...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_92mF-n-g.mjs");
  const { cmpVersion } = await import("./update_BjD6N7MW.mjs");
  await import("./index_Cbs5PFmm.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_C7rooffR.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dsr7xbg2.mjs");
  const { kvWritesToday, KV_WRITE_FREE_LIMIT, kvWriteLimit } = await import("./kv_CLOebsry.mjs");
  ...
  const { reviewQueue } = await import("./users_CJsKDrbc.mjs");
  const { backupAlert } = await import("./backup_NU6GHmBC.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BIgCZPwg.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_UGqxcZD2.mjs");
  const { getWorkersPaid } = await import("./settings_CBzcH790.mjs");
  const { monthNeurons, monthUsd, monthTokens } = await import("./usage_gjxCIj9B.mjs");
