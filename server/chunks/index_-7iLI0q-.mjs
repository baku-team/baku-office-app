...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_C0wNwuGo.mjs");
  const { cmpVersion } = await import("./update_BxH63k1a.mjs");
  await import("./index_DbT-Uvf1.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_DFrbpPbI.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_LzIpyIdO.mjs");
  ...
  const { reviewQueue } = await import("./users_CTvd8Ywn.mjs");
  const { backupAlert } = await import("./backup_BeO-c_ZZ.mjs");
