...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_TWPTdJdo.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_Ds5oncnk.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DHYPZETu.mjs");
