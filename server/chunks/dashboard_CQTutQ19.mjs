...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec } = await import("./client_BCrexcwr.mjs");
  const { cmpVersion } = await import("./update_CuFDP5vg.mjs");
  await import("./index_C75Q_n4c.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_AfFtxawQ.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_A84_kyQi.mjs");
  ...
  const { reviewQueue } = await import("./users_Ck_fNAQ4.mjs");
  const { backupAlert } = await import("./backup_QVWqwlx2.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_Ish93S4w.mjs").then((n) => n.z);
  ...
  const { storageMode } = await import("./storage_aIMHFm6C.mjs");
