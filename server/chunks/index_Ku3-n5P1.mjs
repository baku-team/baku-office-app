...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CoFUCzQz.mjs");
  const ses = await getSession(env, Astro2.request);
  if (!ses) return Astro2.redirect("/login", 302);
  const { cachedEntitlement, hasApiKey } = await import("./client_CY40n_Ph.mjs");
  ...
  const ttsCloudReady = await (await import("./capabilities_DDmgSIQr.mjs")).ttsConfigured(env).catch(() => false);
  ...
  const [{ getMemberModel }, { listSkills }, { listScheduledTasks }] = await Promise.all([
    ...
    import("./skills_CXKDAbMV.mjs"),
    import("./scheduled-tasks_BsIP5_Ud.mjs")
  ]);
  ...
  await import("./index_aE8388t6.mjs");
