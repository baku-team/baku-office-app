...
import { kvPut } from "./kv_DJFUfwu0.mjs";
import { AppError, explainStop } from "./errors_Bw4GoHz5.mjs";
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, fileBelongsTo, storageMode } from "./storage_BF4jK8tm.mjs";
import { getApiKey, getToken, hostFetch, entitlementForGate, masterKeyCtx } from "./client_vij6I9Jq.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification } from "./notifications_CdoRv0_M.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CPDZKemR.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_BgEl515W.mjs";
import "./index_B3tnatTI.mjs";
import { recordUsage, recordTokens, overBudget, estimateUsd } from "./usage_Y47VMyUf.mjs";
import { claudeModelId, geminiModelId, openaiModelId, DEFAULT_MODELS, isValidClaudeModel, isValidWorkersAiModel, workersAiModelId, isValidGeminiModel, isValidOpenAiModel, isValidGrokModel, grokModelId, isValidGithubModelsModel, githubModelsModelId } from "./config_DIXTf4RD.mjs";
import { googleFetch } from "./google_LDFRfhIk.mjs";
...
import { validateDefinition, APP_SCHEMA, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, RENDER_HTML_MAX, opCatalogText } from "./appdef_ByA9vXJX.mjs";
import { runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_Dutk61oa.mjs";
import { getWorkersPaid, setNotifyWebhook, setWorkersPaid, setWorkersAiModel, setBookkeepingMode, setAgentName, setCustomPrompt, setAiEngine, getAiEngine, getBookkeepingMode, getCustomPrompt, getAgentName, resolveModelSelection, getMemberModel, getWorkersAiModel, maxParallelAgents, agentMaxHops } from "./settings_BY28V0AV.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_B9718pEB.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_XigEDpj9.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, autonomyReady, AUTONOMY_TOOLS, runAutonomyTool, AUTONOMY_POLICY } from "./autonomy_BdH0pB7d.mjs";
import { logDiag } from "./diag_Ca0PUS5y.mjs";
import { i as isRunnableDefinition } from "./preflight_BpEyu-un.mjs";
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage, collectionFromDefinition } from "./public-pages_kcQ20wlf.mjs";
import { g as getAppDesign, m as repairOutputLiterals, n as createDraft, o as getAppMeta, q as setAppMeta, t as suggestAppMeta, i as installLocalApp, v as setAppAllowedRoles, b as listExternalApps, w as installedAppDefs, x as appsBrief } from "./external-apps_Dk15ONnL.mjs";
import { a as appendMessage } from "./chat-sessions_BLqLinNd.mjs";
import { a as resolveChannel } from "./connectors-store_CHWkgRyl.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DvB2WseM.mjs";
import { e as runInstalledApp, r as runDraftApp, c as appRunNeedsApproval, a as authorizeAppRun } from "./app-runtime_DGZDs5Yw.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_DK4CePgC.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DRIY_sOG.mjs";
import { setCustomDomain } from "./custom-domain_0fz0VPJf.mjs";
import { p as pushToUser } from "./push_DbE3q74f.mjs";
...
async function runAgent(ctx, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  if (!geminiKey && !claudeKey && !openaiKey && !grokKey && !githubModelsKey && !env.LOCAL_AI_BASE_URL && !env.AI) return "AI機能が未設定です。管理画面の『連携設定』で Gemini / Claude / OpenAI / Grok / GitHub Models のいずれかのAPIキーを登録してください。";
  ...
  const { disabledBuiltins } = await import("./client_vij6I9Jq.mjs");
...
${custom}`].filter(Boolean).join("\n");
  ...
  let model = null;
  let provider = "gemini";
  const wantLocal = want === "local" || !geminiKey && !claudeKey && !openaiKey && !grokKey && !githubModelsKey;
  ...
  if (want === "grok" && grokKey) {
    ...
    if (!model) return "選択中のエンジンが未設定です。『設定 → 連携設定』で Gemini / Claude / OpenAI / Grok / GitHub Models のAPIキーを登録してください。";
  }
  let fellBack = false;
  let switchErr;
  if (model && (provider === "gemini" || provider === "claude" || provider === "openai" || provider === "grok" || provider === "github_models") && env.AI) {
...
` + (res.gate === "ready" ? "「アプリ」画面の「アプリ開発」から確認・有効化できます。" : "一部の事前確認に課題があります。「アプリ」画面の「アプリ開発」で内容をご確認ください。");
    ...
    try {
      ...
      const { getWorkersPaid: getWorkersPaid2 } = await import("./settings_BY28V0AV.mjs");
