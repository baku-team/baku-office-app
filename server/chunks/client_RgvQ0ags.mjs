...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_X9wJ7pe3.mjs").then((n) => n.Y), import("./notifications_CzJHBeD1.mjs")]);
    ...
    await (await import("./ctx_X9wJ7pe3.mjs").then((n) => n.U)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_X9wJ7pe3.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_B81bKWy7.mjs");
...
async function entitlementForGate(env, maxAgeSec = 6 * 3600, waitUntil) {
  ...
  const fresh = Number(await env.LICENSE.get(KV_ENTITLEMENT_AT));
  if (!Number.isFinite(fresh) || nowSec() - fresh > 30 * 86400) return "free";
