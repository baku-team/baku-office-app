...
import { r as renderComponent } from "./worker-entry_CSO5K3OJ.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_CowLAIlK.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_CNSCg6Zs.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_DcvmK-U8.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BaY8k7XO.mjs");
  await import("./index_J60yRFFB.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CpqQ2B4g.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_BgddlvmG.mjs");
  ...
  const { reviewQueue } = await import("./users_plxkvjND.mjs");
  const { backupAlert } = await import("./backup_CMH8t5--.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_CZwKXiCg.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_CtLBMcNN.mjs");
