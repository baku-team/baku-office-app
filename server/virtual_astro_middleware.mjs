...
import { masterKeySource, getToken } from "./chunks/client_Bib4yTGq.mjs";
import { e as ensureSchema } from "./chunks/migrate_BC3gUe6z.mjs";
import { kvPut } from "./chunks/kv_ryUreqJI.mjs";
import { logDiag } from "./chunks/diag_-P_u6Fn4.mjs";
import { sameOrigin, getSession } from "./chunks/auth_DlJYu5pd.mjs";
import { n as needsConsent } from "./chunks/consent_DrWTD5Jd.mjs";
import { H as buildCtx } from "./chunks/ctx_CVhIpi7y.mjs";
...
import { a as isCuratedHost } from "./chunks/cdn-allowlist_rKupC5M_.mjs";
...
function appPreviewCsp(allowHosts) {
  const lib = (allowHosts || []).map((h) => String(h).trim().toLowerCase()).filter((h) => isCuratedHost(h)).map((h) => "https://" + h).join(" ");
  if (!lib) return CSP;
  return CSP.replace("script-src 'self' 'unsafe-inline'", `script-src 'self' 'unsafe-inline' ${lib}`).replace("style-src 'self' 'unsafe-inline'", `style-src 'self' 'unsafe-inline' ${lib}`);
}
...
const onRequest$1 = defineMiddleware(async (context, next) => {
  ...
  try {
    ...
    else if (context.locals.appPreviewAllowHosts) out.headers.set("Content-Security-Policy", appPreviewCsp(context.locals.appPreviewAllowHosts));
    return out;
  } catch (e) {
