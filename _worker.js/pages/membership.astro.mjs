...
import { e as createComponent, k as renderComponent, r as renderTemplate, h as createAstro, m as maybeRenderHead, g as addAttribute, s as spreadAttributes } from '../chunks/astro/server_CfYoLHqm.mjs';
...
const $$Membership = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession, canAccess } = await import('../chunks/auth_ujH5pbJJ.mjs');
  const ses = await getSession(env, Astro2.request);
  if (!ses || ses.ctx !== "org") return Astro2.redirect("/login", 302);
  const canEdit = canAccess(ses.role, "accounting");
  const { listMembers, memberStats, FEE_STATUSES, FEE_LABEL } = await import('../chunks/membership_DUxPJliV.mjs');
