...
async function saveAutonomyConfig(env, a) {
  ...
  if (a.cfToken) {
    ...
    if (!a.cfAccount) {
      ...
      if (det) {
        await env.LICENSE.put(KV_CF_ACCT, det);
        out.cfAccount = det;
  ...
  if (a.ghRepo !== void 0) await env.LICENSE.put(KV_GH_REPO, a.ghRepo.trim());
  return out;
}
...
async function resolveGhClientId(env) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (j.clientId) {
        await env.LICENSE.put("gh_client_id", j.clientId, { expirationTtl: 86400 });
        return j.clientId;
      }
    }
  } catch {
...
export { AUTONOMY_POLICY, AUTONOMY_TOOLS, autonomyReady, cfCreateD1, cfCreateKv, cfDeploy, cfDetectAccount, cfListResources, getAutonomyConfig, ghCommitFile, ghCreateBranch, ghDeviceAvailable, ghDevicePoll, ghDeviceStart, ghListRepos, ghMergePr, ghOpenPr, ghReadFile, isAutonomyOn, resolveGhClientId, runAutonomyTool, saveAutonomyConfig, setAutonomy };
