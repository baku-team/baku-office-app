...
async function geminiJSON(key, sys, prompt) {
...
async function claudeJSON(key, sys, prompt) {
...
${input}` }]
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "x-api-key": key, "anthropic-version": "2023-06-01", "content-type": "application/json" },
    body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 4e3, system: `あなたは次のスキル手順に従う作成アシスタント。本文(Markdown)のみ出力。
...
export { createSkill, deleteSkill, generateSkill, listSkills, runSkill, setSkillEnabled };
