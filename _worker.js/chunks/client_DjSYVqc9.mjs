...
import { g as generateMasterKey, d as decryptField, e as encryptField } from './crypto_D21r3Dwx.mjs';
...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    await env.LICENSE.put(KV_ENTITLEMENT_AT, String(nowSec()));
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import('./reports_CSdFXBi2.mjs');
...
class MasterKeyMissingError extends Error {
  constructor() {
...
export { APP_VERSION, MasterKeyMissingError, cachedEntitlement, deleteApiKey, disabledBuiltins, entitlementForGate, getApiKey, getLicenseId, getToken, getVerifyJwk, hasApiKey, hostFetch, isProduction, masterKey, masterKeyCtx, masterKeySource, nowSec, pollHost, saveApiKey, saveToken, validateApiKey };
