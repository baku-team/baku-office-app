...
import { getApiKey, hostFetch, getVerifyJwk, getLicenseId } from './client_KUuDosgV.mjs';
import { v as verifyEnvelope, i as importVerifyKey, p as payloadOf, r as randomId } from './crypto_BhRWVEcj.mjs';
import { nowSec } from './accounting_BOhbglhy.mjs';
import { getStorageUsage } from './storage-usage_Zli-bXJH.mjs';
import { getLimits, monthTotals } from './usage_CVlW6nuV.mjs';
...
async function preflight(ctx, d) {
  ...
  const month = await monthTotals(env).catch(() => ({}));
  const cap = limits.gemini?.monthlyCap ?? limits.claude?.monthlyCap;
...
async function fetchAndInstall(ctx, id) {
  ...
  let r;
  try {
    r = await hostFetch(env, "/api/registry/fetch?id=" + encodeURIComponent(id));
  } catch {
...
async function listExternalApps(ctx) {
  const { results } = await ctx.db.prepare("SELECT id,name,version,category,description,permissions FROM external_apps ORDER BY installed_at DESC").all();
...
async function listDrafts(ctx) {
  const { results } = await ctx.db.prepare("SELECT id,name,version,description,spec,permissions,status,gate_status,preflight FROM app_drafts ORDER BY created_at DESC").all();
...
async function submitDraft(ctx, id) {
  const env = ctx.env;
  const d = await ctx.db.prepare("SELECT * FROM app_drafts WHERE id=?").bind(id).first();
  ...
  let r;
  try {
    r = await hostFetch(env, "/api/registry/submit", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ licenseId, app }) });
  } catch {
    return { ok: false, error: "ホストへ接続できません" };
  }
  if (!r.ok) {
...
export { createDraft, deleteDraft, fetchAndInstall, listDrafts, listExternalApps, submitDraft, uninstallExternal };
